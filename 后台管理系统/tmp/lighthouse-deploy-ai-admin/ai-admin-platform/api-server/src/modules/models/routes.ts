...
router.get('/models', requireAuth, asyncHandler(async (req, res) => {
  ...
  ok(res, {
    items: models.map(model => {
      const adapter = model.adapter || model.provider.adapter || '';
      ...
      endpointPath: model.endpointPath || model.provider.endpointPath,
      statusEndpointPath: model.statusEndpointPath || model.provider.statusEndpointPath,
      ...
      protocol: {
        ...
        adapter,
        endpointPath: model.endpointPath || model.provider.endpointPath || undefined,
        statusEndpointPath: model.statusEndpointPath || model.provider.statusEndpointPath || undefined,
...
router.patch('/admin/models/:id', requireAuth, requireRole(adminRoles), asyncHandler(async (req, res) => {
  ...
  if (simpleBody.success && ['baseUrl', 'adapter', 'model', 'modelNick', 'key', 'uploadMode', 'imageMainModel', 'upstreamStreamConfigMode', 'upstreamStreamMode', 'upstreamStreamModes', 'asyncTaskConfigMode', 'asyncTaskMode', 'asyncTaskModes', 'responseTypeMode', 'responseType', 'responseTypes'].some(key => key in req.body)) {
    ...
    const next = {
      ...
      uploadMode: simpleBody.data.uploadMode !== undefined ? simpleBody.data.uploadMode : existing.uploadMode || existing.provider.uploadMode || undefined,
    ...
    const model = await prisma.aiModel.update({
      ...
      data: {
        ...
        statusEndpointPath: normalized.statusEndpointPath,
        uploadMode: normalized.uploadMode,
        protocol: mergeJson(jsonObject(existing.protocol), jsonObject(normalized.protocol)),
        supports: normalized.supports,
...
function normalizeSimpleModelConfig(input: { baseUrl: string; adapter: string; model: string; modelNick: string; key: string; type?: ModelType; uploadMode?: string | null; imageMainModel?: string | null; upstreamStreamConfigMode?: string | null; upstreamStreamMode?: string | null; upstreamStreamModes?: Record<string, string> | null; asyncTaskConfigMode?: string | null; asyncTaskMode?: string | null; asyncTaskModes?: Record<string, string> | null; responseTypeMode?: string | null; responseType?: string | null; responseTypes?: Record<string, string> | null }) {
  ...
  return {
    ...
    capabilities: type === 'VIDEO' ? (videoCapabilityPreset(model, modelNick) as Prisma.InputJsonValue) : undefined,
  };
}
