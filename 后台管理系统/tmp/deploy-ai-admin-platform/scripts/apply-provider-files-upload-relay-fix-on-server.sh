...
PKG="${1:-}"
if [[ -z "$PKG" ]]; then
  PKG="$(ls -t /tmp/provider-files-upload-relay-fix-*.tar.gz 2>/dev/null | head -1 || true)"
fi
if [[ -z "$PKG" || ! -f "$PKG" ]]; then
  echo "找不到部署包。请先把 provider-files-upload-relay-fix-*.tar.gz 上传到服务器 /tmp/" >&2
  exit 1
fi
...
DEPLOY_DIR="/tmp/provider-files-upload-relay-fix-$STAMP"
BACKUP_DIR="$REMOTE_ROOT/backups/provider-files-upload-relay-fix-$STAMP"
...
grep -q "callUpstreamMultipart(target.provider, '/v1/files'" "$REMOTE_APP_ROOT/api-server/dist/modules/workbench-compat/routes.js"
echo "provider /v1/files upstream target patched: ok"
grep -q "readUploadFile" "$REMOTE_APP_ROOT/api-server/dist/object-storage.js"
echo "multipart parser export patched: ok"
grep -q "uploadReferenceImageToPlatformFiles(file,progressHandler,normalizedModel)" "$REMOTE_WORKBENCH_ROOT/image-studio-canvas-next.html"
echo "main canvas provider file upload patched: ok"
grep -q "uploadReferenceImageToBackend(file,uploadMode,model)" "$REMOTE_WORKBENCH_ROOT/canvas-next/generation-service.js"
echo "canvas-next provider file upload patched: ok"
if grep -q "CanvasAccountGate.request('/v1/files'" "$REMOTE_WORKBENCH_ROOT/image-studio-canvas-next.html" "$REMOTE_WORKBENCH_ROOT/canvas-next/generation-service.js"; then
  echo "仍检测到画布直接请求平台 /v1/files，请检查包内容" >&2
  exit 1
fi
echo "direct platform /v1/files calls removed: ok"
