...
if [[ -z "$TARBALL" || ! -f "$TARBALL" ]]; then
  echo "用法: sudo $0 /tmp/openai-edits-default-reference-payload-fix-YYYYMMDDHHMMSS.tar.gz" >&2
  exit 1
fi
...
NAME="openai-edits-default-reference-payload-fix-${STAMP}"
WORKDIR="/tmp/${NAME}"
...
echo "==> 校验"
grep -q 'image: primaryFileIds' "$APP_ROOT/api-server/dist/modules/generation/adapters/registry.js" && echo "files image array payload patched: ok"
grep -q 'remoteUrl: entry.remoteUrl' "$APP_ROOT/api-server/dist/modules/generation/adapters/registry.js" && echo "files images metadata payload patched: ok"
grep -q 'image: refs' "$APP_ROOT/api-server/dist/modules/generation/adapters/registry.js" && echo "cos image url array payload patched: ok"
grep -q 'image_url: url' "$APP_ROOT/api-server/dist/modules/generation/adapters/registry.js" && echo "cos images metadata payload patched: ok"
