...
import { Alert, AutoComplete, Button, Card, ConfigProvider, Form, Input, InputNumber, Layout, Menu, Modal, Popconfirm, Progress, Select, Space, Statistic, Table, Tabs, Tag, App as AntApp, message } from 'antd';
...
const API_DOC_URL = '/docs/customer-generation-api.html';
function openApiDocs() {
  window.open(API_DOC_URL, '_blank', 'noopener,noreferrer');
}
...
function Login({ onDone }: { onDone: (mode: TokenMode) => void }) {
  ...
  async function submit(values: { account: string; password: string }) {
    ...
    try {
      ...
      const data = await api<{ token: string }>(path, { method: 'POST', body: JSON.stringify({ account: values.account.trim(), password: values.password }) });
...
function AccountCenter() {
  ...
  const createApiToken = useMutation({
    ...
    onSuccess: data => {
      ...
      Modal.success({
        title: 'API Key 创建成功',
        width: 720,
        content: <Space direction="vertical" style={{ width: '100%' }}><Alert type="warning" showIcon message="请立即复制保存，关闭后不再展示明文。" /><Input.TextArea value={data.token?.plainToken || ''} autoSize readOnly /><Space><Button type="primary" onClick={() => copyText(data.token?.plainToken || '')}>复制 API Key</Button><Button onClick={openApiDocs}>查看对接文档</Button></Space></Space>,
      });
    },
  ...
  return (
    <Space direction="vertical" size={16} style={{ width: '100%' }}>
      ...
      <Card title="API 接入" extra={<Button onClick={openApiDocs}>打开 API 对接文档</Button>}>
        <Space direction="vertical" size={12} style={{ width: '100%' }}>
          <Alert type="info" showIcon message="个人 API Key 长期不过期；会员折扣、账号状态、积分余额实时跟随当前账号。开放 API 使用 /api/open/personal/*，不会影响画布原接口。" />
          <Alert type="success" showIcon message="新手接入：创建 API Key 后先复制保存，再打开文档复制示例代码，把示例里的 sk_live_xxx 换成你的 Key 即可。" action={<Button size="small" onClick={openApiDocs}>查看文档</Button>} />
...
function GenerationResultDetailButton({ task }: { task: any }) {
  ...
  return (
    <>
      ...
      <Modal
        title={detailHasPreview ? (detailTask.type === 'VIDEO' ? '视频预览' : '图片预览') : '任务详情'}
        open={open}
        footer={null}
        width="92vw"
        centered
...
function isStoryboardTask(task: any) {
  ...
  const text = [task.mode, params.mode, params.taskType, params.template, params.sourceNodeType, params.sourceType, params.kind, task.prompt].filter(Boolean).join(' ').toLowerCase();
...
function normalizeModelPricingSubmitValues(model: any, values: any) {
  ...
  return {
    ...
    creditsPerUsdCost: Number(rest.creditsPerUsdCost || 0),
    markupRate: Number(rest.markupRate || 1),
  };
}
...
function showGeneratedCodesModal(title: string, codes: string[] | string, options?: { apiDocs?: boolean; copyLabel?: string }) {
    ...
    content: (
      <div className="trialCodeBox">
        <Space style={{ marginBottom: 8 }} wrap>
          ...
          {options?.apiDocs && <Button size="small" onClick={openApiDocs}>查看 API 对接文档</Button>}
        </Space>
        <pre>{text}</pre>
      </div>
    ),
...
function EnterpriseTasks() {
  ...
  return <Card title="企业生成任务">
    ...
    <Table rowKey="id" loading={q.isLoading} dataSource={apiListItems(q.data)} scroll={{ x: 1200 }} columns={[
      { title: '提交时间', dataIndex: 'createdAt', width: 170, render: dateTime },
      { title: '类型', dataIndex: 'type', width: 100, render: (v, row: any) => <Tag color="blue">{generationTypeLabel(v, row.mode)}</Tag> },
...
function EnterpriseAccounts() {
  ...
  return <Card title="企业 API 管理" extra={<Space><Button onClick={openApiDocs}>查看 API 对接文档</Button><Button type="primary" onClick={() => setOpen(true)}>创建企业</Button></Space>}>
...
function EnterpriseCreateTokenButton({ account }: { account: any }) {
  ...
  const m = useMutation({
    ...
    onSuccess: data => { message.success('企业 Token 已创建'); setOpen(false); form.resetFields(); qc.invalidateQueries({ queryKey: ['enterpriseAccounts'] }); showGeneratedCodesModal('企业 API Token（仅展示一次）', data.token?.plainToken || '', { apiDocs: true, copyLabel: '复制企业 Token' }); },
    onError: err => message.error(err instanceof Error ? err.message : '创建失败'),
  });
  return <>
  ...
  return <>
    ...
    <Modal title={`企业 Token：${account.name}`} open={open} footer={null} onCancel={() => setOpen(false)} width={820}>
      <Table rowKey="id" dataSource={account.tokens || []} pagination={false} columns={[
        ...
        { title: '过期时间', dataIndex: 'expiredAt', render: dateTime },
        { title: '最近调用', dataIndex: 'lastUsedAt', render: dateTime },
        { title: '操作', render: (_, row: any) => row.status === 'ACTIVE' ? <Popconfirm title="禁用该 Token？" onConfirm={() => disable.mutate(row.id)}><Button size="small" danger loading={disable.isPending}>禁用</Button></Popconfirm> : '-' },
      ]} />
    </Modal>
  </>;
}
