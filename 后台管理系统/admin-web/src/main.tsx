...
import { Alert, AutoComplete, Button, Card, Checkbox, ConfigProvider, DatePicker, Form, Input, InputNumber, Layout, Menu, Modal, Popconfirm, Progress, Segmented, Select, Space, Statistic, Switch, Table, Tabs, Tag, App as AntApp, message } from 'antd';
...
type ApiPagination = { limit?: number; offset?: number; total?: number; hasMore?: boolean };
type ApiList<T> = { ok: true; items: T[]; total?: number; pagination?: ApiPagination };
...
type UserMembershipScope = 'all' | 'expiringSoon';

const EXPIRING_MEMBERSHIP_DAYS = 3;

function Users() {
  const [userFilters, setUserFilters] = useState<{ page: number; pageSize: number; keyword: string; membershipScope: UserMembershipScope }>({
    page: 1,
    pageSize: 20,
    keyword: '',
    membershipScope: 'all',
  ...
  const q = useQuery({
    ...
    queryFn: () => {
      ...
      if (userFilters.membershipScope === 'expiringSoon') {
        params.set('membershipExpiringDays', String(EXPIRING_MEMBERSHIP_DAYS));
      }
  ...
  const isExpiringSoon = userFilters.membershipScope === 'expiringSoon';
  return <Card title={isExpiringSoon ? `${EXPIRING_MEMBERSHIP_DAYS}天内到期会员` : '用户列表'} extra={<Space wrap>
    <Segmented
      value={userFilters.membershipScope}
      options={[
        { label: '全部用户', value: 'all' },
        { label: `${EXPIRING_MEMBERSHIP_DAYS}天内到期会员`, value: 'expiringSoon' },
      ]}
      onChange={value => {
        setSelectedRowKeys([]);
        setUserFilters(prev => ({ ...prev, page: 1, membershipScope: value as UserMembershipScope }));
      }}
    />
    <Input.Search
      allowClear
    ...
    />
    {isSuperAdmin && !isExpiringSoon && <BatchOpenMembershipButton users={selectedUsers} keyword={userFilters.keyword} onDone={() => setSelectedRowKeys([])} />}
    <CreateUserButton />
  </Space>}><Table
    rowKey="id"
    loading={q.isLoading}
    ...
    rowSelection={isSuperAdmin && !isExpiringSoon ? {
      selectedRowKeys,
    ...
    { title: '会员', render: (_, row) => <MembershipStatusTag membership={activeMembership(row)} showDaysLeft={isExpiringSoon} /> },
    ...
    { title: '创建时间', dataIndex: 'createdAt', render: dateTime },
    {
      ...
      render: (_, row) => {
        ...
        return (
          <Space wrap>
            {isSuperAdmin && membership && <RenewMembershipButton user={row} membership={membership} />}
...
function MembershipStatusTag({ membership, showDaysLeft = false }: { membership: any; showDaysLeft?: boolean }) {
  if (!membership) return <Tag>非会员</Tag>;
  const daysText = showDaysLeft ? ` · 剩余 ${membershipDaysLeftText(membership.expiredAt)}` : '';
  return <Tag color={showDaysLeft ? 'orange' : 'gold'}>{membership.plan?.name || '会员'} · 到期 {dateTime(membership.expiredAt)}{daysText}</Tag>;
}

function membershipDaysLeftText(value?: string | null) {
  if (!value) return '-';
  ...
  return `${Math.ceil(diff / 86400000)}天`;
}

function RenewMembershipButton({ user, membership }: { user: any; membership: any }) {
  const [open, setOpen] = useState(false);
  const [form] = Form.useForm();
  const qc = useQueryClient();
  const plans = useQuery({ queryKey: ['membershipPlans'], queryFn: () => api<ApiList<any>>('/api/membership/plans') });
  const m = useMutation({
    mutationFn: (values: any) => api<any>(`/api/admin/membership/users/${user.id}/renew`, {
      method: 'POST',
      body: JSON.stringify({
    ...
    onSuccess: data => {
      message.success(`会员已续期至 ${dateTime(data.membership?.expiredAt)}`);
      setOpen(false);
      form.resetFields();
      ['users', 'dashboard', 'commissions'].forEach(key => qc.invalidateQueries({ queryKey: [key] }));
    },
    onError: err => message.error(err instanceof Error ? err.message : '续期失败'),
  });
  function openModal() {
    form.setFieldsValue({ planId: membership?.plan?.id, remark: undefined });
    setOpen(true);
  }
  return (
    <>
      <Button size="small" type="primary" disabled={user.status !== 'ACTIVE'} onClick={openModal}>续期会员</Button>
      <Modal
        title={`直接续期会员：${user.nickname || user.phone || user.email || user.id}`}
        open={open}
        ...
        destroyOnHidden
      >
        <Alert type="info" showIcon style={{ marginBottom: 16 }} message={`当前会员：${membership?.plan?.name || '会员'}，到期时间 ${dateTime(membership?.expiredAt)}。续期会从用户当前最新到期时间后顺延。`} />
        <Form form={form} layout="vertical" onFinish={values => m.mutate(values)}>
          <Form.Item name="planId" label="续期套餐" rules={[{ required: true, message: '请选择续期套餐' }]}>
          ...
          <Form.Item name="remark" label="备注">
            <Input placeholder="例如：到期续费 / 售后补偿 / 线下续期" />
          </Form.Item>
        </Form>
      </Modal>
    </>
...
function commissionRmb(value: number | string | null | undefined) {
  return `￥${(Number(value || 0) / 100).toFixed(2)}`;
}

function rmbNumber(value: number | string | null | undefined) {
...
type GenerationReportPeriod = 'day' | 'month';
type GenerationResultStatusTab = 'ALL' | 'SUCCESS' | 'FAILED';
const GENERATION_RESULT_PAGE_SIZE = 20;
...
type GenerationReportDailyRow = GenerationReportSummary & {
  key: string;
  date: string;
  label: string;
};

type GenerationReportData = {
  category: GenerationResultTab | null;
  scope?: 'page' | 'range';
  pagination?: { limit: number; offset: number; count: number };
  period: GenerationReportPeriod;
  label: string;
  startAt: string;
  endAt: string;
  summary: GenerationReportSummary;
  daily: GenerationReportDailyRow[];
  statusBreakdown: Array<{ status: string; count: number }>;
};

const emptyGenerationReportSummary: GenerationReportSummary = {
  totalCount: 0,
  successCount: 0,
  failedCount: 0,
  pendingCount: 0,
...
const GENERATION_RESULT_STATUS_TABS: Array<{ key: GenerationResultStatusTab; label: string; status: string }> = [
  { key: 'ALL', label: '全部', status: '' },
  { key: 'SUCCESS', label: '成功', status: 'SUCCESS' },
  { key: 'FAILED', label: '失败', status: 'FAILED' },
];

function generationResultCategoryParam(tab: GenerationResultTab) {
  const map: Record<GenerationResultTab, string> = {
    IMAGE: 'image',
    ASSET_DESIGN: 'assetDesign',
    SHOT_STORYBOARD: 'shotStoryboard',
...
function generationResultTabLabel(tab: GenerationResultTab) {
  const map: Record<GenerationResultTab, string> = {
    IMAGE: '图片',
    ASSET_DESIGN: '资产设计',
    SHOT_STORYBOARD: '分镜图',
...
function appendGenerationReportPeriodParams(params: URLSearchParams, period: GenerationReportPeriod, date: string, month: string) {
...
function generationReportSuccessRate(summary: GenerationReportSummary) {
  if (!summary.totalCount) return '0%';
  return `${Math.round((summary.successCount / summary.totalCount) * 100)}%`;
}

function generationResultStatusTabFromStatus(status: string): GenerationResultStatusTab {
  if (status === 'SUCCESS') return 'SUCCESS';
  if (status === 'FAILED') return 'FAILED';
  return 'ALL';
}

function statusFromGenerationResultStatusTab(tab: string) {
  return GENERATION_RESULT_STATUS_TABS.find(item => item.key === tab)?.status || '';
}
...
function GenerationResults() {
  ...
  const [reportPeriod, setReportPeriod] = useState<GenerationReportPeriod>('month');
  const [reportDate, setReportDate] = useState(currentDateValue());
  const [reportMonth, setReportMonth] = useState(currentMonthValue());
  const [page, setPage] = useState(1);
  const q = useQuery({
    queryKey: ['generationResults', activeTab, filters, reportPeriod, reportDate, reportMonth, page],
    queryFn: () => {
      const params = new URLSearchParams();
      if (filters.status) params.set('status', filters.status);
      if (filters.userId) params.set('userId', filters.userId.trim());
      params.set('type', activeTab === 'VIDEO' ? 'VIDEO' : 'IMAGE');
      params.set('category', generationResultCategoryParam(activeTab));
      appendGenerationReportPeriodParams(params, reportPeriod, reportDate, reportMonth);
      params.set('limit', String(GENERATION_RESULT_PAGE_SIZE));
      params.set('offset', String((page - 1) * GENERATION_RESULT_PAGE_SIZE));
      return api<ApiList<any>>(`/api/generation/admin/tasks?${params.toString()}`);
    },
  });
  const reportQuery = useQuery({
    queryKey: ['generationResultsReport', activeTab, filters, reportPeriod, reportDate, reportMonth],
    queryFn: () => {
      const params = new URLSearchParams();
      if (filters.status) params.set('status', filters.status);
      if (filters.userId) params.set('userId', filters.userId.trim());
      ...
      params.set('category', generationResultCategoryParam(activeTab));
      appendGenerationReportPeriodParams(params, reportPeriod, reportDate, reportMonth);
      return api<GenerationReportData>(`/api/generation/admin/tasks/report?${params.toString()}`);
    },
  });
  const rows = apiListItems(q.data);
  const pagination = apiListPagination(q.data);
  const errorMessage = q.error instanceof Error ? q.error.message : q.error ? String(q.error) : '';
  const reportErrorMessage = reportQuery.error instanceof Error ? reportQuery.error.message : reportQuery.error ? String(reportQuery.error) : '';
  const refetchAll = () => {
    q.refetch();
    reportQuery.refetch();
  };
  const activeStatusTab = generationResultStatusTabFromStatus(filters.status);
  const renderTabContent = () => (
    <Space direction="vertical" size={16} style={{ width: '100%' }}>
      <GenerationResultToolbar
        ...
        setReportPeriod={value => { setReportPeriod(value); setPage(1); }}
        reportDate={reportDate}
        setReportDate={value => { setReportDate(value); setPage(1); }}
        reportMonth={reportMonth}
        setReportMonth={value => { setReportMonth(value); setPage(1); }}
        setPage={setPage}
        fetching={q.isFetching || reportQuery.isFetching}
        refetch={refetchAll}
      />
      {reportErrorMessage && <Alert type="error" showIcon message="结果报表加载失败" description={reportErrorMessage} />}
      <GenerationResultReport
        activeTab={activeTab}
        period={reportPeriod}
        data={reportQuery.data}
        loading={reportQuery.isLoading || reportQuery.isFetching}
      />
      {errorMessage && <Alert type="error" showIcon message="结果列表加载失败" description={errorMessage} />}
      <GenerationResultTable
        rows={rows}
        loading={q.isLoading}
  ...
  const renderStatusTabs = () => (
    <Tabs
      className="generationResultStatusTabs"
      activeKey={activeStatusTab}
      onChange={key => { setFilters(prev => ({ ...prev, status: statusFromGenerationResultStatusTab(key) })); setPage(1); }}
      items={GENERATION_RESULT_STATUS_TABS.map(item => ({
        key: item.key,
        label: item.label,
  ...
  return (
    <Card title="生图生视频结果">
      <Tabs
        activeKey={activeTab}
        onChange={key => { setActiveTab(key as GenerationResultTab); setPage(1); }}
        items={[
          { key: 'IMAGE', label: '图片', children: renderStatusTabs() },
          { key: 'ASSET_DESIGN', label: '资产设计', children: renderStatusTabs() },
          { key: 'SHOT_STORYBOARD', label: '分镜图', children: renderStatusTabs() },
          { key: 'STORYBOARD', label: '故事板', children: renderStatusTabs() },
          { key: 'VIDEO', label: '视频', children: renderStatusTabs() },
        ]}
      />
    </Card>
  );
...
function GenerationResultToolbar({ activeTab, filters, setFilters, reportPeriod, setReportPeriod, reportDate, setReportDate, reportMonth, setReportMonth, setPage, fetching, refetch }: {
  activeTab: GenerationResultTab;
  filters: { status: string; userId: string };
  setFilters: React.Dispatch<React.SetStateAction<{ status: string; userId: string }>>;
  reportPeriod: GenerationReportPeriod;
  setReportPeriod: (value: GenerationReportPeriod) => void;
  reportDate: string;
  ...
  return (
    <div className="generationReportToolbar">
      <Space wrap>
        <Tag color="blue">{generationResultTabLabel(activeTab)}</Tag>
        <Segmented
          value={reportPeriod}
          options={[
            { label: '按天', value: 'day' },
          ...
          onChange={value => setReportPeriod(value as GenerationReportPeriod)}
        />
        {reportPeriod === 'day' ? (
          <DatePicker
            allowClear={false}
            ...
            format="YYYY-MM-DD"
            onChange={value => setReportDate(value ? value.format('YYYY-MM-DD') : currentDateValue())}
          />
        ) : (
          <DatePicker
            picker="month"
            ...
            format="YYYY年MM月"
            onChange={value => setReportMonth(value ? value.format('YYYY-MM') : currentMonthValue())}
          />
        )}
        <Input.Search
          key={`${activeTab}-${filters.userId}`}
          allowClear
          placeholder="用户 UID"
          style={{ width: 260 }}
          onSearch={value => { setPage(1); setFilters(prev => ({ ...prev, userId: value.trim() })); }}
        />
...
function GenerationResultReport({ activeTab, period, data, loading }: {
  activeTab: GenerationResultTab;
  period: GenerationReportPeriod;
  data?: GenerationReportData;
  loading: boolean;
}) {
  const summary = data?.summary || emptyGenerationReportSummary;
  const daily = data?.daily || [];
  const periodLabel = data?.label || (period === 'day' ? currentDateValue() : currentMonthValue());
  return (
    <div className="generationReportPanel">
      <div className="generationReportHeader">
        <div>
          <h3>{generationResultTabLabel(activeTab)}数据概览</h3>
          <span>{periodLabel}</span>
        </div>
        <div className="dashboardInlineLegend">
          <span><i className="legendGenerationTotal" />调用</span>
          <span><i className="legendGenerationSuccess" />成功</span>
          <span><i className="legendGenerationFailed" />失败</span>
        </div>
      </div>
      <div className="generationReportMetricGrid">
        <div className="generationReportMetric"><span>任务总数</span><strong>{formatNumber(summary.totalCount)}</strong><small>成功率 {generationReportSuccessRate(summary)}</small></div>
        <div className="generationReportMetric"><span>成功次数</span><strong>{formatNumber(summary.successCount)}</strong><small>图片 {formatNumber(summary.imageCount)} / 视频 {formatNumber(summary.videoCount)}</small></div>
        <div className="generationReportMetric"><span>失败次数</span><strong>{formatNumber(summary.failedCount)}</strong><small>退款 {formatNumber(summary.refundedCount)}</small></div>
        <div className="generationReportMetric"><span>进行中/等待</span><strong>{formatNumber(summary.pendingCount)}</strong><small>扣费 {formatNumber(summary.chargedCredits)} 积分</small></div>
      </div>
      <GenerationDailyBarChart rows={daily} loading={loading} />
      <Table<GenerationReportDailyRow>
        rowKey="key"
        size="small"
        loading={loading}
        dataSource={daily}
        pagination={period === 'month' ? { pageSize: 10, showSizeChanger: false } : false}
        scroll={{ x: 900 }}
        columns={[
          { title: '日期', dataIndex: 'date', width: 130 },
          { title: '调用次数', dataIndex: 'totalCount', width: 120, render: formatNumber },
          { title: '成功次数', dataIndex: 'successCount', width: 120, render: value => <Tag color="green">{formatNumber(value)}</Tag> },
          { title: '失败次数', dataIndex: 'failedCount', width: 120, render: value => <Tag color={Number(value || 0) > 0 ? 'red' : 'default'}>{formatNumber(value)}</Tag> },
          { title: '进行中/等待', dataIndex: 'pendingCount', width: 130, render: formatNumber },
          { title: '退款', dataIndex: 'refundedCount', width: 100, render: formatNumber },
          { title: '扣费积分', dataIndex: 'chargedCredits', width: 120, render: formatNumber },
        ]}
      />
    </div>
  );
...
function GenerationDailyBarChart({ rows, loading }: { rows: GenerationReportDailyRow[]; loading: boolean }) {
  const maxValue = Math.max(1, ...rows.flatMap(row => [row.totalCount, row.successCount, row.failedCount]));
  const labelStep = rows.length > 18 ? Math.ceil(rows.length / 10) : 1;
  const hasData = rows.some(row => row.totalCount > 0 || row.successCount > 0 || row.failedCount > 0);
  return (
    <div className="generationReportBarWrap">
      <div className="generationReportBarChart" style={{ gridTemplateColumns: `repeat(${Math.max(rows.length, 1)}, minmax(30px, 1fr))` }}>
        {rows.map((row, index) => {
          const totalHeight = row.totalCount > 0 ? Math.max(4, (row.totalCount / maxValue) * 100) : 0;
          const successHeight = row.successCount > 0 ? Math.max(4, (row.successCount / maxValue) * 100) : 0;
          const failedHeight = row.failedCount > 0 ? Math.max(4, (row.failedCount / maxValue) * 100) : 0;
          const title = `${row.date} 调用 ${formatNumber(row.totalCount)} / 成功 ${formatNumber(row.successCount)} / 失败 ${formatNumber(row.failedCount)}`;
          return (
            <div className="generationReportBarGroup" key={row.key} title={title}>
              <div className="generationReportBarSlot">
                <span className="generationReportBar generationReportBar-total" style={{ height: `${totalHeight}%` }} />
                <span className="generationReportBar generationReportBar-success" style={{ height: `${successHeight}%` }} />
                <span className="generationReportBar generationReportBar-failed" style={{ height: `${failedHeight}%` }} />
              </div>
              <div className="generationReportBarLabel">{index % labelStep === 0 || index === rows.length - 1 ? row.label : ''}</div>
            </div>
          );
        })}
      </div>
      {!hasData && <div className="dashboardChartEmpty">{loading ? '正在加载统计数据' : '当前筛选区间暂无生成记录'}</div>}
    </div>
  );
}

function GenerationResultTable({ rows, loading, fetching, page, pagination, setPage }: {
  rows: any[];
  loading: boolean;
  fetching: boolean;
  page: number;
  ...
  const total = generationResultPaginationTotal(pagination, rows, page);
  return (
    <Table
        rowKey="id"
        ...
        pagination={{
          current: page,
          pageSize: GENERATION_RESULT_PAGE_SIZE,
          total,
        ...
        columns={[
          ...
          { title: '进度', dataIndex: 'progress', width: 170, render: (v: number, row) => <Progress percent={progressValue(v, row.status)} size="small" status={progressStatus(row.status)} /> },
...
function generationTaskAdminCategory(task: any): GenerationResultTab {
  const serverCategory = normalizeGenerationResultCategory(task?.adminCategory || task?.generationCategory || task?.resultCategory);
...
function normalizeGenerationResultCategory(value: any): GenerationResultTab | null {
  const key = normalizeGenerationNodeTypeKey(value);
  if (!key) return null;
...
function generationTaskNodeCategory(task: any): GenerationResultTab | null {
...
function generationTaskNodeTypeKeys(task: any) {
  const params = generationJsonRecord(task?.paramsJson);
  const result = generationJsonRecord(task?.resultJson);
  const paramsExtra = generationJsonRecord(params.extra);
  const resultExtra = generationJsonRecord(result.extra);
  const paramsMetadata = generationJsonRecord(params.metadata);
  const resultMetadata = generationJsonRecord(result.metadata);
  const fields = [
    params.nodeType,
    params.node_type,
    result.nodeType,
...
function normalizeGenerationNodeTypeKey(value: any) {
  return String(value || '').trim().toLowerCase().replace(/[-_\s/]+/g, '');
}

function isStoryboardTask(task: any) {
  return generationTaskAdminCategory(task) === 'STORYBOARD';
}

function isAssetDesignTask(task: any) {
  return generationTaskAdminCategory(task) === 'ASSET_DESIGN';
}

function isShotStoryboardTask(task: any) {
  return generationTaskAdminCategory(task) === 'SHOT_STORYBOARD';
}
...
function generationJsonRecord(value: any) {
...
function apiListPagination(data: any): ApiPagination {
  if (data?.pagination) return data.pagination;
  if (data?.data?.pagination) return data.data.pagination;
  return {};
}

function generationResultPaginationTotal(pagination: ApiPagination, rows: any[], page: number) {
  const total = Number(pagination.total);
  if (Number.isFinite(total) && total >= 0) return total;
  const offset = Math.max(0, page - 1) * GENERATION_RESULT_PAGE_SIZE;
  return offset + rows.length + (pagination.hasMore ? GENERATION_RESULT_PAGE_SIZE : 0);
}
...
function progressValue(value: number, status: string) {
  if (['SUCCESS', 'FAILED', 'REFUNDED', 'CANCELLED', 'TIMEOUT'].includes(String(status || '').toUpperCase())) return 100;
  const num = Number(value || 0);
  return Math.max(0, Math.min(100, num));
}

function progressStatus(status: string): 'success' | 'exception' | 'normal' | 'active' {
  const normalized = String(status || '').toUpperCase();
  if (['FAILED', 'CANCELLED', 'TIMEOUT'].includes(normalized)) return 'exception';
  if (normalized === 'SUCCESS') return 'success';
  if (['RUNNING', 'PENDING', 'CREATED'].includes(normalized)) return 'active';
  return 'normal';
}
...
function generationTypeLabel(type: string, mode?: string, task?: any) {
  ...
  const category = task ? generationTaskAdminCategory(task) : null;
  if (category === 'ASSET_DESIGN') return '资产设计';
  if (category === 'SHOT_STORYBOARD') {
    const params = task?.paramsJson && typeof task.paramsJson === 'object' ? task.paramsJson : {};
    const result = task?.resultJson && typeof task.resultJson === 'object' ? task.resultJson : {};
  ...
  if (category === 'STORYBOARD' || (type === 'IMAGE' && /storyboard/i.test(normalizedMode))) return '故事板';
...
function patchCanvasNodeConfigData(data: any, type: string, enabled: boolean) {
  if (!data || typeof data !== 'object') return data;
  const updatedAt = new Date().toISOString();
  const patchList = (items: any) => Array.isArray(items)
    ? items.map((item: any) => item && item.type === type ? { ...item, enabled, updatedAt } : item)
    : items;
  const patchDisabledTypes = (items: any) => {
    const disabled = new Set((Array.isArray(items) ? items : []).map((item: any) => String(item || '').trim()).filter(Boolean));
    if (enabled) disabled.delete(type);
    else disabled.add(type);
    return [...disabled];
  };
  const next = { ...data };
  if (Array.isArray(next.items)) next.items = patchList(next.items);
  if (Array.isArray(next.nodes)) next.nodes = patchList(next.nodes);
  if (Array.isArray(next.disabledNodeTypes)) next.disabledNodeTypes = patchDisabledTypes(next.disabledNodeTypes);
  if (Array.isArray(next.data)) {
    next.data = patchList(next.data);
  } else if (next.data && typeof next.data === 'object') {
    next.data = { ...next.data };
    if (Array.isArray(next.data.items)) next.data.items = patchList(next.data.items);
    if (Array.isArray(next.data.nodes)) next.data.nodes = patchList(next.data.nodes);
    if (Array.isArray(next.data.disabledNodeTypes)) next.data.disabledNodeTypes = patchDisabledTypes(next.data.disabledNodeTypes);
  }
  return next;
}
...
function CanvasNodeManagement() {
  ...
  const toggle = useMutation<any, Error, { type: string; enabled: boolean }, { previous?: any }>({
    mutationFn: (input: { type: string; enabled: boolean }) => api<any>(`/api/admin/canvas-nodes/${encodeURIComponent(input.type)}`, {
      method: 'PATCH',
      body: JSON.stringify({ enabled: input.enabled }),
    }),
    onMutate: async variables => {
      await qc.cancelQueries({ queryKey: ['canvasNodes'] });
      const previous = qc.getQueryData<any>(['canvasNodes']);
      qc.setQueryData<any>(['canvasNodes'], (current: any) => patchCanvasNodeConfigData(current, variables.type, variables.enabled));
      return { previous };
    },
    onSuccess: (data, variables) => {
      message.success(variables.enabled ? '节点已启用' : '节点已禁用');
      qc.setQueryData(['canvasNodes'], data);
    },
    onError: (err, _variables, context) => {
      if (context?.previous) qc.setQueryData(['canvasNodes'], context.previous);
      message.error(err instanceof Error ? err.message : '节点状态更新失败');
    },
    onSettled: () => {
        ...
        <Table<CanvasNodeConfigItem>
          ...
          columns={[
            ...
            { title: '最近更新', width: 180, dataIndex: 'updatedAt', render: dateTime },
            {
              ...
              fixed: 'right',
              render: (_, row) => {
                const rowPending = toggle.isPending && toggle.variables?.type === row.type;
                const otherPending = toggle.isPending && toggle.variables?.type !== row.type;
                return (
                  <Space>
                    ...
                    <Button
                      ...
                      danger={row.enabled}
                      data-testid="canvas-node-direct-toggle-v1"
                      loading={rowPending}
                      disabled={otherPending}
                      onClick={() => requestToggle(row, !row.enabled)}
                    >
                      {row.enabled ? '禁用' : '启用'}
                    </Button>
                  </Space>
...
function ModelList() {
  ...
  const providerQ = useQuery({ queryKey: ['providers'], queryFn: () => api<ApiList<any>>('/api/admin/upstream-providers') });
  const [baseUrlFilter, setBaseUrlFilter] = useState('');
  const [pricingModeFilter, setPricingModeFilter] = useState<ModelPricingMode>('all');
  const items = q.data?.items || [];
  const baseUrlOptions = useMemo(
    () => adminModelBaseUrlOptions(items, providerQ.data?.items || []),
    [items, providerQ.data?.items],
  );
  const filteredItems = useMemo(
    () => filterAndSortAdminModels(items, { baseUrl: baseUrlFilter, pricingMode: pricingModeFilter }),
    [items, baseUrlFilter, pricingModeFilter],
  );
  const selectedBaseUrl = normalizeAdminModelBaseUrl(baseUrlFilter);
  const renderTable = (type: string) => <ModelStatusTabs loading={q.isLoading} items={filteredItems.filter((item: any) => item.type === type)} />;
  return (
    <Card title="模型列表" extra={<Space><ApplyAntigravityPresetButton /><CreateModelButton /></Space>}>
      <div className="modelFilterBar">
        <Select
          className="modelBaseUrlSelect"
          allowClear
          showSearch
          loading={providerQ.isLoading}
          placeholder="按渠道 Base URL 筛选"
          value={baseUrlFilter || undefined}
          optionFilterProp="label"
          options={baseUrlOptions}
          onChange={value => setBaseUrlFilter(value || '')}
        />
        <Select
          className="modelPricingModeSelect"
          value={pricingModeFilter}
          options={[
            { value: 'all', label: '全部定价' },
            { value: 'per_second', label: '视频按秒' },
            { value: 'per_generation', label: '视频按次' },
          ]}
          onChange={value => setPricingModeFilter(value)}
        />
        <Tag color="blue">{filteredItems.length} 个模型</Tag>
      </div>
      {selectedBaseUrl ? (
        <div className="selectedBaseUrlLinkBar">
          <a href={selectedBaseUrl} target="_blank" rel="noopener noreferrer" title={`打开 ${selectedBaseUrl}`}>
            {selectedBaseUrl}
          </a>
        </div>
      ) : null}
...
type ModelPricingMode = 'all' | 'per_second' | 'per_generation';

function normalizeAdminModelBaseUrl(value: string) {
...
function adminModelBaseUrl(model: any) {
  return normalizeAdminModelBaseUrl(model?.provider?.baseUrl || model?.baseUrl || model?.url || '');
}

function adminModelBaseUrlOptions(models: any[], providers: any[]) {
  const urls = [...providers.map(provider => provider?.baseUrl), ...models.map(adminModelBaseUrl)]
    .map(value => normalizeAdminModelBaseUrl(String(value || '')))
    .filter(Boolean);
...
function filterAndSortAdminModels(items: any[], filters: { baseUrl: string; pricingMode: ModelPricingMode }) {
  const wantedBaseUrl = normalizeAdminModelBaseUrl(filters.baseUrl);
  return items
    .filter(item => {
      if (wantedBaseUrl && adminModelBaseUrl(item) !== wantedBaseUrl) return false;
      if (item.type === 'VIDEO' && filters.pricingMode !== 'all' && videoBillingModeFromModel(item) !== filters.pricingMode) return false;
      return true;
    })
    .slice()
    .sort(compareAdminModelPriceAsc);
}

function compareAdminModelPriceAsc(a: any, b: any) {
  const priceA = adminModelSortPrice(a);
  const priceB = adminModelSortPrice(b);
  if (priceA !== priceB) return priceA - priceB;
  return String(a?.displayName || a?.name || '').localeCompare(String(b?.displayName || b?.name || ''));
}

function adminModelSortPrice(model: any) {
  if (model?.type === 'IMAGE') return Number(model?.salePrice || 0);
  if (model?.type === 'VIDEO') {
    if (videoBillingModeFromModel(model) === 'per_generation') {
      const prices = videoPricingResolutionList(model)
        .map(resolution => videoTierGenerationMemberPrice(videoPricingTier(model, resolution)))
        .filter((value): value is number => Number.isFinite(value));
      if (prices.length) return Math.min(...prices);
      return videoFlatMemberPrice(model);
    }
    const prices = videoPricingResolutionList(model)
      .map(resolution => videoTierMemberPrice(videoPricingTier(model, resolution)))
      .filter((value): value is number => Number.isFinite(value));
    if (prices.length) return Math.min(...prices);
    return Number(model?.pricePerSecond || 0);
  }
  if (model?.type === 'LLM') return Number(model?.creditsPerUsdCost || 0);
  return Number.MAX_SAFE_INTEGER;
}

function ModelStatusTabs({ loading, items }: { loading: boolean; items: any[] }) {
  const activeItems = items.filter(item => item.status === 'ACTIVE');
  const disabledItems = items.filter(item => item.status !== 'ACTIVE');
  return (
    <Tabs
      size="small"
      items={[
        { key: 'ACTIVE', label: `启用 (${activeItems.length})`, children: <ModelTypeTable loading={loading} items={activeItems} /> },
        { key: 'DISABLED', label: `禁用 (${disabledItems.length})`, children: <ModelTypeTable loading={loading} items={disabledItems} /> },
      ]}
    />
  );
}
  ...
  if (videoBillingModeFromModel(model) === 'per_generation') {
    const tierText = tiers
      .map((tier: any) => {
        ...
        const price = videoTierGenerationMemberPrice(tier);
        if (!label || !Number.isFinite(price)) return '';
        return `${label} ${price}`;
      })
      .filter(Boolean);
    if (tierText.length) return `${tierText.join(' / ')} 积分/次`;
    return `${videoFlatMemberPrice(model)} 积分/次`;
  }
  const tierText = tiers
    .map((tier: any) => {
      const label = String(tier?.resolution || tier?.label || '').trim();
      const scenarioLabel = videoScenarioPriceLabel(tier, label);
...
function ModelTypeTable({ loading, items }: { loading: boolean; items: any[] }) {
  ...
  return <Table rowKey="id" loading={loading || toggleStatus.isPending} dataSource={items} scroll={{ x: 1200 }} columns={[
    ...
    { title: '视频定价', render: (_, row) => row.type === 'VIDEO' ? videoSecondPriceLabel(row) : money(row.pricePerSecond || 0) },
...
type ModelUsageType = 'VIDEO' | 'IMAGE' | 'LLM';
type ModelUsageStatsPeriod = 'day' | 'month' | 'all';

type ModelUsageStatsItem = {
  key: string;
  period: string;
  modelId?: string;
  modelName?: string;
...
const MODEL_USAGE_TYPE_TABS: Array<{ type: ModelUsageType; label: string }> = [
  { type: 'VIDEO', label: '视频' },
  { type: 'IMAGE', label: '图片' },
  { type: 'LLM', label: '大语言' },
];

function modelUsageTypeLabel(type: ModelUsageType) {
  return MODEL_USAGE_TYPE_TABS.find(item => item.type === type)?.label || type;
}

function modelUsagePeriodTitle(period: ModelUsageStatsPeriod) {
  if (period === 'month') return '月份';
  if (period === 'day') return '日期';
  return '';
}

function modelUsageSuccessRate(row: ModelUsageStatsItem) {
...
function ModelUsages() {
  return (
    <Tabs items={MODEL_USAGE_TYPE_TABS.map(item => ({
      key: item.type,
      label: item.label,
      children: <ModelUsageTypePanel type={item.type} />,
    }))} />
  );
}

function ModelUsageTypePanel({ type }: { type: ModelUsageType }) {
  return (
    <Space direction="vertical" size={16} style={{ width: '100%' }}>
      <ModelUsageStats type={type} />
      <ModelUsageList type={type} />
    </Space>
  );
}

function ModelUsageStats({ type }: { type: ModelUsageType }) {
  const [period, setPeriod] = useState<ModelUsageStatsPeriod>('day');
  const q = useQuery({
    queryKey: ['modelUsageStats', type, period],
    queryFn: () => api<ApiList<ModelUsageStatsItem>>(`/api/admin/model-usage-stats?type=${type}&period=${period}&limit=1000`),
  });
  const rows = apiListItems(q.data) as ModelUsageStatsItem[];
  const periodTitle = modelUsagePeriodTitle(period);
  return (
    <Card
      title={`${modelUsageTypeLabel(type)}调用统计`}
      extra={(
        <Segmented
          value={period}
          options={[
          ...
          onChange={value => setPeriod(value as ModelUsageStatsPeriod)}
        />
      )}
    >
      <Table<ModelUsageStatsItem>
        rowKey="key"
        loading={q.isLoading}
        dataSource={rows}
        pagination={period === 'all' ? false : { pageSize: 20 }}
        scroll={{ x: 920 }}
        columns={[
          ...(periodTitle ? [{ title: periodTitle, dataIndex: 'period', width: 130 }] : []),
          {
            title: '启动渠道',
            width: 320,
            render: (_, row) => (
              <Space direction="vertical" size={2}>
                <b>{row.modelDisplayName || row.providerName || row.providerKey || '-'}</b>
                <span className="muted">{row.channelKey || row.providerKey || row.providerId}</span>
              </Space>
            ),
          },
          { title: '调用次数', dataIndex: 'totalCount', width: 130, defaultSortOrder: 'descend', sorter: (a, b) => a.totalCount - b.totalCount, render: formatNumber },
          { title: '成功次数', dataIndex: 'successCount', width: 130, sorter: (a, b) => a.successCount - b.successCount, render: value => <Tag color="green">{formatNumber(value)}</Tag> },
          { title: '失败次数', dataIndex: 'failedCount', width: 130, sorter: (a, b) => a.failedCount - b.failedCount, render: value => <Tag color={Number(value || 0) > 0 ? 'red' : 'default'}>{formatNumber(value)}</Tag> },
          { title: '成功率', width: 110, render: (_, row) => modelUsageSuccessRate(row) },
        ]}
      />
    </Card>
  );
...
function ModelUsageList({ type }: { type: ModelUsageType }) {
  const [filters, setFilters] = useState({ status: '', limit: 200 });
  const q = useQuery({
    queryKey: ['modelUsages', type, filters],
    queryFn: () => {
      const params = new URLSearchParams();
      if (filters.status) params.set('status', filters.status);
  ...
  return (
    <Card title={`${modelUsageTypeLabel(type)}调用明细`}>
      ...
      <Table
        ...
        columns={[
          { title: '用户', dataIndex: ['user', 'nickname'] },
          { title: '模型', dataIndex: ['model', 'displayName'] },
          { title: '渠道', render: (_, row) => row.model?.provider?.name || row.model?.provider?.providerKey || '-' },
...
function Agents() {
  ...
  return <Card title="代理列表" extra={<CreateAgentButton />}><Table rowKey="id" loading={q.isLoading} dataSource={q.data?.items || []} columns={[
    ...
    { title: '待结佣金', dataIndex: ['stats', 'pendingCommissionAmount'], render: commissionRmb },
    { title: '状态', dataIndex: 'status' },
    ...
    <>
      ...
      <Modal title={`${agent.name} 的客户`} open={open} onCancel={() => setOpen(false)} footer={null} width={920}>
        <Table rowKey="id" loading={q.isLoading} dataSource={q.data?.items || []} columns={[
          ...
          { title: '核销数', dataIndex: ['stats', 'redemptionCount'] },
          { title: '佣金', dataIndex: ['stats', 'commissionAmount'], render: commissionRmb },
...
const membershipCardReconciliationTypes = new Set(['MEMBERSHIP_CARD_GRANT', 'MEMBERSHIP_CARD_REQUEST']);

function isAgentCreditReconciliationOrder(row: any) {
  return !membershipCardReconciliationTypes.has(String(row.relatedType || ''));
}

function AgentReconciliationSummary({ items }: { items: any[] }) {
  const pending = items.filter(item => item.status === 'PENDING');
  const confirmed = items.filter(item => item.status === 'CONFIRMED');
  const settled = items.filter(item => item.status === 'SETTLED');
  return (
    <div className="grid4">
      <Card><Statistic title="对账单数" value={items.length} /></Card>
      <Card><Statistic title="充值积分" value={sumBy(items, item => item.credits)} suffix="积分" /></Card>
      <Card><Statistic title="收款总额" value={rmbNumber(sumBy(items, item => item.amountCents))} precision={2} prefix="￥" /></Card>
      <Card><Statistic title="待处理金额" value={rmbNumber(sumBy(pending, item => item.amountCents))} precision={2} prefix="￥" /></Card>
      <Card><Statistic title="已确认金额" value={rmbNumber(sumBy(confirmed, item => item.amountCents))} precision={2} prefix="￥" /></Card>
      <Card><Statistic title="已结算金额" value={rmbNumber(sumBy(settled, item => item.amountCents))} precision={2} prefix="￥" /></Card>
    </div>
  );
}

function AgentReconciliationTable({
  admin = false,
  title,
  showSummary = false,
  agentCreditOnly = false,
}: {
  admin?: boolean;
  title?: string;
  showSummary?: boolean;
  ...
  const settle = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ['myAgentReconciliationOrders'] });
    },
    onError: err => message.error(err instanceof Error ? err.message : '结算失败'),
  });
  const approveGrant = useMutation({
  ...
  const items = agentCreditOnly ? rawItems.filter(isAgentCreditReconciliationOrder) : rawItems;
  return (
    <Space direction="vertical" size={16} style={{ width: '100%' }}>
      {showSummary ? <AgentReconciliationSummary items={items} /> : null}
      <Card title={title || (admin ? '代理对账订单' : '我的对账订单')}>
        <Table rowKey="id" loading={q.isLoading} dataSource={items} columns={[
          { title: '订单号', dataIndex: 'orderNo' },
...
function Commissions() {
  return <Tabs items={[
    { key: 'membershipCommissions', label: '会员佣金结算', children: <MembershipCommissionSettlementTab /> },
    { key: 'agentCreditReconciliation', label: '代理充值积分对账', children: <AgentCreditReconciliationTab /> },
  ]} />;
}

function MembershipCommissionSettlementTab() {
  return (
    <Space direction="vertical" size={16} style={{ width: '100%' }}>
      <CommissionTable />
      <Settlements />
...
function CommissionTable() {
  ...
  return (
    <Space direction="vertical" size={16} style={{ width: '100%' }}>
      <CommissionSummary items={items} />
      <Card title="会员佣金明细" extra={<SettleAgentButton />}>
        <Table rowKey="id" loading={q.isLoading} dataSource={items} columns={[
          { title: '代理', dataIndex: ['agent', 'name'] },
          { title: '客户', render: (_, row) => commissionCustomerText(row) },
          { title: '套餐', render: (_, row) => row.redemption?.plan?.name || row.membership?.plan?.name || '-' },
          { title: '会员账号', render: (_, row) => row.redemption?.account?.code || '-' },
          { title: '来源', dataIndex: 'sourceType' },
          { title: '佣金', dataIndex: 'amount', render: commissionRmb },
...
function commissionCustomerText(row: any) {
  const user = row.redemption?.user || row.user;
  return user?.nickname || user?.phone || user?.email || '-';
}

function CommissionSummary({ items }: { items: any[] }) {
  const pending = items.filter(item => item.status === 'PENDING');
  const eligible = pending.filter(item => item.settleEligible);
  const settled = items.filter(item => item.status === 'SETTLED');
  return (
    <div className="grid4">
      <Card><Statistic title="佣金总额" value={rmbNumber(sumBy(items, item => item.amount))} precision={2} prefix="￥" /></Card>
      <Card><Statistic title="待结佣金" value={rmbNumber(sumBy(pending, item => item.amount))} precision={2} prefix="￥" /></Card>
      <Card><Statistic title="可结佣金" value={rmbNumber(sumBy(eligible, item => item.amount))} precision={2} prefix="￥" /></Card>
      <Card><Statistic title="已结佣金" value={rmbNumber(sumBy(settled, item => item.amount))} precision={2} prefix="￥" /></Card>
    </div>
  );
}
...
function SettleAgentButton() {
  ...
  const agentOptions = (agents.data?.items || []).map((agent: any) => {
    ...
    return {
      value: agent.id,
      disabled: eligible <= 0,
      label: `${agent.name} · 可结 ${commissionRmb(eligible)} · 待结 ${commissionRmb(pending)} · 对账期 ${delay} 天${pending > 0 && eligible <= 0 ? ' · 未到期' : ''}`,
    };
  });
  const m = useMutation({
    ...
    onSuccess: (data: any) => {
      message.success(`已结算 ${commissionRmb(data.settlement.amount)}，共 ${data.count} 笔`);
      setOpen(false);
      form.resetFields();
  ...
  return (
    <Space direction="vertical" size={16} style={{ width: '100%' }}>
      ...
      <Card title="会员佣金对账单">
        <Table rowKey="id" loading={q.isLoading} dataSource={items} columns={[
          { title: '代理', dataIndex: ['agent', 'name'] },
          { title: '金额', dataIndex: 'amount', render: commissionRmb },
          { title: '状态', dataIndex: 'status' },
          { title: '备注', dataIndex: 'remark' },
          { title: '创建时间', dataIndex: 'createdAt', render: dateTime },
...
function SettlementSummary({ items }: { items: any[] }) {
  const pending = items.filter(item => item.status === 'PENDING');
  const settled = items.filter(item => item.status === 'SETTLED');
  ...
  return (
    <div className="grid4">
      <Card><Statistic title="对账单数" value={items.length} /></Card>
      <Card><Statistic title="结算总额" value={rmbNumber(sumBy(items, item => item.amount))} precision={2} prefix="￥" /></Card>
      <Card><Statistic title="待结单数" value={pending.length} /></Card>
      <Card><Statistic title="已结金额" value={rmbNumber(sumBy(settled, item => item.amount))} precision={2} prefix="￥" /></Card>
      <Card><Statistic title="已取消" value={cancelled.length} /></Card>
    </div>
  );
}

function AgentCreditReconciliationTab() {
  return <AgentReconciliationTable admin title="代理充值积分对账单" showSummary agentCreditOnly />;
}
  ...
  return (
    <>
      <div className="formGrid2">
        ...
        <Form.Item name="adapter" label="Adapter" rules={[{ required: true }]}><Input placeholder="openai-image / openai-generations / aiyunzhi-gpt-image-2 / midjourney-imagine / grok-image / gemini-image-generate / gemini-video / gemini-chat" /></Form.Item>
...
function ApplyAntigravityPresetButton() {
  const [open, setOpen] = useState(false);
  const [form] = Form.useForm();
  const qc = useQueryClient();
  const m = useMutation({
    mutationFn: (values: any) => api('/api/admin/model-presets/antigravity/apply', { method: 'POST', body: JSON.stringify(values) }),
    onSuccess: async (data: any) => {
      message.success(`Antigravity/45 已接入：新增 ${data?.createdCount ?? 0} 个，更新 ${data?.updatedCount ?? 0} 个`);
      setOpen(false);
      form.resetFields();
      await Promise.all([
        refreshModelCaches(qc),
        qc.invalidateQueries({ queryKey: ['providers'] }),
      ]);
    },
    onError: err => message.error(err instanceof Error ? err.message : '接入失败'),
  });
  function openModal() {
    form.resetFields();
    form.setFieldsValue({ baseUrl: 'http://45.77.211.38:8317', status: 'ACTIVE', overwrite: true });
    setOpen(true);
  }
  return (
    <>
      <Button onClick={openModal}>接入 Antigravity/45</Button>
      <Modal title="接入 Antigravity/45 模型预设" open={open} onCancel={() => setOpen(false)} onOk={() => form.submit()} confirmLoading={m.isPending} width={680} okText="写入模型配置">
        <Alert
          type="info"
          showIcon
          style={{ marginBottom: 16 }}
          message="会写入 Claude/Gemini 语言模型和 Gemini Flash Image 图片模型；重复执行会更新现有配置，不会创建重复模型。"
        />
        <Form form={form} layout="vertical" onFinish={values => m.mutate(values)}>
          <Form.Item
            name="baseUrl"
            label="45 Base URL"
            tooltip="这里不要带 /v1；后台会按模型分别拼接 /v1/chat/completions 和 /v1beta/models/{model}:generateContent。"
            rules={[{ required: true }]}
          >
            <AutoComplete
            ...
            >
              <Input placeholder="http://45.77.211.38:8317" />
            </AutoComplete>
          </Form.Item>
          <Form.Item
            name="apiKey"
            label="API Key"
            tooltip="首次接入必须填写；后续留空会保留数据库里已有的 Key。"
          >
            <Input.Password placeholder="首次接入填写 45 / Antigravity 的 API Key；留空保留已有 Key" />
          </Form.Item>
          <div className="formGrid2">
            <Form.Item name="status" label="启用状态" rules={[{ required: true }]}>
              <Select options={[{ value: 'ACTIVE', label: '启用' }, { value: 'DISABLED', label: '停用' }]} />
            </Form.Item>
            <Form.Item name="overwrite" label="更新已有模型" valuePropName="checked">
              <Switch checkedChildren="覆盖" unCheckedChildren="仅启用" />
            </Form.Item>
          </div>
        </Form>
      </Modal>
  ...
  return (
    <>
      ...
      <Modal title="新增模型" open={open} onCancel={() => setOpen(false)} onOk={() => form.submit()} confirmLoading={m.isPending} width={640}>
        <Form form={form} layout="vertical" initialValues={{ type: 'IMAGE', adapter: 'openai-edits', baseUrl: 'http://localhost:8317/v1', uploadMode: '', imageResolutions: defaultImageResolutionSupportKeys(), imageAspectRatios: defaultImageAspectRatioSupportKeys() }} onFinish={values => m.mutate(values)}>
...
function SimpleModelFormFields({ editing = false, model }: { editing?: boolean; model?: any }) {
  ...
  const watchedNick = Form.useWatch('modelNick', form);
  const selectedType = (watchedType || form.getFieldValue('type') || model?.type || 'IMAGE') as 'IMAGE' | 'VIDEO' | 'LLM';
  const selectedAdapter = String(watchedAdapter ?? form.getFieldValue('adapter') ?? model?.adapter ?? model?.provider?.adapter ?? '');
  const selectedModel = String(watchedModel ?? form.getFieldValue('model') ?? model?.name ?? '');
  const selectedNick = String(watchedNick ?? form.getFieldValue('modelNick') ?? model?.displayName ?? '');
  ...
  const adapterOptions = useMemo(() => modelAdapterOptions.filter(option => option.type === selectedType), [selectedType]);
  ...
  const watchedVideoDurations = Form.useWatch('videoDurations', form);
  useEffect(() => {
    const currentAdapter = form.getFieldValue('adapter');
    if (currentAdapter && adapterOptions.some(option => option.value === currentAdapter)) return;
    const modelAdapter = editing ? String(model?.adapter || model?.provider?.adapter || '') : '';
    const fallback = adapterOptions.find(option => option.value === modelAdapter)?.value || adapterOptions[0]?.value;
    if (fallback && fallback !== currentAdapter) form.setFieldValue('adapter', fallback);
  }, [adapterOptions, editing, form, model?.adapter, model?.provider?.adapter]);
	  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const current = normalizeImageAspectRatioSupportValue(form.getFieldValue('imageAspectRatios'));
    if (!current.length) form.setFieldValue('imageAspectRatios', defaultImageAspectRatioSupportKeys());
  }, [form, selectedType]);
  useEffect(() => {
    if (selectedType !== 'VIDEO') return;
    if (Array.isArray(watchedVideoDurations) && watchedVideoDurations.length) return;
    form.setFieldValue('videoDurations', normalizeVideoDurationOptionsValue(model?.capabilities?.durations || model?.capabilities?.videoDurations || model?.capabilities?.allowedDurations || model?.capabilities?.durationOptions));
  }, [form, model?.capabilities, selectedType, watchedVideoDurations]);
  return (
    <>
      ...
      {selectedType === 'IMAGE' && (
        <>
          ...
          <Form.Item
            ...
            label="支持分辨率"
            tooltip="保存到模型 capabilities.resolutions；画布生图节点只显示这里勾选的分辨率。"
            rules={[{
              validator: (_, value) => Array.isArray(value) && value.length
                ? Promise.resolve()
                : Promise.reject(new Error('请至少选择一个分辨率')),
            }]}
          >
		            <Checkbox.Group options={imageResolutionResponseTypeFields.map(item => ({ value: item.key, label: item.label }))} />
		          </Form.Item>
          <Form.Item
            name="imageAspectRatios"
            label="支持比例"
            tooltip="保存到模型 capabilities.aspectRatios；画布生图节点只显示这里勾选的比例。"
            rules={[{
              validator: (_, value) => Array.isArray(value) && value.length
                ? Promise.resolve()
                : Promise.reject(new Error('请至少选择一个比例')),
            }]}
          >
            <Checkbox.Group options={imageAspectRatioResponseTypeFields.map(item => ({ value: item.key, label: item.label }))} />
          </Form.Item>
	        </>
	      )}
      {selectedType === 'VIDEO' && (
        <>
          <Form.Item
            name="videoDurations"
            label="可选视频时长（秒）"
            tooltip="勾选后保存到模型 capabilities.durations；生视频节点的时长下拉框只显示这里勾选的秒数。"
            rules={[{
              validator: (_, value) => Array.isArray(value) && value.length
                ? Promise.resolve()
                : Promise.reject(new Error('请至少选择一个视频时长')),
            }]}
          >
            <Checkbox.Group options={videoDurationCheckboxOptions} />
          </Form.Item>
          <div className="formGrid2">
...
const modelAdapterOptions = [
  ...
  { value: 'openai-generations', label: 'OpenAI 图片生成 /v1/images/generations', type: 'IMAGE' },
  { value: 'openai-edits', label: 'OpenAI 图片编辑 /images/edits', type: 'IMAGE' },
  { value: 'aiyunzhi-gpt-image-2', label: 'Aiyunzhi GPT Image 2 /v1/images/generations + edits', type: 'IMAGE' },
  { value: 'openai-responses-image', label: 'OpenAI Responses 生图 /responses', type: 'IMAGE' },
  { value: 'openai-chat-image', label: 'OpenAI Chat 生图 /v1/chat/completions', type: 'IMAGE' },
  ...
  { value: 'midjourney-imagine', label: 'Midjourney Imagine /mj/submit/imagine', type: 'IMAGE' },
  { value: 'grok_image', label: 'Grok 生图统一 自动文生图/图生图', type: 'IMAGE' },
  { value: 'sora-video', label: 'Sora v3 视频 /videos + /videos/{id}', type: 'VIDEO' },
  { value: 'seedance2', label: 'Artifex Seedance 2 /videos + /videos/{id}', type: 'VIDEO' },
  { value: 'seedance-task', label: 'Seedance 2.0 Task /api/v3/contents/generations/tasks', type: 'VIDEO' },
  { value: 'seedance2-sd', label: 'SD / Seedance 2.0 /video/generations', type: 'VIDEO' },
  { value: 'seedance2-vip', label: 'Seedance 2.0 VIP 视频 /videos + /videos/{id}', type: 'VIDEO' },
  { value: 'lingdong-sd-2-vip', label: '灵动 sd-2-vip 专属折扣 /videos + /video/generations/{id}', type: 'VIDEO' },
  { value: 'zaomeng-seedance2', label: '造梦 Seedance 2.0 /v1/video/generations + /v1/videos/{id}', type: 'VIDEO' },
  { value: 'toapis-seedance2', label: 'ToAPIs Seedance 2 /v1/videos/generations', type: 'VIDEO' },
  ...
  { value: 'grok-llm', label: 'Grok LLM /chat/completions', type: 'LLM' },
...
const upstreamBaseUrlOptions = [
  { value: 'http://127.0.0.1:8086', label: 'Midjourney Proxy 本机 - http://127.0.0.1:8086' },
  { value: 'http://124.156.137.236:8086', label: 'Midjourney Proxy 124 - http://124.156.137.236:8086' },
  { value: 'https://api.aiid.edu.kg', label: 'AIID Seedance - https://api.aiid.edu.kg' },
  { value: 'http://45.77.211.38:8317', label: 'Antigravity / CLIProxy 45 - http://45.77.211.38:8317' },
...
const imageResponseTypeOptions = [
  ...
  { value: 'server_async_object_storage', label: '124 异步转存 COS（先预览）' },
  { value: 'server_object_storage', label: '124 同步服务器转存 COS' },
  { value: 'server_base64_object_storage', label: '124 base64 转存 COS（返回预览）' },
  { value: 'server_base64_async_object_storage', label: '124 base64 异步转存 COS（先本地预览）' },
...
const imageAspectRatioResponseTypeFields = [
  { key: 'auto', label: 'auto' },
  { key: '1:1', label: '1:1' },
  { key: '3:2', label: '3:2' },
  { key: '2:3', label: '2:3' },
...
function defaultImageResolutionSupportKeys() {
  return imageResolutionResponseTypeFields.map(item => item.key);
}

function normalizeImageResolutionSupportValue(value: any) {
  const source = Array.isArray(value)
    ? value
    : (value && typeof value === 'object'
      ? Object.entries(value).filter(([, enabled]) => Boolean(enabled)).map(([key]) => key)
      : String(value || '').split(/[,\s/]+/));
  const allowed = new Set(defaultImageResolutionSupportKeys());
  const normalized = Array.from(new Set(source
    .map(item => String(item || '').trim().toLowerCase())
    .filter(item => allowed.has(item))));
  return normalized.length ? normalized : defaultImageResolutionSupportKeys();
}

function defaultImageAspectRatioSupportKeys() {
  return imageAspectRatioResponseTypeFields.map(item => item.key);
}

function normalizeImageAspectRatioSupportKey(value: any) {
  return String(value || '')
    .trim()
    .toLowerCase()
    .replace('：', ':')
    .replace('×', 'x')
    .replace(/^(\d+(?:\.\d+)?)x(\d+(?:\.\d+)?)$/i, '$1:$2');
}

function normalizeImageAspectRatioSupportValue(value: any) {
  const source = Array.isArray(value)
    ? value
    : (value && typeof value === 'object'
      ? Object.entries(value).filter(([, enabled]) => Boolean(enabled)).map(([key]) => key)
      : String(value || '').split(/[,\s/]+/));
  const allowed = new Set(defaultImageAspectRatioSupportKeys());
  return Array.from(new Set(source
    .map(normalizeImageAspectRatioSupportKey)
    .filter(item => allowed.has(item))));
}
...
const videoDurationCheckboxOptions = Array.from({ length: 15 }, (_, index) => {
  const value = index + 1;
...
function normalizeSimpleModelFormValues(model: any) {
  ...
  return {
    type: modelType,
    adapter: model.adapter || model.provider?.adapter || 'openai-image',
    ...
    imageResolutions: normalizeImageResolutionSupportValue(capabilities.resolutions || capabilities.imageResolutions || capabilities.image_resolutions),
    imageAspectRatios: (() => {
      const ratios = normalizeImageAspectRatioSupportValue(capabilities.aspectRatios || capabilities.aspect_ratios || capabilities.ratios || capabilities.sizeRatios || capabilities.size_ratios);
      return ratios.length ? ratios : defaultImageAspectRatioSupportKeys();
    })(),
    videoDurations: normalizeVideoDurationOptionsValue(capabilities.durations || capabilities.videoDurations || capabilities.video_durations || capabilities.allowedDurations || capabilities.allowed_durations || capabilities.durationOptions || capabilities.duration_options),
...
function normalizeSimpleModelSubmitValues(values: any, editing = false) {
  ...
  const defaultResponseType = modelType === 'VIDEO' ? 'provider_url' : 'object_storage';
    ...
    imageResolutions: normalizeImageResolutionSupportValue(values.imageResolutions),
    imageAspectRatios: normalizeImageAspectRatioSupportValue(values.imageAspectRatios),
    videoDurations: normalizeVideoDurationOptionsValue(values.videoDurations),
  ...
  if (payload.type === 'LLM') {
    ...
    delete payload.imageResolutions;
    delete payload.imageAspectRatios;
...
function normalizeVideoDurationOptionsValue(value: any) {
  const source = Array.isArray(value)
    ? value
    : (value && typeof value === 'object'
      ? Object.entries(value).filter(([, enabled]) => Boolean(enabled)).map(([key]) => key)
      : String(value || '').split(/[,\s/]+/));
  const normalized = Array.from(new Set(source
    .map(item => normalizePositiveIntegerValue(item))
    .filter(item => item >= 1 && item <= 15)))
    .sort((a, b) => a - b);
  return normalized.length ? normalized : videoDurationCheckboxOptions.map(item => item.value);
}
...
function ModelPricingFormFields({ model }: { model: any }) {
  ...
  const watchedVideoBillingMode = Form.useWatch('videoBillingMode');
  const common = (
    <>
  ...
  if (type === 'VIDEO') {
    ...
    const billingMode = watchedVideoBillingMode || videoBillingModeFromModel(model);
    const isPerGeneration = billingMode === 'per_generation';
    const hasScenarioPricing = supportsVideoInputScenarioPricing(model) && !isPerGeneration;
    return (
      <>
        <div className="formGrid2">{common}</div>
        <div className="formGrid2">
          <Form.Item name="videoBillingMode" label="视频计费方式" rules={[{ required: true, message: '请选择视频计费方式' }]}>
            <Select options={[
              { value: 'per_second', label: '按秒计费' },
              { value: 'per_generation', label: '按次计费' },
            ]} />
          </Form.Item>
          {isPerGeneration ? (
            <Form.Item name="videoFlatPrice" label="默认会员积分/次（兜底）" rules={[{ required: true, message: '请填写按次默认价格' }]}>
              <InputNumber min={0} precision={0} style={{ width: '100%' }} />
            </Form.Item>
          ) : null}
        </div>
        {isPerGeneration ? (
          <div className="formGrid2">
            {resolutions.map(resolution => (
              <Form.Item key={resolution} name={['videoGenerationTierPrices', resolution]} label={`${resolution}会员积分/次`} rules={[{ required: true, message: `请填写 ${resolution} 次价` }]}>
                <InputNumber min={0} precision={0} style={{ width: '100%' }} />
              </Form.Item>
            ))}
          </div>
          ...
          <div style={{ display: 'grid', gap: 12 }}>
            {resolutions.map(resolution => (
              <div key={resolution} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12 }}>
                <Form.Item name={['videoScenarioTierPrices', resolution, 'noVideo']} label={`${resolution}无视频输出/秒`} rules={[{ required: true, message: `请填写 ${resolution} 无视频输出秒价` }]}>
                  <InputNumber min={0} precision={0} style={{ width: '100%' }} />
                </Form.Item>
                <Form.Item name={['videoScenarioTierPrices', resolution, 'withVideoOutput']} label={`${resolution}有视频输出/秒`} rules={[{ required: true, message: `请填写 ${resolution} 有视频输出秒价` }]}>
                  <InputNumber min={0} precision={0} style={{ width: '100%' }} />
                </Form.Item>
                <Form.Item name={['videoScenarioTierPrices', resolution, 'withVideoInput']} label={`${resolution}有视频输入/秒`} rules={[{ required: true, message: `请填写 ${resolution} 有视频输入秒价` }]}>
                  <InputNumber min={0} precision={0} style={{ width: '100%' }} />
                </Form.Item>
              </div>
            ))}
          ...
          <div className="formGrid2">
            {!isPerGeneration && resolutions.map(resolution => (
        ...
        <div style={{ color: '#888', fontSize: 12 }}>
          {hasScenarioPricing ? 'ToAPIs Seedance 2 原价按 9 折价自动反推，成本按会员价 70% 自动写入；' : '原价按会员价 4 折自动反推，成本按会员价 70% 自动写入；'}
          {isPerGeneration ? '按次计费时按用户选择的分辨率档位扣每次价格；未命中档位时使用默认兜底价。' : '按秒计费时，扣费按生成秒数和所选分辨率计算。'}
        </div>
      </>
    );
  }
...
function videoTierGenerationMemberPrice(tier: any) {
  const value = Number(
...
function supportsVideoInputScenarioPricing(model: any) {
  ...
  if (text.includes('toapis') && text.includes('seedance')) return true;
  const pricing = model?.defaults?.pricing && typeof model.defaults.pricing === 'object' ? model.defaults.pricing : {};
  const tiers = Array.isArray(pricing?.resolutionTiers) ? pricing.resolutionTiers : [];
  return tiers.some((tier: any) => Array.isArray(tier?.scenarioTiers) && tier.scenarioTiers.length > 0);
}

function videoScenarioPriceLabel(tier: any, resolutionLabel: string) {
  const noVideo = videoScenarioTier(tier, 'no_video_input');
  const withVideo = videoScenarioTier(tier, 'with_video_input');
  if (!noVideo && !withVideo) return '';
  const label = String(resolutionLabel || tier?.resolution || tier?.label || '').trim();
  const noVideoPrice = videoScenarioMemberPrice(noVideo);
  const withOutput = videoScenarioPartPrice(withVideo, ['memberOutputCreditsPerSecond', 'chargedOutputCreditsPerSecond', 'outputCreditsPerSecond']);
  const withInput = videoScenarioPartPrice(withVideo, ['memberInputCreditsPerSecond', 'chargedInputCreditsPerSecond', 'inputCreditsPerSecond']);
  const withTotal = videoScenarioMemberPrice(withVideo);
  if (!label) return '';
  const parts = [];
  if (Number.isFinite(noVideoPrice)) parts.push(`无视频 ${noVideoPrice}`);
  if (Number.isFinite(withOutput) || Number.isFinite(withInput)) {
    parts.push(`有视频 ${Number.isFinite(withOutput) ? withOutput : 0}+${Number.isFinite(withInput) ? withInput : 0}`);
  } else if (Number.isFinite(withTotal)) {
    parts.push(`有视频 ${withTotal}`);
  }
  return parts.length ? `${label} ${parts.join(' / ')}` : '';
}

function videoScenarioTier(tier: any, scenario: 'no_video_input' | 'with_video_input') {
  const tiers = Array.isArray(tier?.scenarioTiers) ? tier.scenarioTiers : [];
  const byFlag = tiers.find((item: any) => typeof item?.hasVideoInput === 'boolean' && item.hasVideoInput === (scenario === 'with_video_input'));
  if (byFlag) return byFlag;
  return tiers.find((item: any) => normalizeVideoScenarioKey(item?.scenario || item?.key || item?.name || item?.label) === scenario) || null;
}

function normalizeVideoScenarioKey(value: unknown) {
  const raw = String(value || '').trim().toLowerCase();
...
function videoScenarioMemberPrice(tier: any) {
  if (!tier) return undefined;
  const direct = Number(tier.memberCreditsPerSecond ?? tier.chargedCreditsPerSecond ?? tier.creditsPerSecond ?? tier.pricePerSecond);
  if (Number.isFinite(direct) && direct >= 0) return direct;
  const output = videoScenarioPartPrice(tier, ['memberOutputCreditsPerSecond', 'chargedOutputCreditsPerSecond', 'outputCreditsPerSecond']);
  const input = videoScenarioPartPrice(tier, ['memberInputCreditsPerSecond', 'chargedInputCreditsPerSecond', 'inputCreditsPerSecond']);
  if (!Number.isFinite(output) && !Number.isFinite(input)) return undefined;
  const outputValue = Number.isFinite(output) ? Number(output) : 0;
  const inputValue = Number.isFinite(input) ? Number(input) : 0;
  return outputValue + inputValue;
}

function videoScenarioPartPrice(tier: any, keys: string[]) {
  if (!tier) return undefined;
  for (const key of keys) {
...
function videoBillingModeFromModel(model: any) {
  const pricing = model?.defaults?.pricing && typeof model.defaults.pricing === 'object' ? model.defaults.pricing : {};
  ...
  if (['generation', 'per_generation', 'per-generation', 'flat', 'request', 'per_request', 'per-request'].includes(unit)) return 'per_generation';
  if (['generation', 'per_generation', 'per-generation', 'flat', 'request', 'per_request', 'per-request'].includes(mode)) return 'per_generation';
  return 'per_second';
}

function videoFlatMemberPrice(model: any) {
  const pricing = model?.defaults?.pricing && typeof model.defaults.pricing === 'object' ? model.defaults.pricing : {};
  const value = Number(
...
function normalizeModelPricingFormValues(model: any) {
  ...
  const videoScenarioTierPrices = Object.fromEntries(videoPricingResolutionList(model).map(resolution => {
    const item = videoPricingTier(model, resolution);
    const noVideo = videoScenarioTier(item, 'no_video_input');
    const withVideo = videoScenarioTier(item, 'with_video_input');
    const fallback = videoTierMemberPrice(item) ?? Number(model.pricePerSecond || 0);
    const withTotal = videoScenarioMemberPrice(withVideo);
    return [resolution, {
      noVideo: videoScenarioPartPrice(noVideo, ['memberOutputCreditsPerSecond', 'chargedOutputCreditsPerSecond', 'outputCreditsPerSecond']) ?? videoScenarioMemberPrice(noVideo) ?? fallback,
      withVideoOutput: videoScenarioPartPrice(withVideo, ['memberOutputCreditsPerSecond', 'chargedOutputCreditsPerSecond', 'outputCreditsPerSecond']) ?? (Number.isFinite(withTotal) ? Math.ceil(Number(withTotal) / 2) : fallback),
      withVideoInput: videoScenarioPartPrice(withVideo, ['memberInputCreditsPerSecond', 'chargedInputCreditsPerSecond', 'inputCreditsPerSecond']) ?? (Number.isFinite(withTotal) ? Math.floor(Number(withTotal) / 2) : fallback),
    }];
  }));
  const videoGenerationTierPrices = Object.fromEntries(videoPricingResolutionList(model).map(resolution => {
    const item = videoPricingTier(model, resolution);
    return [resolution, videoTierGenerationMemberPrice(item) ?? videoFlatMemberPrice(model)];
  }));
  return {
    type: model.type,
    unit: model.unit,
    ...
    videoBillingMode: videoBillingModeFromModel(model),
    videoFlatPrice: videoFlatMemberPrice(model),
    videoGenerationTierPrices,
    imageTierPrices,
    videoTierPrices,
    videoScenarioTierPrices,
...
function normalizeModelPricingSubmitValues(model: any, values: any) {
  const { imageTierPrices, videoTierPrices, videoGenerationTierPrices, videoScenarioTierPrices, videoBillingMode, videoFlatPrice, ...rest } = values;
  ...
  if (type === 'VIDEO') return normalizeVideoPricingSubmitValues(model, rest, videoTierPrices || {}, videoBillingMode, videoFlatPrice, videoGenerationTierPrices || {}, videoScenarioTierPrices || {});
...
function normalizeVideoPricingSubmitValues(
  model: any,
  rest: any,
  videoTierPrices: Record<string, unknown>,
  videoBillingMode: unknown,
  videoFlatPrice: unknown,
  videoGenerationTierPrices: Record<string, unknown>,
  videoScenarioTierPrices: Record<string, any>,
) {
  const existingDefaults = model?.defaults && typeof model.defaults === 'object' && !Array.isArray(model.defaults) ? model.defaults : {};
  const existingPricing = existingDefaults.pricing && typeof existingDefaults.pricing === 'object' && !Array.isArray(existingDefaults.pricing) ? existingDefaults.pricing : {};
  if (String(videoBillingMode || '').trim() === 'per_generation') {
    const defaultMemberCredits = Math.max(0, Number(videoFlatPrice || 0));
    const members = videoPricingResolutionList(model)
      .map(resolution => {
        const raw = videoGenerationTierPrices[resolution] ?? defaultMemberCredits;
        return { resolution, value: Number(raw) };
      })
      .filter(item => Number.isFinite(item.value) && item.value >= 0);
    const resolutionTiers = members.map(item => ({
      ...
      originalCreditsPerGeneration: memberOriginal(item.value),
      costCreditsPerGeneration: memberCost(item.value),
    }));
    const fallbackMemberCredits = Number.isFinite(defaultMemberCredits)
      ? defaultMemberCredits
      : Number(resolutionTiers[0]?.chargedCreditsPerGeneration || 0);
    const originalCredits = memberOriginal(fallbackMemberCredits);
    const costCredits = memberCost(fallbackMemberCredits);
    return {
      type: rest.type,
      status: rest.status,
      unit: 'generation',
      salePrice: fallbackMemberCredits,
      pricePerSecond: 0,
      costPrice: costCredits,
      defaults: {
        ...existingDefaults,
        pricing: {
          ...existingPricing,
          ...
          currency: existingPricing.currency || 'credits',
          memberDiscountRate: 0.4,
          chargedCreditsPerGeneration: fallbackMemberCredits,
          memberCreditsPerGeneration: fallbackMemberCredits,
  ...
  if (supportsVideoInputScenarioPricing(model)) {
    return normalizeVideoScenarioPricingSubmitValues(model, rest, videoScenarioTierPrices, existingDefaults, existingPricing);
  }
  const members = videoPricingResolutionList(model)
...
function normalizeVideoScenarioPricingSubmitValues(model: any, rest: any, videoScenarioTierPrices: Record<string, any>, existingDefaults: any, existingPricing: any) {
  const members = videoPricingResolutionList(model)
    .map(resolution => {
      const row = videoScenarioTierPrices?.[resolution] || {};
      return {
        resolution,
        noVideo: Number(row.noVideo),
        withVideoOutput: Number(row.withVideoOutput),
        withVideoInput: Number(row.withVideoInput),
      };
    })
    .filter(item => Number.isFinite(item.noVideo) && item.noVideo >= 0
      && Number.isFinite(item.withVideoOutput) && item.withVideoOutput >= 0
      && Number.isFinite(item.withVideoInput) && item.withVideoInput >= 0);
  if (!members.length) return rest;
  const resolutionTiers = members.map(item => {
    const withVideoMember = item.withVideoOutput + item.withVideoInput;
    const withVideoOriginalOutput = toapisOriginal(item.withVideoOutput);
    const withVideoOriginalInput = toapisOriginal(item.withVideoInput);
    return {
      resolution: item.resolution,
      chargedCreditsPerSecond: item.noVideo,
      memberCreditsPerSecond: item.noVideo,
      originalCreditsPerSecond: toapisOriginal(item.noVideo),
      costCreditsPerSecond: memberCost(item.noVideo),
      scenarioTiers: [
        {
          scenario: 'no_video_input',
          label: '无视频输入',
          hasVideoInput: false,
          outputCreditsPerSecond: item.noVideo,
          inputCreditsPerSecond: 0,
          chargedCreditsPerSecond: item.noVideo,
          memberCreditsPerSecond: item.noVideo,
          originalOutputCreditsPerSecond: toapisOriginal(item.noVideo),
        ...
        },
        {
          ...
          label: '有视频输入',
          hasVideoInput: true,
          outputCreditsPerSecond: item.withVideoOutput,
          inputCreditsPerSecond: item.withVideoInput,
          chargedCreditsPerSecond: withVideoMember,
          memberCreditsPerSecond: withVideoMember,
          originalOutputCreditsPerSecond: withVideoOriginalOutput,
          originalInputCreditsPerSecond: withVideoOriginalInput,
          originalCreditsPerSecond: withVideoOriginalOutput + withVideoOriginalInput,
          costCreditsPerSecond: memberCost(withVideoMember),
        },
      ],
    };
  });
  ...
  return {
    ...
    defaults: {
      ...
      pricing: {
        ...
        billingMode: 'per_second',
        currency: existingPricing.currency || 'credits',
        creditsPerCny: existingPricing.creditsPerCny || 100,
...
function toapisOriginal(member: number) {
  return Math.max(member, Math.ceil(member / 0.9));
}
...
function AgentRedemptions() {
  ...
  return <Table style={{ marginTop: 16 }} rowKey="id" loading={q.isLoading} dataSource={q.data?.items || []} columns={[
    ...
    { title: '套餐', dataIndex: ['plan', 'name'] },
    { title: '佣金', dataIndex: 'commissionAmount', render: commissionRmb },
...
function AgentSubAgents() {
  ...
  return <Card title="下级代理" extra={<Space><GrantSubAgentMembershipCardsButton /><CreateSubAgentButton /></Space>}>
    ...
    { title: '核销数', dataIndex: ['stats', 'redemptionCount'] },
    { title: '佣金', dataIndex: ['stats', 'commissionAmount'], render: commissionRmb },
    { title: '待结', dataIndex: ['stats', 'pendingCommissionAmount'], render: commissionRmb },
    { title: '状态', dataIndex: 'status' },
  ]} />
  </Card>;
}
...
function EnterpriseTasks() {
  ...
  return <Card title="企业生成任务">
    ...
    <Table rowKey="id" loading={q.isLoading} dataSource={apiListItems(q.data)} scroll={{ x: 1200 }} columns={[
      ...
      { title: '进度', dataIndex: 'progress', width: 160, render: (v: number, row: any) => <Progress percent={progressValue(v, row.status)} size="small" status={progressStatus(row.status)} /> },
