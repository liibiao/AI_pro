...
async function upsertGrokVideoModel(providerIdValue: string) {
  ...
  const defaultVideoModelName = process.env.CLIPROXY_XAI_VIDEO_MODEL || 'grok-imagine-video';
  const defaultVideoMaxDurationSeconds = /grok-imagine-video-1\.5(?:-preview|-2026-05-30)?/i.test(defaultVideoModelName) ? 15 : 10;
  ...
  const defaultDurations = Array.from({ length: defaultVideoMaxDurationSeconds }, (_, index) => index + 1);
  const defaultCapabilities = {
    ...
    maxVideoDurationSeconds: defaultVideoMaxDurationSeconds,
    max_video_duration_seconds: defaultVideoMaxDurationSeconds,
    maxImageReferenceDurationSeconds: defaultVideoMaxDurationSeconds,
    max_image_reference_duration_seconds: defaultVideoMaxDurationSeconds,
    maxVideoDurationSecondsByMode: { 'text-to-video': defaultVideoMaxDurationSeconds, 'image-to-video': defaultVideoMaxDurationSeconds, 'reference-to-video': defaultVideoMaxDurationSeconds },
    max_video_duration_seconds_by_mode: { 'text-to-video': defaultVideoMaxDurationSeconds, 'image-to-video': defaultVideoMaxDurationSeconds, 'reference-to-video': defaultVideoMaxDurationSeconds },
    maxDurationByMode: { 'text-to-video': defaultVideoMaxDurationSeconds, 'image-to-video': defaultVideoMaxDurationSeconds, 'reference-to-video': defaultVideoMaxDurationSeconds },
