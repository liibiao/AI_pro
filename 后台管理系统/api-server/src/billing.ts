...
export function calculateCredits(
  ...
  if (model.type === 'VIDEO') {
    const flatPrice = videoFlatPriceForInput(model, input);
    if (flatPrice) {
      ...
      return {
        ...
        pricingResolution: flatPrice.resolution || null,
        discountActive: benefit.discountActive,
        discountRate: benefit.discountRate,
        discountLabel: benefit.label,
        discountExpiresAt: benefit.expiresAt || null,
...
function videoPriceForInput(
  ...
  const resolution = videoResolutionForInput(input);
...
function videoFlatPriceForInput(
  ...
  const configuredMember = Number(
    matched?.memberCreditsPerGeneration ??
    matched?.chargedCreditsPerGeneration ??
    matched?.creditsPerGeneration ??
    matched?.pricePerGeneration ??
    matched?.memberCreditsPerRequest ??
    matched?.chargedCreditsPerRequest ??
    matched?.creditsPerRequest ??
    matched?.pricePerRequest ??
  ...
  const original = Number(
    matched?.originalCreditsPerGeneration ??
    matched?.listCreditsPerGeneration ??
    matched?.standardCreditsPerGeneration ??
    matched?.originalCreditsPerRequest ??
  ...
  const cost = Number(
    matched?.costCreditsPerGeneration ??
    matched?.costCreditsPerRequest ??
    matched?.costPerGeneration ??
    matched?.costPerRequest ??
...
function videoResolutionForInput(input: CreditInput) {
  const candidate = [
    input.resolution,
    input.imageSize,
    input.size,
    stringParam(input.params, [
      'resolution',
      'requestedResolution',
      'videoResolution',
      'video_resolution',
  ...
  ].find(hasNonEmptyValue);
  return normalizeVideoResolution(candidate);
}
