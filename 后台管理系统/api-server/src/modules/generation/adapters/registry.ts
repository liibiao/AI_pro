...
const registry: Record<string, GenerationAdapter> = {
  ...
  'aiyunzhi-gpt-image-2': { submit: submitAiyunzhiGptImage2 },
...
const SEEDANCE_FULL_RESILIENT_STATUS_HOST = '103.236.54.113:8081';
const SEEDANCE_FULL_STATUS_RETRY_DELAYS_MS = parseDelayMsList(process.env.SEEDANCE_FULL_STATUS_RETRY_DELAYS_MS, [
  1000, 1500, 2000, 3000, 4000,
  5000, 6000, 8000, 10000, 12000,
  15000, 15000, 20000, 20000, 30000,
]);
const SEEDANCE_FULL_STATUS_REQUEST_TIMEOUT_MS = Math.max(3000, Number(process.env.SEEDANCE_FULL_STATUS_REQUEST_TIMEOUT_MS || 12000));
...
async function submitAiyunzhiGptImage2(ctx: AdapterContext): Promise<AdapterSubmitResult> {
  ...
  if (!refGroups.length) {
    const upstream = await callUpstreamJson(ctx.provider, resolveEndpointPath(resolveAiyunzhiGptImage2Endpoint(ctx, 'generation'), ctx), requestJson, ctx.timeoutMs);
    return normalizeImageSubmit(ctx.provider, upstream, requestJson, resolveImageResponseType(ctx));
  }
  ...
  const upstream = await callUpstreamMultipart(ctx.provider, resolveEndpointPath(resolveAiyunzhiGptImage2Endpoint(ctx, 'edit'), ctx), form, ctx.timeoutMs);
  return normalizeImageSubmit(ctx.provider, upstream, requestPreview, resolveImageResponseType(ctx));
}
...
async function querySeedanceFullVideo(ctx: AdapterContext & { upstreamTaskId: string }): Promise<AdapterQueryResult> {
  ...
  try {
    const upstream = await callSeedanceFullStatusJson(ctx, endpoint);
    return normalizeSeedanceFullTaskResult(ctx, upstream, { taskId: ctx.upstreamTaskId } as Prisma.InputJsonValue);
  } catch (err) {
    if (!isSeedanceFullResilientStatusProvider(ctx) || !isSeedanceFullTransientStatusError(err)) throw err;
    const message = err instanceof Error ? err.message : String(err || 'unknown error');
    console.warn(`[generation seedance-full] status query still unreachable after resilient reconnects, keep task running: ${message}`);
    return seedanceFullTransientRunningResult(ctx, message);
  }
}

async function callSeedanceFullStatusJson(ctx: AdapterContext & { upstreamTaskId: string }, endpoint: string) {
  if (!isSeedanceFullResilientStatusProvider(ctx)) {
    return callUpstreamGetJson(ctx.provider, endpoint, {}, ctx.timeoutMs);
  }
  let lastErr: unknown;
  const attempts = SEEDANCE_FULL_STATUS_RETRY_DELAYS_MS.length + 1;
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      return await callSeedanceFullStatusJsonOnce(ctx, endpoint);
    } catch (err) {
      lastErr = err;
      if (!isSeedanceFullTransientStatusError(err) || attempt >= attempts) throw err;
      const delayMs = SEEDANCE_FULL_STATUS_RETRY_DELAYS_MS[attempt - 1] || 0;
      const message = err instanceof Error ? err.message : String(err || 'unknown error');
      console.warn(`[generation seedance-full] transient status query network error, reconnect ${attempt}/${attempts - 1} after ${delayMs}ms: ${message}`);
...
async function callSeedanceFullStatusJsonOnce(ctx: AdapterContext & { upstreamTaskId: string }, endpoint: string) {
  const apiKey = decryptSecret(ctx.provider.apiKeyEncrypted);
  if (!apiKey) fail(400, '上游 API Key 未配置', 'PROVIDER_KEY_MISSING');
  const url = new URL(buildEndpoint(ctx.provider.baseUrl, endpoint));
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), SEEDANCE_FULL_STATUS_REQUEST_TIMEOUT_MS);
  try {
    const resp = await fetch(url, {
      method: 'GET',
      headers: {
    ...
    const data = parseSeedanceFullStatusJson(text);
    if (!resp.ok) {
      const message = extractErrorMessage(data) || `上游请求失败 HTTP ${resp.status}`;
    ...
    if (err instanceof Error && err.name === 'AbortError') {
      throw new HttpError(504, `Seedance2.0 满血状态查询超时（${SEEDANCE_FULL_STATUS_REQUEST_TIMEOUT_MS}ms）`, 'UPSTREAM_TIMEOUT');
    }
    throw err;
  } finally {
    clearTimeout(timer);
...
function parseSeedanceFullStatusJson(text: string) {
...
function isSeedanceFullResilientStatusProvider(ctx: AdapterContext) {
  const raw = String(ctx.provider.baseUrl || '').trim();
  if (raw.includes(SEEDANCE_FULL_RESILIENT_STATUS_HOST)) return true;
  try {
    const url = new URL(raw);
    const port = url.port || (url.protocol === 'https:' ? '443' : '80');
    return `${url.hostname}:${port}` === SEEDANCE_FULL_RESILIENT_STATUS_HOST;
  } catch {
    return false;
  }
}

function isSeedanceFullTransientStatusError(err: unknown) {
  const message = err instanceof Error ? err.message : String(err || '');
  if (/connect\s+ETIMEDOUT|ETIMEDOUT|ECONNRESET|ECONNREFUSED|EAI_AGAIN|ENOTFOUND|socket hang up|connection reset|network|fetch failed|timeout|timed out|aborted/i.test(message)) return true;
  if (err instanceof HttpError) {
    return err.code === 'UPSTREAM_NETWORK_ERROR'
...
function seedanceFullTransientRunningResult(ctx: AdapterContext & { upstreamTaskId: string }, message: string): AdapterQueryResult {
  const responseJson = {
    status: 'query_retrying',
    taskId: ctx.upstreamTaskId,
    message: `Seedance2.0 满血状态查询网络暂时不可达，任务保持运行并等待下一轮轮询：${message}`,
  } as Prisma.InputJsonValue;
  return {
    status: 'RUNNING',
    upstreamTaskId: ctx.upstreamTaskId,
    upstreamRequestId: ctx.upstreamTaskId,
    responseJson,
    resultJson: { url: '', outputs: [], status: 'query_retrying' } as Prisma.InputJsonValue,
    resultUrls: [],
  };
}
...
function resolveSeedance2ModelName(ctx: AdapterContext) {
  const sdModel = String(ctx.model.name || ctx.provider.defaultModel || ctx.params.model || '').trim();
  ...
  const mappedModel = resolveSeedance2ModelByResolution(ctx, requestedResolution);
  if (mappedModel) return mappedModel;
  if (isArtifexSeedance2ModelName(sdModel)) return sdModel.toLowerCase();
  if (/^(?:sd2-(?:720p|1080p)(?:-[a-z0-9._]+)*|seedance-2)$/i.test(sdModel)) return sdModel.toLowerCase();
  ...
  const sd2Variant = /^sd2-([a-z0-9][a-z0-9._-]*)$/i.exec(sdModel);
  if (sd2Variant) return `sd2-${requestedResolution}-${sd2Variant[1].toLowerCase()}`;
  const allowed = ['video-fast-480p', 'video-fast-720p', 'video-pro-480p', 'video-pro-720p'];
  const raw = sdModel.trim().toLowerCase();
...
function resolveSeedance2ModelByResolution(ctx: AdapterContext, resolution: string) {
  const protocol = isRecord(ctx.model.protocol) ? ctx.model.protocol : {};
  const defaults = isRecord(ctx.model.defaults) ? ctx.model.defaults : {};
  const providerDefaults = isRecord(ctx.provider.defaultParams) ? ctx.provider.defaultParams : {};
  const modelByResolution = firstDefined(
  ...
  if (!isRecord(modelByResolution)) return '';
  return String(modelByResolution[resolution] || modelByResolution[resolution.toLowerCase()] || '').trim().toLowerCase();
}
...
function isAiyunzhiSeedance2Protocol(ctx: AdapterContext, modelName = resolveSeedance2ModelName(ctx)) {
  ...
  return /(?:^|\s)(?:sd2-(?:720p|1080p)(?:-[a-z0-9._]+)*|sd2-[a-z0-9._-]+|seedance-2)(?:\s|$)/.test(` ${hay} `)
