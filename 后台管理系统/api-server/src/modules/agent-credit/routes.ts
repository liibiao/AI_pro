...
const directRechargeSchema = voucherRequestSchema.omit({ validDays: true });

router.post('/agent/credit/direct-recharge', requireAuth, asyncHandler(async (req, res) => {
  const agent = await currentAgent(req.user!.id);
  const body = directRechargeSchema.parse(req.body);
  const payload = await applyCreditsFromPaidAmount(body);
  const result = await agentDirectRechargeCredits({ agent, ...payload });
  ok(res, result);
}));
