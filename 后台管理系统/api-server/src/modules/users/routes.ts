...
type ExpiringMembershipUserIdRow = { id: string };
type CountRow = { total: bigint | number | null };

router.get('/', requireAuth, requireRole(adminRoles), asyncHandler(async (req, res) => {
  const page = Math.max(Number(req.query.page || 1), 1);
  const pageSize = Math.min(Math.max(Number(req.query.pageSize || 20), 1), 100);
  const keyword = String(req.query.keyword || '').trim();
  const membershipExpiringDays = parseExpiringDays(req.query.membershipExpiringDays);
  if (membershipExpiringDays) {
    const result = await listUsersWithExpiringMemberships({ page, pageSize, keyword, days: membershipExpiringDays });
    ok(res, result);
    return;
  }
  const where = keyword
...
async function listUsersWithExpiringMemberships(input: { page: number; pageSize: number; keyword: string; days: number }) {
  const now = new Date();
  const expiresBefore = addDays(now, input.days);
  const keywordPattern = `%${input.keyword}%`;
  const keywordSql = input.keyword
    ? Prisma.sql`AND (u."phone" LIKE ${keywordPattern} OR u."email" LIKE ${keywordPattern} OR u."nickname" LIKE ${keywordPattern})`
    : Prisma.empty;
    ...
    prisma.$queryRaw<CountRow[]>`
      WITH latest_memberships AS (
        SELECT
          ...
          MAX(um."expired_at") AS "latestExpiredAt"
        FROM "user_memberships" um
        WHERE um."expired_at" > ${now}
        GROUP BY um."user_id"
        HAVING MAX(um."expired_at") <= ${expiresBefore}
      )
      SELECT COUNT(*) AS "total"
      FROM "users" u
      INNER JOIN latest_memberships lm ON lm."userId" = u."id"
      WHERE 1 = 1
      ${keywordSql}
    `,
    prisma.$queryRaw<ExpiringMembershipUserIdRow[]>`
      WITH latest_memberships AS (
        SELECT
          um."user_id" AS "userId",
          MAX(um."expired_at") AS "latestExpiredAt"
      ...
      WHERE 1 = 1
      ${keywordSql}
      ORDER BY lm."latestExpiredAt" ASC, u."created_at" DESC
      LIMIT ${input.pageSize}
      OFFSET ${offset}
    `,
  ]);
  const ids = idRows.map(row => row.id);
  if (!ids.length) return { items: [], total: Number(countRows[0]?.total || 0), page: input.page, pageSize: input.pageSize };
  ...
  const items = ids.map(id => usersById.get(id)).filter(Boolean);
  return { items, total: Number(countRows[0]?.total || 0), page: input.page, pageSize: input.pageSize };
}

function parseExpiringDays(value: unknown) {
