...
WORK_ROOT="/tmp/aiyunzhi-response-format-adapter-fix-$$"
BACKUP_ROOT="/var/www/ai-admin/backups/aiyunzhi-response-format-adapter-fix-$(date +%Y%m%d%H%M%S)"
...
grep -q "function resolveAiyunzhiGptImage2ResponseType" "$REG"
grep -q "if (responseType === 'base64')" "$REG"
grep -q "return 'url'" "$REG"
...
echo "Aiyunzhi response_format adapter fix deployed."
