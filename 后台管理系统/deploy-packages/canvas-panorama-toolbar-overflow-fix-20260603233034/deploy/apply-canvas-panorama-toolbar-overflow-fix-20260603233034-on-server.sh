...
PKG="canvas-panorama-toolbar-overflow-fix-20260603233034"
...
log "verify markers"
grep -Fq "tapnow-rewrite.css?v=20260603-panorama-toolbar-overflow-fix" "$HTML_DEST"
grep -Fq "keep every panorama toolbar capsule inside the toolbar border" "$CSS_DEST"
grep -Fq ".node.node-type-imageToPanorama .pano-gen-bar>select" "$CSS_DEST"
grep -Fq ".node.node-type-imageToPanorama .pano-gen-bar>.credit-estimate-pill" "$CSS_DEST"
grep -Fq ".node.node-type-imageToPanorama .pano-gen-bar>.aio-go" "$CSS_DEST"
