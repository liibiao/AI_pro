...
export async function listGenerationTasksForPrincipal(principal: OpenApiPrincipal, limit: unknown, offset: unknown = 0, filters: { clientRequestId?: unknown } = {}) {
  ...
  const items = tasks.filter(task => task.userId === ownerUserId).map(task => sanitizeGenerationTaskListItem(task));
...
router.get('/admin/tasks', requireAuth, requireRole(adminRoles), asyncHandler(async (req, res) => {
  ...
  if (requestedCategory) {
    if (requestedCategory === 'IMAGE' || requestedCategory === 'VIDEO') {
      ...
      ok(res, {
        items: tasks.map(task => sanitizeGenerationTaskListItem(task as unknown as Record<string, unknown>, requestedCategory)),
  ...
  ok(res, {
    items: tasks.map(task => sanitizeGenerationTaskListItem(task)),
    pagination: adminGenerationPagination(take, skip, total, skip + tasks.length < total),
  });
}));
