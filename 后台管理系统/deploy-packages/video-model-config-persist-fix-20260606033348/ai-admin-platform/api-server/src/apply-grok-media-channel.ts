...
async function upsertGrokVideoModel(providerIdValue: string) {
  ...
  const defaultSupports = { txt2video: true, img2video: true };
  ...
  const updateData: Prisma.AiModelUncheckedUpdateInput = overwriteExistingGrokMedia ? {
    ...
    providerId: providerIdValue,
    name: existing?.name || 'grok-imagine-video',
    ...
    supports: mergeJsonObjects(defaultSupports, jsonObject(existing?.supports)) as Prisma.InputJsonValue,
    defaults: mergeJsonObjects(defaultDefaults as Record<string, unknown>, jsonObject(existing?.defaults)) as Prisma.InputJsonValue,
    capabilities: mergeJsonObjects(defaultCapabilities, jsonObject(existing?.capabilities)) as Prisma.InputJsonValue,
