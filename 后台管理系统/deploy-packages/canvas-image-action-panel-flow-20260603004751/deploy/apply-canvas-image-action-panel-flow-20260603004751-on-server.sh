...
PKG="canvas-image-action-panel-flow-20260603004751"
...
log "verify markers"
grep -Fq "tapnow-rewrite.css?v=20260603-image-action-panel-flow" "$HTML_DEST"
grep -Fq "const IMAGE_ACTION_REGISTRY=" "$HTML_DEST"
grep -Fq "function renderImageActionPanel(n)" "$HTML_DEST"
grep -Fq "async function executeSingleImageAction(id)" "$HTML_DEST"
grep -Fq "function findSpawnPositionNearNode(sourceId,type" "$HTML_DEST"
grep -Fq "setSingleImageActionPanel(n.id,'edit','');" "$HTML_DEST"
grep -Fq "setSingleImageActionPanel(sourceId,'panorama','panorama');" "$HTML_DEST"
grep -Fq "await executeSingleImageAction(n.id)" "$HTML_DEST"
grep -Fq ".image-action-toolbar" "$CSS_DEST"
grep -Fq ".image-action-config" "$CSS_DEST"
grep -Fq ".image-action-run" "$CSS_DEST"
