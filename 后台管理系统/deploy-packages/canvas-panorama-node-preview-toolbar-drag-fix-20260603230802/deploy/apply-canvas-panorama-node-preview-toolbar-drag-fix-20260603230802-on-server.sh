...
PKG="canvas-panorama-node-preview-toolbar-drag-fix-20260603230802"
...
log "verify markers"
grep -Fq "tapnow-rewrite.css?v=20260603-panorama-node-preview-toolbar-drag-fix" "$HTML_DEST"
grep -Fq "function renderNodePano(id)" "$HTML_DEST"
grep -Fq "const showFallback=()=>{canvas.style.opacity='0'" "$HTML_DEST"
grep -Fq "uploadPanoTextureTo(st,st.img)" "$HTML_DEST"
grep -Fq "stopPanoDragButtonClick" "$HTML_DEST"
grep -Fq "keep the panorama generator controls centered" "$CSS_DEST"
grep -Fq ".pano-node-drag" "$CSS_DEST"
