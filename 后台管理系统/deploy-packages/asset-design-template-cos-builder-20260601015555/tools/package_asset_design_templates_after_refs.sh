...
if len(items) < 17:
    raise SystemExit(f"COS 映射不足 17 条，当前 {len(items)} 条")
for item in items[:17]:
...
本包已完成 17 张参考图到 COS 的映射，并写入新画布资产设计节点。
...
test -f "\$MANIFEST" || { echo "未检测到 17 张参考图 COS 映射 manifest.json，拒绝部署占位缩略图" >&2; exit 2; }
...
test "\$MANIFEST_COUNT" -ge 17 || { echo "COS 映射不足 17 条，当前 \$MANIFEST_COUNT 条，拒绝部署" >&2; exit 2; }
