...
PKG="generation-gpt-image-retry-grok-video-timeout-fix-20260610104602"
...
grep -Fq "OPENAI_RESPONSES_IMAGE_RETRY_DELAYS_MS" "$SRC_ROOT/api-server/src/modules/generation/adapters/registry.ts"
grep -Fq "callOpenAiResponsesImageJson" "$SRC_ROOT/api-server/dist/modules/generation/adapters/registry.js"
grep -Fq "grokVideoMaxRunningMinutes" "$SRC_ROOT/api-server/src/config.ts"
grep -Fq "GROK_VIDEO_TASK_TIMEOUT" "$SRC_ROOT/api-server/dist/modules/generation/routes.js"

log "backup dir: $BACKUP_DIR"
run_sudo mkdir -p "$BACKUP_DIR"

log "install api files"
install_file "api-server/src/config.ts" "$API_DIR/src/config.ts"
install_file "api-server/dist/config.js" "$API_DIR/dist/config.js"
...
run_sudo grep -Fq "callOpenAiResponsesImageJson" "$API_DIR/dist/modules/generation/adapters/registry.js"
run_sudo grep -Fq "GROK_VIDEO_TASK_TIMEOUT" "$API_DIR/dist/modules/generation/routes.js"
run_sudo grep -Fq "grokVideoMaxRunningMinutes" "$API_DIR/dist/config.js"
curl -fsS http://127.0.0.1:4000/api/health >/dev/null 2>&1 && log "api health: ok" || log "api health: skipped/failed"

log "done"
echo "backup: $BACKUP_DIR"
echo "GPT image retry and Grok video timeout fix deployed"
