...
PKG="${1:-/tmp/video-frame-thumbnails-proxy-fix-20260521.tar.gz}"
...
WORKDIR="$(mktemp -d /tmp/video-frame-thumbnails-proxy-fix-XXXXXX)"
...
grep -n "inline thumbnails failed" "$CANVAS_DIR/tools/workbench-web/image-studio-canvas-next.html"
grep -n "captureReadableVideoFrame(video,src" "$CANVAS_DIR/tools/workbench-web/image-studio-canvas-next.html"
