...
PKG="canvas-panorama-toolbar-center-fix-20260602230115"
...
log "verify markers"
grep -Fq "tapnow-rewrite.css?v=20260602-merged-panorama-viewer-toolbar-center" "$HTML_DEST"
grep -Fq "function imageToPanoramaViewerReady(n)" "$HTML_DEST"
grep -Fq "const genBar=viewerReady?'':" "$HTML_DEST"
grep -Fq "Keep the panorama generation toolbar centered and prevent the run button from overlapping controls." "$CSS_DEST"
grep -Fq ".node.node-type-imageToPanorama .pano-gen-layout>.pano-gen-bar" "$CSS_DEST"
grep -Fq "align-self:center!important" "$CSS_DEST"
grep -Fq "width:max-content!important" "$CSS_DEST"
grep -Fq "justify-content:center!important" "$CSS_DEST"
grep -Fq "position:static!important" "$CSS_DEST"
grep -Fq "width:178px!important" "$CSS_DEST"
grep -Fq "width:72px!important" "$CSS_DEST"
grep -Fq "width:64px!important" "$CSS_DEST"
