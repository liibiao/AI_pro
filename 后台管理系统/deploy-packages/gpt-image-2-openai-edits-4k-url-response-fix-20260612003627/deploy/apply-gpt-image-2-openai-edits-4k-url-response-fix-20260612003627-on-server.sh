...
PKG="gpt-image-2-openai-edits-4k-url-response-fix-20260612003627"
...
grep -Fq "delete request.size" "$REG_SRC"
grep -Fq "aspect_ratio: resolveGptImage2AspectRatio" "$REG_SRC"
grep -Fq "resolution: resolveGptImage2Resolution" "$REG_SRC"
grep -Fq "return '/images/generations';" "$REG_SRC"
grep -Fq "delete request.size" "$REG_DIST"
grep -Fq "aspect_ratio: resolveGptImage2AspectRatio" "$REG_DIST"
grep -Fq "resolution: resolveGptImage2Resolution" "$REG_DIST"
grep -Fq "return '/images/generations';" "$REG_DIST"
...
run_sudo grep -Fq "delete request.size" "$APP_DIR/api-server/dist/modules/generation/adapters/registry.js"
run_sudo grep -Fq "aspect_ratio: resolveGptImage2AspectRatio" "$APP_DIR/api-server/dist/modules/generation/adapters/registry.js"
run_sudo grep -Fq "resolution: resolveGptImage2Resolution" "$APP_DIR/api-server/dist/modules/generation/adapters/registry.js"
run_sudo grep -Fq "return '/images/generations';" "$APP_DIR/api-server/dist/modules/generation/adapters/registry.js"
