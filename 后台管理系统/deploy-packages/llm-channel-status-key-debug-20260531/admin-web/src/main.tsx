...
function ModelTypeTable({ loading, items }: { loading: boolean; items: any[] }) {
  ...
  return <Table rowKey="id" loading={loading || toggleStatus.isPending} dataSource={items} scroll={{ x: 1200 }} columns={[
    ...
    { title: '渠道', render: (_, row) => row.provider?.providerKey || row.providerKey || '-' },
    ...
    { title: '模型状态', dataIndex: 'status', render: value => <Tag color={value === 'ACTIVE' ? 'green' : 'red'}>{value === 'ACTIVE' ? '启用' : '禁用'}</Tag> },
    { title: '渠道状态', render: (_, row) => <Tag color={row.provider?.status === 'ACTIVE' ? 'green' : 'red'}>{row.provider?.status === 'ACTIVE' ? '启用' : '禁用'}</Tag> },
    {
      title: '操作',
      width: 300,
  ...
  return (
    <>
      ...
      <Modal title="新增模型" open={open} onCancel={() => setOpen(false)} onOk={() => form.submit()} confirmLoading={m.isPending} width={640}>
        <Form form={form} layout="vertical" initialValues={{ type: 'IMAGE', adapter: 'openai-edits', baseUrl: 'http://localhost:8317/v1', uploadMode: '', status: 'ACTIVE' }} onFinish={values => m.mutate(values)}>
  ...
  return (
    <>
      ...
      <Form.Item
        name="status"
        label="模型/渠道状态"
        tooltip="启用会同时启用该模型和它绑定的上游渠道；停用会让联调和画布调用不再选择它。"
        rules={[{ required: true }]}
      >
...
function normalizeSimpleModelFormValues(model: any) {
  ...
  return {
    ...
    status: model.provider?.status === 'DISABLED' || model.status === 'DISABLED' ? 'DISABLED' : 'ACTIVE',
