...
PKG="${1:-/tmp/llm-channel-status-key-debug-20260531.tar.gz}"
APP_DIR="${APP_DIR:-/var/www/ai-admin/ai-admin-platform}"
PM2_APP="${PM2_APP:-ai-admin-api}"
PM2_USER="${PM2_USER:-ubuntu}"
STAMP="$(date +%Y%m%d%H%M%S)"
WORK="$(mktemp -d /tmp/llm-channel-status-key-debug-20260531-XXXXXX)"
...
tar -xzf "$PKG" -C "$WORK"
SRC="$WORK/llm-channel-status-key-debug-20260531"
...
BACKUP_ROOT="/var/www/ai-admin/backups/llm-channel-status-key-debug-20260531-$STAMP"
...
run_sudo grep -q "模型/渠道状态" "$APP_DIR/admin-web/dist/assets/"*.js
