...
PKG="${1:-/tmp/prompt-zoom-editor-20260531234308.tar.gz}"
...
WORK="$(mktemp -d /tmp/prompt-zoom-editor-20260531234308-XXXXXX)"
BACKUP="/var/www/ai-admin/backups/prompt-zoom-editor-20260531234308-$STAMP"
...
tar -xzf "$PKG" -C "$WORK"
SRC="$WORK/prompt-zoom-editor-20260531234308"
...
log "verify markers"
run_sudo grep -q "function openPromptZoomEditor" "$WORKBENCH_DIR/image-studio-canvas-next.html"
run_sudo grep -q "prompt-zoom-btn" "$WORKBENCH_DIR/image-studio-canvas-next.html"
run_sudo grep -q "bindPromptZoomControls(el,n)" "$WORKBENCH_DIR/image-studio-canvas-next.html"
