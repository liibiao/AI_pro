...
PKG="seedance2-two-models-20260608151814"
...
remove_old_file(){
  ...
  if [ -f "$dest" ]; then
    backup_file "$dest"
    run_sudo rm -f "$dest"
    log "removed old $dest"
  fi
}

log "verify package markers"
grep -Fq "requestedResolution" "$SRC_ROOT/api-server/dist/modules/generation/adapters/registry.js"
grep -Fq "resolutions: ['480p', '720p']" "$SRC_ROOT/api-server/dist/modules/models/routes.js"
grep -Fq "resolutionTiers" "$SRC_ROOT/api-server/dist/sync-canvas-models.js"
grep -Fq "seedance2-fast" "$SRC_ROOT/workbench-web/model-registry.json"
grep -Fq "SD 2.0 Fast" "$SRC_ROOT/workbench-web/models/seedance2-fast.json"
grep -Fq "video-pro-{resolution}" "$SRC_ROOT/workbench-web/models/seedance2-pro.json"
...
install_optional "api-server/src/modules/generation/routes.ts" "$APP_DIR/api-server/src/modules/generation/routes.ts"
install_optional "api-server/dist/modules/generation/routes.js" "$APP_DIR/api-server/dist/modules/generation/routes.js"
install_optional "api-server/src/modules/workbench-compat/routes.ts" "$APP_DIR/api-server/src/modules/workbench-compat/routes.ts"
install_optional "api-server/dist/modules/workbench-compat/routes.js" "$APP_DIR/api-server/dist/modules/workbench-compat/routes.js"
...
for model_file in seedance2-fast.json seedance2-pro.json; do
  install_file "workbench-web/models/$model_file" "$WEB_ROOT/workbench-web/models/$model_file"
done
for old_model_file in seedance2-fast-480p.json seedance2-fast-720p.json seedance2-pro-480p.json seedance2-pro-720p.json; do
  remove_old_file "$WEB_ROOT/workbench-web/models/$old_model_file"
done
...
if run_sudo test -d "$MIRROR_ROOT"; then
  ...
  for old_model_file in seedance2-fast-480p.json seedance2-fast-720p.json seedance2-pro-480p.json seedance2-pro-720p.json; do
    remove_old_file "$MIRROR_ROOT/tools/workbench-web/models/$old_model_file"
  done
  ...
  for old_model_file in seedance2-fast-480p.json seedance2-fast-720p.json seedance2-pro-480p.json seedance2-pro-720p.json; do
    remove_old_file "$MIRROR_ROOT/smart-vision/canvas/legacy-workbench/workbench-web/models/$old_model_file"
  done
fi
...
log "upsert seedance2 provider and collapsed models"
...
const modelRows = [
  {
    ...
    cnyPerSecond: 0.35,
    resolutionTiers: [
      { resolution: '480p', cnyPerSecond: 0.27, chargedCreditsPerSecond: 27, originalCreditsPerSecond: 27, costCreditsPerSecond: 18.9 },
      { resolution: '720p', cnyPerSecond: 0.35, chargedCreditsPerSecond: 35, originalCreditsPerSecond: 35, costCreditsPerSecond: 24.5 },
    ],
    modelAssembly: { type: 'template', template: 'video-fast-{resolution}' },
  },
  {
    id: 'canvas-seedance2-pro',
    modelKey: 'seedance2-pro',
    name: 'video-pro',
    displayName: 'SD 2.0 Pro',
    badge: 'PRO',
    badgeColor: '#22c55e',
    pricePerSecond: 52,
    cnyPerSecond: 0.52,
    resolutionTiers: [
      { resolution: '480p', cnyPerSecond: 0.35, chargedCreditsPerSecond: 35, originalCreditsPerSecond: 35, costCreditsPerSecond: 24.5 },
      { resolution: '720p', cnyPerSecond: 0.52, chargedCreditsPerSecond: 52, originalCreditsPerSecond: 52, costCreditsPerSecond: 36.4 },
    ],
...
for (const row of modelRows) {
  const pricing = {
    ...
    chargedCreditsPerSecond: row.pricePerSecond,
    originalCreditsPerSecond: row.pricePerSecond,
    costCreditsPerSecond: Number((row.pricePerSecond * 0.7).toFixed(2)),
    grossMarginRate: 0.3,
    resolutionTiers: row.resolutionTiers,
  };
  const capabilities = {
  ...
  const modelData = {
    ...
    defaults: { pricing },
    capabilities,
    modelAssembly: row.modelAssembly,
...
const rows = await prisma.aiModel.findMany({
  where: {
    OR: [
      { id: { in: [...oldModelIds, ...modelRows.map(row => row.id)] } },
      { providerId: provider.id, adapter: 'seedance2' },
    ],
  },
  orderBy: { id: 'asc' },
...
console.log(JSON.stringify({
  disabledOldModels: disabled.count,
  models: rows.map(row => ({
    id: row.id,
    modelKey: row.modelKey,
    displayName: row.displayName,
    ...
    resolutions: row.capabilities?.resolutions,
  })),
}, null, 2));
...
run_sudo grep -Fq "requestedResolution" "$APP_DIR/api-server/dist/modules/generation/adapters/registry.js"
run_sudo grep -Fq "seedance2-fast" "$WEB_ROOT/workbench-web/model-registry.json"
run_sudo grep -Fq "SD 2.0 Pro" "$WEB_ROOT/workbench-web/models/seedance2-pro.json"
for old_model_file in seedance2-fast-480p.json seedance2-fast-720p.json seedance2-pro-480p.json seedance2-pro-720p.json; do
  run_sudo test ! -f "$WEB_ROOT/workbench-web/models/$old_model_file"
done
