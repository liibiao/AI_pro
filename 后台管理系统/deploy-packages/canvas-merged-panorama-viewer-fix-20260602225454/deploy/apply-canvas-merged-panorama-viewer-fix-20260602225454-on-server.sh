...
PKG="canvas-merged-panorama-viewer-fix-20260602225454"
...
log "verify markers"
grep -Fq "imageToPanorama:{cat:'generate',name:'720全景图'" "$HTML_DEST"
grep -Fq "panoramaViewer:{cat:'output',name:'交互式 720° 全景查看',icon:'PANO',desc:'双击节点沉浸查看 720° 全景图，支持 720° 连续拖拽',w:380,hidden:true" "$HTML_DEST"
grep -Fq "function imageToPanoramaResultUrl(n)" "$HTML_DEST"
grep -Fq "function imageToPanoramaViewerReady(n)" "$HTML_DEST"
grep -Fq "function markImageToPanoramaViewerReady(id)" "$HTML_DEST"
grep -Fq "const genBar=viewerReady?'':" "$HTML_DEST"
grep -Fq "resetImageToPanoramaViewerReady(nd);" "$HTML_DEST"
grep -Fq "markImageToPanoramaViewerReady(id)" "$HTML_DEST"
grep -Fq "tapnow-rewrite.css?v=20260602-merged-panorama-viewer" "$HTML_DEST"
grep -Fq "requestAnimationFrame(()=>focusNodeLarge(panoId));" "$HTML_DEST"
grep -Fq "toast('已打开全景图工具栏，选择模型和参数后点击执行','ok');" "$HTML_DEST"
grep -Fq "const nid=makeNode('imageToPanorama',pos.x,pos.y,{images:panoImages,displayRatio:'default'});" "$HTML_DEST"
grep -Fq "The merged panorama generator must inherit the full 720 viewer surface and director UI." "$CSS_DEST"
grep -Fq ".node-type-imageToPanorama .pano-viewer-bar" "$CSS_DEST"
grep -Fq ".node-type-imageToPanorama.director-mode .pano-char-layer" "$CSS_DEST"

if grep -Fq "await runNode(panoId" "$HTML_DEST"; then
  fail "unexpected panorama auto-run call remains"
fi
if grep -Fq "await confirmAiRefine(targetId)" "$HTML_DEST"; then
  fail "unexpected refine auto-run call remains"
fi
if grep -Fq "target='panoramaViewer'" "$HTML_DEST"; then
