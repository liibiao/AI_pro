...
PKG="${1:-/tmp/text-prompt-agent-name-fix-20260601041419.tar.gz}"
...
WORK="$(mktemp -d /tmp/text-prompt-agent-name-fix-20260601041419-XXXXXX)"
BACKUP="/var/www/ai-admin/backups/text-prompt-agent-name-fix-20260601041419-$STAMP"
...
tar -xzf "$PKG" -C "$WORK"
SRC="$WORK/text-prompt-agent-name-fix-20260601041419"
HTML="$SRC/workbench-web/image-studio-canvas-next.html"

test -f "$HTML" || { echo "部署包缺少 image-studio-canvas-next.html" >&2; exit 1; }

log "verify package markers"
grep -q "agentPackDisplayTitle" "$HTML"
grep -q "title:'智能推演'" "$HTML"
grep -q "Agent 数据包.*智能推演" "$HTML"
...
run_sudo grep -q "agentPackDisplayTitle" "$WORKBENCH_DIR/image-studio-canvas-next.html"
run_sudo grep -q "title:'智能推演'" "$WORKBENCH_DIR/image-studio-canvas-next.html"
run_sudo grep -q "Agent 数据包.*智能推演" "$WORKBENCH_DIR/image-studio-canvas-next.html"
