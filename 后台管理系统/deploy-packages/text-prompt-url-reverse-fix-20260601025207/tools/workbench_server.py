...
def start_video_background_download(task_id: str, output_dir: str | None = None) -> None:
    def worker() -> None:
        try:
            ...
            with urlopen(request, timeout=600) as response, tmp_path.open("wb") as fp:
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/workbench/image-studio/video/content":
            try:
                ...
                try:
                    response_ctx = urlopen(request, timeout=600)
                except HTTPError as exc:
        ...
        if parsed.path == "/":
            ...
            self.path = "/漫剧创作库/tools/workbench-web/image-studio-canvas.html"
