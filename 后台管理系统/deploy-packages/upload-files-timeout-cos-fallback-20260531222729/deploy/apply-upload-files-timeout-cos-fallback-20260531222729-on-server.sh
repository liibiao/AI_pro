...
PKG="${1:-/tmp/upload-files-timeout-cos-fallback-20260531222729.tar.gz}"
...
WORK="$(mktemp -d /tmp/upload-files-timeout-cos-fallback-20260531222729-XXXXXX)"
BACKUP="/var/www/ai-admin/backups/upload-files-timeout-cos-fallback-20260531222729-$STAMP"
...
tar -xzf "$PKG" -C "$WORK"
SRC="$WORK/upload-files-timeout-cos-fallback-20260531222729"
...
log "verify markers"
run_sudo grep -q "upstreamTimeoutMs" "$APP_DIR/api-server/dist/modules/workbench-compat/routes.js"
run_sudo grep -q "FILES失败，切换COS" "$WORKBENCH_DIR/image-studio-canvas-next.html"
run_sudo grep -q "准备 COS 上传" "$WORKBENCH_DIR/image-studio-canvas-next.html"
