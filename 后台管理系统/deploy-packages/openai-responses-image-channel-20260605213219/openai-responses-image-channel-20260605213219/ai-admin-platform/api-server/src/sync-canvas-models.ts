...
async function main() {
  ...
  for (const file of files) {
    ...
    || (['veo-chat', 'veo-3.1', 'openai-responses-image', 'grok-image', 'grok-image-edit', 'sora-video', 'seedance2-vip', 'seedance2.0-vip', 'grok-video', 'gemini-image-generate', 'gemini-image-edit', 'gemini-video', 'veo-video'].includes(adapterValue) ? 'object_storage' : null);
    const requestMethodValue = String(config.protocol?.method || '').trim()
    || (['veo-chat', 'veo-3.1', 'openai-responses-image', 'grok-image', 'grok-image-edit', 'grok-video', 'grok-chat', 'grok-llm', 'gemini-image-generate', 'gemini-image-edit', 'gemini-chat', 'gemini-llm'].includes(adapterValue) ? 'sync' : (['sora-video', 'seedance2-vip', 'seedance2.0-vip', 'gemini-video', 'veo-video'].includes(adapterValue) ? 'async-poll' : null));
