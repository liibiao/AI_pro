...
PKG="image-resolution-support-config-20260610004815"
...
grep -Fq "支持分辨率" "$SRC/ai-admin-platform/admin-web/src/main.tsx"
grep -Fq "imageResolutions" "$SRC/ai-admin-platform/admin-web/src/main.tsx"
grep -Rqs "支持分辨率" "$SRC/ai-admin-platform/admin-web/dist"
grep -Fq "IMAGE_RESOLUTION_SUPPORT_KEYS" "$SRC/ai-admin-platform/api-server/src/modules/models/routes.ts"
grep -Fq "imageResolutions" "$SRC/ai-admin-platform/api-server/dist/modules/models/routes.js"
grep -Fq "normalizeImageResolutionSupportOptions" "$SRC/workbench-web/image-studio-canvas-next.html"
grep -Fq "getImageResolutionOptionsForModel" "$SRC/workbench-web/image-studio-canvas-next.html"
...
run_sudo grep -Fq "支持分辨率" "$REMOTE_APP_ROOT/admin-web/src/main.tsx"
run_sudo grep -Rqs "支持分辨率" "$REMOTE_APP_ROOT/admin-web/dist"
run_sudo grep -Fq "IMAGE_RESOLUTION_SUPPORT_KEYS" "$REMOTE_APP_ROOT/api-server/src/modules/models/routes.ts"
run_sudo grep -Fq "imageResolutions" "$REMOTE_APP_ROOT/api-server/dist/modules/models/routes.js"
run_sudo grep -Fq "normalizeImageResolutionSupportOptions" "$WORKBENCH_DIR/image-studio-canvas-next.html"
run_sudo grep -Fq "getImageResolutionOptionsForModel" "$WORKBENCH_DIR/image-studio-canvas-next.html"
