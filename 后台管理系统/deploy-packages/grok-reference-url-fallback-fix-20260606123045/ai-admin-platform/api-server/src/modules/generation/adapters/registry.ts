...
async function submitGrokUnifiedImage(ctx: AdapterContext): Promise<AdapterSubmitResult> {
  assertImageModelName(ctx);
  const refGroups = await collectPublicImageReferenceUrlGroups(ctx);
...
async function submitGrokImageEdit(ctx: AdapterContext): Promise<AdapterSubmitResult> {
  assertImageModelName(ctx);
  const refGroups = (await collectPublicImageReferenceUrlGroups(ctx)).slice(-3);
...
async function submitGeminiUnifiedImageEdit(ctx: AdapterContext): Promise<AdapterSubmitResult> {
  assertImageModelName(ctx);
  const refGroups = (await collectPublicImageReferenceUrlGroups(ctx)).slice(-4);
...
async function submitGrokVideo(ctx: AdapterContext): Promise<AdapterSubmitResult> {
  const references = buildVideoReferenceUrls(ctx).slice(0, 7);
  ...
  const seconds = resolveVideoDurationSeconds(ctx, resolution, ctx.params.seconds || ctx.params.duration || ctx.params.durationSeconds || ctx.params.duration_seconds, 6, Array.from({ length: 15 }, (_, index) => index + 1));
  const videoParams = withoutKeys(ctx.params, [
    'model', 'prompt', 'stream',
    'duration', 'durationSeconds', 'duration_seconds', 'seconds',
    'aspectRatio', 'aspect_ratio', 'requestedRatio',
  ...
  return {
    status,
    progress: failed || urls.length ? 100 : Number((upstream as any)?.progress || (upstream as any)?.data?.progress || 0),
    upstreamTaskId: taskId,
...
async function resolvePublicImageReferenceCandidate(value: unknown, timeoutMs: number) {
  if (!value) return '';
  if (isRecord(value)) {
    return resolvePublicImageReferenceCandidate(value.url || value.image_url || value.imageUrl || value.remoteUrl, timeoutMs);
  }
  const raw = String(value || '').trim();
  if (!raw) return '';
...
async function resolvePublicImageReferenceCandidateGroup(values: unknown[], timeoutMs: number) {
  const refs: string[] = [];
  for (const value of values) {
    const ref = await resolvePublicImageReferenceCandidate(value, timeoutMs);
    if (ref) refs.push(ref);
  }
  return Array.from(new Set(refs));
}

async function collectPublicImageReferenceUrlGroups(ctx: AdapterContext, timeoutMs = 15000) {
  ...
  for (const rawGroup of rawGroups) {
    const group = await resolvePublicImageReferenceCandidateGroup(rawGroup, timeoutMs);
...
const imageReferenceFetchHeaders = {
...
async function imageResultUrl(provider: UpstreamProvider, result: { url?: string; b64?: string; mimeType?: string }, responseType: ImageResponseType = 'object_storage') {
  ...
  if (responseType === 'server_base64_async_object_storage') {
    if (result.b64) return persistInlineImageLocalPreview(result.b64, result.mimeType || 'image/png');
    if (/^data:image\//i.test(directUrl)) return persistInlineImageLocalPreview(directUrl, result.mimeType || 'image/png');
    return directUrl || '';
  }
