...
export async function agentDirectRechargeCredits(input: {
  agent: Agent;
  userId: string;
  credits: number;
  amountCents?: number;
  ...
  return prisma.$transaction(async tx => {
    ...
    const accountChanged = await tx.agentCreditAccount.updateMany({
      where: { agentId: input.agent.id, availableCredits: { gte: input.credits } },
      data: { availableCredits: { decrement: input.credits }, usedCredits: { increment: input.credits } },
    });
    if (accountChanged.count !== 1) fail(400, '代理可用额度不足', 'AGENT_CREDIT_NOT_ENOUGH');
    const account = await tx.agentCreditAccount.findUnique({ where: { agentId: input.agent.id } });
    ...
    const order = await tx.rechargeOrder.create({
      data: {
        ...
        userId: input.userId,
        channel: 'AGENT_VOUCHER',
        status: 'PAID',
        amountCents: input.amountCents || 0,
        credits: input.credits,
        qrCode: `agent-direct-recharge://${input.agent.id}`,
        paymentTradeNo: input.transferNo || undefined,
        notifyPayload: {
          agentId: input.agent.id,
          agentName: input.agent.name,
          source: 'AGENT_DIRECT_RECHARGE',
          transferChannel: input.transferChannel,
          transferNo: input.transferNo,
          proofImageUrl: input.proofImageUrl,
        },
    ...
    const walletChange = await applyWalletDelta(tx, { userId: input.userId, delta: input.credits });
    ...
    const walletLog = await tx.walletLog.create({
      data: {
        ...
        relatedId: order.id,
        remark: input.remark || `代理 ${input.agent.name} 直接充值 ${input.credits} 积分`,
      },
    });
    ...
    const ledger = await tx.agentCreditLedger.create({
      data: {
        ...
        relatedType: 'AGENT_DIRECT_RECHARGE',
        relatedId: order.id,
        remark: input.remark || `直接给客户 ${input.userId} 充值 ${input.credits} 积分`,
      },
    });

    const reconciliation = await tx.agentReconciliationOrder.create({
      data: {
        orderNo: makeReconciliationOrderNo(),
        agentId: input.agent.id,
        type: 'VOUCHER_REDEEM',
        ...
        relatedType: 'AGENT_DIRECT_RECHARGE',
        relatedId: order.id,
        remark: input.remark || `代理直接充值客户 ${input.userId}`,
      },
    });

    return { order, wallet, walletLog, account, ledger, reconciliation, balance: walletChange.balanceAfter };
  });
}
