...
function GenerationResults() {
  ...
  const q = useQuery({
    queryKey: ['generationResults', activeTab, filters, page],
    queryFn: () => {
      const params = new URLSearchParams();
  ...
  const renderTable = () => <GenerationResultTable
    rows={rows}
    loading={q.isLoading}
  ...
  return (
    <Card title="生图生视频结果列表">
      {errorMessage && <Alert type="error" showIcon style={{ marginBottom: 12 }} message="结果列表加载失败" description={errorMessage} />}
      <Tabs
        activeKey={activeTab}
        onChange={key => { setActiveTab(key as GenerationResultTab); setPage(1); }}
        items={[
          { key: 'IMAGE', label: '图片', children: renderTable() },
          { key: 'ASSET_DESIGN', label: '资产设计', children: renderTable() },
          { key: 'SHOT_STORYBOARD', label: '分镜图', children: renderTable() },
          { key: 'STORYBOARD', label: '故事板', children: renderTable() },
          { key: 'VIDEO', label: '视频', children: renderTable() },
        ]}
      />
    </Card>
  );
...
function GenerationResultTable({ rows, loading, fetching, filters, setFilters, page, pagination, setPage, refetch }: {
  rows: any[];
  loading: boolean;
  fetching: boolean;
    ...
    <>
      <Space style={{ marginBottom: 12 }} wrap>
        <Select
          ...
          onChange={value => { setPage(1); setFilters(prev => ({ ...prev, status: value || '' })); }}
        />
        <Input.Search
          allowClear
          placeholder="用户 UID"
