...
router.post('/register', asyncHandler(async (req, res) => {
  ...
  const token = signToken({ id: user.id, role: user.role, nickname: user.nickname, status: user.status, sessionId, clientType: 'CANVAS' });
...
const loginSchema = z.object({
  ...
  clientType: z.enum(['ADMIN_WEB', 'CANVAS']).optional().default('CANVAS'),
});
...
router.post('/login', asyncHandler(async (req, res) => {
  ...
  const sessionId = newSessionId();
  if (body.clientType === 'CANVAS') {
    await prisma.user.update({ where: { id: user.id }, data: { currentSessionId: sessionId } });
  }
  const token = signToken({ id: user.id, role: user.role, nickname: user.nickname, status: user.status, sessionId, clientType: body.clientType });
  ok(res, { token, user: publicUser(user) });
}));
...
router.post('/logout', requireAuth, asyncHandler(async (req, res) => {
  if (req.user?.sessionId && (req.user.clientType || 'CANVAS') === 'CANVAS') {
