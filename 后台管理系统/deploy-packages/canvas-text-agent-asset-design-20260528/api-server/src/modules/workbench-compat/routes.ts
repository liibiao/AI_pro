...
function resolveCompatImageMode(body: z.infer<typeof imageGenerateSchema>) {
  const text = [body.mode, body.taskType, body.template, body.prompt].filter(Boolean).join(' ').toLowerCase();
  if (/storyboard|故事板|分镜|panel|comic/.test(text)) return 'storyboard';
