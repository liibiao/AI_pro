...
function isOpenApiPath(path: string) {
  ...
  try {
    ...
    return pathname === '/api/open' || pathname.startsWith('/api/open/');
  } catch {
    return raw === '/api/open' || raw.startsWith('/api/open/');
  }
}
...
export async function api<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  try {
    ...
    if (!resp.ok || data.ok === false) {
      ...
      const usedLoginToken = !hasExplicitAuthorization || explicitAuthorization === `Bearer ${token}`;
      if ((resp.status === 401 || data.code === 'SESSION_REPLACED') && usedLoginToken && !isOpenApiPath(path)) {
        clearToken();
