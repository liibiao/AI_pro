...
PKG_NAME="large-paint-open-entry-fix-20260601142051"
...
grep -q "pointerdown','mousedown" "$TARGET"
grep -q "进入大图绘画失败" "$TARGET"
grep -q "openLightboxImage(src,{nodeId:id,entryIdx:0,sourceEl})===false" "$TARGET"
