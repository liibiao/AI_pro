...
function requireAuthWithQueryToken(req: Request, res: Response, next: NextFunction) {
  const token = queryText(req.query.authToken) || queryText(req.query.accessToken) || queryText(req.query.token);
...
router.get('/workbench/image-studio/models', requireAuth, asyncHandler(async (_req, res) => {
  ...
  ok(res, {
    models: models.map(model => {
      ...
      const capabilities = isVideo ? normalizeCompatWorkbenchVideoCapabilities(model) : model.capabilities || {};
      const protocol = {
...
router.post('/workbench/image-studio/video/start', requireAuth, asyncHandler(async (req, res) => {
  ...
  const resolution = compatVideoResolution(body);
  const cost = await calculateCreditsForUser(prisma, req.user!.id, model, {
    quantity: 1,
    durationSeconds,
    inputTokens: 0,
    ...
    if (runtime.generationMockMode) {
      ...
      const submit = await adapter.submit({
        ...
        params: {
          ...
          aspectRatio: body.aspectRatio,
          resolution,
          quality: body.quality || resolution,
          requestedResolution: body.requestedResolution || resolution,
          videoResolution: body.videoResolution || resolution,
          size: body.size,
...
router.get('/workbench/image-studio/video/content', requireAuthWithQueryToken, asyncHandler(async (req, res) => {
  const taskId = String(req.query.taskId || '').trim();
...
router.get('/workbench/image-studio/video/proxy', requireAuthWithQueryToken, asyncHandler(async (req, res) => {
...
async function proxySmartMidjourneyPrompt(body: z.infer<typeof smartMidjourneyPromptSchema>) {
...
function fallbackSmartMidjourneyPrompt(body: z.infer<typeof smartMidjourneyPromptSchema>) {
...
function inferSmartMidjourneyAspectRatio(...values: unknown[]) {
  const text = values.map(value => String(value || '')).join(' ').toLowerCase();
  const explicit = text.match(/\b(\d{1,2})\s*[:x×]\s*(\d{1,2})\b/);
  if (explicit) return `${explicit[1]}:${explicit[2]}`;
  if (/竖屏|纵向|portrait|vertical|手机|短视频|9:16/.test(text)) return '9:16';
  if (/方图|头像|square|1:1/.test(text)) return '1:1';
  if (/海报|poster|3:4|4:5/.test(text)) return '3:4';
  if (/横版|宽屏|电影|cinematic|landscape|16:9/.test(text)) return '16:9';
  return '16:9';
}
...
function isCompatGenerationAdapter(adapter: string, type: CompatModelType) {
  ...
  if (type === 'IMAGE') return ['openai-image', 'openai-generations', 'openai-edits', 'openai-responses-image', 'openai-chat-image', 'aiyunzhi-gpt-image-2', 'gpt-image-v2', 'grok_image', 'grok-image-unified', 'grok-image', 'grok-image-edit', 'gemini-image-generate', 'gemini-image-edit'].includes(value);
  if (type === 'VIDEO') return ['sora-video', 'sora-video-pro', 'seedance-task', 'seedance2', 'seedance2.0', 'seedance2-sd', 'sd-seedance2', 'sd-video', 'toapis-seedance2', 'seedance2-vip', 'seedance2.0-vip', 'lingdong-sd-2-vip', 'zaomeng-seedance2', 'notevideo', 'grok-video', 'gemini-video', 'veo-video'].includes(value);
  return false;
}
...
function defaultEndpointPath(adapter: string, type: CompatModelType) {
  switch (adapter) {
    ...
    case 'openai-image':
    case 'openai-generations': return '/images/generations';
    case 'aiyunzhi-gpt-image-2': return '/v1/images/generations';
...
function normalizeCompatWorkbenchVideoCapabilities(model: { capabilities: Prisma.JsonValue | null; defaults: Prisma.JsonValue | null }) {
  const base = asRecord(model.capabilities);
  const next: Record<string, unknown> = { ...base };
  const disabled = normalizeCompatVideoResolutionList(
    next.disabledVideoResolutions ?? next.disabled_video_resolutions ?? next.disabledResolutions ?? next.disabled_resolutions,
  );
  const disabledSet = new Set(disabled);
  const configured = normalizeCompatVideoResolutionList(next.resolutions ?? next.videoResolutions ?? next.video_resolutions);
  const pricingResolutions = compatPricingVideoResolutions(model);
  const resolutions: string[] = [];
  for (const resolution of [...configured, ...pricingResolutions]) {
    if (disabledSet.has(resolution)) continue;
    if (!resolutions.includes(resolution)) resolutions.push(resolution);
  }
  if (!resolutions.length) {
    for (const resolution of configured.length ? configured : ['720p']) {
      if (!resolutions.includes(resolution)) resolutions.push(resolution);
    }
  }

  const durationMap = normalizeCompatVideoDurationMap(next.durationsByResolution ?? next.durations_by_resolution);
  let durations = normalizeCompatVideoDurationList(
    next.durations ?? next.videoDurations ?? next.video_durations ?? next.allowedDurations ?? next.allowed_durations ?? next.durationOptions ?? next.duration_options,
  );
  const durationUnion = Object.values(durationMap).flat();
  if (!durations.length && durationUnion.length) durations = Array.from(new Set(durationUnion)).sort((a, b) => a - b);
  const fallbackDurations = durations.length ? durations : [5, 10, 15];
  for (const resolution of resolutions) {
    if (!durationMap[resolution]?.length) durationMap[resolution] = [...fallbackDurations];
  }
  durations = Array.from(new Set([...fallbackDurations, ...Object.values(durationMap).flat()])).sort((a, b) => a - b);

  const defaultResolution = normalizeCompatVideoResolution(next.defaultResolution ?? next.default_resolution);
  next.resolutions = resolutions;
  next.videoResolutions = resolutions;
  next.video_resolutions = resolutions;
  next.defaultResolution = resolutions.includes(defaultResolution) ? defaultResolution : resolutions[0];
  ...
  next.durationsByResolution = durationMap;
  next.durations_by_resolution = durationMap;
  if (disabled.length) {
    next.disabledVideoResolutions = disabled;
    next.disabled_video_resolutions = disabled;
    next.disabledResolutions = disabled;
    next.disabled_resolutions = disabled;
  }
  return next;
}

function compatPricingVideoResolutions(model: { defaults: Prisma.JsonValue | null }) {
  const pricing = asRecord(asRecord(model.defaults).pricing);
  const tiers = Array.isArray(pricing.resolutionTiers) ? pricing.resolutionTiers : [];
  return normalizeCompatVideoResolutionList(tiers.map(row => {
    const record = asRecord(row);
    return record.resolution ?? record.label ?? record.quality ?? record.size ?? record.pixelSize ?? record.pixel_size;
  }));
}

function normalizeCompatVideoResolutionList(value: unknown) {
  ...
  const values = source
    .map(item => normalizeCompatVideoResolution(item))
    .filter(item => allowed.has(item));
  return Array.from(new Set(values));
}

function normalizeCompatVideoDurationList(value: unknown) {
...
function normalizeCompatVideoDurationMap(value: unknown) {
  const out: Record<string, number[]> = {};
  for (const [rawKey, rawValue] of Object.entries(asRecord(value))) {
    const key = normalizeCompatVideoResolution(rawKey);
    const durations = normalizeCompatVideoDurationList(rawValue);
    if (key && durations.length) out[key] = durations;
  }
  return out;
}
...
function pickCompatParams(body: Record<string, unknown>) {
  ...
  [
    'size', 'pixelSize', 'pixel_size', 'n', 'background', 'resolution', 'requestedResolution', 'videoResolution', 'video_resolution', 'aspectRatio', 'imageSize', 'image_size',
...
function compatVideoResolution(body: z.infer<typeof videoStartSchema>) {
  const raw = body.resolution
    ...
    || body.size
    || body.pixelSize;
  return normalizeCompatVideoResolution(raw);
}

function normalizeCompatVideoResolution(value: unknown) {
  ...
  if (raw === 'fullhd' || raw === 'fhd') return '1080p';
  if (raw === 'hd') return '720p';
  return raw;
}

function normalizeVideoStatus(upstream: unknown, resultUrl = '', taskId = '') {
  const raw = extractVideoTaskStatus(upstream);
  if (resultUrl) return 'completed';
  if (isCompatVideoDoneStatus(raw)) return 'completed';
  if (taskId && isCompatQueuedOrBusyPayload(upstream)) return 'processing';
...
function isCompatQueuedOrBusyPayload(upstream: unknown) {
  const raw = extractVideoTaskStatus(upstream);
  if (isCompatQueuedOrRunningStatus(raw)) return true;
  return isCompatQueuedOrBusyMessage(extractVideoFailureMessage(upstream));
}

function isCompatQueuedOrRunningStatus(status: string) {
...
function isCompatQueuedOrBusyMessage(message: string) {
