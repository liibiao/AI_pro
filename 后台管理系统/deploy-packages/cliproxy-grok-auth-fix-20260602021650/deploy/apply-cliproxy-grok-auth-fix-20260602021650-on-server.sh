...
PKG="${1:-/tmp/cliproxy-grok-auth-fix-20260602021650.tar.gz}"
PKG_NAME="cliproxy-grok-auth-fix-20260602021650"
INSTALL_DIR="${INSTALL_DIR:-/opt/cliproxy}"
PATCHED_BIN="${PATCHED_BIN:-$INSTALL_DIR/CLIProxyAPI-grok-auth-fix}"
...
tar -xzf "$PKG" -C "$WORK"
SRC="$WORK/$PKG_NAME"
BIN="$SRC/bin/CLIProxyAPI-grok-auth-fix-amd64"
...
if [ -f "$PATCHED_BIN" ]; then
  ...
  run_sudo cp "$PATCHED_BIN" "$INSTALL_DIR/backups/$(basename "$PATCHED_BIN").bak-$STAMP"
fi

log "install patched CLIProxyAPI binary"
run_sudo install -m 0755 "$BIN" "$PATCHED_BIN"
...
verify_xai_route() {
  log "verify xAI auth route is registered"
  STATUS="$(curl -sS -o /tmp/cliproxy-xai-route-check.out -w '%{http_code}' 'http://127.0.0.1:8317/v0/management/xai-auth-url?is_webui=true' || true)"
  case "$STATUS" in
    200|401|403|500)
      log "xAI auth route reachable: HTTP $STATUS"
      ;;
    404)
      echo "xAI auth route still returns 404; deployment did not take effect" >&2
      cat /tmp/cliproxy-xai-route-check.out >&2 || true
      exit 1
      ;;
    *)
      log "xAI auth route check returned HTTP $STATUS; service is up, please verify management auth/proxy logs if needed"
      ;;
  esac
}
...
exec "$PATCHED_BIN" --config "$CONFIG_FILE"
EOF
...
echo "cliproxy Grok/xAI OAuth auth-url fix deployed"
echo "binary: $PATCHED_BIN"
