...
PKG="grok-reference-local-fallback-fix-20260606154831"
...
grep -Fq "CANVAS_LOCAL_ASSET_DIRS" "$REGISTRY_SRC"
grep -Fq "runninghub_outputs/image-studio-v3" "$REGISTRY_SRC"
grep -Fq "err instanceof HttpError && err.code" "$ROUTES_SRC"
grep -Fq "compactUnifiedInputFileObject(img)||{}" "$CANVAS_PUBLIC"
grep -Fq "图生图参考图读取失败" "$CANVAS_PUBLIC"
...
backup_file "$APP_DIR/api-server/src/modules/generation/routes.ts" "$BACKUP_DIR/ai-admin-platform/api-server/src/modules/generation/routes.ts"
backup_file "$APP_DIR/api-server/dist/modules/generation/routes.js" "$BACKUP_DIR/ai-admin-platform/api-server/dist/modules/generation/routes.js"
...
run_sudo grep -Fq "CANVAS_LOCAL_ASSET_DIRS" "$APP_DIR/api-server/dist/modules/generation/adapters/registry.js"
run_sudo grep -Fq "runninghub_outputs/image-studio-v3" "$APP_DIR/api-server/dist/modules/generation/adapters/registry.js"
run_sudo grep -Fq "err instanceof HttpError && err.code" "$APP_DIR/api-server/dist/modules/generation/routes.js"
if run_sudo test -f "$WORKBENCH_DIR/image-studio-canvas-next.html"; then
  run_sudo grep -Fq "图生图参考图读取失败" "$WORKBENCH_DIR/image-studio-canvas-next.html"
fi
