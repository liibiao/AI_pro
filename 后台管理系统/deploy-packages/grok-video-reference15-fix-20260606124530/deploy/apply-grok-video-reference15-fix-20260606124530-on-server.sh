...
PKG="grok-video-reference15-fix-20260606124530"
...
grep -Fq "resolveGrokVideoWorkflowMode" "$REGISTRY_SRC"
grep -Fq "image: grokVideoImageReferencePayload" "$REGISTRY_SRC"
grep -Fq "reference_images: referenceImages.map" "$REGISTRY_SRC"
grep -Fq "grok-imagine-video-1.5-preview" "$GROK_APPLY_SRC"
grep -Fq "maxDurationByMode" "$GROK_APPLY_SRC"
grep -Fq "const GROK_VIDEO_REFERENCE_TO_VIDEO_MAX_DURATION=15;" "$CANVAS_PUBLIC"
grep -Fq "const videoImageInputs=" "$CANVAS_PUBLIC"
...
log "patch Grok video model capabilities in database"
cd "$APP_DIR/api-server"
run_sudo node --input-type=module <<'NODE'
import { prisma } from './dist/db.js';

const durations = Array.from({ length: 15 }, (_, index) => index + 1);
const modeMax = { 'text-to-video': 15, 'image-to-video': 15, 'reference-to-video': 15 };
const modeDurations = {
...
const models = await prisma.aiModel.findMany({
  where: {
    ...
    OR: [
      ...
      { adapter: 'grok-video' },
      { name: { startsWith: 'grok-imagine-video' } },
      { displayName: { contains: 'Grok' } },
    ],
  },
  select: { id: true, name: true, capabilities: true, supports: true },
});
for (const model of models) {
  ...
  await prisma.aiModel.update({
    where: { id: model.id },
    data: {
      ...(model.id === 'cliproxy-grok-video' ? { name: process.env.CLIPROXY_XAI_VIDEO_MODEL || 'grok-imagine-video-1.5-preview' } : {}),
      capabilities: { ...capabilities, ...patch },
      supports: { ...supports, ...supportsPatch },
    },
  });
}
console.log(`patched grok video models: ${models.map(model => `${model.id}/${model.name}`).join(', ')}`);
...
run_sudo grep -Fq "image: grokVideoImageReferencePayload" "$APP_DIR/api-server/dist/modules/generation/adapters/registry.js"
run_sudo grep -Fq "grok-imagine-video-1.5-preview" "$APP_DIR/api-server/dist/apply-grok-media-channel.js"
if run_sudo test -f "$WORKBENCH_DIR/image-studio-canvas-next.html"; then
  run_sudo grep -Fq "const videoImageInputs=" "$WORKBENCH_DIR/image-studio-canvas-next.html"
fi
if run_sudo test -f "$MIRROR_TARGET/tools/workbench-web/image-studio-canvas-next.html"; then
  run_sudo grep -Fq "const videoImageInputs=" "$MIRROR_TARGET/tools/workbench-web/image-studio-canvas-next.html"
fi
