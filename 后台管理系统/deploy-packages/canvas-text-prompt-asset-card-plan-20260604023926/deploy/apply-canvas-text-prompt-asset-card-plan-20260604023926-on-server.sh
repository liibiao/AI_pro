...
PKG="canvas-text-prompt-asset-card-plan-20260604023926"
...
log "verify markers"
grep -Fq "tapnow-rewrite.css?v=20260604-text-prompt-asset-card-plan" "$HTML_DEST"
grep -Fq "data-prompt-action=\"deriveAssets\"" "$HTML_DEST"
grep -Fq "ASSET_CARD_PLAN" "$HTML_DEST"
grep -Fq "function startTextPromptAssetCardDerivation(id)" "$HTML_DEST"
grep -Fq "runAssetCardPlanNodes(created)" "$HTML_DEST"
grep -Fq "text prompt asset-card derivation confirmation table" "$CSS_DEST"
grep -Fq ".asset-card-plan-modal" "$CSS_DEST"
