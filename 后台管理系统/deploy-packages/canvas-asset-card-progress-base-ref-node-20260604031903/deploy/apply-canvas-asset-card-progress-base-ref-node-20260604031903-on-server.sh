...
PKG="canvas-asset-card-progress-base-ref-node-20260604031903"
...
log "verify markers"
grep -Fq "tapnow-rewrite.css?v=20260604-asset-card-progress-base-ref-node" "$HTML_DEST"
grep -Fq "function renderAssetCardDeriveProgress(n)" "$HTML_DEST"
grep -Fq "function renderAssetCardDeriveCreditPill(n)" "$HTML_DEST"
grep -Fq "data-asset-card-base-row" "$HTML_DEST"
grep -Fq "function makeAssetCardReferenceImageNode(sourceId,assetId,card,assetPos,index,total)" "$HTML_DEST"
grep -Fq "Promise.allSettled(live.map" "$HTML_DEST"
grep -Fq "固定资产底图模式需要至少上传一张底图" "$HTML_DEST"
grep -Fq ".asset-card-row-base-upload" "$CSS_DEST"
grep -Fq ".asset-card-derive-progress" "$CSS_DEST"
