...
PKG="${PKG:-/tmp/asset-library-user-isolation-20260529212359.tar.gz}"
...
BACKUP="${BACKUP:-/var/www/ai-admin/backups/asset-library-user-isolation-20260529212359-$STAMP}"
TMP="$(mktemp -d /tmp/asset-library-user-isolation-20260529212359.XXXXXX)"
...
mkdir -p "$BACKUP/workbench-web" \
  "$BACKUP/backend/api-server/src/modules/generation" \
  "$BACKUP/backend/api-server/dist/modules/generation" \
  "$BACKUP/backend/api-server/src/modules/workbench-compat" \
  "$BACKUP/backend/api-server/dist/modules/workbench-compat"
cp -a "$WORKBENCH_DIR/image-studio-canvas-next.html" "$BACKUP/workbench-web/image-studio-canvas-next.html" 2>/dev/null || true
cp -a "$APP_DIR/api-server/src/modules/generation/routes.ts" "$BACKUP/backend/api-server/src/modules/generation/routes.ts" 2>/dev/null || true
cp -a "$APP_DIR/api-server/dist/modules/generation/routes.js" "$BACKUP/backend/api-server/dist/modules/generation/routes.js" 2>/dev/null || true
...
install -m 0644 "$TMP/backend/api-server/src/modules/generation/routes.ts" "$APP_DIR/api-server/src/modules/generation/routes.ts"
install -m 0644 "$TMP/backend/api-server/dist/modules/generation/routes.js" "$APP_DIR/api-server/dist/modules/generation/routes.js"
...
grep -q "ownerUserId" "$APP_DIR/api-server/dist/modules/generation/routes.js"
grep -q "filterAssetTasksForCurrentUser" "$WORKBENCH_DIR/image-studio-canvas-next.html"
