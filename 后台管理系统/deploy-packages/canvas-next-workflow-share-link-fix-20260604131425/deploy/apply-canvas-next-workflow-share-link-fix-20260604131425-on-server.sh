...
PKG="canvas-next-workflow-share-link-fix-20260604131425"
...
log "verify markers"
grep -Fq "data-workflow-share" "$HTML_DEST"
grep -Fq "function ensureProjectWorkflowCloudRecord(localWorkflowId)" "$HTML_DEST"
grep -Fq "function updateLocalProjectWorkflowCloudId(projectId,workflowId,cloudWorkflowId)" "$HTML_DEST"
grep -Fq "cloudWorkflowId" "$HTML_DEST"
grep -Fq "工作流调用链接已复制" "$HTML_DEST"
grep -Fq "workflowShare" "$HTML_DEST"
grep -Fq "function storyboardTableTextFromData(data)" "$HTML_DEST"
grep -Fq "if(isShotSurface)return;" "$HTML_DEST"
