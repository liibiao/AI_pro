  ...
  return (
    <>
      ...
      <Modal title="新增模型" open={open} onCancel={() => setOpen(false)} onOk={() => form.submit()} confirmLoading={m.isPending} width={640}>
        <Form form={form} layout="vertical" initialValues={{ type: 'IMAGE', adapter: 'openai-edits', baseUrl: 'http://localhost:8317/v1', uploadMode: '', imageResolutions: defaultImageResolutionSupportKeys() }} onFinish={values => m.mutate(values)}>
...
function SimpleModelFormFields({ editing = false, model }: { editing?: boolean; model?: any }) {
  ...
  const selectedBaseUrl = String(watchedBaseUrl ?? form.getFieldValue('baseUrl') ?? model?.provider?.baseUrl ?? '');
  ...
  const limitGptImage2ProResolutions = isGptImage2ProOfficialProxyConfig(selectedType, selectedAdapter, selectedModel, selectedNick, selectedBaseUrl);
  const selectedImageResolutionFields = limitGptImage2ProResolutions
    ? imageResolutionResponseTypeFields.filter(item => item.key === '1k' || item.key === '2k')
    : imageResolutionResponseTypeFields;
  ...
  useEffect(() => {
    if (selectedType !== 'IMAGE' || !limitGptImage2ProResolutions) return;
    const allowed = new Set(['1k', '2k']);
    const current = normalizeImageResolutionSupportValue(form.getFieldValue('imageResolutions')).filter(item => allowed.has(item));
    form.setFieldValue('imageResolutions', current.length ? current : ['1k', '2k']);
  }, [form, limitGptImage2ProResolutions, selectedType]);
  useEffect(() => {
    ...
    <>
      ...
      {selectedType === 'IMAGE' && (
        <>
          ...
          >
	            <Checkbox.Group options={selectedImageResolutionFields.map(item => ({ value: item.key, label: item.label }))} />
	          </Form.Item>
        </>
      )}
      {selectedType === 'VIDEO' && (
...
function isGptImage2ProOfficialProxyConfig(type: string, adapter: string, model: string, modelNick: string, baseUrl: string) {
  const text = `${adapter} ${model} ${modelNick} ${baseUrl}`.toLowerCase().replace(/[\s_]+/g, '-');
  if (type !== 'IMAGE') return false;
  if (text.includes('openai-responses-image') || text.includes('openai-chat-image')) return false;
  return text.includes('gpt-image-2-pro')
    || text.includes('canvas-gpt-image-2-pro')
    || ((text.includes('localhost:8317') || text.includes('127.0.0.1:8317') || text.includes('45.77.211.38:8317'))
      && /gpt-image-2/.test(text)
      && /openai-edits|openai-image|gpt-image/.test(text));
}
...
function normalizeSimpleModelSubmitValues(values: any, editing = false) {
  ...
  if (isGptImage2ProOfficialProxyConfig(payload.type, payload.adapter, payload.model, payload.modelNick, payload.baseUrl)) {
    const allowed = new Set(['1k', '2k']);
    const normalized = normalizeImageResolutionSupportValue(payload.imageResolutions).filter(item => allowed.has(item));
    payload.imageResolutions = normalized.length ? normalized : ['1k', '2k'];
  }
