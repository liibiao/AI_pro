...
PKG="gpt-image-2-openai-edits-resolution-model-fix-20260611234913"
...
[ -f "$REG_SRC" ] || fail "package missing api-server/src/modules/generation/adapters/registry.ts"
[ -f "$REG_DIST" ] || fail "package missing api-server/dist/modules/generation/adapters/registry.js"

log "verify package markers"
grep -Fq "withGptImage2ResolutionModel" "$REG_SRC"
grep -Fq "resolveGptImage2ResolutionModelName" "$REG_SRC"
grep -Fq "model: resolveGptImage2ResolutionModelName" "$REG_SRC"
grep -Fq "withGptImage2ResolutionModel" "$REG_DIST"
grep -Fq "resolveGptImage2ResolutionModelName" "$REG_DIST"
grep -Fq "model: resolveGptImage2ResolutionModelName" "$REG_DIST"
...
run_sudo grep -Fq "withGptImage2ResolutionModel" "$APP_DIR/api-server/dist/modules/generation/adapters/registry.js"
run_sudo grep -Fq "resolveGptImage2ResolutionModelName" "$APP_DIR/api-server/dist/modules/generation/adapters/registry.js"
run_sudo grep -Fq "model: resolveGptImage2ResolutionModelName" "$APP_DIR/api-server/dist/modules/generation/adapters/registry.js"
