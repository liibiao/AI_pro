...
function withGptImage2ResolutionModel(ctx: AdapterContext, requestJson: Prisma.InputJsonValue) {
  ...
  return compactJson({
    ...request,
    model: resolveGptImage2ResolutionModelName(ctx, request.model, request.size),
  });
}
...
function resolveGptImage2ResolutionModelName(ctx: AdapterContext, requestModel: unknown, requestSize: unknown) {
  ...
  return resolveGptImageV2ModelName(requestModel || ctx.params.model || ctx.model.name || ctx.provider.defaultModel, resolution);
}
