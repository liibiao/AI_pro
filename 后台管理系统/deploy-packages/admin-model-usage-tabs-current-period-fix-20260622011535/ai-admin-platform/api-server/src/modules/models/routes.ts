...
router.get('/admin/model-usage-stats', requireAuth, requireRole(adminRoles), asyncHandler(async (req, res) => {
  ...
  if (type === 'IMAGE' || type === 'VIDEO') {
    ...
    const rows = await prisma.$queryRaw<ModelUsageStatsRow[]>`
      SELECT
        ${bucket} AS period,
        gt."model_id" AS model_id,
        am."name" AS model_name,
        am."display_name" AS model_display_name,
        gt."channel_key" AS channel_key,
        up."id" AS provider_id,
        up."provider_key" AS provider_key,
      ...
      ORDER BY total_count DESC, success_count DESC, failed_count ASC, model_display_name ASC, channel_key ASC, period DESC
      LIMIT ${take}
    `;
    ok(res, {
      type,
      ...
      items: rows.map(row => ({
        key: `${row.period || 'all'}:${row.channel_key}:${row.model_id}`,
  ...
  const rows = await prisma.$queryRaw<ModelUsageStatsRow[]>`
    SELECT
      ...
      am."name" AS model_name,
      am."display_name" AS model_display_name,
      up."provider_key" AS channel_key,
      up."id" AS provider_id,
      up."provider_key" AS provider_key,
      up."name" AS provider_name,
    ...
    GROUP BY 1, 2, 3, 4, 5, 6, 7, 8
    ORDER BY total_count DESC, success_count DESC, failed_count ASC, model_display_name ASC, provider_key ASC, period DESC
    LIMIT ${take}
  `;
  ok(res, {
    type,
