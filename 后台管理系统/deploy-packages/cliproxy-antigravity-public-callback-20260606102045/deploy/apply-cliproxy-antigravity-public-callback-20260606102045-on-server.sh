...
PKG="${1:-/tmp/cliproxy-antigravity-public-callback-20260606102045.tar.gz}"
PKG_NAME="cliproxy-antigravity-public-callback-20260606102045"
...
verify_routes() {
  log "verify OAuth/media routes are registered"
  verify_not_404 GET "/v0/management/antigravity-auth-url?is_webui=true"
  verify_not_404 POST /v0/management/oauth-callback
...
tar -xzf "$PKG" -C "$WORK"
SRC="$WORK/$PKG_NAME"
BIN="$SRC/bin/CLIProxyAPI-antigravity-public-callback-amd64"
...
log "install CLIProxyAPI binary with Antigravity public callback fix"
