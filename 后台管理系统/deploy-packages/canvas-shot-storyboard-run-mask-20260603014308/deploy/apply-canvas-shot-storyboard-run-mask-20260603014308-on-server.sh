...
PKG="canvas-shot-storyboard-run-mask-20260603014308"
...
log "verify markers"
grep -Fq "tapnow-rewrite.css?v=20260603-shot-storyboard-run-mask" "$HTML_DEST"
grep -Fq "function shotStoryboardRunStage(n)" "$HTML_DEST"
grep -Fq "function renderShotStoryboardRunMask(n)" "$HTML_DEST"
grep -Fq "renderShotStoryboardRunMask(n)" "$HTML_DEST"
grep -Fq ".shot-run-mask" "$CSS_DEST"
grep -Fq ".shot-run-card" "$CSS_DEST"
grep -Fq ".shot-run-step.active" "$CSS_DEST"
