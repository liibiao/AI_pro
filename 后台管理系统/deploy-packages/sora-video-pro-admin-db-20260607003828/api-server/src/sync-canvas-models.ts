...
async function main() {
  ...
  for (const file of files) {
    ...
    || (['veo-chat', 'veo-3.1', 'openai-responses-image', 'openai-chat-image', 'grok_image', 'grok-image-unified', 'grok-image', 'grok-image-edit', 'sora-video', 'sora-video-pro', 'seedance2-vip', 'seedance2.0-vip', 'grok-video', 'gemini-image-generate', 'gemini-image-edit', 'gemini-video', 'veo-video'].includes(adapterValue) ? 'object_storage' : null);
    const requestMethodValue = String(config.protocol?.method || '').trim()
    || (['veo-chat', 'veo-3.1', 'openai-responses-image', 'openai-chat-image', 'grok_image', 'grok-image-unified', 'grok-image', 'grok-image-edit', 'grok-chat', 'grok-llm', 'gemini-image-generate', 'gemini-image-edit', 'gemini-chat', 'gemini-llm'].includes(adapterValue) ? 'sync' : (['sora-video', 'sora-video-pro', 'seedance2-vip', 'seedance2.0-vip', 'grok-video', 'gemini-video', 'veo-video'].includes(adapterValue) ? 'async-poll' : null));
...
function normalizeEndpointConfig(config: CanvasModelConfig, modelType: SyncedModelType) {
  ...
  const fallbackEndpoint = adapter === 'sora-video' || adapter === 'sora-video-pro'
    ? '/videos'
