...
async function submitSoraVideoPro(ctx: AdapterContext): Promise<AdapterSubmitResult> {
  const imageUrls = buildSoraVideoProImageUrls(ctx).slice(0, 9);
  const explicitImageUrl = buildSoraVideoProUrlList(ctx, ['image_url', 'imageUrl', 'firstFrame', 'first_frame'], 'image', 1)[0] || imageUrls[0] || '';
  const videoUrls = buildSoraVideoProUrlList(ctx, [
  ...
  const audioUrls = buildSoraVideoProUrlList(ctx, [
