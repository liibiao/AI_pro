...
function normalizeModelPricingSubmitValues(model: any, values: any) {
  const { imageTierPrices, videoTierPrices, videoBillingMode, videoFlatPrice, ...rest } = values;
  ...
  if (type === 'VIDEO') return normalizeVideoPricingSubmitValues(model, rest, videoTierPrices || {}, videoBillingMode, videoFlatPrice);
...
function normalizeVideoPricingSubmitValues(model: any, rest: any, videoTierPrices: Record<string, unknown>, videoBillingMode: unknown, videoFlatPrice: unknown) {
  const existingDefaults = model?.defaults && typeof model.defaults === 'object' && !Array.isArray(model.defaults) ? model.defaults : {};
  const existingPricing = existingDefaults.pricing && typeof existingDefaults.pricing === 'object' && !Array.isArray(existingDefaults.pricing) ? existingDefaults.pricing : {};
