...
PKG="toapis-seedance2-full-chain-20260613145346"
...
grep -Fq "submitToapisSeedance2Video" "$SRC/api-server/dist/modules/generation/adapters/registry.js"
grep -Fq "toapis-seedance2" "$SRC/api-server/dist/modules/generation/routes.js"
grep -Fq "ToAPIs Seedance 2 /v1/videos/generations" "$SRC/admin-web/src/main.tsx"
grep -Fq "TOAPIS_ENABLE_CHANNELS" "$SRC/api-server/dist/apply-toapis-seedance2-channels.js"
...
log "upsert ToAPIs Seedance 2 channels"
...
const modelIds = ['canvas-toapis-seedance-2', 'canvas-toapis-seedance-2-fast'];
const rows = await prisma.aiModel.findMany({ where: { id: { in: modelIds } }, include: { provider: true }, orderBy: { id: 'asc' } });
for (const id of modelIds) {
  const row = rows.find(item => item.id === id);
  if (!row) throw new Error(`missing ToAPIs model: ${id}`);
  if (row.adapter !== 'toapis-seedance2') throw new Error(`bad adapter for ${id}: ${row.adapter}`);
  if (row.endpointPath !== '/v1/videos/generations') throw new Error(`bad endpoint for ${id}: ${row.endpointPath}`);
  if (row.provider.adapter !== 'toapis-seedance2') throw new Error(`bad provider adapter for ${id}: ${row.provider.adapter}`);
  console.log(`${row.id}: model=${row.name} modelStatus=${row.status} provider=${row.provider.providerKey} providerStatus=${row.provider.status}`);
}
await prisma.$disconnect();
NODE

grep -Fq "submitToapisSeedance2Video" "$APP_DIR/api-server/dist/modules/generation/adapters/registry.js"
grep -Fq "ToAPIs Seedance 2 /v1/videos/generations" "$APP_DIR/admin-web/dist/assets/"*.js
