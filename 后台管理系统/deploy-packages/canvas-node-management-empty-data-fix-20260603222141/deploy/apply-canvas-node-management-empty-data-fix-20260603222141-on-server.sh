...
PKG="canvas-node-management-empty-data-fix-20260603222141"
...
grep -Fq "data: { nodes" "$SRC/ai-admin-platform/api-server/src/modules/canvas-node-config/routes.ts"
grep -Fq "data: { nodes" "$SRC/ai-admin-platform/api-server/dist/modules/canvas-node-config/routes.js"
grep -Fq "canvasNodeConfigItems" "$SRC/ai-admin-platform/admin-web/src/main.tsx"
grep -Fq "画布节点配置加载失败" "$SRC/ai-admin-platform/admin-web/src/main.tsx"
grep -Fq "没有读取到节点配置" "$SRC/ai-admin-platform/admin-web/src/main.tsx"
grep -Fq "canvasNodeIcon" "$SRC/ai-admin-platform/admin-web/src/styles.css"
grep -Rqs "画布节点配置加载失败" "$SRC/ai-admin-platform/admin-web/dist"
grep -Rqs "没有读取到节点配置" "$SRC/ai-admin-platform/admin-web/dist"
...
run_sudo grep -Fq "data: { nodes" "$REMOTE_APP_ROOT/api-server/dist/modules/canvas-node-config/routes.js"
run_sudo grep -Fq "canvasNodeConfigItems" "$REMOTE_APP_ROOT/admin-web/src/main.tsx"
run_sudo grep -Fq "画布节点配置加载失败" "$REMOTE_APP_ROOT/admin-web/src/main.tsx"
run_sudo grep -Rqs "画布节点配置加载失败" "$REMOTE_APP_ROOT/admin-web/dist"
run_sudo grep -Rqs "没有读取到节点配置" "$REMOTE_APP_ROOT/admin-web/dist"
