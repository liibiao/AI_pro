...
router.get('/admin/tasks', requireAuth, requireRole(adminRoles), asyncHandler(async (req, res) => {
  const take = ADMIN_GENERATION_RESULT_PAGE_SIZE;
  const skip = Math.min(Math.max(Number(req.query.offset || 0), 0), 100000);
...
router.get('/admin/tasks/report', requireAuth, requireRole(adminRoles), asyncHandler(async (req, res) => {
  ...
  const reportTasks = (needsAdminGenerationCategoryScan(requestedCategory)
    ? tasks.filter(task => generationTaskAdminCategory(task as unknown as Record<string, unknown>) === requestedCategory)
    : tasks) as AdminGenerationReportTask[];
