...
async function main() {
  ...
  const requestMethodValue = String(config.protocol?.method || '').trim()
    ...
    const data: Prisma.AiModelUncheckedCreateInput = {
      ...
      unit: overwritePricing
        ? configPricingUnit(config, modelType) ?? existing?.unit ?? defaultUnit(modelType)
        : existing?.unit ?? configPricingUnit(config, modelType) ?? defaultUnit(modelType),
...
function statusEndpointPath(config: CanvasModelConfig, endpointPath: string) {
  ...
  if (adapter === 'seedance2-sd' || adapter === 'sd-seedance2' || adapter === 'sd-video') return '/video/generations/{taskId}';
