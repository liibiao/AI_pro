...
PKG="sd-seedance2-three-channels-20260610163035"
...
log "install canvas and three consolidated model definitions"
...
log "sync three SD / Seedance 2.0 models"
...
log "migrate historical tasks and remove five legacy model rows"
...
log "verify markers and installed model files"
grep -Fq "key.includes('sd2-fast')" "$WEB_ROOT/image-studio-canvas-next.html"
grep -Fq "sd2-{resolution}-fast" "$WEB_ROOT/tools/workbench-web/models/sd2-fast.json"
grep -Fq "sd2-{resolution}" "$WEB_ROOT/tools/workbench-web/models/sd2.json"
grep -Fq '"model": "seedance-2"' "$WEB_ROOT/tools/workbench-web/models/sd2-full.json"
grep -Fq "resolveSeedance2ModelName" "$APP_DIR/api-server/dist/modules/generation/adapters/registry.js"
for legacy_file in sd2-720p-fast.json sd2-720p.json sd2-1080p-fast.json sd2-1080p.json seedance-2.json; do
  [ ! -e "$WEB_ROOT/tools/workbench-web/models/$legacy_file" ] || fail "legacy file still exists: $legacy_file"
done
curl -fsS http://127.0.0.1:4000/api/health >/dev/null 2>&1 && log "api health ok" || log "api health skipped/failed"

log "done"
echo "backup: $BACKUP_DIR"
echo "Active SD channels: SD 2.0 Full, SD 2.0 Fast, SD 2.0"
