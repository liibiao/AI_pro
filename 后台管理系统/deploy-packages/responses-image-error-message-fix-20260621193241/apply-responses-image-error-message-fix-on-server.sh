...
PKG="responses-image-error-message-fix-20260621193241"
...
verify_markers(){
  local file="$1"
  grep -Fq "extractResponsesOutputTextMessage" "$file"
  grep -Fq "isNonUserFacingErrorMetadataKey" "$file"
  grep -Fq "unable\\s+to\\s+generate" "$file"
}
