...
PKG="${1:-/tmp/image-reference-temp-url-fix-20260601042714.tar.gz}"
...
WORK="$(mktemp -d /tmp/image-reference-temp-url-fix-20260601042714-XXXXXX)"
BACKUP="/var/www/ai-admin/backups/image-reference-temp-url-fix-20260601042714-$STAMP"
...
tar -xzf "$PKG" -C "$WORK"
SRC="$WORK/image-reference-temp-url-fix-20260601042714"
...
run_sudo grep -q "isProviderReachableReferenceUrl" "$WORKBENCH_DIR/image-studio-canvas-next.html"
