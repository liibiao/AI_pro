...
PKG="grok-media-video-config-fix-20260606021120"
...
ADMIN_SRC="$SRC_ROOT/ai-admin-platform/admin-web/src"
...
[ -f "$ADMIN_SRC/main.tsx" ] || fail "package missing admin main.tsx"
[ -f "$ADMIN_DIST/index.html" ] || fail "package missing admin dist"
...
grep -Fq "VIDEO_RESULT_MISSING" "$API_SRC/modules/generation/adapters/registry.ts"
grep -Fq "isVideoResultUrlCandidate" "$API_SRC/modules/generation/adapters/registry.ts"
grep -Fq "looksLikeImageResultUrl" "$API_SRC/upstream.ts"
grep -Fq "adapter: model.adapter || model.provider.adapter" "$API_SRC/modules/models/routes.ts"
grep -Fq "adapter: model.adapter || model.provider?.adapter || 'openai-image'" "$ADMIN_SRC/main.tsx"
grep -Fq "Array.from({ length: 15 }, (_, index) => index + 1)" "$API_SRC/apply-grok-media-channel.ts"
grep -Fq '"model": "grok-imagine-video"' "$MODEL_SRC/grok-video.json"
grep -Fq '"maxVideoDurationSeconds": 15' "$MODEL_SRC/grok-video.json"
...
if run_sudo test -f "$ADMIN_WEB_DIR/src/main.tsx"; then
  run_sudo cp -a "$ADMIN_WEB_DIR/src/main.tsx" "$BACKUP_DIR/ai-admin-platform/admin-web/src/main.tsx"
fi
if run_sudo test -d "$ADMIN_WEB_DIR/dist"; then
  run_sudo cp -a "$ADMIN_WEB_DIR/dist/." "$BACKUP_DIR/ai-admin-platform/admin-web/dist/"
fi
...
log "install admin-web build"
run_sudo mkdir -p "$ADMIN_WEB_DIR/src" "$ADMIN_WEB_DIR/dist"
run_sudo install -m 0644 "$ADMIN_SRC/main.tsx" "$ADMIN_WEB_DIR/src/main.tsx"
run_sudo rm -rf "$ADMIN_WEB_DIR/dist"
run_sudo mkdir -p "$ADMIN_WEB_DIR/dist"
run_sudo cp -a "$ADMIN_DIST/." "$ADMIN_WEB_DIR/dist/"
...
run_sudo grep -Fq "VIDEO_RESULT_MISSING" "$APP_DIR/api-server/dist/modules/generation/adapters/registry.js"
run_sudo grep -Fq "looksLikeImageResultUrl" "$APP_DIR/api-server/dist/upstream.js"
run_sudo grep -Fq "adapter: model.adapter || model.provider?.adapter || 'openai-image'" "$ADMIN_WEB_DIR/src/main.tsx"
run_sudo grep -Fq "grok-imagine-video" "$APP_DIR/api-server/dist/apply-grok-media-channel.js"
