...
router.get('/tasks', requireAuth, asyncHandler(async (req, res) => {
  const tasks = await listGenerationTasksForPrincipal(personalPrincipalFromUser(req.user!.id, 'CANVAS'), req.query.limit);
  ok(res, { items: tasks });
}));
...
export async function listGenerationTasksForPrincipal(principal: OpenApiPrincipal, limit: unknown) {
  ...
  return tasks.filter(task => task.userId === ownerUserId).map(sanitizeGenerationTaskListItem);
}
...
router.get('/admin/tasks', requireAuth, requireRole(adminRoles), asyncHandler(async (req, res) => {
  ...
  if (wantsStoryboard) {
    const tasks = await findAdminStoryboardGenerationTasks(where, take);
...
async function findAdminStoryboardGenerationTasks(where: Prisma.GenerationTaskWhereInput, take: number) {
  ...
  while (items.length < take && skip < maxScanned) {
    ...
    items.push(...tasks.filter(isStoryboardGenerationTask));
    skip += tasks.length;
  }
