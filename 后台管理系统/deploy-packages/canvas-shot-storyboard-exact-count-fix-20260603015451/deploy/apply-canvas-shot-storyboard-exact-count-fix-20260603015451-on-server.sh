...
PKG="canvas-shot-storyboard-exact-count-fix-20260603015451"
...
log "verify markers"
grep -Fq "function shotStoryboardExpectedCount(n)" "$HTML_DEST"
grep -Fq "function shotStoryboardMaxTokens(n)" "$HTML_DEST"
grep -Fq "function ensureShotStoryboardExactCount(id,text,source,activeModel,modelId)" "$HTML_DEST"
grep -Fq "硬性数量约束" "$HTML_DEST"
grep -Fq "不能只生成 4 个" "$HTML_DEST"
grep -Fq "补帧要求" "$HTML_DEST"
grep -Fq "expectedShotCount" "$HTML_DEST"
grep -Fq "storyboard\\s*sheet" "$HTML_DEST"
grep -Fq "panels?\\s*arranged" "$HTML_DEST"
grep -Fq "[|｜]" "$HTML_DEST"
