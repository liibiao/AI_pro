...
PKG_NAME="large-paint-tool-active-fix-20260601131125"
...
grep -q ".lb-workbar .paint-tool-btn.active" "$TARGET"
grep -q "aria-pressed" "$TARGET"
grep -q "item.classList.toggle('active',active)" "$TARGET"
