    ...
    <div className="dashboardBarWrap">
      <div className="dashboardBarChart" style={{ gridTemplateColumns: `repeat(${rows.length}, minmax(30px, 1fr))` }}>
        {rows.map((row, index) => {
          ...
          const title = `${row.period} 充值 ${formatNumber(row.rechargeCredits)} / 消耗 ${formatNumber(row.consumeCredits)}${showAgentQuota ? ` / 代理配额 ${formatNumber(row.agentQuotaCredits)}` : ''}`;
          return (
      ...
      {ranked.map(agent => (
        <div className="dashboardAgentBarRow" key={agent.agentId}>
          ...
          <div className="dashboardAgentBarTracks">
            <div className="dashboardAgentTrack" title={`充值 ${formatNumber(agent.rechargeCredits)}`}>
...
function Dashboard() {
  ...
  const donutSegments = useMemo(() => [
    { label: '用户充值', value: summary.rechargeCredits, color: '#2563eb' },
  ...
  ], [excludeAgentQuota, summary.agentQuotaCredits, summary.consumeCredits, summary.rechargeCredits]);
  const detailColumns = [
    { title: granularity === 'day' ? '日期' : '月份', dataIndex: 'period', width: 130, render: (value: string) => <span className="dashboardPeriodCell">{value}</span> },
    { title: '充值积分', dataIndex: 'rechargeCredits', render: (value: number) => formatNumber(value) },
    { title: '充值笔数', dataIndex: 'rechargeCount', render: (value: number) => formatNumber(value) },
  ...
  return (
    <Space direction="vertical" size={16} className="dashboardPage">
      ...
      <div className="dashboardMetricGrid">
        <DashboardMetric label="本期充值积分" value={summary.rechargeCredits} detail={`${formatNumber(summary.rechargeCount)} 笔，实收 ${formatYuanFromCents(summary.rechargeAmountCents)}`} tone="blue" />
        <DashboardMetric label="本期消耗积分" value={summary.consumeCredits} detail={`${formatNumber(summary.consumeCount)} 笔消费流水`} tone="orange" />
        <DashboardMetric label="本期净增积分" value={summary.netCredits} detail="充值 + 配额 - 消耗" tone={summary.netCredits >= 0 ? 'green' : 'orange'} />
      ...
      <div className="dashboardChartGrid">
        <Card className="dashboardPanel" title="充值与消耗趋势" loading={q.isLoading && !data} extra={
          <div className="dashboardInlineLegend">
            <span><i className="legendRecharge" />充值</span>
      ...
      <Card className="dashboardPanel" title="代理充值 / 消耗 / 余额排行" loading={q.isLoading && !data} extra={
        <div className="dashboardInlineLegend">
          <span><i className="legendRecharge" />充值</span>
          <span><i className="legendConsume" />消耗</span>
      ...
      <Card className="dashboardPanel" title="代理明细" extra={<span className="dashboardMuted">按代理绑定客户归集充值、消耗与余额</span>}>
...
function videoDurationResolutionFieldsForModel(model: any, modelName: string, modelNick: string) {
  ...
  const values = [...fromCaps, ...fromMap, ...preset]
...
function videoDurationPresetResolutionList(modelName: any, modelNick: any) {
  ...
  if (text.includes('sora-v3') || text.includes('seedance') || text.includes('sora-vip3') || text.includes('veo')) return ['720p', '1080p'];
...
function AgentDashboard() {
  ...
  const donutSegments = useMemo(() => [
    { label: '客户充值', value: summary.rechargeCredits, color: '#2563eb' },
    { label: '客户消耗', value: summary.consumeCredits, color: '#f97316' },
    { label: '下级配额', value: summary.agentQuotaCredits, color: '#7c3aed' },
  ], [summary.agentQuotaCredits, summary.consumeCredits, summary.rechargeCredits]);
  const detailColumns = [
    { title: granularity === 'day' ? '日期' : '月份', dataIndex: 'period', width: 130, render: (value: string) => <span className="dashboardPeriodCell">{value}</span> },
    { title: '客户充值', dataIndex: 'rechargeCredits', render: (value: number) => formatNumber(value) },
    { title: '充值笔数', dataIndex: 'rechargeCount', render: (value: number) => formatNumber(value) },
  ...
  return (
    <Space direction="vertical" size={16} className="dashboardPage">
      ...
      <div className="dashboardMetricGrid">
        <DashboardMetric label="客户充值积分" value={summary.rechargeCredits} detail={`${formatNumber(summary.rechargeCount)} 笔，实收 ${formatYuanFromCents(summary.rechargeAmountCents)}`} tone="blue" />
        <DashboardMetric label="客户消耗积分" value={summary.consumeCredits} detail={`${formatNumber(summary.consumeCount)} 笔消费流水`} tone="orange" />
        <DashboardMetric label="本期净增积分" value={summary.netCredits} detail="客户充值 + 下级配额 - 客户消耗" tone={summary.netCredits >= 0 ? 'green' : 'orange'} />
      ...
      <div className="dashboardChartGrid">
        <Card className="dashboardPanel" title="客户充值与消耗趋势" loading={q.isLoading && !data} extra={
          <div className="dashboardInlineLegend">
            <span><i className="legendRecharge" />充值</span>
            <span><i className="legendConsume" />消耗</span>
      ...
      <Card className="dashboardPanel" title="团队代理充值 / 消耗 / 余额排行" loading={q.isLoading && !data} extra={
        <div className="dashboardInlineLegend">
          <span><i className="legendRecharge" />充值</span>
          <span><i className="legendConsume" />消耗</span>
          <span><i className="legendBalance" />客户余额</span>
