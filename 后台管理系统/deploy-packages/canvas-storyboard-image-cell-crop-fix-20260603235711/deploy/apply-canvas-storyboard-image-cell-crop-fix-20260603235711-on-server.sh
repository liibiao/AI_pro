...
PKG="canvas-storyboard-image-cell-crop-fix-20260603235711"
...
if [ -d "$(dirname "$MIRROR_HTML_DEST")" ]; then
  ...
  log "updated mirror canvas html"
fi

log "verify markers"
grep -Fq "storyboardImageEntryOwnDisplayUrl" "$HTML_DEST"
grep -Fq "storyboardImageInvalidCroppedCount" "$HTML_DEST"
grep -Fq "ensureStoryboardImageCroppedCells" "$HTML_DEST"
grep -Fq "scheduleStoryboardImageRecropIfNeeded" "$HTML_DEST"
grep -Fq "renderStoryboardImageResolutionSelect" "$HTML_DEST"
grep -Fq "normalizeStoryboardImageResolutionValue" "$HTML_DEST"
grep -Fq "storygrid-resolution-select" "$HTML_DEST"
grep -Fq "sourceRatio:ratio" "$HTML_DEST"
grep -Fq "preserveAspectRatio:true" "$HTML_DEST"
grep -Fq "storygrid-shot-progress" "$HTML_DEST"
grep -Fq "shot-row-table-dialog" "$HTML_DEST"
grep -Fq "storyboardImageSavedPreviewEntry" "$HTML_DEST"
