...
PKG="${1:-/tmp/admin-dashboard-agent-analytics-filter-fix-20260603112416.tar.gz}"
REMOTE_ROOT="${REMOTE_ROOT:-/var/www/ai-admin}"
REMOTE_APP_ROOT="${REMOTE_APP_ROOT:-$REMOTE_ROOT/ai-admin-platform}"
PM2_NAME="${PM2_NAME:-ai-admin-api}"
PM2_USER="${PM2_USER:-ubuntu}"
STAMP="$(date +%Y%m%d%H%M%S)"
WORKDIR="$(mktemp -d /tmp/admin-dashboard-agent-analytics-filter-fix-XXXXXX)"
BACKUP_DIR="$REMOTE_ROOT/backups/admin-dashboard-agent-analytics-filter-fix-20260603112416-$STAMP"
...
grep -Fq "agentBreakdown" "$SRC/api-server/src/modules/admin-logs/routes.ts"
grep -Fq "DashboardAgentBars" "$SRC/admin-web/src/main.tsx"
grep -Fq "DatePicker" "$SRC/admin-web/src/main.tsx"
grep -Fq "代理充值 / 消耗 / 余额排行" "$SRC/admin-web/src/main.tsx"
grep -Fq ".dashboardAgentBars" "$SRC/admin-web/src/styles.css"
grep -Rqs "代理明细" "$SRC/admin-web/dist" "$SRC/admin-web/src/main.tsx"
...
log "install dashboard analytics fix"
...
grep -Fq "agentBreakdown" "$REMOTE_APP_ROOT/api-server/dist/modules/admin-logs/routes.js"
grep -Fq "DashboardAgentBars" "$REMOTE_APP_ROOT/admin-web/src/main.tsx"
grep -Fq "DatePicker" "$REMOTE_APP_ROOT/admin-web/src/main.tsx"
grep -Fq ".dashboardAgentBars" "$REMOTE_APP_ROOT/admin-web/src/styles.css"
grep -Rqs "代理明细" "$REMOTE_APP_ROOT/admin-web/dist" "$REMOTE_APP_ROOT/admin-web/src/main.tsx"
