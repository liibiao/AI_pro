...
router.patch('/admin/models/:id', requireAuth, requireRole(adminRoles), asyncHandler(async (req, res) => {
  ...
  if (simpleBody.success && ['baseUrl', 'adapter', 'model', 'modelNick', 'key', 'uploadMode', 'imageMainModel', 'upstreamStreamConfigMode', 'upstreamStreamMode', 'upstreamStreamModes', 'asyncTaskConfigMode', 'asyncTaskMode', 'asyncTaskModes', 'responseTypeMode', 'responseType', 'responseTypes', 'maxVideoDurationSeconds'].some(key => key in req.body)) {
