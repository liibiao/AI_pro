...
import { callUpstreamJson } from '../../upstream.js';
...
const textAgentSchema = z.object({
  ...
  agentPackId: z.string().default('manga-writer'),
...
async function runLlm(modelKey = '', prompt: string) {
  ...
  const model = await prisma.aiModel.findUnique({ where: { id: modelKey }, include: { provider: true } });
  if (!model || model.status !== 'ACTIVE' || model.type !== 'LLM') fail(400, 'LLM 模型不可用', 'LLM_MODEL_UNAVAILABLE');
  const protocol = isRecord(model.protocol) ? model.protocol : {};
  const endpointPath = model.endpointPath || String(protocol.endpointPath || '') || '/v1/chat/completions';
  const payload = {
    model: model.name,
    messages: [
