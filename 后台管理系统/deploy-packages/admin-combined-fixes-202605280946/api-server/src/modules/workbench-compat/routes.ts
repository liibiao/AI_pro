...
router.post('/workbench/image-studio/video/extract-audio', requireAuth, asyncHandler(async (req, res) => {
  ...
  if (!/^https?:\/\//i.test(remoteUrl)) fail(400, '缺少可抽取音频的视频地址', 'VIDEO_AUDIO_SOURCE_MISSING');
  const extracted = await extractRemoteVideoAudio(remoteUrl);
  res.setHeader('Content-Type', extracted.contentType);
  res.setHeader('Content-Length', String(extracted.size));
  res.setHeader('Content-Disposition', `attachment; filename="${extracted.filename}"`);
  res.setHeader('Cache-Control', 'no-store');
  res.sendFile(extracted.path, err => {
    fs.rm(extracted.dir, { recursive: true, force: true }).catch(() => undefined);
    if (err && !res.headersSent) res.status(500).json({ ok: false, error: '音频文件发送失败', code: 'VIDEO_AUDIO_SEND_FAILED' });
  });
}));

router.get('/workbench/image-studio/file', requireAuth, asyncHandler(async (req, res) => {
  const filePath = resolveExistingLocalFile(req.query.path || '');
  if (!filePath) fail(404, '图片文件不存在', 'WORKBENCH_FILE_NOT_FOUND');
  res.setHeader('Content-Type', inferLocalContentType(filePath));
  res.setHeader('Cache-Control', 'private, max-age=300');
  res.sendFile(filePath, err => {
    if (err && !res.headersSent) res.status(404).json({ ok: false, error: '图片文件不存在', code: 'WORKBENCH_FILE_NOT_FOUND' });
  });
}));
...
router.post('/workbench/image-studio/upload-provider-file', requireAuth, asyncHandler(async (req, res) => {
  ...
  const upstream = await callUpstreamMultipart(target.provider, '/v1/files', form, 0);
...
function buildRemoteVideoHeaders(reqHeaders: Request['headers'] = {}, remoteUrl: string, range = '') {
  const remote = new URL(remoteUrl);
  const headers: Record<string, string> = {
    Accept: 'video/*,application/octet-stream,*/*',
    'User-Agent': String(reqHeaders['user-agent'] || 'Mozilla/5.0'),
...
async function sendRemoteVideo(req: Request, res: Response, remoteUrl: string) {
  ...
  const headers = buildRemoteVideoHeaders(req.headers, remoteUrl, range);
  const resp = await fetch(remoteUrl, { headers, redirect: 'follow' });
  if (!resp.ok) fail(resp.status >= 500 ? 502 : 400, `在线视频代理失败 HTTP ${resp.status}`, 'REMOTE_VIDEO_PROXY_FAILED');
  const contentType = String(resp.headers.get('content-type') || '').split(';')[0].trim().toLowerCase();
  const inferredType = inferRemoteVideoContentType(remoteUrl);
  if (contentType && !contentType.startsWith('video/') && contentType !== 'application/octet-stream' && !/^binary\//i.test(contentType)) {
  ...
  res.setHeader('Content-Type', contentType && contentType !== 'application/octet-stream' ? contentType : inferredType);
...
async function downloadRemoteVideoToTemp(remoteUrl: string) {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'workbench-video-audio-'));
  const input = path.join(dir, `source${path.extname(new URL(remoteUrl).pathname) || '.mp4'}`);
  const resp = await fetch(remoteUrl, { headers: buildRemoteVideoHeaders({}, remoteUrl), redirect: 'follow' });
  if (!resp.ok) fail(resp.status >= 500 ? 502 : 400, `在线视频下载失败 HTTP ${resp.status}`, 'REMOTE_VIDEO_DOWNLOAD_FAILED');
  const buffer = Buffer.from(await resp.arrayBuffer());
  if (!buffer.length) fail(400, '在线视频内容为空', 'REMOTE_VIDEO_EMPTY');
  const maxBytes = Number(process.env.WORKBENCH_VIDEO_AUDIO_MAX_BYTES || 0) || 500 * 1024 * 1024;
  if (buffer.length > maxBytes) fail(413, '在线视频过大，无法抽取音频', 'REMOTE_VIDEO_AUDIO_TOO_LARGE');
  await fs.writeFile(input, buffer);
  return { dir, input };
}

async function extractRemoteVideoAudio(remoteUrl: string) {
  const ffmpeg = process.env.FFMPEG_BIN || 'ffmpeg';
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'workbench-video-audio-'));
  const output = path.join(dir, 'audio.m4a');
  try {
    const headers = buildRemoteVideoHeaders({}, remoteUrl);
    const headerText = Object.entries(headers).filter(([key]) => key.toLowerCase() !== 'range').map(([key, value]) => `${key}: ${value}`).join('\r\n');
    await new Promise<void>((resolve, reject) => {
      const args = ['-y', '-headers', headerText, '-i', remoteUrl, '-vn', '-acodec', 'aac', '-b:a', '192k', output];
    ...
    if (!stat.size) throw new Error('抽取音频为空');
    return { dir, path: output, size: stat.size, filename: 'extracted-audio.m4a', contentType: 'audio/mp4' };
  } catch (err) {
    await fs.rm(dir, { recursive: true, force: true }).catch(() => undefined);
    fail(501, `服务端音频抽取失败：${err instanceof Error ? err.message : String(err)}`, 'VIDEO_AUDIO_EXTRACT_FAILED');
  }
}
