...
export function buildCreditDiscountBenefit(membership?: { source?: string | null; expiredAt?: Date | string | null; plan?: { name?: string | null } | null } | null): CreditDiscountBenefit {
  ...
  return {
    ...
    label: '原价',
    expiresAt: null,
    message: '当前为原价计费；开通会员或兑换体验卡后，生图/生视频按 VIP 4 折扣积分。',
  };
}
...
export function calculateCredits(
  ...
  const chargedCredits = Math.ceil(costUsd * Number(model.creditsPerUsdCost) * Number(model.markupRate));
  return { chargedCredits, memberCredits: chargedCredits, originalCredits: chargedCredits, costAmount: 0, costUsd, discountActive: false, discountRate: 1, discountLabel: '原价', discountExpiresAt: null };
}
...
function videoOriginalCreditsPerSecond(model: { defaults?: Prisma.JsonValue | null }, memberPerSecond: number) {
  const pricing = recordValue(recordValue(model.defaults)?.pricing);
  const configured = Number(pricing?.originalCreditsPerSecond ?? pricing?.listCreditsPerSecond ?? pricing?.standardCreditsPerSecond ?? NaN);
  if (Number.isFinite(configured) && configured >= memberPerSecond) return configured;
  return originalCreditsFromMember(memberPerSecond);
}
...
function videoPriceForInput(
  ...
  const fallbackMember = model.pricePerSecond || VIDEO_PRICING.chargedCreditsPerSecond;
  const member = Number.isFinite(memberCreditsPerSecond) && memberCreditsPerSecond >= 0 ? memberCreditsPerSecond : fallbackMember;
  const original = Number.isFinite(originalCreditsPerSecond) && originalCreditsPerSecond >= member
    ? originalCreditsPerSecond
    : videoOriginalCreditsPerSecond(model, member);
  return {
