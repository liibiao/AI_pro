      ...
      {selectedType === 'VIDEO' && (
        <>
          <Form.Item
            ...
            label="最大视频时长（秒）"
            tooltip="保存到模型 capabilities.maxVideoDurationSeconds，并自动生成画布可选时长。留空则使用模型默认能力。"
          >
            <InputNumber min={1} precision={0} style={{ width: '100%' }} placeholder="例如 15 / 30 / 60" />
          </Form.Item>
          <Form.Item
            name="responseType"
            label="response_type"
...
function normalizeSimpleModelSubmitValues(values: any, editing = false) {
  ...
  if (payload.type !== 'VIDEO') delete payload.maxVideoDurationSeconds;
