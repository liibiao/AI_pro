...
async function onClick(e){
  ...
  try{
    if(action==='create-node'){
      ...
      render();
    }else if(action==='agent-file-action'){
      await runAgentFileAction(id,Number(target.dataset.index||0),target.dataset.mode||'parse');
...
async function runAgentFileAction(id,index,mode){
  const n=state.nodes[id];if(!n)return;
  const file=(n.values.files||[])[index];
  if(!file)throw new Error('未找到导入素材');
  const oldMode=n.values.mode;
  const oldFiles=Array.isArray(n.values.files)?n.values.files:[];
  const nextMode=mode==='reverse'?'reverse':'parse';
  n.values.mode=nextMode;
  n.values.files=[file];
  await runNode(id);
  const cur=state.nodes[id];
  const text=cur?.data?.text||cur?.values?.outputText||'';
  cur.values.files=oldFiles;
  cur.values.mode=oldMode;
  if(text){
    cur.values.inputText=text;
    cur.values.outputText=text;
    cur.data={...(cur.data||{}),text,stageLabel:nextMode==='reverse'?'反推结果已写入输入框':'解析结果已写入输入框'};
    saveWorkflow();render();
    toast(nextMode==='reverse'?'已反推并写入输入框':'已解析并写入输入框','ok');
  }else{
    saveWorkflow();render();
  }
}
...
async function handleVideoFile(input){
  ...
  n.data={url:dataUrl,videoUrl:dataUrl,contentUrl:dataUrl,dataUrl,file,name:file.name,type:file.type||'video/mp4',size:file.size||0};
  n.status='success';n.progress=100;n.error='';
  propagateOutputs(n.id);
  saveWorkflow();render();
}
...
function onPointerUp(e){
  if(state.connect){
    ...
    const target=hit?.closest?.('.port');
    const targetNode=hit?.closest?.('.node');
    const targetQuickAdd=hit?.closest?.('[data-action="quick-add"]');
    if(target&&finishConnectToPort(target)){
      ...
      suppressQuickAddClickUntil=Date.now()+350;
      if(targetQuickAdd)e.preventDefault();
    }else{
