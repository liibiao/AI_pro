...
const BACKEND_GENERATION_TIMEOUT_MS=30*60*1000;
...
async function submitBackendGeneration({model,type,mode,prompt,inputFiles=[],params={},onProgress}){
  ...
  for(;;){
    if(Date.now()-startedAt>BACKEND_GENERATION_TIMEOUT_MS){
      throw new Error('生成任务等待超时，请稍后在资产库查看结果，或修改提示词后重新生成');
    }
...
function mediaUrl(entry){
  if(!entry)return '';
  return entry.uploadRef||entry.providerRef||entry.fileId||entry.id||entry.remoteUrl||entry.videoUrl||entry.contentUrl||entry.url||entry.localUrl||entry.dataUrl||'';
}

function isProviderRef(value){
  const raw=String(value||'');
  return /^file-[\w-]+$/i.test(raw)||/^https?:\/\//i.test(raw);
}

async function entryToFile(entry,index,{kind='image'}={}){
  const E=engine();
  if(entry.file instanceof File||entry.file instanceof Blob)return entry.file;
  if(entry.dataUrl){
    const blob=E.dataUrlToBlob(entry.dataUrl);
    return new File([blob],entry.name||`reference-${index+1}.${kind==='video'?'mp4':'png'}`,{type:blob.type||(kind==='video'?'video/mp4':'image/png')});
  }
  const url=entry.localUrl||entry.videoUrl||entry.contentUrl||entry.url;
  if(url){
    const resp=await fetch(url);
    const blob=await resp.blob();
    return new File([blob],entry.name||`reference-${index+1}.${kind==='video'?'mp4':'png'}`,{type:blob.type||(kind==='video'?'video/mp4':'image/png')});
  }
  throw new Error(kind==='video'?'参考视频缺少可上传的视频内容':'参考图缺少可上传的图片内容');
}
...
export async function ensureUploadedImages(images,model,{forVideo=false,onProgress}={}){
  ...
  for(let i=0;i<(images||[]).length;i++){
    ...
    const file=await entryToFile(entry,i,{kind:'image'});
...
async function ensureUploadedVideos(videos,model,{onProgress}={}){
  const E=engine();
  const uploadMode='object_storage';
  const refs=[];
  for(let i=0;i<(videos||[]).length;i++){
    const entry=videos[i];
    const existing=mediaUrl(entry);
    if(isProviderRef(existing)){
      refs.push(existing);
      continue;
    }
    onProgress?.(`上传参考视频 ${i+1}/${videos.length}`);
    const file=await entryToFile(entry,i,{kind:'video'});
    const uploaded=modelHasBackendGateway(model)
      ? await uploadReferenceImageToBackend(file,uploadMode,model)
      : await E.uploadReferenceImage(file,{mode:uploadMode,baseUrl:model?.baseUrl||model?.url||'',apiKey:model?.apiKey||model?.key||''});
    const ref=uploaded.uploadRef||uploaded.providerRef||uploaded.fileId||uploaded.id||uploaded.remoteUrl||uploaded.url;
    if(!ref)throw new Error('参考视频上传后未返回可用引用');
    Object.assign(entry,{uploadRef:ref,providerRef:ref,fileId:uploaded.fileId||uploaded.id||ref,id:uploaded.id||ref,remoteUrl:uploaded.remoteUrl||uploaded.url||'',videoUrl:uploaded.remoteUrl||uploaded.url||ref,uploaded:true,uploadMode,status:'done'});
    refs.push(ref);
  }
  return refs;
}
...
export async function runTextPromptAgent(node,inputs,models,{onProgress}={}){
  const values=node.values||{};
  const inputText=[inputs.text,values.inputText].filter(Boolean).join('\n\n').trim();
  if(values.mode==='default'){
    return {text:inputText,outputType:values.outputType||'text',stageLabel:'默认模式：已原文透传'};
  }
  if(!window.CanvasAccountGate?.request)throw new Error('后台账号网关未加载');
  onProgress?.('提交文本 Agent 任务');
  const data=await window.CanvasAccountGate.request('/api/workbench/text-agent/run',{
    method:'POST',
    body:JSON.stringify({
      mode:values.mode||'default',
      modelKey:values.llmModelKey||'',
      agentPackId:values.agentPackId||'manga-writer',
      outputType:values.outputType||'text',
      inputText,
      files:values.files||[],
  ...
  return {text:data.text||data.outputText||'',raw:data,outputType:values.outputType||'text',stageLabel:'文本 Agent 执行完成'};
}

export async function runAssetDesign(node,inputs,models,{onProgress}={}){
  const values=node.values||{};
  if(!window.CanvasAccountGate?.request)throw new Error('后台账号网关未加载');
  onProgress?.('提交资产设计任务');
  const data=await window.CanvasAccountGate.request('/api/workbench/asset-design/run',{
    method:'POST',
    body:JSON.stringify({
      ...values,
      inputText:[inputs.text,values.prompt].filter(Boolean).join('\n\n').trim(),
      referenceFiles:values.referenceFiles||[],
      images:inputs.images||[]
    })
  });
  return {prompt:data.prompt||data.outputPrompt||'',text:data.prompt||data.outputPrompt||'',images:data.images||[],raw:data,stageLabel:'资产设计完成'};
}
...
async function runVideo(node,inputs,models,{onProgress}={}){
  ...
  const videoRefs=await ensureUploadedVideos(inputs.videos||[],model,{onProgress});
  const inputRefs=[...imageRefs,...videoRefs];
  const modelInfo=modelPayload(model);
  onProgress?.('提交视频任务');
  const backendMode=inputRefs.length?'image-to-video':'text-to-video';
  const backendPayload={
    prompt,
    videoMode:videoRefs.length?'video-to-video':backendMode,
    refMode:values.refMode||'full',
    images:imageRefs,
    videos:videoRefs,
    video:videoRefs[0]||'',
    sourceVideo:videoRefs[0]||'',
  ...
  if(modelHasBackendGateway(model)){
    const task=await submitBackendGeneration({model,type:'VIDEO',mode:backendPayload.videoMode,prompt,inputFiles:inputRefs,params:backendPayload,onProgress});
  ...
  const start=await E.startVideoGeneration({
    ...
    prompt,videoMode:videoRefs.length?'video-to-video':(imageRefs.length?'image-to-video':'text-to-video'),refMode:values.refMode||'full',
    images:imageRefs,videos:videoRefs,video:videoRefs[0]||'',sourceVideo:videoRefs[0]||'',duration:values.duration||'5',seconds:values.duration||'5',
