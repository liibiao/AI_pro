...
async function submitAiyunzhiGptImage2(ctx: AdapterContext): Promise<AdapterSubmitResult> {
  ...
  const geometry = resolveAiyunzhiGptImage2Geometry(ctx);
  ...
  form.set('size', size);
  if (geometry.aspect_ratio) form.set('aspect_ratio', String(geometry.aspect_ratio));
  if (geometry.requested_size) form.set('requested_size', String(geometry.requested_size));
  form.set('n', String(n));
...
function resolveAiyunzhiGptImage2Geometry(ctx: AdapterContext) {
  const aspectRatio = resolveOpenAiImageAspectRatio(ctx.params);
  const requestedSize = String(firstDefined(ctx.params.requestedPixelSize, ctx.params.pixelSize, ctx.params.size) || '').trim().toLowerCase().replace('×', 'x').replace(/\s+/g, '');
  return compactJson({
    aspect_ratio: aspectRatio,
    requested_size: /^\d{3,5}x\d{3,5}$/.test(requestedSize) ? requestedSize : undefined,
  }) as Record<string, unknown>;
}
...
function resolveAiyunzhiGptImage2ResponseFormat(ctx: AdapterContext) {
  ...
  const raw = String(ctx.params.response_format || ctx.params.responseFormat || openAiImageResponseFormat(ctx) || 'url').trim().toLowerCase();
