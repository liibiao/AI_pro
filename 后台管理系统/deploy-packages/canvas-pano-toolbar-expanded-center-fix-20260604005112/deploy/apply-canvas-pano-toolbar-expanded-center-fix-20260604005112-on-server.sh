...
PKG="canvas-pano-toolbar-expanded-center-fix-20260604005112"
...
log "verify markers"
grep -Fq "tapnow-rewrite.css?v=20260604-pano-toolbar-expanded-center-fix" "$HTML_DEST"
grep -Fq "expanded centered panorama toolbar, no squeezed capsules" "$CSS_DEST"
grep -Fq "min-width:760px!important" "$CSS_DEST"
grep -Fq "width:230px!important" "$CSS_DEST"
grep -Fq "dragging the panorama node handle must never open the screenshot grid" "$CSS_DEST"
grep -Fq "setPanoDragButtonActive(true)" "$HTML_DEST"
grep -Fq "S.suppressPanoShotUntil=Date.now()+1000" "$HTML_DEST"
