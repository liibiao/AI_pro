...
export async function callUpstreamJson(
  provider: UpstreamProvider,
  endpointPath: string,
  payload: unknown,
  timeoutMs?: number,
  options: Pick<UpstreamCallOptions, 'maxAttempts'> = {},
) {
  return callUpstream(provider, endpointPath, { method: 'POST', payload, timeoutMs, ...options });
}
...
function upstreamHttpAttempts(options: UpstreamCallOptions) {
  if (Number.isFinite(options.maxAttempts) && Number(options.maxAttempts) > 0) {
    return Math.max(1, Math.min(5, Math.floor(Number(options.maxAttempts))));
  }
...
export function extractUpstreamTaskId(payload: any) {
  ...
  return firstTaskIdCandidate(
    ...
    first?.id,
    first?.name,
    typeof first === 'string' || typeof first === 'number' || typeof first === 'bigint' ? first : '',
  ) || findNestedVideoTaskId(payload);
}

function findNestedVideoTaskId(payload: unknown) {
  const seen = new Set<unknown>();
  const strongKeys = /^(?:task|job|prediction|generation|operation)[_-]?id$/i;
  const wrapperKeys = /^(?:data|result|response|output)$/i;
  const taskContainers = /(?:task|job|prediction|generation|operation|response|result|data)/i;
  const visit = (value: unknown, parentKey = '', depth = 0): string => {
    if (value === undefined || value === null || depth > 6) return '';
    if (typeof value === 'string' || typeof value === 'number' || typeof value === 'bigint') {
      return wrapperKeys.test(parentKey) && isLikelyTaskIdValue(value) ? String(value).trim() : '';
    }
    if (typeof value !== 'object' || seen.has(value)) return '';
    seen.add(value);
    if (Array.isArray(value)) {
      for (const item of value) {
        const found = visit(item, parentKey, depth + 1);
        if (found) return found;
      }
      return '';
    }
    ...
    for (const [key, item] of Object.entries(record)) {
      if (strongKeys.test(key) && isLikelyTaskIdValue(item)) return String(item).trim();
    }
    if (taskContainers.test(parentKey)) {
      for (const key of ['id', 'name']) {
        if (isLikelyTaskIdValue(record[key])) return String(record[key]).trim();
      }
    }
    for (const [key, item] of Object.entries(record)) {
...
function isLikelyTaskIdValue(value: unknown) {
  if (typeof value !== 'string' && typeof value !== 'number' && typeof value !== 'bigint') return false;
  const raw = String(value).trim();
  if (raw.length < 6 || raw.length > 512) return false;
  if (/^https?:\/\//i.test(raw) || /^data:/i.test(raw)) return false;
  if (/^(?:success|succeeded|completed|done|running|pending|queued|failed|error|null|undefined|true|false)$/i.test(raw)) return false;
  return /[0-9]/.test(raw) || /^(?:operations?|tasks?|jobs?|predictions?|generations?)\//i.test(raw);
}
