...
PKG="text-agent-timeout-backend-20260624103054"
...
UPSTREAM_SRC="$SRC_ROOT/ai-admin-platform/api-server/src/upstream.ts"
UPSTREAM_DIST="$SRC_ROOT/ai-admin-platform/api-server/dist/upstream.js"
...
[ -f "$UPSTREAM_SRC" ] || fail "package missing upstream.ts"
[ -f "$UPSTREAM_DIST" ] || fail "package missing upstream.js"
...
grep -Fq "maxAttempts?: number" "$UPSTREAM_SRC"
grep -Fq "Number.isFinite(options.maxAttempts)" "$UPSTREAM_SRC"
grep -Fq "callUpstreamJson(model.provider, endpointPath, payload, timeoutMs, { maxAttempts: 1 })" "$TEXT_AGENT_SRC"
grep -Fq "body.requestTimeoutMs" "$TEXT_AGENT_SRC"
grep -Fq "return clampNumber(Math.max(...requested), 1000, 600000)" "$TEXT_AGENT_SRC"
grep -Fq "maxAttempts" "$UPSTREAM_DIST"
grep -Fq "maxAttempts: 1" "$TEXT_AGENT_DIST"
...
run_sudo cp -a "$APP_DIR/api-server/src/upstream.ts" "$BACKUP_DIR/ai-admin-platform/api-server/src/upstream.ts" 2>/dev/null || true
run_sudo cp -a "$APP_DIR/api-server/dist/upstream.js" "$BACKUP_DIR/ai-admin-platform/api-server/dist/upstream.js" 2>/dev/null || true
...
run_sudo install -m 0644 "$UPSTREAM_SRC" "$APP_DIR/api-server/src/upstream.ts"
run_sudo install -m 0644 "$UPSTREAM_DIST" "$APP_DIR/api-server/dist/upstream.js"
...
run_sudo grep -Fq "Number.isFinite(options.maxAttempts)" "$APP_DIR/api-server/src/upstream.ts"
run_sudo grep -Fq "maxAttempts: 1" "$APP_DIR/api-server/dist/modules/workbench/text-agent-routes.js"
run_sudo grep -Fq "return clampNumber(Math.max(...requested), 1000, 600000)" "$APP_DIR/api-server/src/modules/workbench/text-agent-routes.ts"
