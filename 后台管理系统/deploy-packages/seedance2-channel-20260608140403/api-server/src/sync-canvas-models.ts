...
async function main() {
  ...
  for (const file of files) {
    ...
    || (['veo-chat', 'veo-3.1', 'openai-responses-image', 'openai-chat-image', 'midjourney-imagine', 'midjourney', 'mj-imagine', 'grok_image', 'grok-image-unified', 'grok-image', 'grok-image-edit', 'sora-video', 'sora-video-pro', 'seedance-task', 'seedance2', 'seedance2.0', 'seedance2-vip', 'seedance2.0-vip', 'grok-video', 'gemini-image-generate', 'gemini-image-edit', 'gemini-video', 'veo-video'].includes(adapterValue) ? 'object_storage' : null);
  const requestMethodValue = String(config.protocol?.method || '').trim()
    || (['veo-chat', 'veo-3.1', 'openai-responses-image', 'openai-chat-image', 'grok_image', 'grok-image-unified', 'grok-image', 'grok-image-edit', 'grok-chat', 'grok-llm', 'gemini-image-generate', 'gemini-image-edit', 'gemini-chat', 'gemini-llm'].includes(adapterValue) ? 'sync' : (['midjourney-imagine', 'midjourney', 'mj-imagine', 'sora-video', 'sora-video-pro', 'seedance-task', 'seedance2', 'seedance2.0', 'seedance2-vip', 'seedance2.0-vip', 'grok-video', 'gemini-video', 'veo-video'].includes(adapterValue) ? 'async-poll' : null));
    ...
    const data: Prisma.AiModelUncheckedCreateInput = {
      ...
      pricePerSecond: overwritePricing
        ? modelPricing.pricePerSecond ?? existing?.pricePerSecond ?? (modelType === 'VIDEO' ? VIDEO_PRICING.chargedCreditsPerSecond : 0)
        : existing?.pricePerSecond ?? modelPricing.pricePerSecond ?? (modelType === 'VIDEO' ? VIDEO_PRICING.chargedCreditsPerSecond : 0),
      ...
      defaults: overwriteExisting
        ? (overwritePricing ? syncedDefaults : preservedDefaults)
...
function normalizeModelType(value: unknown) {
  ...
  if (raw.includes('video') || raw.includes('sora') || raw.includes('veo') || raw.includes('seedance') || raw.includes('artifex')) return 'VIDEO' as const;
...
function statusEndpointPath(config: CanvasModelConfig, endpointPath: string) {
  ...
  if (adapter === 'seedance2' || adapter === 'seedance2.0') return '/videos/{taskId}';
...
function pricingForCanvasModel(type: SyncedModelType, model: { id: string; name: string; model: string; displayName: string; adapter: string }) {
  ...
  if (type === 'VIDEO' && (key.includes('seedance2') || key.includes('seedance 2')) && (key.includes('video-fast-') || key.includes('video-pro-')) && !key.includes('vip')) {
    const chargedCreditsPerSecond = key.includes('video-pro-720p')
      ? 52
      : key.includes('video-fast-480p')
        ? 27
        : 35;
    const pricing = {
