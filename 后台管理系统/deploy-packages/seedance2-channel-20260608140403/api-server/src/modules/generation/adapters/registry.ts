...
function buildMidjourneyImaginePrompt(ctx: AdapterContext, promptUrls: string[]) {
  ...
  if (promptParams) suffixes.push(promptParams);
...
async function normalizeMidjourneyImagineQueryResult(ctx: AdapterContext & { upstreamTaskId: string }, upstream: unknown) {
...
async function buildSeedanceTaskRequestJson(ctx: AdapterContext) {
...
function resolveSeedance2ModelName(ctx: AdapterContext) {
  ...
  const quality = key.includes('pro') || key.includes('高质量') ? 'pro' : 'fast';
  const resolution = key.includes('480p') ? '480p' : '720p';
...
function buildSoraVideoProUrlList(ctx: AdapterContext, keys: string[], mediaType: 'image' | 'video' | 'audio', limit: number) {
  const refs = new Set<string>();
  const push = (value: unknown) => {
    if (!value) return;
    if (Array.isArray(value)) {
