...
def build_midjourney_imagine_payload(payload: dict[str, Any], images: list[str] | None = None) -> tuple[ImageStudioConfig, dict[str, Any]]:
    ...
    image_refs = _normalize_midjourney_image_refs(images if images is not None else (payload.get("reference_images") or payload.get("image")))
    ...
    final_prompt = " ".join([*prompt_urls, prompt]).strip()
    extra_params = str(payload.get("mjParams") or payload.get("midjourneyParams") or payload.get("midjourney_params") or "").strip()
...
def generate_panorama(payload: dict[str, Any], reference_images: list[str], output_dir: Path) -> dict[str, Any]:
    ...
    config, request_payload = build_image_payload(payload, images=reference_values)
    prompt = str(request_payload.get("prompt") or "").strip()
    if config.adapter not in {"gpt-image-v2", "gemini-image", "midjourney-imagine"} and "seam_fix" not in request_payload:
...
def _artifex_video_resolution(model: Any) -> str:
    return "480p" if "480p" in str(model or "").lower() else "720p"
...
def build_seedance_task_request_body(payload: dict[str, Any], config: VideoStudioConfig) -> dict[str, Any]:
    ...
    if "generateAudio" in payload or "generate_audio" in payload:
        request_body["generate_audio"] = payload.get("generateAudio", payload.get("generate_audio")) is not False
...
def start_sora_video_pro_video(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    resolution = _artifex_video_resolution(config.model)
    create_resp = _request_json(config.endpoint_url, headers, request_body, timeout=900)
