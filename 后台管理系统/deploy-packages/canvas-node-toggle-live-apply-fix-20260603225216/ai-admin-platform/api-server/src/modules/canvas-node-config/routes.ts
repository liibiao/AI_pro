...
const registrySettingKey = 'canvas.node.config';
const registrySettingLabel = '画布节点启停配置';
const registrySettingGroup = 'canvas';
...
router.get('/canvas/nodes', requireAuth, asyncHandler(async (_req, res) => {
  ...
  const disabledNodeTypes = nodes.filter(node => !node.enabled).map(node => node.type);
  ok(res, {
    nodes,
    items: nodes,
    data: { nodes, items: nodes, disabledNodeTypes, updatedAt: registry.updatedAt },
    disabledNodeTypes,
...
router.get('/admin/canvas-nodes', requireAuth, requireRole(superAdminRoles), asyncHandler(async (_req, res) => {
  const registry = await readRegistry();
  const disabledNodeTypes = registry.nodes.filter(node => !node.enabled).map(node => node.type);
  ok(res, {
    nodes: registry.nodes,
    items: registry.nodes,
    data: { nodes: registry.nodes, items: registry.nodes, disabledNodeTypes, updatedAt: registry.updatedAt },
    disabledNodeTypes,
    updatedAt: registry.updatedAt,
  });
}));
...
async function readRegistry(): Promise<CanvasNodeRegistry> {
  const databaseRegistry = await readDatabaseRegistry();
  if (databaseRegistry) return databaseRegistry;

  const legacyFileRegistry = await readLegacyFileRegistry();
  if (legacyFileRegistry) return legacyFileRegistry;

  return normalizeRegistry({});
}

async function readDatabaseRegistry(): Promise<CanvasNodeRegistry | null> {
  const row = await prisma.systemSetting.findUnique({ where: { key: registrySettingKey } });
  if (!row?.value) return null;
  try {
    const raw = JSON.parse(row.value);
    return normalizeRegistry({
      ...(raw && typeof raw === 'object' && !Array.isArray(raw) ? raw : {}),
      updatedAt: typeof raw?.updatedAt === 'string' ? raw.updatedAt : row.updatedAt.toISOString(),
    });
  } catch (error) {
    return normalizeRegistry({ updatedAt: row.updatedAt.toISOString() });
  }
}

async function readLegacyFileRegistry(): Promise<CanvasNodeRegistry | null> {
  try {
...
async function writeRegistry(registry: CanvasNodeRegistry): Promise<CanvasNodeRegistry> {
  ...
  await prisma.systemSetting.upsert({
    where: { key: registrySettingKey },
    update: {
      label: registrySettingLabel,
      group: registrySettingGroup,
      value: JSON.stringify(normalized),
      valueType: 'STRING',
      isSecret: false,
      remark: 'Canvas node enable/disable registry',
    },
    create: {
      key: registrySettingKey,
      label: registrySettingLabel,
      group: registrySettingGroup,
      value: JSON.stringify(normalized),
      valueType: 'STRING',
