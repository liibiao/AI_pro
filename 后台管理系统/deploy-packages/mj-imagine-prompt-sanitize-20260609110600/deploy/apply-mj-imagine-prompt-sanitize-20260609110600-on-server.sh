...
PKG="mj-imagine-prompt-sanitize-20260609110600"
...
if command -v npm >/dev/null 2>&1; then
  ...
  log "node/typescript not found, using packaged dist file"
fi
...
log "verify markers"
grep -Fq "cleanPromptTemplateArguments" "$WEB_ROOT/workbench-web/image-studio-canvas-next.html"
grep -Fq "payload.prompt=cleanMidjourneyPromptText(prompt)" "$WEB_ROOT/workbench-web/image-studio-canvas-next.html"
grep -Fq "buildMidjourneyParamSuffix" "$APP_DIR/api-server/dist/modules/generation/adapters/registry.js"
grep -Fq "cleanMidjourneyPromptText(ctx.prompt)" "$APP_DIR/api-server/dist/modules/generation/adapters/registry.js"
