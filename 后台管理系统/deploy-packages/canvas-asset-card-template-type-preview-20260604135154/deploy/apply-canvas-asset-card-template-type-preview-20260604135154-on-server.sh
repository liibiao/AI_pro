...
PKG="canvas-asset-card-template-type-preview-20260604135154"
...
log "installed web canvas files"
...
if [ -d "$(dirname "$MIRROR_HTML_DEST")" ]; then
  ...
  log "updated mirror canvas files"
fi

log "verify markers"
grep -Fq "20260604-asset-card-template-type-preview" "$HTML_DEST"
grep -Fq "scene_environment_board" "$HTML_DEST"
grep -Fq "function assetTemplatesForAssetType(type)" "$HTML_DEST"
grep -Fq "function getAssetTemplateDefinitionForType(id,type)" "$HTML_DEST"
grep -Fq "data-asset-card-template-option" "$HTML_DEST"
grep -Fq "function showSharedAssetTemplatePreview" "$HTML_DEST"
grep -Fq ".asset-card-template-option-thumb-wrap:hover .asset-card-template-option-thumb" "$CSS_DEST"
grep -Fq "brightness(1.06)" "$CSS_DEST"
! grep -Fq "transform:scale(1.16)" "$CSS_DEST"
