...
PKG="canvas-next-storyboard-drag-capsule-fix-20260604025154"
...
grep -Fq "<span class=\"storygrid-panel-no\">\${esc(no)}</span>" "$HTML_DEST"
...
grep -Fq "try{e.dataTransfer?.setDragImage(handle" "$HTML_DEST"
grep -Fq "if(Date.now()-Number(btn._shotToggleAt||0)<450)return;" "$HTML_DEST"
grep -Fq "right:8px;bottom:8px;z-index:5;width:25px;height:25px;display:grid" "$HTML_DEST"
