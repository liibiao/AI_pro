...
PKG="canvas-quick-downstream-anchor-fix-20260603213852"
...
grep -Fq "function quickDownstream(id,opts={})" "$HTML_DEST"
grep -Fq "anchorEl=null" "$HTML_DEST"
grep -Fq "const ar=anchorEl?.getBoundingClientRect?.();" "$HTML_DEST"
grep -Fq "positionConnTargetPopup(popup,{clientX:opts.clientX,clientY:opts.clientY,node:n,anchorEl:opts.anchorEl});" "$HTML_DEST"
