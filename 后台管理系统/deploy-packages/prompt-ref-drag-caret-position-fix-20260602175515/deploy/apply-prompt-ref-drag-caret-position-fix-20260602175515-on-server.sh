...
PKG="prompt-ref-drag-caret-position-fix-20260602175515"
...
log "verify markers"
grep -Fq "function promptRichGeometryRangeFromPoint(input,x,y)" "$DEST"
grep -Fq "function textareaCaretIndexFromPoint(textarea,x,y)" "$DEST"
grep -Fq "function updatePromptReferenceDropCaret(target,e)" "$DEST"
grep -Fq "_promptRefDropContext={target:key,range:range.cloneRange?.()||range" "$DEST"
grep -Fq "insertPromptTextareaReference(target.input,payload,cachedIdx==null?textareaCaretIndexFromPoint" "$DEST"
