...
log "强制校正 canvas_seedance2-vip 数据库闭环"
node --input-type=module <<'NODE'
import { PrismaClient } from '@prisma/client';
const prisma = new PrismaClient();
const modelAssembly = { type: 'template', template: '{model}-{resolution}' };
await prisma.upstreamProvider.updateMany({
  where: { providerKey: 'canvas_seedance2-vip' },
  data: {
    adapter: 'seedance2-vip',
    defaultModel: 'sora-vip3-pro',
    endpointPath: '/videos',
...
if (row?.adapter !== 'seedance2-vip' || row?.provider?.adapter !== 'seedance2-vip' || row?.name !== 'sora-vip3-pro') {
  throw new Error('canvas-seedance2-vip 数据库校验失败');
}
...
log "部署完成：seedance2.0-vip 会走 seedance2-vip 适配器，并向 43 发送 sora-vip3-pro-720p / sora-vip3-pro-1080p"
