...
function adminModelSortPrice(model: any) {
  ...
  if (model?.type === 'VIDEO') {
    if (videoBillingModeFromModel(model) === 'per_generation') return videoFlatMemberPrice(model);
          ...
          ...(periodTitle ? [{ title: periodTitle, dataIndex: 'period', width: 130 }] : []),
          {
            ...
            render: (_, row) => (
              <Space direction="vertical" size={2}>
                <b>{row.providerName || row.providerKey || '-'}</b>
                <span className="muted">{row.providerKey || row.providerId}</span>
              </Space>
            ),
          },
    ...
    return (
      <>
        ...
        <div style={{ color: '#888', fontSize: 12 }}>
          ...
          {isPerGeneration ? '按次计费时，不管用户选择多少秒，每次生成都扣统一价格。' : '按秒计费时，扣费按生成秒数和所选分辨率计算。'}
        </div>
      </>
    );
  }
...
function normalizeVideoPricingSubmitValues(
  ...
  if (String(videoBillingMode || '').trim() === 'per_generation') {
    const memberCredits = Math.max(0, Number(videoFlatPrice || 0));
    const originalCredits = memberOriginal(memberCredits);
    const costCredits = memberCost(memberCredits);
    return {
      type: rest.type,
      status: rest.status,
      unit: 'generation',
      salePrice: memberCredits,
      pricePerSecond: 0,
      ...
      defaults: {
        ...
        pricing: {
          ...
          currency: existingPricing.currency || 'credits',
          memberDiscountRate: 0.4,
          chargedCreditsPerGeneration: memberCredits,
          memberCreditsPerGeneration: memberCredits,
