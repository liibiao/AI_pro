...
func (s *Server) setupRoutes() {
	...
	v1.Use(AuthMiddleware(s.accessManager))
	{
		...
		v1.GET("/videos/:request_id", openaiHandlers.VideosRetrieve)
	...
	s.engine.GET("/oauth2callback", func(c *gin.Context) {
		code := c.Query("code")
		state := c.Query("state")
		errStr := c.Query("error")
		if errStr == "" {
	...
	s.engine.GET("/oauth-callback", func(c *gin.Context) {
		code := c.Query("code")
		state := c.Query("state")
		errStr := c.Query("error")
		if errStr == "" {
	...
	s.engine.GET("/callback", func(c *gin.Context) {
		code := c.Query("code")
		state := c.Query("state")
		errStr := c.Query("error")
		if errStr == "" {
		...
		if state != "" {
			if provider, _, ok := managementHandlers.GetOAuthSession(state); ok {
