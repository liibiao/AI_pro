...
PKG="${1:-/tmp/canvas-video-inline-controls-20260522.tar.gz}"
...
WORK="/tmp/canvas-video-inline-controls-$(date +%Y%m%d%H%M%S)"
mkdir -p "$WORK"
tar -xzf "$PKG" -C "$WORK"
...
echo "==> 校验视频内嵌控制条和 COS 缩略图标记"
grep -n "function bindInlineVideoControls" "$CANVAS_DIR/tools/workbench-web/image-studio-canvas-next.html"
grep -n "data-video-inline-play" "$CANVAS_DIR/tools/workbench-web/image-studio-canvas-next.html"
grep -n "function cosPreviewThumbUrl" "$CANVAS_DIR/tools/workbench-web/image-studio-canvas-next.html"
if [[ -d "$ONLINE_WORKBENCH_DIR" ]]; then
  grep -n "function bindInlineVideoControls" "$ONLINE_WORKBENCH_DIR/image-studio-canvas-next.html"
fi

echo "部署完成：视频节点内嵌控制条可点击，COS 预览缩略图逻辑保留。"
