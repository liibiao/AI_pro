...
PKG="${1:-}"
if [[ -z "$PKG" || ! -f "$PKG" ]]; then
  echo "用法: $0 /tmp/firefly-chat-route-runtime-guard-*.tar.gz" >&2
  exit 1
fi
...
DEPLOY_DIR="/tmp/firefly-chat-route-runtime-guard-$STAMP"
BACKUP_DIR="$REMOTE_ROOT/backups/firefly-chat-route-runtime-guard-$STAMP"
...
REPAIR_SCRIPT="$DEPLOY_DIR/ai-admin-platform/api-server/scripts/repair-firefly-chat-route-runtime-guard.mjs"
...
cp -a "$REMOTE_API_ROOT/api-server/scripts/repair-firefly-chat-route-runtime-guard.mjs" \
  "$BACKUP_DIR/ai-admin-platform/api-server/scripts/repair-firefly-chat-route-runtime-guard.mjs" 2>/dev/null || true
...
cp -a "$REPAIR_SCRIPT" "$REMOTE_API_ROOT/api-server/scripts/repair-firefly-chat-route-runtime-guard.mjs"
...
(cd "$REMOTE_API_ROOT/api-server" && node scripts/repair-firefly-chat-route-runtime-guard.mjs)
...
const targets = [
  ...
  ['canvas-aiyunzhi-gpt-image-2', 'literal'],
];
