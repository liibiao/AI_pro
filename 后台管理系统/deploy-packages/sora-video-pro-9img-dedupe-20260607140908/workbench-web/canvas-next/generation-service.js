...
function imageRequestGeometry(model,values={}){
  ...
  const rawRatio=values.aspectRatio||values.aspect_ratio||values.requestedRatio||values.ratio||model?.defaults?.aspectRatio||model?.defaults?.aspect_ratio||'';
  const aspectRatio=grokImage?normalizeImageAspectRatio(rawRatio,fallbackSize):normalizeImageAspectRatio('',fallbackSize);
  const resolution=normalizeImageResolution(values.resolution||values.requestedResolution||model?.defaults?.resolution||'1K');
  const size=grokImage
    ? imageSizeFromRatioResolution(aspectRatio,resolution,fallbackSize)
    : fallbackSize;
...
function normalizeImageAspectRatio(value,fallbackSize=''){
  ...
  const direct=['1:1','16:9','9:16','4:3','3:4','3:2','2:3','2:1','1:2','19.5:9','9:19.5','20:9','9:20','auto'].find(item=>item===raw);
...
function imageSizeFromRatioResolution(aspectRatio,resolution,fallbackSize){
  ...
  const supported={
    '1K':{'1:1':'1024x1024','16:9':'1792x1024','9:16':'1024x1792','4:3':'4:3','3:4':'3:4','3:2':'1536x1024','2:3':'1024x1536','2:1':'2:1','1:2':'1:2','19.5:9':'19.5:9','9:19.5':'9:19.5','20:9':'20:9','9:20':'9:20','auto':'auto'},
    '2K':{'1:1':'2048x2048','16:9':'3584x2048','9:16':'2048x3584','4:3':'4:3','3:4':'3:4','3:2':'3072x2048','2:3':'2048x3072','2:1':'2:1','1:2':'1:2','19.5:9':'19.5:9','9:19.5':'9:19.5','20:9':'20:9','9:20':'9:20','auto':'auto'}
  };
...
function normalizeSupportedGrokImageSize(value){
  const raw=String(value||'').trim().toLowerCase().replace('×','x');
  if(['1:1','16:9','9:16','4:3','3:4','3:2','2:3','2:1','1:2','19.5:9','9:19.5','20:9','9:20','auto','1024x1024','2048x2048','1792x1024','1024x1792','1536x1024','1024x1536','1280x720','720x1280','3584x2048','2048x3584','3072x2048','2048x3072'].includes(raw))return raw;
...
function ratioFromImageSize(value){
  ...
  const candidates=[['20:9',20/9],['19.5:9',19.5/9],['2:1',2],['16:9',16/9],['3:2',3/2],['4:3',4/3],['1:1',1],['3:4',3/4],['2:3',2/3],['9:16',9/16],['1:2',1/2],['9:19.5',9/19.5],['9:20',9/20]];
...
function normalizeVideoRequestValues(model,values={}){
  ...
  const durations=capabilityList(caps.durations,grok?GROK_VIDEO_DURATIONS:['5','10','15']).map(String);
  ...
  const resolution=resolutions.includes(values.resolution)?values.resolution:(resolutions[0]||'720p');
...
export async function runPanorama(node,inputs,models,{onProgress}={}){
  ...
  if(modelHasBackendGateway(model)){
    const payload={mode:'panorama',prompt,size:node.values.size||'2048x1024',resolution,image:refs,images:refs,outputDir:OUTPUT_DIR};
  ...
  const data=await E.generateImageStandalone({
    ...
    size:node.values.size||'2048x1024',resolution,image:refs,images:refs,background:'auto',outputDir:OUTPUT_DIR
  });
...
async function runVideo(node,inputs,models,{onProgress}={}){
  ...
  const soraVideoProImages=imageRefs.slice(0,9);
  const soraVideoProFields=usesSoraVideoPro?{
    model:modelInfo.model,
    adapter:modelInfo.protocol.adapter||'sora-video-pro',
    protocolAdapter:modelInfo.protocol.adapter||'sora-video-pro',
    requestMethod:modelInfo.protocol.method||'async-poll',
    extra_images:soraVideoProImages,
    extraImages:soraVideoProImages,
  ...
  const start=await E.startVideoGeneration({
    ...
    aspectRatio:normalizedVideo.aspectRatio,generateAudio:values.generateAudio!==false,outputDir:OUTPUT_DIR,
    ...soraVideoProFields
  });
