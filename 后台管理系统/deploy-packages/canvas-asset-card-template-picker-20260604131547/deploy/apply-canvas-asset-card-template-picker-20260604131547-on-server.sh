...
PKG="canvas-asset-card-template-picker-20260604131547"
...
log "verify markers"
grep -Fq "20260604-asset-card-template-picker" "$HTML_DEST"
grep -Fq "function renderAssetCardTemplatePicker(card,idx)" "$HTML_DEST"
grep -Fq "data-asset-card-template-option" "$HTML_DEST"
grep -Fq "function showSharedAssetTemplatePreview" "$HTML_DEST"
grep -Fq ".asset-card-template-picker" "$CSS_DEST"
