#!/usr/bin/env bash
set -euo pipefail
PKG_NAME="canvas-asset-multi-image-ref-fix-20260531100901"
...
grep -q "renderPromptReferenceArea(n,{uploadKinds:'image'})" "$TARGET"
grep -q "最多支持 5 张图片参考" "$TARGET"
grep -q "支持上传多张图片参考" "$TARGET"
rm -rf "$WORKDIR"
echo "[deploy] done"
