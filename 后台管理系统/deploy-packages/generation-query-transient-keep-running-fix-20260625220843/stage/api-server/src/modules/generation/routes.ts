...
const generationTaskListReadRefreshInFlight = new Set<string>();
  ...
  try {
    task = await prisma.generationTask.create({
      data: {
        ...
        inputFilesJson: taskBody.inputFiles as Prisma.InputJsonValue,
        paramsJson: taskBody.params as Prisma.InputJsonValue,
        chargedCredits: 0,
        costAmount: 0,
        costUsd: new Prisma.Decimal(0),
      },
...
async function refreshGenerationTaskOnRead(
  task: Pick<RuntimeGenerationTask, 'id' | 'status' | 'upstreamTaskId' | 'updatedAt' | 'errorCode' | 'errorMessage' | 'refundReason'>,
  ownerUserId?: string,
) {
  if (!shouldRefreshGenerationTaskOnRead(task)) return false;
  try {
    const runtimeTask = await prisma.generationTask.findFirst({
      where: { id: task.id, ...(ownerUserId ? { userId: ownerUserId } : {}) },
      include: { provider: true, model: true },
    });
    if (!runtimeTask || !shouldRefreshGenerationTaskOnRead(runtimeTask)) return false;
...
async function refreshGenerationTaskListOnRead(
  tasks: Array<Pick<RuntimeGenerationTask, 'id' | 'status' | 'upstreamTaskId' | 'updatedAt' | 'errorCode' | 'errorMessage' | 'refundReason'>>,
  ownerUserId?: string,
  options: { wait?: boolean; limit?: number; maxWaitMs?: number } = {},
) {
  const limit = Math.max(1, Math.min(
    Number(options.limit || GENERATION_TASK_LIST_READ_REFRESH_LIMIT),
    GENERATION_TASK_LIST_READ_REFRESH_LIMIT,
  ));
...
async function refreshGenerationTaskListCandidate(
  task: Pick<RuntimeGenerationTask, 'id' | 'status' | 'upstreamTaskId' | 'updatedAt' | 'errorCode' | 'errorMessage' | 'refundReason'>,
  ownerUserId?: string,
) {
  if (generationTaskListReadRefreshInFlight.has(task.id)) return false;
...
function waitForGenerationTaskListRefresh(promise: Promise<boolean>, maxWaitMs: number) {
...
function shouldRefreshGenerationTaskOnRead(task: Pick<RuntimeGenerationTask, 'status' | 'upstreamTaskId' | 'updatedAt' | 'errorCode' | 'errorMessage' | 'refundReason'>) {
  if (isTerminalStatus(String(task.status || '')) && !canRecoverRefundedQueryFailure(task)) return false;
...
function generationTaskLightListSelect(options: { includeUser: boolean; includeRefunds: boolean }) {
  return {
    id: true,
    userId: true,
    clientRequestId: true,
...
type AdminGenerationDatePeriod = 'day' | 'month';

type AdminGenerationDateRange = {
  period: AdminGenerationDatePeriod;
  start: Date;
  end: Date;
  label: string;
  bucketKeys: string[];
...
type AdminGenerationTaskFilters = {
  ...
  requestedCategory: AdminGenerationResultCategory | null;
  dateRange: AdminGenerationDateRange | null;
};

type AdminGenerationReportTask = {
  id: string;
  createdAt: Date;
  type: ModelType;
  mode: string;
...
type AdminGenerationReportAggregateRow = {
  date_key: string;
  type: string;
  status: string;
  total_count: bigint | number | string;
  charged_credits: bigint | number | string | null;
  refund_credits: bigint | number | string | null;
};
...
function adminGenerationResultPageParams(query: Request['query']) {
...
function shanghaiLocalStartUtc(year: number, month: number, day: number) {
  return new Date(Date.UTC(year, month - 1, day) - SHANGHAI_UTC_OFFSET_MS);
}

function addShanghaiDays(year: number, month: number, day: number, days: number) {
...
function buildMonthBucketKeys(year: number, month: number) {
  const days = new Date(Date.UTC(year, month, 0)).getUTCDate();
...
function adminGenerationDateRange(query: Request['query'], fallbackToCurrentMonth = false): AdminGenerationDateRange | null {
  ...
  const period: AdminGenerationDatePeriod | '' = periodRaw === 'day' || periodRaw === 'month'
    ? periodRaw
    : hasDay
      ? 'day'
...
function buildAdminGenerationTaskFilters(query: Request['query'], fallbackToCurrentMonth = false): AdminGenerationTaskFilters {
...
function reportStatusKind(status: string) {
  const raw = String(status || '').toUpperCase();
  if (raw === 'SUCCESS') return 'success';
  if (['FAILED', 'TIMEOUT', 'CANCELLED'].includes(raw)) return 'failed';
  if (raw === 'REFUNDED') return 'refunded';
  return 'pending';
}

function emptyGenerationReportBucket(date: string) {
  return {
    key: date,
    date,
    label: date.slice(5),
...
function aggregateGenerationReport(tasks: AdminGenerationReportTask[], dateRange: AdminGenerationDateRange) {
...
function numberFromSqlAggregate(value: bigint | number | string | null | undefined) {
  if (typeof value === 'bigint') return Number(value);
  const num = Number(value || 0);
  return Number.isFinite(num) ? num : 0;
}

function aggregateGenerationReportRows(rows: AdminGenerationReportAggregateRow[], dateRange: AdminGenerationDateRange) {
  const byDate = new Map(dateRange.bucketKeys.map(key => [key, emptyGenerationReportBucket(key)]));
  const summary = {
    totalCount: 0,
    successCount: 0,
...
function needsAdminGenerationCategoryScan(category: AdminGenerationResultCategory | null) {
...
router.get('/admin/tasks', requireAuth, requireRole(adminRoles), asyncHandler(async (req, res) => {
  ...
  if (requestedCategory) {
    if (requestedCategory === 'IMAGE' || requestedCategory === 'VIDEO') {
      ...
      ok(res, {
        items: pageTasks.map(task => sanitizeGenerationTaskListItem(task as unknown as Record<string, unknown>, requestedCategory)),
...
function adminGenerationPagination(limit: number, offset: number, total: number, hasMore: boolean) {
  return { limit, offset, total, hasMore };
}

async function findAdminGenerationTaskPage(where: Prisma.GenerationTaskWhereInput, offset: number, take: number, predicate: (task: Record<string, unknown>) => boolean) {
  const ids: string[] = [];
  const chunkSize = 100;
  ...
  while (ids.length < take + 1 && scanned < maxScanned) {
    ...
    tasks.forEach(task => {
      if (!predicate(task as Record<string, unknown>)) return;
      if (matched >= offset) ids.push(String(task.id));
      matched += 1;
    });
  ...
  const items = uniqueIds.map(id => byId.get(id)).filter(Boolean) as Record<string, unknown>[];
  return { items, pagination: adminGenerationPagination(take, offset, total, hasMore) };
}

function adminGenerationReportSqlWhere(query: Request['query'], dateRange: AdminGenerationDateRange, requestedCategory: AdminGenerationResultCategory | null) {
  const status = queryText(query.status);
  const channelKey = queryText(query.channelKey);
  const userId = queryText(query.userId);
  ...
  if (status) conditions.push(Prisma.sql`"status" = ${status}::"GenerationTaskStatus"`);
  if (channelKey) conditions.push(Prisma.sql`"channel_key" = ${channelKey}`);
  ...
  if (type) conditions.push(Prisma.sql`"type" = ${type}::"ModelType"`);
  if (mode && !requestedCategory) conditions.push(Prisma.sql`"mode" ILIKE ${`%${mode}%`}`);
  return Prisma.join(conditions, ' AND ');
}

async function aggregateAdminGenerationReportWithSql(query: Request['query'], dateRange: AdminGenerationDateRange, requestedCategory: AdminGenerationResultCategory | null) {
  if (needsAdminGenerationCategoryScan(requestedCategory)) return null;
  const whereSql = adminGenerationReportSqlWhere(query, dateRange, requestedCategory);
  const rows = await prisma.$queryRaw<AdminGenerationReportAggregateRow[]>`
    SELECT
...
router.get('/admin/tasks/report', requireAuth, requireRole(adminRoles), asyncHandler(async (req, res) => {
  ...
  if (!dateRange) fail(400, '统计周期参数无效', 'GENERATION_REPORT_PERIOD_INVALID');
  ...
  const report = sqlReport || aggregateGenerationReport((needsAdminGenerationCategoryScan(requestedCategory)
    ...
    })).filter(task => generationTaskAdminCategory(task as unknown as Record<string, unknown>) === requestedCategory)
    : []) as AdminGenerationReportTask[], dateRange);
  ok(res, {
  ...
  for (const task of tasks) {
    try {
      if (task.upstreamTaskId) {
        ...
        if ('refund' in result && result.refund?.refundedCredits) refunded += Number(result.refund.refundedCredits || 0);
        continue;
      }
...
function resolveGenerationAdapterName(
  provider: { adapter?: string | null; providerKey?: string | null; name?: string | null; baseUrl?: string | null; endpointPath?: string | null; statusEndpointPath?: string | null },
  model: { adapter?: string | null; name?: string | null; displayName?: string | null; modelKey?: string | null; endpointPath?: string | null; statusEndpointPath?: string | null; protocol?: Prisma.JsonValue | null },
) {
  const configured = String(model.adapter || provider.adapter || '').trim();
  const configuredLower = configured.toLowerCase();
  const modelProtocol = model.protocol && typeof model.protocol === 'object' && !Array.isArray(model.protocol) ? model.protocol as Record<string, unknown> : {};
  const protocolAdapter = String(modelProtocol.adapter || modelProtocol.protocolAdapter || modelProtocol.protocol_adapter || '').trim().toLowerCase();
  const modelHint = [
  ...
  if (configuredLower === 'fullblood-video'
    ...
    || hint.includes('canvas-fullblood-video')
    || hint.includes('fullblood-video')
    || hint.includes('fullblood-seedance-2')
    || hint.includes('fullblood-omni-video-2')) return 'fullblood-video';
  if (configuredLower === 'openai-responses-image' || configuredLower === 'openai-chat-image') return configuredLower;
  if (configuredLower === 'aiyunzhi-gpt-image-2') return 'aiyunzhi-gpt-image-2';
  if (hint.includes('aiyunzhi-gpt-image-2-api') || hint.includes('canvas-aiyunzhi-gpt-image-2-api') || hint.includes('canvas_aiyunzhi-gpt-image-2-api')) return 'aiyunzhi-gpt-image-2';
  if (configuredLower === 'aiyunzhi-firefly-gpt-image') return 'aiyunzhi-firefly-gpt-image';
  if (hint.includes('aiyunzhi-firefly-gpt-image') || hint.includes('firefly-gpt-image')) return 'aiyunzhi-firefly-gpt-image';
  if (configuredLower === 'gpt-image-v2') return 'gpt-image-v2';
  if (configuredLower === 'gpt-image') return 'openai-edits';
  ...
  if (configuredLower === 'seedance-full'
    ...
    || hint.includes('/seedance-full/task')) return 'seedance-full';
  ...
  if (configuredLower === 'lingdong-sd-2-vip' || configuredLower === 'sd-2-vip' || hint.includes('lingdong-sd-2-vip') || hint.includes('lingdongapi.com') || /\bsd-2-vip\b/.test(hint)) return 'lingdong-sd-2-vip';
  if (configuredLower === 'zaomeng-seedance2'
    ...
    || hint.includes('seedance-2.0-fast')) return 'zaomeng-seedance2';
  if (configuredLower === 'toapis-seedance2'
    ...
    || hint.includes('toapis.com')) return 'toapis-seedance2';
  ...
  if (configuredLower === 'seedance2-sd' || configuredLower === 'sd-seedance2' || configuredLower === 'sd-video' || /\bsd2-(?:720p|1080p)(?:-fast)?\b/.test(hint) || hint.includes('aiyunzhi.top')) return 'seedance2-sd';
  if (configuredLower === 'seedance2' || configuredLower === 'seedance2.0' || hint.includes('seedance2') || hint.includes('seedance 2') || hint.includes('seedance-2')) return 'seedance2';
...
async function queryAndUpdateGenerationTask(task: RuntimeGenerationTask) {
  ...
  if (isTerminalStatus(task.status) && !canRecoverRefundedQueryFailure(task)) return { task };
  if (!task.upstreamTaskId) {
    const completed = await completeTaskFromStoredResultUrls(task);
    if (completed) return { task: completed };
  ...
  if (locallyCompleted) return { task: locallyCompleted };
  ...
  try {
    ...
    return markGenerationQueryTransientFailure(task, message);
  }
...
async function markGenerationQueryTransientFailure(task: RuntimeGenerationTask, message: string) {
  const terminal = isTerminalStatus(String(task.status || ''));
  const retryCount = Math.max(0, Number(task.retryCount || 0)) + 1;
  const detail = `上游任务状态查询暂时失败，任务保留继续轮询，不自动关闭或退款（第 ${retryCount} 次）：${message}`;
  const updated = await prisma.generationTask.update({
    where: { id: task.id },
    data: {
      ...(terminal ? {} : { status: 'RUNNING', failedAt: null }),
      retryCount: { increment: 1 },
      errorCode: 'GENERATION_QUERY_TRANSIENT',
      errorMessage: detail,
    },
    include: { provider: true, model: true },
  });
...
function canRecoverRefundedQueryFailure(task: Pick<RuntimeGenerationTask, 'status' | 'upstreamTaskId' | 'errorCode' | 'errorMessage' | 'refundReason'>) {
  if (!task.upstreamTaskId) return false;
  const code = String(task.errorCode || '').toUpperCase();
  const text = [task.errorMessage, task.refundReason].map(value => String(value || '')).join(' ');
  return code === 'GENERATION_QUERY_FAILED'
    || code === 'GENERATION_QUERY_TRANSIENT'
    || /上游任务状态查询失败|状态查询暂时失败|connect\s+ETIMEDOUT|ECONN|ETIMEDOUT|UPSTREAM_NETWORK_ERROR/i.test(text);
}

type MidjourneyTaskManagementAction = 'fetch' | 'seed' | 'cancel';

async function manageMidjourneyTaskForPrincipal(
  id: string,
  principal: OpenApiPrincipal,
  action: MidjourneyTaskManagementAction,
  input: z.infer<typeof mjTaskOperationSchema>,
) {
  const task = await prisma.generationTask.findFirst({
    where: { id, userId: principal.billingUserId },
    include: { provider: true, model: true },
  ...
  if (!task) fail(404, '生成任务不存在', 'GENERATION_TASK_NOT_FOUND');
  if (!isMidjourneyGenerationTask(task)) fail(400, '当前任务不是 Midjourney 图片任务', 'MIDJOURNEY_TASK_REQUIRED');
  const upstreamTaskId = resolveMidjourneyUpstreamTaskId(task, input);
  if (!upstreamTaskId) fail(400, '缺少 Midjourney 上游 taskId', 'MIDJOURNEY_TASK_ID_MISSING');
  ...
  if (action === 'cancel') {
    ...
    if (Number.isFinite(nextProgress)) data.progress = Math.max(0, Math.min(100, nextProgress));
  }
  const updated = await prisma.generationTask.update({
    where: { id: task.id },
    data,
...
function isMidjourneyGenerationTask(task: RuntimeGenerationTask) {
  const params = isRecord(task.paramsJson) ? task.paramsJson : {};
  const result = isRecord(task.resultJson) ? task.resultJson : {};
...
function resolveMidjourneyUpstreamTaskId(task: RuntimeGenerationTask, input: z.infer<typeof mjTaskOperationSchema>) {
  const result = isRecord(task.resultJson) ? task.resultJson : {};
  ...
  return firstNonEmptyString(
    ...
    input.taskId,
    task.upstreamTaskId,
    mjTask.upstreamTaskId,
    mjTask.taskId,
    mjTask.id,
    result.upstreamTaskId,
    result.taskId,
    isRecord(result.result) ? result.result.taskId : '',
    isRecord(result.result) ? result.result.id : '',
    response.taskId,
    isRecord(response.result) ? response.result.taskId : '',
    isRecord(response.result) ? response.result.id : '',
    params.upstreamTaskId,
    params.taskId,
  );
}

function normalizeMidjourneyManagedTask(
  upstream: unknown,
  task: RuntimeGenerationTask,
  meta: { action: MidjourneyTaskManagementAction; upstreamTaskId: string },
) {
  ...
  const seed = firstNonEmptyString(
    payload.seed,
    properties.seed,
    root.seed,
    meta.action === 'seed' && (typeof resultValue === 'string' || typeof resultValue === 'number') ? resultValue : '',
    previous.seed,
  );
  ...
  Object.keys(normalized).forEach(key => {
    ...
    if (value == null || value === '' || (Array.isArray(value) && !value.length)) delete normalized[key];
  });
  return normalized as Prisma.InputJsonObject;
}

function mergeMidjourneyManagedTaskResultJson(
  current: unknown,
  mjTask: Prisma.InputJsonObject,
  action: MidjourneyTaskManagementAction,
  upstream: unknown,
) {
...
function normalizeMidjourneyProgress(value: unknown) {
...
function resolveGenerationQueryProgress(task: RuntimeGenerationTask, result: { progress?: number }, status: string) {
  if (status !== 'RUNNING') return result.progress ?? task.progress;
  ...
  if (!isMidjourneyActionGenerationTask(task)) return base;
  return Math.max(base, slowMidjourneyActionFallbackProgress(task));
}

function isMidjourneyActionGenerationTask(task: RuntimeGenerationTask) {
  const params = isRecord(task.paramsJson) ? task.paramsJson : {};
  const hay = [
    task.mode,
    params.mjTaskKind,
...
function slowMidjourneyActionFallbackProgress(task: RuntimeGenerationTask) {
...
async function closeInterruptedGenerationTask(task: RuntimeGenerationTask, reason?: string, errorCode = 'GENERATION_TASK_INTERRUPTED') {
  const completed = await completeTaskFromStoredResultUrls(task);
  if (completed) return { task: completed };
...
async function closeExpiredRunningGenerationTask(task: RuntimeGenerationTask) {
  if (task.status !== 'RUNNING' && task.status !== 'PENDING' && task.status !== 'CREATED') return null;
  if (!isGrokVideoGenerationTask(task)) return null;
...
function isGrokVideoGenerationTask(task: RuntimeGenerationTask) {
...
async function materializeStoredGenerationResultUrls(task: RuntimeGenerationTask, urls: string[]) {
  ...
  if (task.type === 'VIDEO') {
    if (!shouldMaterializeVideoResults(task, urls)) return urls;
...
function shouldMaterializeVideoResults(task: Pick<RuntimeGenerationTask, 'type' | 'provider'> & { paramsJson?: unknown; requestJson?: unknown; resultJson?: unknown; responseJson?: unknown }, urls: string[]) {
  if (task.type !== 'VIDEO') return false;
  if (urls.some(url => isProtectedProviderResultUrl(task.provider, url))) return true;
...
function collectAsyncServerObjectStorageTemporaryUrls(task: { paramsJson?: unknown; requestJson?: unknown; resultUrlsJson?: unknown; resultJson?: unknown; responseJson?: unknown }) {
  ...
  return urls.filter(url => {
    ...
    if (/^data:(?:image|video|audio)\//i.test(raw)) return responseType === 'server_base64_async_object_storage';
    if (isGenerationResultPreviewUrl(raw)) return responseType === 'server_base64_async_object_storage';
...
async function materializeServerObjectStorageTask(taskId: string) {
  ...
  await prisma.generationTask.update({
    where: { id: task.id },
    data: { resultJson, resultUrlsJson: generationResultPublicUrls(sourceUrls) as Prisma.InputJsonValue },
  });
}
...
function findImageResponseType(value: unknown): '' | 'object_storage' | 'server_object_storage' | 'server_async_object_storage' | 'server_base64_object_storage' | 'server_base64_async_object_storage' | 'provider_url' | 'base64' {
  if (!value) return '';
...
function normalizeImageResponseTypeValue(value: unknown): '' | 'object_storage' | 'server_object_storage' | 'server_async_object_storage' | 'server_base64_object_storage' | 'server_base64_async_object_storage' | 'provider_url' | 'base64' {
...
function sanitizeGenerationTaskListItem<T extends Record<string, unknown>>(task: T, adminCategory?: AdminGenerationResultCategory): T & { adminCategory: AdminGenerationResultCategory } {
  const sanitized = {
    ...task,
    adminCategory: adminCategory ?? generationTaskAdminCategory(task),
  } as T & {
    adminCategory: AdminGenerationResultCategory;
    inputFilesJson?: unknown;
    paramsJson?: unknown;
    resultJson?: unknown;
    resultUrlsJson?: unknown;
  };
  if (Object.prototype.hasOwnProperty.call(task, 'inputFilesJson')) sanitized.inputFilesJson = sanitizeGenerationListJson(task.inputFilesJson, 'inputFilesJson');
  if (Object.prototype.hasOwnProperty.call(task, 'paramsJson')) sanitized.paramsJson = sanitizeGenerationListJson(task.paramsJson, 'paramsJson');
  if (Object.prototype.hasOwnProperty.call(task, 'resultJson')) sanitized.resultJson = sanitizeGenerationListJson(task.resultJson, 'resultJson');
  if (Object.prototype.hasOwnProperty.call(task, 'resultUrlsJson')) sanitized.resultUrlsJson = sanitizeGenerationListJson(task.resultUrlsJson, 'resultUrlsJson');
  return sanitized;
}
...
function sanitizeGenerationTaskResponse<T extends Record<string, unknown>>(payload: T): T {
  if (!payload || typeof payload !== 'object' || Array.isArray(payload)) return payload;
  const task = payload.task;
  if (!task || typeof task !== 'object' || Array.isArray(task)) return payload;
  return {
    ...payload,
    task: sanitizeGenerationTaskDetailItem(task as Record<string, unknown>),
  };
}
...
function normalizeStoredGenerationResultJson(resultJson: unknown, urls: string[]) {
  ...
  return { url: publicUrls[0], outputs: publicUrls } as Prisma.InputJsonValue;
}

function generationResultPublicUrls(urls: string[]) {
  const resolved: string[] = [];
  for (const url of urls) {
...
function generationResultPublicUrl(value: unknown) {
  ...
  if (/^\/api\/generation\/results\//i.test(raw)) return `${publicBase}${raw}`;
  return raw;
}

function decodeGenerationResultUrlEntities(value: unknown) {
...
function normalizeGenerationTaskParamsForModel(
  params: Record<string, unknown>,
  provider: { providerKey?: string | null; name?: string | null; baseUrl?: string | null; defaultModel?: string | null },
  model: { id?: string | null; modelKey?: string | null; name?: string | null; displayName?: string | null },
  type: ModelType,
) {
  if (type !== 'IMAGE' || !isGptImage2ProOfficialProxyModel(provider, model)) return params;
  const resolution = normalizeGptImage2ProResolutionParam(firstNonEmpty(
    params.requestedResolution,
    params.resolution,
    params.imageSize,
    params.image_size,
    params.requestedPixelSize,
    params.pixelSize,
    params.size,
  ));
  const ratio = normalizeGptImage2ProRatioParam(firstNonEmpty(
    params.aspect_ratio,
    params.aspectRatio,
    params.requestedRatio,
    params.ratio,
    params.size,
    params.requestedPixelSize,
    params.pixelSize,
...
function isGptImage2ProOfficialProxyModel(
  provider: { providerKey?: string | null; name?: string | null; baseUrl?: string | null; defaultModel?: string | null },
  model: { id?: string | null; modelKey?: string | null; name?: string | null; displayName?: string | null },
) {
  const hint = [
  ...
  if (hint.includes('hongniaoai.com') || hint.includes('hongniao') || hint.includes('gpt-image-2(pro)')) return false;
...
function normalizeGptImage2ProResolutionParam(value: unknown) {
  const raw = String(value || '').trim().toLowerCase().replace(/\s+/g, '').replace('×', 'x');
  if (raw === '1k') return '1k';
  if (/^[2-4]k$/.test(raw)) return '2k';
  const match = raw.match(/(\d{3,5})x(\d{3,5})/i);
  if (match) {
    const longSide = Math.max(Number(match[1]), Number(match[2]));
    return longSide >= 1500 ? '2k' : '1k';
  }
  return '1k';
}

function normalizeGptImage2ProRatioParam(value: unknown) {
...
function aspectRatioNumberFromParam(value: string) {
...
function gptImage2ProPixelSize(ratio: string, resolution: string) {
