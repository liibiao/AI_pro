...
WORK_DIR="$(mktemp -d)"
BACKUP_DIR="${APP_DIR}/backups/generation-query-transient-keep-running-fix-$(date +%Y%m%d%H%M%S)"
...
grep -q "markGenerationQueryTransientFailure" "${ROUTES_SRC}"
grep -q "GENERATION_QUERY_TRANSIENT" "${ROUTES_SRC}"
grep -q "markGenerationQueryTransientFailure" "${ROUTES_DIST}"
grep -q "GENERATION_QUERY_TRANSIENT" "${ROUTES_DIST}"
...
echo "Generation query transient keep-running fix deployed."
