...
PKG="${1:-/tmp/asset-design-template-cos-final-20260601020426.tar.gz}"
...
WORK="$(mktemp -d /tmp/asset-design-template-cos-final-20260601020426-XXXXXX)"
BACKUP="/var/www/ai-admin/backups/asset-design-template-cos-final-20260601020426-$STAMP"
...
tar -xzf "$PKG" -C "$WORK"
SRC="$WORK/asset-design-template-cos-final-20260601020426"
...
test -f "$MANIFEST" || { echo "未检测到 19 张参考图 COS 映射 manifest.json，拒绝部署占位缩略图" >&2; exit 2; }
...
test "$MANIFEST_COUNT" -ge 19 || { echo "COS 映射不足 19 条，当前 $MANIFEST_COUNT 条，拒绝部署" >&2; exit 2; }
