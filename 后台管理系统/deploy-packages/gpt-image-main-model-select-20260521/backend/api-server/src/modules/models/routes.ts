...
router.patch('/admin/models/:id', requireAuth, requireRole(adminRoles), asyncHandler(async (req, res) => {
  ...
  if (simpleBody.success && ['baseUrl', 'adapter', 'model', 'modelNick', 'key', 'uploadMode', 'imageMainModel'].some(key => key in req.body)) {
    ...
    const model = await prisma.aiModel.update({
      ...
      data: {
        ...
        supports: normalized.supports,
        defaults: mergeJson(jsonObject(normalized.defaults), jsonObject(existing.defaults), imageMainModelJsonPatch(normalized.protocol)),
        capabilities: normalized.capabilities,
...
function normalizeSimpleModelConfig(input: { baseUrl: string; adapter: string; model: string; modelNick: string; key: string; type?: ModelType; uploadMode?: string | null; imageMainModel?: string | null }) {
