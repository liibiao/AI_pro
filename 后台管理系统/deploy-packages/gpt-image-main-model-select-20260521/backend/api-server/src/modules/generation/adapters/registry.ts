...
import { callUpstreamGetJson, callUpstreamJson, callUpstreamMultipart, extractImageResult, extractUpstreamTaskId, extractVideoResultUrl, extractVideoTask } from '../../../upstream.js';
...
async function submitOpenAiEdits(ctx: AdapterContext): Promise<AdapterSubmitResult> {
  ...
  const requestJson = withGptImage2MainModel(ctx, compactJson({
    ...
    size: ctx.params.size || ctx.params.imageSize || '1024x1024',
    n: ctx.params.n || ctx.params.quantity || 1,
    ...withoutKeys(ctx.params, ['model', 'prompt', 'image', 'images', 'input_image', 'reference_images', 'referenceImages', 'mask', 'size', 'imageSize', 'n', 'quantity', 'response_format', 'responseFormat', 'main_model', 'mainModel', 'responses_model', 'responsesModel', 'codex_model', 'codexModel', 'imageMainModel']),
  ...
  if (!collectOpenAiPrimaryFileRefs(requestJson).length) {
    ...
    if (imageUrlRequestJson) {
      ...
      return normalizeImageSubmit(ctx.provider, upstream, imageUrlRequestJson);
    }
  }
  try {
    ...
    try {
      ...
      return normalizeImageSubmit(ctx.provider, fallbackUpstream, fallbackRequestJson);
    } catch (fallbackErr) {
...
async function submitOpenAiTextImage(ctx: AdapterContext): Promise<AdapterSubmitResult> {
  ...
  const requestJson = withGptImage2MainModel(ctx, compactJson({
    ...
    ...withoutKeys(ctx.params, [
      ...
      'mask', 'response_format', 'responseFormat',
      'main_model', 'mainModel', 'responses_model', 'responsesModel', 'codex_model', 'codexModel', 'imageMainModel',
    ]),
    response_format: responseFormat,
  }));
...
async function submitOpenAiChat(ctx: AdapterContext): Promise<AdapterSubmitResult> {
  const requestJson = compactJson({
    ...
    ...withoutKeys(ctx.params, ['messages', 'temperature']),
  });
  const upstream = await callUpstreamJson(ctx.provider, resolveEndpointPath(ctx.provider.endpointPath || ctx.model.endpointPath || '/chat/completions', ctx), requestJson, ctx.timeoutMs);
  return {
    status: 'SUCCESS',
    progress: 100,
    ...
    resultJson: upstream as Prisma.InputJsonValue,
  };
}
...
async function submitGrokImage(ctx: AdapterContext): Promise<AdapterSubmitResult> {
  ...
  const result = await normalizeImageSubmit(ctx.provider, upstream, requestJson);
...
async function queryGenericImage(ctx: AdapterContext & { upstreamTaskId: string }): Promise<AdapterQueryResult> {
  ...
  for (const endpointPath of imageStatusEndpointCandidates(ctx)) {
    ...
    try {
      ...
      return await normalizeGenericImageQueryResult(ctx.provider, ctx.upstreamTaskId, upstream);
    } catch (err) {
      lastErr = err;
...
async function normalizeGenericImageQueryResult(provider: UpstreamProvider, upstreamTaskId: string, upstream: unknown): Promise<AdapterQueryResult> {
  const statusText = extractGenericTaskStatus(upstream);
  const urls = await materializeImageResultUrls(provider, absolutizeProviderUrls(provider, extractUrls(upstream).filter(url => !/^data:image\//i.test(url))));
  const failed = ['failed', 'error', 'cancelled', 'canceled'].includes(statusText);
  ...
  return {
    ...
    resultJson: { url: urls[0] || '', outputs: urls } as Prisma.InputJsonValue,
    resultUrls: urls,
    errorCode: failed ? 'IMAGE_TASK_FAILED' : undefined,
    errorMessage: failed ? extractErrorMessage(upstream) : undefined,
  };
}
...
async function submitGrokImageEdit(ctx: AdapterContext): Promise<AdapterSubmitResult> {
  assertImageModelName(ctx);
  const refs = collectGrokImageRefs(ctx).slice(-3);
...
async function submitGptImageV2(ctx: AdapterContext): Promise<AdapterSubmitResult> {
  ...
  const referenceImages = buildGptImageV2ReferenceImages(ctx);
  const requestJson = compactJson({
  ...
  const fallback = await normalizeImageSubmit(ctx.provider, upstream, requestJson);
  ...
  return {
    ...
    resultUrls: [],
    errorCode: 'GPT_IMAGE_V2_RESULT_MISSING',
    errorMessage: 'GPT-Image-v2 上游响应未包含 data[0].url 或 task.result_urls',
  };
}

async function submitGeminiImage(ctx: AdapterContext): Promise<AdapterSubmitResult> {
  assertImageModelName(ctx);
  const aspectRatio = String(ctx.params.aspectRatio || ctx.params.aspect_ratio || ctx.params.requestedRatio || ctx.params.size || '1:1');
  const resolution = String(ctx.params.resolution || ctx.params.requestedResolution || ctx.params.quality || '').toUpperCase();
  ...
  const resultUrl = await imageResultUrl(ctx.provider, result);
  const urls = resultUrl ? [resultUrl] : [];
  return {
    status: 'SUCCESS',
    progress: 100,
    upstreamRequestId: String((upstream as any)?.responseId || (upstream as any)?.id || ''),
    requestJson,
    responseJson: sanitizeUpstreamPayload(upstream) as Prisma.InputJsonValue,
    resultJson: { url: resultUrl, mimeType: result.mimeType, outputs: urls } as Prisma.InputJsonValue,
    resultUrls: urls,
  };
}
...
async function submitGeminiUnifiedImageEdit(ctx: AdapterContext): Promise<AdapterSubmitResult> {
  assertImageModelName(ctx);
  const refs = collectGrokImageRefs(ctx).slice(-4);
...
function shouldPreferInlineImageResult(ctx: AdapterContext) {
  ...
  return !mode || mode === 'object_storage' || mode === 'cos' || mode === 'tencent_cos';
}
...
async function callOpenAiImageJson(ctx: AdapterContext, endpoint: string, requestJson: Prisma.InputJsonValue) {
  try {
    ...
    return callUpstreamJson(ctx.provider, endpoint, fallbackJson, ctx.timeoutMs);
  }
}
...
async function buildOpenAiEditsReferencePayload(ctx: AdapterContext) {
  ...
  if (primaryFileIds.length) {
    return {
      ...
      images: primaryFileRefs.map(entry => compactJson({
        id: entry.id || entry.fileId,
        name: entry.name || 'reference.png',
        width: entry.width,
        fileId: entry.fileId || entry.id,
        height: entry.height,
        remoteUrl: entry.remoteUrl || entry.fileId || entry.id,
...
async function submitGenericVideo(ctx: AdapterContext): Promise<AdapterSubmitResult> {
  ...
  const requestJson = compactJson({
    ...
    prompt: ctx.prompt,
    negative_prompt: ctx.negativePrompt,
    duration: ctx.params.duration || ctx.params.durationSeconds || ctx.params.seconds || 5,
    seconds: ctx.params.seconds || ctx.params.duration || ctx.params.durationSeconds || 5,
  ...
  const urls = resultUrl ? [resultUrl] : extractUrls(upstream);
  const statusText = String((upstream as any)?.status || (upstream as any)?.data?.status || '').toLowerCase();
  const failed = ['failed', 'error', 'cancelled', 'canceled'].includes(statusText);
  const done = ['success', 'succeeded', 'completed', 'done'].includes(statusText);
  return {
    status: failed ? 'FAILED' : (done && urls.length ? 'SUCCESS' : 'RUNNING'),
...
async function submitSoraVideo(ctx: AdapterContext): Promise<AdapterSubmitResult> {
  ...
  const secondsNumber = Math.max(1, Math.min(120, Number.parseInt(String(ctx.params.seconds || ctx.params.duration || ctx.params.durationSeconds || 5).replace(/s$/i, ''), 10) || 5));
  const requestJson = compactJson({
...
async function submitSeedance2VipVideo(ctx: AdapterContext): Promise<AdapterSubmitResult> {
  ...
  const seconds = normalizeSeedance2VipSeconds(ctx.params.seconds || ctx.params.duration || ctx.params.durationSeconds || ctx.params.duration_seconds);
  const videoParams = withoutKeys(ctx.params, [
    'model', 'prompt', 'negative_prompt', 'stream',
    'duration', 'durationSeconds', 'duration_seconds', 'seconds',
    'aspectRatio', 'aspect_ratio', 'requestedRatio',
...
async function submitVeoChatVideo(ctx: AdapterContext): Promise<AdapterSubmitResult> {
  ...
  const resolution = normalizeAllowedString(ctx.params.resolution || ctx.params.quality, ['720p', '1080p'], '720p');
  const duration = normalizeAllowedNumber(ctx.params.duration || ctx.params.durationSeconds || ctx.params.seconds, [4, 6, 8], 6);
...
async function submitGeminiVideo(ctx: AdapterContext): Promise<AdapterSubmitResult> {
  ...
  const aspectRatio = normalizeAllowedString(ctx.params.aspectRatio || ctx.params.aspect_ratio, ['16:9', '9:16'], '16:9');
  const resolution = normalizeAllowedString(ctx.params.resolution || ctx.params.quality, ['720p', '1080p'], '720p');
  const duration = normalizeAllowedNumber(ctx.params.duration || ctx.params.durationSeconds || ctx.params.seconds || ctx.params.duration_seconds, [4, 6, 8], 8);
  const videoParams = withoutKeys(ctx.params, [
    'model', 'messages', 'prompt', 'stream',
    'duration', 'durationSeconds', 'duration_seconds', 'seconds',
    'aspectRatio', 'aspect_ratio', 'requestedRatio',
  ...
  const upstream = await callUpstreamJson(ctx.provider, endpoint, requestJson, ctx.timeoutMs);
  return normalizeGeminiVideoResult(ctx.provider, extractGeminiVideoTask(upstream), upstream, requestJson);
}
...
async function queryGeminiVideo(ctx: AdapterContext & { upstreamTaskId: string }): Promise<AdapterQueryResult> {
  ...
  return normalizeGeminiVideoResult(ctx.provider, ctx.upstreamTaskId, upstream);
}
...
async function submitGrokVideo(ctx: AdapterContext): Promise<AdapterSubmitResult> {
  ...
  const seconds = Math.max(6, Math.min(30, Number.parseInt(String(ctx.params.seconds || ctx.params.duration || ctx.params.durationSeconds || 6), 10) || 6));
  ...
  const upstream = await callUpstreamJson(ctx.provider, endpoint, requestJson, ctx.timeoutMs);
  const resultUrl = extractVideoResultUrl(upstream);
  const urls = absolutizeProviderUrls(ctx.provider, resultUrl ? [resultUrl] : extractUrls(upstream));
  const statusText = String((upstream as any)?.status || (upstream as any)?.data?.status || '').toLowerCase();
  const failed = ['failed', 'error', 'cancelled', 'canceled'].includes(statusText);
  return {
...
async function queryGenericVideo(ctx: AdapterContext & { upstreamTaskId: string }): Promise<AdapterQueryResult> {
  ...
  for (const endpointPath of endpoints) {
    ...
    try {
      ...
      return normalizeVideoQueryResult(ctx.upstreamTaskId, upstream);
    } catch (err) {
      if (!isRetryableVideoStatusEndpointError(err)) throw err;
...
function normalizeVideoQueryResult(upstreamTaskId: string, upstream: unknown): AdapterQueryResult {
  const statusText = String((upstream as any)?.status || (upstream as any)?.data?.status || '').toLowerCase();
  const failed = ['failed', 'error', 'cancelled', 'canceled'].includes(statusText);
  const done = ['success', 'succeeded', 'completed', 'done'].includes(statusText);
  const resultUrl = extractVideoResultUrl(upstream);
...
function normalizeGeminiVideoResult(provider: UpstreamProvider, upstreamTaskId: string, upstream: unknown, requestJson: Prisma.InputJsonValue): AdapterSubmitResult;
function normalizeGeminiVideoResult(provider: UpstreamProvider, upstreamTaskId: string, upstream: unknown, requestJson?: undefined): AdapterQueryResult;
function normalizeGeminiVideoResult(provider: UpstreamProvider, upstreamTaskId: string, upstream: unknown, requestJson?: Prisma.InputJsonValue): AdapterSubmitResult | AdapterQueryResult {
  const statusText = extractGeminiVideoStatus(upstream);
  const failed = ['failed', 'error', 'cancelled', 'canceled'].includes(statusText);
  const resultUrl = extractVideoResultUrl(upstream);
  const urls = absolutizeProviderUrls(provider, resultUrl ? [resultUrl] : extractUrls(upstream));
...
function collectGrokImageRefs(ctx: AdapterContext) {
  const refs = new Set<string>();
  const push = (value: unknown) => {
    if (!value) return;
    if (typeof value === 'string') {
...
async function normalizeImageSubmit(provider: UpstreamProvider, upstream: unknown, requestJson: Prisma.InputJsonValue): Promise<AdapterSubmitResult> {
  const result = extractImageResult(upstream);
  const resultUrl = await imageResultUrl(provider, result);
  const urls = resultUrl ? [resultUrl] : await materializeImageResultUrls(provider, absolutizeProviderUrls(provider, extractUrls(upstream).filter(url => !/^data:image\//i.test(url))));
...
async function imageResultUrl(provider: UpstreamProvider, result: { url?: string; b64?: string; mimeType?: string }) {
...
export async function materializeImageResultUrl(provider: UpstreamProvider, value: string) {
  ...
  return isProviderEphemeralImageResultUrl(provider, raw) ? '' : raw;
}
...
async function uploadRemoteImageUrlToObjectStorageBestEffort(provider: UpstreamProvider, rawUrl: string) {
  ...
  try {
    ...
    if (Number.isFinite(contentLength) && contentLength > config.objectStorage.maxBytes) return '';
    const bytes = Buffer.from(await resp.arrayBuffer());
    if (!bytes.length || bytes.length > config.objectStorage.maxBytes) return '';
...
function buildGptImageV2ReferenceImages(ctx: AdapterContext) {
  const refs = new Set<string>();
  const push = (value: unknown) => {
    if (!value) return;
    if (typeof value === 'string') {
  ...
  ctx.inputFiles.forEach(item => push(item));
  const rawImages = ctx.params.reference_images || ctx.params.referenceImages || ctx.params.images || ctx.params.image;
  if (Array.isArray(rawImages)) rawImages.forEach(item => push(item));
  else push(rawImages);
  return Array.from(refs);
}
...
function extractErrorMessage(payload: unknown) {
  ...
  return String(data?.error?.message || data?.error || data?.message || data?.data?.error || data?.data?.message || '上游任务失败');
}
