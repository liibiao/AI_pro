...
function openAiResponsesImageGeometryOptions(ctx: AdapterContext) {
  ...
  if (!isGptImage2Request(ctx)) return compactJson({ size }) as Record<string, unknown>;
  return compactJson({
    size,
    aspect_ratio: resolveGptImage2AspectRatio(ctx, size),
    resolution: resolveGptImage2Resolution(ctx, size),
...
function extractAiyunzhiFireflyImageUrlFromText(value: unknown) {
  ...
  return (imageUrl || matches[0] || '').trim();
}
