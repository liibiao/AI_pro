...
PKG="${1:-/tmp/storyboard-video-preview-url-mode-fix-20260601030109.tar.gz}"
...
WORK="$(mktemp -d /tmp/storyboard-video-preview-url-mode-fix-20260601030109-XXXXXX)"
BACKUP="/var/www/ai-admin/backups/storyboard-video-preview-url-mode-fix-20260601030109-$STAMP"
...
tar -xzf "$PKG" -C "$WORK"
SRC="$WORK/storyboard-video-preview-url-mode-fix-20260601030109"
...
grep -q "buildVideoPreviewSourceCandidates" "$HTML"
grep -q "fallbackUrls:\\[inlineSrc" "$HTML"
grep -q "URL内容提取" "$HTML"
...
run_sudo grep -q "buildVideoPreviewSourceCandidates" "$WORKBENCH_DIR/image-studio-canvas-next.html"
run_sudo grep -q "fallbackUrls:\\[inlineSrc" "$WORKBENCH_DIR/image-studio-canvas-next.html"
run_sudo grep -q "URL内容提取" "$WORKBENCH_DIR/image-studio-canvas-next.html"
