...
function GenerationResults() {
  ...
  const q = useQuery({
    ...
    queryFn: () => {
      ...
      if (activeTab === 'ASSET_DESIGN') params.set('category', 'assetDesign');
      if (activeTab === 'SHOT_STORYBOARD') params.set('category', 'shotStoryboard');
...
function isStoryboardTask(task: any) {
  if (isAssetDesignTask(task) || isShotStoryboardTask(task)) return false;
  const text = generationTaskClassifierText(task);
  return matchesStoryboardText(text) || String(task?.mode || '').trim().toLowerCase() === 'storyboard';
}

function isAssetDesignTask(task: any) {
  if (matchesAssetDesignText(generationTaskClassifierText(task))) return true;
  return /资产设计图|资产设计稿|资产设计提示词|完整资产设计|asset\s+design/i.test(generationTaskPromptText(task));
}

function isShotStoryboardTask(task: any) {
  if (matchesShotStoryboardText(generationTaskClassifierText(task))) return true;
  return /分镜图|分镜表|分镜格|号镜|镜号|storyboard\s+sheet/i.test(generationTaskPromptText(task));
}
...
function generationTaskPromptText(task: any) {
  return String(task?.prompt || '').toLowerCase();
}
