...
PKG="${PKG:-/tmp/canvas-text-asset-node-fix-20260529170520.tar.gz}"
...
BACKUP_DIR="${BACKUP_DIR:-/var/www/ai-admin/backups/canvas-text-asset-node-fix-20260529170520-$STAMP}"
TMP_DIR="$(mktemp -d /tmp/canvas-text-asset-node-fix-20260529170520.XXXXXX)"
...
SRC="$TMP_DIR/workbench-web/image-studio-canvas-next.html"
test -f "$SRC"
...
grep -q "assetDesign" "$TARGET_FILE"
grep -q "textPrompt" "$TARGET_FILE"
grep -q "/api/models?type=LLM" "$TARGET_FILE"
grep -q "prompt-node-card" "$TARGET_FILE"
grep -q "runLlmPromptNode" "$TARGET_FILE"
