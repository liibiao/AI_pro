...
import { Alert, AutoComplete, Button, Card, ConfigProvider, Form, Input, InputNumber, Layout, Menu, Modal, Popconfirm, Progress, Segmented, Select, Space, Statistic, Switch, Table, Tabs, Tag, App as AntApp, message } from 'antd';
...
function Dashboard() {
  ...
  }, [excludeAgentQuota, excludeSuperAdminSelfRecharge, granularity, range.end, range.start]);
    ...
    <Space direction="vertical" size={16} className="dashboardPage">
      <Card className="dashboardControlCard">
        <div className="dashboardControlHeader">
          <div>
            <h2>数据概览</h2>
            <p>{periodText}统计，所有图表和明细使用同一筛选口径</p>
          </div>
          <div className="dashboardControls">
            ...
            />
            {granularity === 'day' ? (
              <Input className="dashboardDateInput" type="month" value={selectedMonth} onChange={e => setSelectedMonth(e.target.value || currentMonthValue())} />
            ) : (
              <InputNumber className="dashboardYearInput" min={2020} max={2099} value={selectedYear} onChange={value => setSelectedYear(Number(value || new Date().getFullYear()))} />
            )}
          </div>
        </div>
      ...
      <div className="llmChatDebugLayout">
        <div className="llmChatConfig">
          <Form form={form} layout="vertical" initialValues={{ maxOutputTokens: 800, prompt: '帮我写一个无限画布 AI 创作流程的简短说明' }} onFinish={values => m.mutate(values)}>
            <Form.Item name="modelId" label="模型" rules={[{ required: true }]}>
              <Select loading={models.isLoading} placeholder="请选择后台已启用的大语言模型" options={modelItems.map((model: any) => ({ value: model.id, label: `${model.provider?.providerKey || '-'} · ${model.displayName || model.name} · ${model.creditsPerUsdCost || 0}积分/刀` }))} />
            </Form.Item>
