...
PKG="gpt-image-base64-server-cos-20260606002658"
...
grep -Fq "persistInlineImage(result.b64" "$API_REGISTRY_DIST"
...
grep -Rqs "124 base64" "$ADMIN_DIST"
grep -Fq "server_base64_object_storage" "$HTML_SRC"
grep -Fq "responseType==='server_base64_object_storage'" "$HTML_SRC"
...
run_sudo grep -Fq "persistInlineImage(result.b64" "$REMOTE_APP_ROOT/api-server/dist/modules/generation/adapters/registry.js"
...
run_sudo grep -Rqs "124 base64" "$REMOTE_APP_ROOT/admin-web/dist"
run_sudo grep -Fq "server_base64_object_storage" "$WORKBENCH_DIR/image-studio-canvas-next.html"
run_sudo grep -Fq "responseType==='server_base64_object_storage'" "$WORKBENCH_DIR/image-studio-canvas-next.html"
