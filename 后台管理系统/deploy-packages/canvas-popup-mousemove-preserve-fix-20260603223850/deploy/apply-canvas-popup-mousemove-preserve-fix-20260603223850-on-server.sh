...
PKG="canvas-popup-mousemove-preserve-fix-20260603223850"
...
log "verify markers"
grep -Fq "function hideAllPopups(opts={})" "$HTML_DEST"
grep -Fq "preserveConnTarget" "$HTML_DEST"
grep -Fq "连接目标弹窗只在点击空白时关闭" "$HTML_DEST"
grep -Fq "hideAllPopups({preserveConnTarget:true})" "$HTML_DEST"
grep -Fq "stage.addEventListener('click'" "$HTML_DEST"
