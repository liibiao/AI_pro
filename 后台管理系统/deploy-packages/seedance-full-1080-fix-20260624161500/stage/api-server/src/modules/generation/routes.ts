...
const GENERATION_TASK_LIST_READ_REFRESH_LIMIT = 5;
...
function refreshGenerationTaskListOnRead(
  ...
  const candidates = tasks
    ...
    .slice(0, GENERATION_TASK_LIST_READ_REFRESH_LIMIT);
  for (const task of candidates) {
    generationTaskListReadRefreshInFlight.add(task.id);
    setTimeout(() => {
      refreshGenerationTaskOnRead(task, ownerUserId)
        .catch(err => {
...
router.get('/admin/tasks', requireAuth, requireRole(adminRoles), asyncHandler(async (req, res) => {
  ...
  if (requestedCategory) {
    if (requestedCategory === 'IMAGE' || requestedCategory === 'VIDEO') {
      ...
      let pageTasks = tasks;
      if (await refreshGenerationTaskListOnRead(tasks)) {
        pageTasks = await prisma.generationTask.findMany({
          where,
          select: generationTaskLightListSelect({ includeUser: true, includeRefunds: true }),
          orderBy: { createdAt: 'desc' },
...
async function queryAndUpdateGenerationTask(task: RuntimeGenerationTask) {
  ...
  try {
    ...
    const updated = await prisma.generationTask.update({
      ...
      data: {
        status: 'RUNNING',
        errorCode: 'GENERATION_QUERY_RETRY_PENDING',
        errorMessage: `上游任务状态暂时无法查询，将自动重试：${message}`,
        retryCount: { increment: 1 },
        failedAt: null,
      },
      include: { provider: true, model: true },
    });
    return { task: updated, transientQueryFailure: true };
  }
...
function canRecoverRefundedQueryFailure(task: Pick<RuntimeGenerationTask, 'status' | 'upstreamTaskId' | 'errorCode' | 'errorMessage' | 'refundReason'>) {
  if (task.status !== 'REFUNDED' || !task.upstreamTaskId) return false;
  const text = [task.errorCode, task.errorMessage, task.refundReason].filter(Boolean).join(' ');
  return task.errorCode === 'GENERATION_QUERY_FAILED' || isTransientGenerationFailureMessage(text);
}
