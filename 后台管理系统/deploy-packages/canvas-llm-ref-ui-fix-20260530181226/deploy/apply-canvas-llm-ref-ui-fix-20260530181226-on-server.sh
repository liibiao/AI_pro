...
PKG="${1:-/tmp/canvas-llm-ref-ui-fix-20260530181226.tar.gz}"
...
WORK="$(mktemp -d /tmp/canvas-llm-ref-ui-fix-20260530181226-XXXXXX)"
...
log "package: $PKG"
tar -xzf "$PKG" -C "$WORK"
SRC="$WORK/canvas-llm-ref-ui-fix-20260530181226"
...
BACKUP_ROOT="/var/www/ai-admin/backups/canvas-llm-ref-ui-fix-20260530181226-$STAMP"
...
log "install api"
run_sudo mkdir -p "$APP_DIR/api-server/src/modules/generation/adapters" "$APP_DIR/api-server/src/modules/generate" "$APP_DIR/api-server/src/modules/workbench" \
  "$APP_DIR/api-server/dist/modules/generation/adapters" "$APP_DIR/api-server/dist/modules/generate" "$APP_DIR/api-server/dist/modules/workbench"
