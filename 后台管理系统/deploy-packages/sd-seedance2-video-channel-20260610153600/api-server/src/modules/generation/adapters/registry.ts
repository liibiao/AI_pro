...
async function submitSeedance2Video(ctx: AdapterContext): Promise<AdapterSubmitResult> {
  ...
  const refGroups = (await collectPublicImageReferenceUrlGroups(ctx, ctx.timeoutMs || 15000, { allowLocalFiles: true })).slice(0, 9);
  const hasReferences = refGroups.length > 0;
  const requestJson = compactJson({
    model: modelName,
    prompt: ctx.prompt,
    aspect_ratio: aspectRatio,
    resolution,
    duration,
    referenceImageCount: refGroups.length,
    ...(hasReferences ? { images: refGroups.map(group => group.map((ref, index) => redactLargeInlineRef(ref, index))) } : {}),
  });

  const upstream = hasReferences
    ? await callUpstreamMultipart(ctx.provider, resolveSeedance2MultipartEndpoint(ctx), await buildSeedance2VideoForm(requestJson, refGroups, ctx.timeoutMs), ctx.timeoutMs)
...
async function querySeedance2Video(ctx: AdapterContext & { upstreamTaskId: string }): Promise<AdapterQueryResult> {
  ...
  const endpoint = resolveEndpointPath(ctx.model.statusEndpointPath || ctx.provider.statusEndpointPath || '/video/generations/{taskId}', { ...ctx, taskId: ctx.upstreamTaskId });
  const upstream = await callUpstreamGetJson(ctx.provider, endpoint, {}, ctx.timeoutMs);
  return await normalizeSeedance2VideoTaskResult(ctx, upstream, { taskId: ctx.upstreamTaskId } as Prisma.InputJsonValue);
}
...
async function buildSeedance2VideoForm(requestJson: Prisma.InputJsonValue, refGroups: string[][], timeoutMs: number) {
  ...
  form.set('prompt', String(request.prompt || ''));
  ...
  for (let i = 0; i < refGroups.length; i += 1) {
    const file = await imageRefCandidatesToBlob(refGroups[i], timeoutMs, (ref, nextTimeoutMs) => compatibleImageRefToBlob(ref, nextTimeoutMs, 'Seedance 2.0 视频'));
