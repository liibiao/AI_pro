...
PKG="${1:-/tmp/shared-preview-prompt-zoom-animation-20260601003023.tar.gz}"
...
WORK="$(mktemp -d /tmp/shared-preview-prompt-zoom-animation-20260601003023-XXXXXX)"
BACKUP="/var/www/ai-admin/backups/shared-preview-prompt-zoom-animation-20260601003023-$STAMP"
...
tar -xzf "$PKG" -C "$WORK"
SRC="$WORK/shared-preview-prompt-zoom-animation-20260601003023"
...
log "verify markers"
run_sudo grep -q "prompt-zoom-native-group" "$WORKBENCH_DIR/image-studio-canvas-next.html"
run_sudo grep -q "animateSharedPreviewClone" "$WORKBENCH_DIR/image-studio-canvas-next.html"
run_sudo grep -q "shared-preview-clone" "$WORKBENCH_DIR/image-studio-canvas-next.html"
