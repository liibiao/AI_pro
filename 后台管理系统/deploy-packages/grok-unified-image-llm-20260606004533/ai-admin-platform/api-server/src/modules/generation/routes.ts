...
function resolveGenerationAdapterName(provider: { adapter?: string | null; providerKey?: string | null; name?: string | null }, model: { adapter?: string | null; name?: string | null; displayName?: string | null; modelKey?: string | null }) {
  ...
  if (configuredLower === 'openai-responses-image') return configuredLower;
  ...
  if (['gemini-image-generate', 'gemini-image-edit', 'gemini-video', 'veo-video', 'gemini-chat', 'gemini-llm', 'veo-chat', 'veo-3.1', 'grok-video', 'grok_image', 'grok-image-unified', 'grok-image', 'grok-image-edit', 'grok-chat', 'grok-llm', 'openai-responses-image'].includes(configuredLower)) return configuredLower;
...
function isGptImage2GenerationHint(modelHint: string, hint: string, configuredLower: string) {
  ...
  return ['openai-edits', 'openai-image', 'openai-generations', 'openai-responses-image', 'gpt-image', 'gpt-image-v2'].includes(configuredLower)
...
function findImageResponseType(value: unknown): '' | 'object_storage' | 'server_object_storage' | 'server_base64_object_storage' | 'provider_url' | 'base64' {
  if (!value) return '';
...
function normalizeImageResponseTypeValue(value: unknown): '' | 'object_storage' | 'server_object_storage' | 'server_base64_object_storage' | 'provider_url' | 'base64' {
