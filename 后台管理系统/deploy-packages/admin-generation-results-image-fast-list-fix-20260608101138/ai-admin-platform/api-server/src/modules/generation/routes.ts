...
router.get('/admin/tasks', requireAuth, requireRole(adminRoles), asyncHandler(async (req, res) => {
  ...
  if (requestedCategory) {
    if (requestedCategory === 'IMAGE') {
      ...
      ok(res, { items: tasks.map(task => sanitizeGenerationTaskListItem(task as unknown as Record<string, unknown>, 'IMAGE')) });
      return;
    }
    const tasks = await findAdminGenerationTasks(where, take, task => generationTaskAdminCategory(task) === requestedCategory);
    ok(res, { items: tasks.map(task => sanitizeGenerationTaskListItem(task, requestedCategory)) });
    return;
  }
  const tasks = await prisma.generationTask.findMany({
    where,
  ...
  ok(res, { items: tasks.map(task => sanitizeGenerationTaskListItem(task)) });
}));
