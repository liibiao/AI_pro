...
export function normalizeModelType(model){
  const raw=String(model?.modelType||model?.type||model?.category||'').trim().toLowerCase();
  if(['image','img','image_generation','text2image','txt2img','image_to_image','img2img'].includes(raw))return 'image';
  if(['video','video_generation','text2video','txt2video','image2video','img2video'].includes(raw))return 'video';
  if(['llm','chat','language','text','text_generation','completion'].includes(raw))return 'llm';
  const hay=[model?.name,model?.model,model?.nick,model?.displayName,model?.label,model?.ui?.label].map(v=>String(v||'')).join(' ').toLowerCase();
  ...
  if(/deepseek|qwen|glm|claude|grok[-_ ]?4|gpt[-_ ]?(?:3|4|4o|5)|gpt\d|chat|llm|语言/.test(hay))return 'llm';
  return 'image';
}
