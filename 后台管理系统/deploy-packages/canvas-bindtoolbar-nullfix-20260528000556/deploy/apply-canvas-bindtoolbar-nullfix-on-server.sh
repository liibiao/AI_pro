...
set -euo pipefail
PKG="${1:-}"
if [[ -z "$PKG" || ! -f "$PKG" ]]; then echo "用法: sudo bash $0 /tmp/canvas-bindtoolbar-nullfix-YYYYMMDDHHMMSS.tar.gz" >&2; exit 1; fi
...
BACKUP="${BACKUP:-/var/www/ai-admin/backups/canvas-bindtoolbar-nullfix-$STAMP}"
TMP="/tmp/canvas-bindtoolbar-nullfix-$STAMP"
...
grep -n "const on=(id,handler,event='onclick')" "$ONLINE_WORKBENCH_DIR/image-studio-canvas-next.html"
