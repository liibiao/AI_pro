...
PKG="prompt-ref-drag-at-insert-20260602171247"
...
grep -Fq "function bindPromptReferenceDragGlobal()" "$DEST"
grep -Fq "promptRefDragAttrs(n.id,'i2i','image'" "$DEST"
grep -Fq "promptRefDragAttrs(n.id,'vn2'" "$DEST"
grep -Fq "storyboardImage'].includes(n?.type)" "$DEST"
