...
PKG="canvas-next-storyboard-grid-select-fix-20260604023815"
...
log "verify markers"
grep -Fq "btn._shotToggleAt=Date.now();" "$HTML_DEST"
grep -Fq "if(Date.now()-Number(btn._shotToggleAt||0)<450)return;" "$HTML_DEST"
grep -Fq "e.stopImmediatePropagation?.();" "$HTML_DEST"
grep -Fq "if(btn.classList.contains('storygrid-panel')||btn.classList.contains('storygrid-cell'))" "$HTML_DEST"
