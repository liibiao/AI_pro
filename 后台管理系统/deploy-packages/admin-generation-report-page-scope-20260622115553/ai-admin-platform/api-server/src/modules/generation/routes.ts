...
async function findAdminGenerationReportTasks(
  where: Prisma.GenerationTaskWhereInput,
  offset: number,
  take: number,
  requestedCategory: AdminGenerationResultCategory | null,
  ...
  if (!needsAdminGenerationCategoryScan(requestedCategory)) {
    return prisma.generationTask.findMany({
      ...
      skip: offset,
      take,
    }) as Promise<AdminGenerationReportTask[]>;
  }

  const tasks: AdminGenerationReportTask[] = [];
  const chunkSize = 100;
  const maxScanned = 10000;
  let scanned = 0;
  let matched = 0;
  while (tasks.length < take && scanned < maxScanned) {
    const chunk = await prisma.generationTask.findMany({
      where,
      select: generationTaskReportSelect(),
      orderBy: { createdAt: 'desc' },
      skip: scanned,
    ...
    chunk.forEach(task => {
      if (generationTaskAdminCategory(task as unknown as Record<string, unknown>) !== requestedCategory) return;
      if (matched >= offset && tasks.length < take) tasks.push(task as unknown as AdminGenerationReportTask);
      matched += 1;
    });
...
router.get('/admin/tasks/report', requireAuth, requireRole(adminRoles), asyncHandler(async (req, res) => {
  ...
  const { take, skip } = adminGenerationResultPageParams(req.query);
  const reportTasks = await findAdminGenerationReportTasks(where, skip, take, requestedCategory);
  const report = aggregateGenerationReport(reportTasks, dateRange);
  ok(res, {
    category: requestedCategory,
    scope: 'page',
    pagination: { limit: take, offset: skip, count: reportTasks.length },
