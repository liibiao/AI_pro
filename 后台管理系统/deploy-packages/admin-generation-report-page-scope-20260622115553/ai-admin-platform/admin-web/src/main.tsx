...
function GenerationResults() {
  ...
  const reportQuery = useQuery({
    queryKey: ['generationResultsReport', activeTab, filters, reportPeriod, reportDate, reportMonth, page],
    queryFn: () => {
      const params = new URLSearchParams();
      if (filters.status) params.set('status', filters.status);
      if (filters.userId) params.set('userId', filters.userId.trim());
...
function GenerationResultReport({ activeTab, period, page, data, loading }: {
  activeTab: GenerationResultTab;
  period: GenerationReportPeriod;
  page: number;
  ...
  return (
    <div className="generationReportPanel">
      <div className="generationReportHeader">
        <div>
          <h3>{generationResultTabLabel(activeTab)}当前页数据概览</h3>
          <span>{periodLabel} · 第 {page} 页</span>
        </div>
        <div className="dashboardInlineLegend">
      ...
      <div className="generationReportMetricGrid">
        <div className="generationReportMetric"><span>当前页任务数</span><strong>{formatNumber(summary.totalCount)}</strong><small>成功率 {generationReportSuccessRate(summary)}</small></div>
  ...
  return (
    <>
      <div className="formGrid2">
        ...
        <Form.Item name="adapter" label="Adapter" rules={[{ required: true }]}><Input placeholder="openai-image / midjourney-imagine / grok-image / gemini-image-generate / gemini-video / gemini-chat" /></Form.Item>
