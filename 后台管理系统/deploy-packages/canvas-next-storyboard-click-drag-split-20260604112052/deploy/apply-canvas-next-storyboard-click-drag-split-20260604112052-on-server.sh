...
PKG="canvas-next-storyboard-click-drag-split-20260604112052"
...
log "verify markers"
grep -Fq "const isShotSurface=btn.classList.contains('storygrid-panel')||btn.classList.contains('storygrid-cell');" "$HTML_DEST"
grep -Fq "if(!dragging&&isShotPanel&&Math.hypot(dx,dy)>7)" "$HTML_DEST"
grep -Fq "else{btn._shotToggleAt=Date.now();toggleShot();}" "$HTML_DEST"
grep -Fq "if(isShotSurface&&Date.now()-Number(btn._shotToggleAt||0)<450)return;" "$HTML_DEST"
grep -Fq "reorderStoryboardImageShot(n,from,to)" "$HTML_DEST"
