...
async function imageResultUrl(provider: UpstreamProvider, result: { url?: string; b64?: string; mimeType?: string }, responseType: ImageResponseType = 'object_storage') {
  ...
  if (responseType === 'server_async_object_storage') {
    ...
    if (result.b64) return await persistInlineImageLocalPreview(inlineImageDataUrl(result.b64, result.mimeType || 'image/png'), result.mimeType || 'image/png') || '';
    if (/^data:image\//i.test(directUrl)) return await persistInlineImageLocalPreview(directUrl, result.mimeType || 'image/png') || '';
    return '';
  }
