...
PKG="${1:-/tmp/video-shared-preview-animation-fix-20260601005752.tar.gz}"
...
WORK="$(mktemp -d /tmp/video-shared-preview-animation-fix-20260601005752-XXXXXX)"
BACKUP="/var/www/ai-admin/backups/video-shared-preview-animation-fix-20260601005752-$STAMP"
...
tar -xzf "$PKG" -C "$WORK"
SRC="$WORK/video-shared-preview-animation-fix-20260601005752"
...
log "verify markers"
run_sudo grep -q "videoPreviewTargetRect" "$WORKBENCH_DIR/image-studio-canvas-next.html"
run_sudo grep -q -- "--vp-preview-aspect" "$WORKBENCH_DIR/image-studio-canvas-next.html"
