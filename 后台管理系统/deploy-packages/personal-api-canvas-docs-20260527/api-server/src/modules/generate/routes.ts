...
router.post('/llm/chat', requireAuth, asyncHandler(async (req, res) => {
  ...
  try {
    const endpointPath = resolveEndpointPath(body.endpointPath || model.endpointPath || '/chat/completions', { model: model.name });
    const upstream = runtime.generationMockMode
...
async function getModel(modelId: string, type: 'IMAGE' | 'VIDEO' | 'LLM') {
  ...
  if (!model || model.status !== 'ACTIVE' || model.type !== type) fail(404, '模型不可用', 'MODEL_NOT_FOUND');
  return model;
}
