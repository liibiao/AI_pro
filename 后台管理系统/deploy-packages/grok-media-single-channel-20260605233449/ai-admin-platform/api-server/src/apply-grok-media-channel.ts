...
import { IMAGE_PRICING_TIERS, VIDEO_PRICING, imagePricingDefaults, videoPricingDefaults } from './pricing.js';
...
async function main() {
  ...
  await upsertGrokImageEditModel(provider.id);
  ...
  console.log(`Grok media channel applied. providerKey=${provider.providerKey}, baseUrl=${baseUrl}`);
}

async function disableLegacyGrokChannels(activeProviderId: string) {
  const grokAdapters = ['grok-image', 'grok-image-edit', 'grok-video', 'grok-chat', 'grok-llm'];
...
async function upsertGrokImageModel(providerIdValue: string) {
  await prisma.aiModel.upsert({
    ...
    update: {
      ...
      displayName: 'Grok 文生图',
      type: 'IMAGE',
      adapter: 'grok-image',
      endpointPath: '/images/generations',
      statusEndpointPath: null,
      uploadMode: 'object_storage',
      protocol: { adapter: 'grok-image', endpointPath: '/images/generations', method: 'sync', uploadMode: 'object_storage' },
      supports: { txt2img: true, img2img: false, imageToImage: false, storyboard: true, panorama: false },
      defaults: { ...imagePricingDefaults(IMAGE_PRICING_TIERS), size: '1024x1024' },
      ui: { label: 'Grok 文生图', badge: 'Grok', badgeColor: '#111827' },
      status: 'ACTIVE',
    },
    create: {
