...
PKG="lingdong-vip-stuck-95-refresh-fix-20260612051310"
...
grep -Fq "queryLingdongSd2VipVideo" "$SRC_ROOT/api-server/src/modules/generation/adapters/registry.ts"
grep -Fq "providerResultFetchHeaders" "$SRC_ROOT/api-server/dist/modules/generation/adapters/registry.js"
grep -Fq "GENERATION_TASK_LIST_READ_REFRESH_LIMIT" "$SRC_ROOT/api-server/src/modules/generation/routes.ts"
grep -Fq "refreshGenerationTaskListOnRead" "$SRC_ROOT/api-server/dist/modules/generation/routes.js"
grep -Fq "lingdong running targets" "$SRC_ROOT/deploy/refresh-lingdong-vip-running-tasks.mjs"
...
log "refresh existing Lingdong SD2 VIP running tasks"
run_sudo bash -lc "cd '$API_DIR' && node '$SRC_ROOT/deploy/refresh-lingdong-vip-running-tasks.mjs'"
...
run_sudo grep -Fq "queryLingdongSd2VipVideo" "$API_DIR/dist/modules/generation/adapters/registry.js"
run_sudo grep -Fq "providerResultFetchHeaders" "$API_DIR/dist/modules/generation/adapters/registry.js"
run_sudo grep -Fq "refreshGenerationTaskListOnRead" "$API_DIR/dist/modules/generation/routes.js"
curl -fsS http://127.0.0.1:4000/api/health >/dev/null 2>&1 && log "api health: ok" || log "api health: skipped/failed"

log "done"
echo "backup: $BACKUP_DIR"
echo "Lingdong VIP stuck 95 refresh fix deployed"
