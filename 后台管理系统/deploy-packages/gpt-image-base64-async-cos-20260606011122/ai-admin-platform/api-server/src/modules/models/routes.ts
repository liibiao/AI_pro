...
function normalizeClientModelSupports(supports: Prisma.JsonValue | null, adapter: string, type: ModelType) {
  ...
  if (['openai-edits', 'openai-responses-image', 'gpt-image-v2', 'gemini-image', 'gemini-image-edit', 'grok-image-edit'].includes(adapter)) {
    return {
      ...base,
...
function defaultImageSupports(adapter: string, type: ModelType): Prisma.InputJsonValue | undefined {
  ...
  const imageToImage = ['openai-edits', 'openai-responses-image', 'gpt-image-v2', 'gemini-image', 'gemini-image-edit', 'grok-image-edit'].includes(adapter);
  return {
...
router.patch('/admin/models/:id', requireAuth, requireRole(adminRoles), asyncHandler(async (req, res) => {
  ...
  const simplePatchKeys = ['baseUrl', 'adapter', 'model', 'modelNick', 'key', 'uploadMode', 'imageMainModel', 'upstreamStreamConfigMode', 'upstreamStreamMode', 'upstreamStreamModes', 'asyncTaskConfigMode', 'asyncTaskMode', 'asyncTaskModes', 'responseTypeMode', 'responseType', 'responseTypes', 'maxVideoDurationSeconds', 'maxVideoDurationSecondsByResolution', 'disabledVideoResolutions'];
...
function normalizeSimpleModelConfig(input: { baseUrl: string; adapter: string; model: string; modelNick: string; key: string; type?: ModelType; uploadMode?: string | null; imageMainModel?: string | null; upstreamStreamConfigMode?: string | null; upstreamStreamMode?: string | null; upstreamStreamModes?: Record<string, string> | null; asyncTaskConfigMode?: string | null; asyncTaskMode?: string | null; asyncTaskModes?: Record<string, string> | null; responseTypeMode?: string | null; responseType?: string | null; responseTypes?: Record<string, string> | null; maxVideoDurationSeconds?: number | null; maxVideoDurationSecondsByResolution?: Record<string, number | null> | null; disabledVideoResolutions?: string[] | null }) {
  ...
  const videoCapabilities = type === 'VIDEO'
    ? normalizeVideoDurationCapabilities({
      ...
      ...(input.disabledVideoResolutions ? { disabledVideoResolutions: input.disabledVideoResolutions } : {}),
    })
    : undefined;
  return {
