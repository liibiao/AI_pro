...
func videosCreateRequestFromForm(c *gin.Context) ([]byte, error) {
	rawJSON := []byte(`{}`)
	for _, field := range []string{"model", "prompt", "duration", "seconds", "size", "aspect_ratio", "resolution"} {
...
func buildXAIVideosCreateRequest(rawJSON []byte, model string) ([]byte, xaiVideoCreateMetadata, error) {
	...
	secondsInput := gjson.GetBytes(rawJSON, "seconds").String()
	if strings.TrimSpace(secondsInput) == "" {
		secondsInput = gjson.GetBytes(rawJSON, "duration").String()
	}
	seconds, duration, err := normalizeXAIVideosSeconds(secondsInput)
	if err != nil {
	...
	if imageURL == "" && len(referenceImages) == 1 && xaiVideosWantsImageToVideo(rawJSON) {
...
func xaiVideosWantsImageToVideo(rawJSON []byte) bool {
	modeParts := []string{
		gjson.GetBytes(rawJSON, "mode").String(),
		gjson.GetBytes(rawJSON, "videoMode").String(),
		gjson.GetBytes(rawJSON, "grokVideoMode").String(),
		gjson.GetBytes(rawJSON, "xaiVideoMode").String(),
	}
	mode := strings.ToLower(strings.Join(modeParts, " "))
	return strings.Contains(mode, "image-to-video") ||
		strings.Contains(mode, "image_to_video") ||
		strings.Contains(mode, "img2video") ||
		strings.Contains(mode, "i2v")
}
