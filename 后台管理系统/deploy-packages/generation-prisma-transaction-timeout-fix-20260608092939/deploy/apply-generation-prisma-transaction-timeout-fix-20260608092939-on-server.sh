...
PKG="generation-prisma-transaction-timeout-fix-20260608092939"
...
grep -Fq "GENERATION_TASK_TRANSACTION_OPTIONS" "$GEN_SRC"
grep -Fq "writeProviderHealthLog" "$GEN_SRC"
grep -Fq "GENERATION_TASK_TRANSACTION_OPTIONS" "$GEN_DIST"
grep -Fq "writeProviderHealthLog" "$GEN_DIST"
if grep -Fq "tx.providerHealthLog.create" "$GEN_SRC" "$GEN_DIST"; then
  fail "providerHealthLog is still written through transaction client"
fi
...
run_sudo grep -Fq "GENERATION_TASK_TRANSACTION_OPTIONS" "$APP_DIR/api-server/src/modules/generation/routes.ts"
run_sudo grep -Fq "writeProviderHealthLog" "$APP_DIR/api-server/dist/modules/generation/routes.js"
if run_sudo grep -Fq "tx.providerHealthLog.create" "$APP_DIR/api-server/src/modules/generation/routes.ts" "$APP_DIR/api-server/dist/modules/generation/routes.js"; then
  fail "installed files still contain tx.providerHealthLog.create"
fi
