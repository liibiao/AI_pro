...
async function refreshGenerationTaskOnRead(
  task: Pick<RuntimeGenerationTask, 'id' | 'status' | 'upstreamTaskId' | 'updatedAt'>,
  ownerUserId?: string,
) {
...
function shouldRefreshGenerationTaskOnRead(task: Pick<RuntimeGenerationTask, 'status' | 'upstreamTaskId' | 'updatedAt'>) {
  if (isTerminalStatus(String(task.status || ''))) return false;
