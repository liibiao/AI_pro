...
PKG="${1:-/tmp/cliproxy-public-oauth-callback-20260611141534.tar.gz}"
PKG_NAME="cliproxy-public-oauth-callback-20260611141534"
...
BACKUP_DIR="${BACKUP_DIR:-$INSTALL_DIR/backups/public-oauth-callback-$STAMP}"
WORK="$(mktemp -d "/tmp/$PKG_NAME-XXXXXX")"

log() { printf '[public-oauth-callback] %s\n' "$*"; }
...
tar -xzf "$PKG" -C "$WORK"
SRC="$WORK/$PKG_NAME"
BIN="$SRC/bin/CLIProxyAPI-public-oauth-callback-amd64"

test -f "$BIN" || { echo "package missing binary: $BIN" >&2; exit 1; }
grep -Fq "publicManagementCallbackURL" "$SRC/src/internal/api/handlers/management/auth_files.go"
