...
func (h *Handler) publicManagementCallbackURL(c *gin.Context, path string) (string, error) {
	...
	scheme := firstForwardedHeader(c, "X-Forwarded-Proto")
	if scheme == "" && c.Request.URL != nil {
		scheme = c.Request.URL.Scheme
	}
	if scheme == "" {
		scheme = "http"
		if h != nil && h.cfg != nil && h.cfg.TLS.Enable {
			scheme = "https"
		}
	}
	scheme = strings.ToLower(strings.TrimSpace(scheme))
	if scheme != "http" && scheme != "https" {
		return "", fmt.Errorf("unsupported callback scheme %q", scheme)
	}

	host := firstForwardedHeader(c, "X-Forwarded-Host")
	if host == "" {
		host = strings.TrimSpace(c.Request.Host)
	}
	if host == "" {
		return "", fmt.Errorf("request host is unavailable")
	}
	return (&url.URL{Scheme: scheme, Host: host, Path: path}).String(), nil
}

func firstForwardedHeader(c *gin.Context, name string) string {
	if c == nil || c.Request == nil {
		return ""
	}
...
func (h *Handler) ServePluginAuthURL(c *gin.Context) bool {
	...
	if isWebUIRequest(c) {
		baseURL, errBaseURL = h.publicManagementCallbackURL(c, "/v0/management/oauth-callback")
	}
	if errBaseURL != nil {
...
func (h *Handler) RequestAnthropicToken(c *gin.Context) {
	...
	if isWebUI {
		redirectURI, err = h.publicManagementCallbackURL(c, "/anthropic/callback")
		if err != nil {
			log.WithError(err).Error("failed to compute anthropic public callback URL")
			c.JSON(http.StatusInternalServerError, gin.H{"error": "callback url unavailable"})
			return
		}
	}
...
func (h *Handler) RequestGeminiCLIToken(c *gin.Context) {
	...
	isWebUI := isWebUIRequest(c)
	if isWebUI {
		redirectURI, errRedirect := h.publicManagementCallbackURL(c, "/google/callback")
		if errRedirect != nil {
			log.WithError(errRedirect).Error("failed to compute gemini public callback URL")
			c.JSON(http.StatusInternalServerError, gin.H{"error": "callback url unavailable"})
			return
		}
...
func (h *Handler) RequestCodexToken(c *gin.Context) {
	...
	if isWebUI {
		redirectURI, err = h.publicManagementCallbackURL(c, "/codex/callback")
		if err != nil {
			log.WithError(err).Error("failed to compute codex public callback URL")
			c.JSON(http.StatusInternalServerError, gin.H{"error": "callback url unavailable"})
			return
		}
	}
...
func (h *Handler) RequestAntigravityToken(c *gin.Context) {
	...
	isWebUI := isWebUIRequest(c)
	if isWebUI {
		redirectURI, errState = h.publicManagementCallbackURL(c, "/antigravity/callback")
		if errState != nil {
			log.WithError(errState).Error("failed to compute antigravity public callback URL")
			c.JSON(http.StatusInternalServerError, gin.H{"error": "callback url unavailable"})
			return
		}
	}
...
func (h *Handler) RequestXAIToken(c *gin.Context) {
	...
	isWebUI := isWebUIRequest(c)
	if isWebUI {
		redirectURI, errPKCE = h.publicManagementCallbackURL(c, "/xai/callback")
		if errPKCE != nil {
			log.WithError(errPKCE).Error("failed to compute xai public callback URL")
			c.JSON(http.StatusInternalServerError, gin.H{"error": "callback url unavailable"})
			return
		}
	}
