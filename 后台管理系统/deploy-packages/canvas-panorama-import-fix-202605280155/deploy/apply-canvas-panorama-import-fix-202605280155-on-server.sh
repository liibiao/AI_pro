#!/usr/bin/env bash
set -euo pipefail
PKG="/tmp/canvas-panorama-import-fix-202605280155.tar.gz"
WORK="/tmp/canvas-panorama-import-fix-202605280155"
TARGET="/var/www/ai-admin/workbench-web"
MIRROR="/home/ubuntu/漫剧创作库/tools/workbench-web"
BACKUP="/var/www/ai-admin/backups/workbench-web-canvas-panorama-import-fix-202605280155-$(date +%Y%m%d%H%M%S)"
if [ ! -f "$PKG" ]; then echo "缺少部署包：$PKG" >&2; exit 1; fi
rm -rf "$WORK"
mkdir -p "$WORK"
tar -xzf "$PKG" -C "$WORK"
if [ ! -d "$WORK/canvas-panorama-import-fix-202605280155/workbench-web" ]; then echo "部署包结构错误" >&2; exit 1; fi
mkdir -p "$(dirname "$TARGET")" /var/www/ai-admin/backups
if [ -d "$TARGET" ]; then cp -a "$TARGET" "$BACKUP"; fi
rm -rf "$TARGET"
cp -a "$WORK/canvas-panorama-import-fix-202605280155/workbench-web" "$TARGET"
