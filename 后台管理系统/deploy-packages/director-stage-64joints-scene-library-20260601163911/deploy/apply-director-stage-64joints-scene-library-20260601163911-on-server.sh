...
PKG="${1:-/tmp/director-stage-64joints-scene-library-20260601163911.tar.gz}"
...
WORK="$(mktemp -d /tmp/director-stage-64joints-scene-library-20260601163911-XXXXXX)"
BACKUP="/var/www/ai-admin/backups/director-stage-64joints-scene-library-20260601163911-$STAMP"
...
tar -xzf "$PKG" -C "$WORK"
SRC="$WORK/director-stage-64joints-scene-library-20260601163911"
...
grep -q "upperChest" "$HTML"
grep -q "function directorStageOpenSceneAssetLibrary" "$HTML"
grep -q "director-stage-scene-modal" "$HTML"
grep -q "data-director-action=\"openSceneAssets\"" "$HTML"
grep -q "directorStageBackgroundFromAssetTask" "$HTML"
grep -q "application/x-mjb-asset-task" "$HTML"
grep -q "makeShellSegment" "$HTML"
grep -q "nose-bridge" "$HTML"
...
run_sudo grep -q "upperChest" "$WORKBENCH_DIR/image-studio-canvas-next.html"
run_sudo grep -q "function directorStageOpenSceneAssetLibrary" "$WORKBENCH_DIR/image-studio-canvas-next.html"
run_sudo grep -q "director-stage-scene-modal" "$WORKBENCH_DIR/image-studio-canvas-next.html"
run_sudo grep -q "data-director-action=\"openSceneAssets\"" "$WORKBENCH_DIR/image-studio-canvas-next.html"
run_sudo grep -q "directorStageBackgroundFromAssetTask" "$WORKBENCH_DIR/image-studio-canvas-next.html"
run_sudo grep -q "application/x-mjb-asset-task" "$WORKBENCH_DIR/image-studio-canvas-next.html"
run_sudo grep -q "makeShellSegment" "$WORKBENCH_DIR/image-studio-canvas-next.html"
run_sudo grep -q "nose-bridge" "$WORKBENCH_DIR/image-studio-canvas-next.html"
