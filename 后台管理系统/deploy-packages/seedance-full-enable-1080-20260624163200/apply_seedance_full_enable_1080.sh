...
WORK_ROOT="/tmp/seedance-full-enable-1080-$$"
BACKUP_ROOT="/var/www/ai-admin/backups/seedance-full-enable-1080-$(date +%Y%m%d%H%M%S)"
...
try {
  ...
  if (!model) {
    console.log(`[seedance-full-enable] model ${modelId} not found, skip db patch`);
  } else {
    ...
    console.log(`[seedance-full-enable] ${providerId}/${modelId} ACTIVE with 720p+1080p`);
  }
} finally {
...
echo "seedance-full enable 1080 installed"
