...
PKG="canvas-next-storyboard-compose-image-node-20260604023120"
...
log "verify markers"
grep -Fq "function normalizeStoryboardShotNoList(n,shots=[])" "$HTML_DEST"
grep -Fq "async function ensureStoryboardImageCroppedShots(n,shots=[])" "$HTML_DEST"
grep -Fq "async function recomposeStoryboardShots(n,opts={})" "$HTML_DEST"
grep -Fq "const selectedOnly=opts.selectedOnly===true" "$HTML_DEST"
grep -Fq "function createStoryboardCompositeImageNode(sourceId,imageEntry,opts={})" "$HTML_DEST"
grep -Fq "function connectStoryboardCompositeImageNode(sourceId,targetId)" "$HTML_DEST"
grep -Fq "async function runStoryboardImageRecomposeAction(id,n)" "$HTML_DEST"
grep -Fq "await recomposeStoryboardShots(n,{selectedOnly:true})" "$HTML_DEST"
grep -Fq "createStoryboardCompositeImageNode(id,image,{shotNos,stageLabel:'合成分镜图'})" "$HTML_DEST"
grep -Fq "if(action==='recomposeShots'&&isStoryboardShotEditNode(n))return runStoryboardImageRecomposeAction(id,n)" "$HTML_DEST"
