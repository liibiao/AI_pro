...
const CANVAS_LOCAL_ASSET_DIR = path.resolve(process.cwd(), '..', '..', 'runninghub_outputs', 'image-studio-v3');
...
function resolveSafeLocalImageReferencePath(value: string) {
  ...
  if (!candidate && path.isAbsolute(raw)) candidate = raw;
  if (!candidate) return '';
  const resolved = path.resolve(candidate);
  const allowedRoots = [GENERATED_IMAGE_DIR, CANVAS_LOCAL_ASSET_DIR].map(root => path.resolve(root));
  const allowed = allowedRoots.some(root => resolved === root || resolved.startsWith(root + path.sep));
  if (!allowed || !/\.(?:png|jpe?g|webp|gif)$/i.test(resolved)) return '';
  return resolved;
}
