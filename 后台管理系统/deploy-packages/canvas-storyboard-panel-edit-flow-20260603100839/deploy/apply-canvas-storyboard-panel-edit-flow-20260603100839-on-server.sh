...
PKG="canvas-storyboard-panel-edit-flow-20260603100839"
...
log "verify markers"
grep -Fq "tapnow-rewrite.css?v=20260603-storyboard-panel-edit-flow" "$HTML_DEST"
grep -Fq "function renderStoryboardImageReferenceArea(n)" "$HTML_DEST"
grep -Fq "function cropStoryboardImageSourceToShots(n,source,shots=null)" "$HTML_DEST"
grep -Fq "function reorderStoryboardImageShot(n,fromNo,toNo)" "$HTML_DEST"
grep -Fq "data-shot-drag" "$HTML_DEST"
grep -Fq "function generateStoryboardImageShotEdit(id,n,entry,activeModel)" "$HTML_DEST"
grep -Fq "async function runStoryboardImageNode(id)" "$HTML_DEST"
grep -Fq "function openShotStoryboardRowEditor(id,shotNo)" "$HTML_DEST"
grep -Fq "局部重绘任务" "$HTML_DEST"
grep -Fq ".storygrid-sliced" "$HTML_DEST"
grep -Fq ".storygrid-ref-area" "$HTML_DEST"
grep -Fq ".shot-row-editor-modal" "$HTML_DEST"
