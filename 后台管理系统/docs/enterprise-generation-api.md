# 企业生成 API 接入文档（AI 智能体可直接调试版）

> 只覆盖企业 API。企业开放接口使用 `ent_live_xxx`，归属和扣费由企业 Token 自动决定。个人 API 请看 `customer-generation-api.md`。

## 1. Token 创建与鉴权

### 1.1 后台创建企业 API Token
...
企业开放 API 只用于 `/api/open/enterprise/*`；费用扣企业结算钱包，请不要把 `ent_live_xxx` 放到公开网页、GitHub、截图或聊天群。
...
```bash
curl -X POST "$BASE_URL/api/open/enterprise/generation/tasks" \
...
```bash
curl -X GET "$BASE_URL/api/open/enterprise/generation/tasks/$TASK_ID" -H "Authorization: Bearer $ENTERPRISE_API_TOKEN"
curl -X POST "$BASE_URL/api/open/enterprise/generation/tasks/$TASK_ID/query" -H "Authorization: Bearer $ENTERPRISE_API_TOKEN"
curl -X GET "$BASE_URL/api/open/enterprise/generation/tasks?limit=100" -H "Authorization: Bearer $ENTERPRISE_API_TOKEN"
```
...
async function api(path, options = {}) {
  const res = await fetch(BASE_URL + path, {
    ...options,
    headers: { Authorization: `Bearer ${ENTERPRISE_API_TOKEN}`, 'Content-Type': 'application/json', ...(options.headers || {}) },
  });
...
async function waitTask(taskId) {
  for (let i = 0; i < 120; i++) {
    const { task } = await api(`/api/open/enterprise/generation/tasks/${taskId}/query`, { method: 'POST' });
...
| `ENTERPRISE_API_TOKEN_INVALID` | 企业 Token 无效或已禁用。 |
| `USER_DISABLED` | 企业账号或结算账号禁用。 |
| `INSUFFICIENT_BALANCE` | 企业余额不足。 |
...
ENTERPRISE_API_TOKEN=ent_live_xxx
创建任务：POST /api/open/enterprise/generation/tasks
查询任务：GET /api/open/enterprise/generation/tasks/:id
刷新任务：POST /api/open/enterprise/generation/tasks/:id/query
