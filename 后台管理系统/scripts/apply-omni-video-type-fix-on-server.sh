...
PKG="${1:-}"
if [[ -z "$PKG" ]]; then
  PKG="$(ls -t /tmp/omni-video-type-fix-*.tar.gz 2>/dev/null | head -1 || true)"
fi
if [[ -z "$PKG" || ! -f "$PKG" ]]; then
  echo "找不到部署包 omni-video-type-fix-*.tar.gz" >&2
  exit 1
fi
...
WORKDIR="$(mktemp -d /tmp/omni-video-type-fix-XXXXXX)"
BACKUP_DIR="$REMOTE_ROOT/backups/omni-video-type-fix-$STAMP"
