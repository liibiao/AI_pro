...
PKG="${1:-}"
if [[ -z "$PKG" ]]; then
  PKG="$(ls -t /tmp/generation-progress-slowdown-fix-*.tar.gz 2>/dev/null | head -1 || true)"
fi
if [[ -z "$PKG" || ! -f "$PKG" ]]; then
  echo "找不到部署包。请先把 generation-progress-slowdown-fix-*.tar.gz 上传到服务器 /tmp/" >&2
  exit 1
fi
...
DEPLOY_DIR="/tmp/generation-progress-slowdown-fix-$STAMP"
BACKUP_DIR="$REMOTE_ROOT/backups/generation-progress-slowdown-fix-$STAMP"
...
echo "main canvas fake generation progress slowed to 30%: ok"
grep -q "7\\*FAKE_GENERATION_PROGRESS_RATE" "$REMOTE_WORKBENCH_ROOT/canvas-next/app.js"
echo "module canvas fake progress slowed to 30%: ok"
