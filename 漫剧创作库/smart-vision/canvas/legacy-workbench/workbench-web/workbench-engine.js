...
;(function (root) {
  ...
  function isPlaceholderApiKey(value) {
    ...
    return !text || text === 'replace-me' || text === 'your-api-key' || text === 'your_api_key' || text.indexOf('replace-with-') === 0 || text.indexOf('placeholder') >= 0;
  }
  function getModelApiKey(raw) {
    var values = raw ? [raw.key, raw.apiKey, raw.api_key, raw.token] : [];
    ...
    return await new Promise(function (resolve, reject) {
      ...
      function armConfirmTimer() {
        ...
        confirmTimer = setTimeout(function () {
          rejectOnce(new Error('参考图已上传 100%，但云端确认超时'), true);
        }, confirmTimeoutMs);
      }
      ...
      xhr.upload.onprogress = function (evt) {
        ...
        if (percent >= 100) {
          onProgress({ loaded: loaded, total: total, percent: 99, phase: 'confirming', mode: mode });
          armConfirmTimer();
        } else {
