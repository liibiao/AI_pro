...
export function getInputs(id){
  const inputs={images:[],videos:[],style:null};
  inputConnections(id).forEach(c=>{
...
function inputConnections(id){
  return state.conns
    .filter(c=>c.to===id)
    .map((c,index)=>({c,index,src:state.nodes[c.from]}))
    .sort((a,b)=>{
      const ay=Number(a.src?.y??0),by=Number(b.src?.y??0);
      if(Math.abs(ay-by)>8)return ay-by;
      const ax=Number(a.src?.x??0),bx=Number(b.src?.x??0);
...
export function getOutputData(node,port){
  ...
  if(node.type==='singleImage'||node.type==='panoramaViewer'){
    const first=node.values.images?.[0]||{};
    const saved=first.saved||{};
    return node.data||{images:node.values.images||[],url:first.url||first.dataUrl||first.previewUrl||first.localUrl||first.remoteUrl||saved.url||saved.remoteUrl||saved.assetPreviewUrl||''};
  }
...
export function normalizeConnections(conns=[],validIds=null){
  const seen=new Set();
  const inputTaken=new Set();
  return (Array.isArray(conns)?conns:[]).filter(c=>{
    if(!c||!validIds?.has(c.from)||!validIds?.has(c.to)||c.from===c.to)return false;
    const sourceDef=NODE_DEFS[state.nodes[c.from]?.type];
    const targetDef=NODE_DEFS[state.nodes[c.to]?.type];
    const output=sourceDef?.outputs?.find(p=>p.id===c.fromPort);
    const input=targetDef?.inputs?.find(p=>p.id===c.toPort);
    if(!output||!input||output.kind!==input.kind)return false;
    const connKind=c.kind||output.kind;
    if(connKind!==output.kind)return false;
    const inputKey=`${c.to}:${c.toPort}`;
    if(input.kind!=='image'&&inputTaken.has(inputKey))return false;
    const key=`${c.from}:${c.fromPort}->${c.to}:${c.toPort}:${connKind}`;
    if(seen.has(key))return false;
    seen.add(key);
    if(input.kind!=='image')inputTaken.add(inputKey);
    c.kind=connKind;
    return true;
  });
}
...
export function loadData(wf){
  ...
  state.conns=normalizeConnections(wf?.conns||[],validIds);
