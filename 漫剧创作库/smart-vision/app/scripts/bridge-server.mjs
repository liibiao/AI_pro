...
const workbenchWebRoot = path.join(workspaceRoot, 'tools/workbench-web');
const imageStudioWordlistsRoot = path.join(workspaceRoot, 'wordlists/mj-image');
const localWorkbenchApiBase = String(process.env.SMART_VISION_WORKBENCH_API_BASE || 'http://127.0.0.1:8877').replace(/\/+$/, '');
const localGenerationTasks = new Map();
...
async function readImageStudioWordlist(wordlistId) {
  const id = String(wordlistId || '').trim().replace(/\.json$/i, '');
  if (!id) {
    return { ok: true, ...(await readJson(path.join(imageStudioWordlistsRoot, 'index.json'), { dimensions: [] })) };
  }
  const safeName = `${path.basename(id)}.json`;
  const filePath = path.join(imageStudioWordlistsRoot, safeName);
  if (!existsSync(filePath)) {
    const error = new Error(`词库不存在：${id}`);
    error.statusCode = 400;
    throw error;
  }
  return { ok: true, ...(await readJson(filePath, {})) };
}
...
function usableApiKey(...values) {
  for (const value of values) {
    const text = String(value || '').trim();
    if (text && !isPlaceholderApiKey(text)) return text;
  }
  return '';
}
...
function isGptImage2ProModelConfig(model = {}) {
...
function forceGptImage2ProBridgeModelConfig(model = {}) {
  if (!isGptImage2ProModelConfig(model)) return model;
  const provider = model.provider || {};
  const configuredUploadMode = String(
    model.uploadMode ||
    model.upload_mode ||
    provider.uploadMode ||
    provider.upload_mode ||
    model.protocol?.uploadMode ||
    model.protocol?.upload_mode ||
    model.defaults?.uploadMode ||
    model.defaults?.upload_mode ||
    ''
  ).trim();
  const protocol = {
    ...(model.protocol || {}),
    ...
    method: model.protocol?.method || 'sync'
  };
  ...
  if (configuredUploadMode) nextProvider.uploadMode = configuredUploadMode;
  return {
    ...model,
    model: 'gpt-image-2',
    adapter: 'openai-edits',
...
async function readCanvasModelConfigs() {
  const preferredModelsDir = path.join(workbenchWebRoot, 'models');
  const legacyModelsDir = path.join(canvasRoot, 'models');
  const modelsDir = existsSync(preferredModelsDir) ? preferredModelsDir : legacyModelsDir;
...
function normalizeBridgeModelConfig(model = {}) {
  model = forceGptImage2ProBridgeModelConfig(model);
  ...
  const apiKey = usableApiKey(model.key, model.apiKey, model.api_key, model.token, model.provider?.apiKey, model.provider?.api_key, model.provider?.key, model.provider?.token);
  return forceGptImage2ProBridgeModelConfig({
    ...model,
...
async function resolveBridgeModel(modelId, expectedType = '') {
  ...
  if (wanted === 'gpt-image-2') {
    const directGptImage2 = candidates.find((model) => isGptImage2ProModelConfig(model));
    if (directGptImage2) return directGptImage2;
  }
  const exactIdentity = candidates.find((model) => [model.id, model.configId, model.modelKey].some((value) => String(value || '').trim() === wanted));
  return exactIdentity || candidates[0] || null;
}

function resolveBridgeLlmTimeoutMs(input = {}, headers = {}) {
  const headerValue = (name) => headers?.[name] ?? headers?.[name.toLowerCase()] ?? headers?.[name.toUpperCase()];
  const candidates = [
    input.timeoutMs,
    input.upstreamTimeoutMs,
    input.requestTimeoutMs,
    input.queryTimeoutMs,
    headerValue('x-upstream-timeout-ms'),
    headerValue('x-request-timeout-ms'),
    headerValue('x-generation-timeout-ms'),
    headerValue('x-timeout-ms')
  ];
  const picked = candidates.map((value) => Number(value)).find((value) => Number.isFinite(value) && value > 0);
  return Math.max(1000, Math.min(Math.floor(picked || 900000), 1800000));
}

function shouldStreamBridgeLlm(input = {}, model = {}, endpointPath = '') {
  if (input.stream === false || input.streamMode === 'disabled' || input.extra?.stream === false) return false;
  if (input.stream === true || input.preferStream === true || input.extra?.preferStream === true) return true;
  const mode = String(input.extra?.canvasPromptMode || input.extra?.mode || input.mode || '').trim().toLowerCase();
  const text = [
    mode,
    model.id,
    model.configId,
  ...
  return mode === 'script' && (text.includes('gpt-5.5') || text.includes('responses') || /\/responses\b/.test(text));
}

async function callBridgeLlmChat(input = {}, requestHeaders = {}) {
  ...
  const apiKey = usableApiKey(model.key, model.apiKey, model.provider?.apiKey, process.env.SMART_VISION_LLM_API_KEY, process.env.OPENAI_API_KEY);
  ...
  const timeoutMs = resolveBridgeLlmTimeoutMs(input, requestHeaders);
  const stream = shouldStreamBridgeLlm(input, model, endpointPath);
  const headers = {
    'content-type': 'application/json',
    authorization: `Bearer ${apiKey}`
  };
  if (isOpenAiResponsesLlmModel(model, endpointPath)) {
    const converted = responsesInputFromChatMessages(messages);
    ...
    const body = {
      ...
      max_output_tokens: Number.isFinite(Number(input.maxTokens ?? input.maxOutputTokens)) ? Number(input.maxTokens ?? input.maxOutputTokens) : undefined,
      stream
    };
    Object.keys(body).forEach((key) => body[key] === undefined && delete body[key]);
    const { response, payload } = await postBridgeChatCompletion(endpoint, headers, body, timeoutMs, 'responses');
    if (!response.ok) throw new Error(payload.error?.message || payload.message || `OpenAI Responses HTTP ${response.status}`);
    const text = normalizeResponsesText(payload);
    return {
      ...payload,
      text,
      choices: payload.choices || [{ message: { role: 'assistant', content: text } }],
      model: payload.model || model.model || model.name,
      modelId: model.id,
      usedStringContentFallback: false,
  ...
  Object.keys(body).forEach((key) => body[key] === undefined && delete body[key]);
  let { response, payload } = await postBridgeChatCompletion(endpoint, headers, body, timeoutMs, 'chat');
  let usedStringContentFallback = false;
  const disallowStringContentFallback = Boolean(input.disallowStringContentFallback || input.extra?.disallowStringContentFallback || input.extra?.requireVisionInput);
  if (!response.ok && chatContentHasArray(messages) && chatContentArraySchemaError(payload)) {
    if (disallowStringContentFallback) {
      throw new Error(payload.error?.message || '当前大语言模型通道拒绝图文数组，无法读取参考图；已阻止纯文本猜图。请切换支持视觉输入的模型或修复聊天接口 schema。');
    }
    usedStringContentFallback = true;
    const retryBody = {
      ...body,
      messages: stringifyChatMessages(messages)
    };
    ({ response, payload } = await postBridgeChatCompletion(endpoint, headers, retryBody, timeoutMs, 'chat'));
  }
...
function normalizeResponsesText(payload = {}) {
...
function isOpenAiResponsesLlmModel(model = {}, endpointPath = '') {
  const text = [
    model.adapter,
    model.protocol?.adapter,
    model.provider?.adapter,
    model.responseApi,
    model.protocol?.responseApi,
    endpointPath,
    model.endpointPath,
    model.protocol?.endpointPath,
    model.provider?.endpointPath
  ].map((value) => String(value || '').trim().toLowerCase()).join(' ');
  return text.includes('responses') || /\/responses\b/.test(text);
}

function chatContentImageUrl(part = {}) {
  return (
...
function responsesContentFromChatContent(content) {
  ...
  const visit = (value) => {
    ...
    if (type.includes('image')) {
      const imageUrl = chatContentImageUrl(value);
      if (imageUrl) {
        const item = { type: 'input_image', image_url: String(imageUrl) };
        const detail = value.detail || value.image_url?.detail || value.input_image?.detail;
...
function chatContentHasArray(messages) {
  return (Array.isArray(messages) ? messages : []).some((message) => Array.isArray(message?.content));
}

function compactChatImageReference(value) {
  const raw = String(value || '').trim();
  if (!raw) return '';
  if (/^data:image\//i.test(raw)) return `内联图片 data URL，长度 ${raw.length}`;
  return raw;
}
...
function chatContentToString(content) {
  ...
  const visit = (value) => {
    ...
    const imageUrl =
      ...
      value.image_url_url ||
      value.url ||
      value.input_image?.image_url ||
      value.input_image?.url;
    if (imageUrl) {
      parts.push(`参考图 URL：${compactChatImageReference(imageUrl)}`);
      return;
    }
    const type = String(value.type || '').toLowerCase();
    if (type.includes('image')) parts.push('参考图：当前接口不支持图文数组，已改用图片引用文本。');
  };
  visit(content);
  return parts.join('\n').replace(/\n{3,}/g, '\n\n').trim();
}
...
function chatContentArraySchemaError(payload) {
  const message = [
  ...
  return /expected\s+string,\s*received\s+array|received\s+array.*expected\s+string|content[^.\n]*string|messages?\.\d+\.content|invalid(?:_type)?.*array/i.test(message);
}
...
function appendStreamDelta(parts, value) {
  const text = typeof value === 'string' ? value : '';
  if (text) parts.push(text);
}

function bridgeStreamPayloadFromText(raw, responseApi = 'chat') {
  const parts = [];
  let finalPayload = {};
  for (const data of parseSseDataLines(raw)) {
    ...
    if (!eventType || eventType.includes('delta')) {
      appendStreamDelta(parts, event.delta);
      appendStreamDelta(parts, event.output_text);
      appendStreamDelta(parts, event.text);
    }
    appendStreamDelta(parts, event.choices?.[0]?.delta?.content);
    if (!eventType || eventType.includes('delta')) appendStreamDelta(parts, event.choices?.[0]?.message?.content);
  }
  const text = parts.join('').trim() || normalizeResponsesText(finalPayload) || normalizeChatCompletionText(finalPayload?.choices?.[0]?.message?.content);
  if (responseApi === 'responses') {
    return {
      ...finalPayload,
      output_text: text || finalPayload.output_text,
      text: text || finalPayload.text,
      rawStreamText: raw
    };
  }
  return {
    ...finalPayload,
    text,
    choices: finalPayload.choices || [{ message: { role: 'assistant', content: text } }],
    rawStreamText: raw
  };
}

async function parseBridgeChatResponse(response, responseApi) {
  const contentType = String(response.headers.get('content-type') || '').toLowerCase();
  if (contentType.includes('text/event-stream')) {
    return bridgeStreamPayloadFromText(await response.text(), responseApi);
  }
  const raw = await response.text();
  if (!raw.trim()) return {};
  if (/^\s*data:/m.test(raw)) return bridgeStreamPayloadFromText(raw, responseApi);
  try {
    return JSON.parse(raw);
  } catch {
    return { text: raw.trim(), rawText: raw };
  }
}

async function postBridgeChatCompletion(endpoint, headers, body, timeoutMs, responseApi = 'chat') {
  let response = null;
  try {
    ...
    throw new Error(`LLM upstream request failed: ${endpoint} (${cause})`);
  }
  const payload = await parseBridgeChatResponse(response, responseApi);
...
async function recognizeCreativeImageImport(file) {
  ...
  let { response, payload } = await postBridgeChatCompletion(endpoint, headers, body, 30000);
  let usedStringContentFallback = false;
  if (!response.ok && chatContentArraySchemaError(payload)) {
    usedStringContentFallback = true;
    ({ response, payload } = await postBridgeChatCompletion(endpoint, headers, { ...body, messages: stringifyChatMessages(messages) }, 30000));
  }
  if (!response.ok) {
...
function sendJson(res, statusCode, payload) {
  ...
  res.writeHead(statusCode, {
    ...
    'access-control-allow-headers': 'content-type, authorization, x-request-timeout-ms, x-generation-timeout-ms, x-upstream-timeout-ms, x-client-request-id, x-timeout-ms',
    'cache-control': 'no-store'
  });
  res.end(body);
}

async function proxyToLocalWorkbench(req, res, url) {
  const targetUrl = `${localWorkbenchApiBase}${url.pathname}${url.search}`;
  const headers = {};
...
function generationTaskPayloadToWorkbenchPayload(input = {}, resolvedModel = null) {
  const params = input.params && typeof input.params === 'object' ? input.params : {};
  const model = resolvedModel && typeof resolvedModel === 'object' ? resolvedModel : {};
  const modelProvider = model.provider && typeof model.provider === 'object' ? model.provider : {};
  const modelProtocol = model.protocol && typeof model.protocol === 'object' ? model.protocol : {};
  const protocol = params.protocol && typeof params.protocol === 'object' ? params.protocol : {};
  const provider = params.provider && typeof params.provider === 'object' ? params.provider : {};
  const modelConfig = params.modelConfig && typeof params.modelConfig === 'object' ? params.modelConfig : {};
  const modelConfigOverride = params.modelConfigOverride && typeof params.modelConfigOverride === 'object' ? params.modelConfigOverride : {};
  const apiKey = usableApiKey(
    ...
    provider.key,
    provider.token,
    modelConfigOverride.apiKey,
    modelConfigOverride.api_key,
    modelConfigOverride.key,
    modelConfigOverride.token,
    modelConfig.apiKey,
    modelConfig.api_key,
    modelConfig.key,
    modelConfig.token,
  ...
  const merged = {
    ...
    negativePrompt: input.negativePrompt || params.negativePrompt || params.negative_prompt || '',
    negative_prompt: input.negativePrompt || params.negative_prompt || params.negativePrompt || '',
    mode: input.mode || params.mode || '',
    action: input.mode || params.action || params.mode || '',
    model: modelConfigOverride.model || modelConfig.model || params.model || model.model || model.name || input.modelId || '',
    adapter: input.adapter || params.adapter || params.protocolAdapter || protocol.adapter || provider.adapter || modelConfigOverride.adapter || modelConfig.adapter || model.adapter || modelProtocol.adapter || modelProvider.adapter || '',
    endpointPath: input.endpointPath || params.endpointPath || protocol.endpointPath || provider.endpointPath || modelConfigOverride.endpointPath || modelConfig.endpointPath || model.endpointPath || modelProtocol.endpointPath || modelProvider.endpointPath || '',
    baseUrl: input.baseUrl || params.baseUrl || provider.baseUrl || modelConfigOverride.baseUrl || modelConfig.baseUrl || model.baseUrl || model.url || modelProvider.baseUrl || '',
    apiKey,
    images: params.images || params.image || input.inputFiles || [],
    reference_images: params.reference_images || params.referenceImages || params.images || input.inputFiles || [],
    inputFiles: input.inputFiles || []
  };
  if (params.size && !merged.size) merged.size = params.size;
  if (params.resolution && !merged.resolution) merged.resolution = params.resolution;
  if (params.aspectRatio && !merged.aspectRatio) merged.aspectRatio = params.aspectRatio;
  if (params.n && !merged.n) merged.n = params.n;
  return merged;
}

function normalizeWorkbenchGenerationResultUrls(payload = {}) {
  const values = [];
  const add = (value) => {
    if (!value) return;
    if (Array.isArray(value)) {
    ...
    if (typeof value === 'object') {
      ...
      [
        ...
        value.images,
        value.outputs,
        value.assetPreviewUrls,
        value.resultUrls,
        value.outputUrls,
        value.imageUrls
      ].forEach(add);
      if (value.saved) add(value.saved);
      if (value.result) add(value.result);
...
async function submitLocalGenerationTask(input = {}) {
  const type = String(input.type || '').toUpperCase();
  const resolvedModel = await resolveBridgeModel(input.modelId || input.modelKey || input.model || '', type);
  const id = `local-gen-${Date.now()}-${Math.random().toString(16).slice(2, 10)}`;
  const now = new Date().toISOString();
  const baseTask = {
    id,
    taskId: id,
    ...
    createdAt: now,
    updatedAt: now,
    paramsJson: taskJson(input.params || {})
  };
  localGenerationTasks.set(id, baseTask);
  try {
    const endpoint = type === 'VIDEO' ? '/api/workbench/image-studio/generate-video' : '/api/workbench/image-studio/generate';
    const workbenchPayload = generationTaskPayloadToWorkbenchPayload(input, resolvedModel);
    const response = await fetch(`${localWorkbenchApiBase}${endpoint}`, {
      method: 'POST',
    ...
    const payload = await response.json().catch(() => ({}));
    if (!response.ok || payload.ok === false) {
      const message = payload.error || payload.message || payload.error?.message || `Workbench generation HTTP ${response.status}`;
      const failed = {
        ...baseTask,
        status: 'FAILED',
        progress: 100,
        errorMessage: String(message || '生成失败'),
        resultJson: taskJson(payload),
        responseJson: taskJson(payload),
        updatedAt: new Date().toISOString()
      };
      localGenerationTasks.set(id, failed);
      return failed;
    }
    const urls = normalizeWorkbenchGenerationResultUrls(payload);
    const resultJson = {
      ...(payload.result && typeof payload.result === 'object' ? payload.result : {}),
      rawResponse: payload,
      url: urls[0] || '',
      urls,
      saved: payload.result?.saved || payload.saved || {}
    };
    const success = {
      ...baseTask,
      status: 'SUCCESS',
      progress: 100,
      resultJson: taskJson(resultJson),
      responseJson: taskJson(payload),
      resultUrls: urls,
      resultUrlsJson: taskJson(urls),
      updatedAt: new Date().toISOString()
    };
    localGenerationTasks.set(id, success);
    return success;
  } catch (error) {
    const failed = {
      ...baseTask,
...
function listLocalGenerationTasks(url) {
  const clientRequestId = String(url.searchParams.get('clientRequestId') || '').trim();
  const limit = Math.max(1, Math.min(200, Number(url.searchParams.get('limit') || 80) || 80));
  let items = Array.from(localGenerationTasks.values()).sort((a, b) => String(b.createdAt || '').localeCompare(String(a.createdAt || '')));
  if (clientRequestId) items = items.filter((task) => String(task.clientRequestId || '') === clientRequestId);
  ...
  try {
    if (req.method === 'OPTIONS') {
      ...
      res.writeHead(200, {
        ...
        'access-control-allow-origin': '*',
        'access-control-allow-methods': 'GET, POST, OPTIONS',
        'access-control-allow-headers': requestedHeaders || 'content-type, authorization, x-request-timeout-ms, x-generation-timeout-ms, x-upstream-timeout-ms, x-client-request-id, x-timeout-ms',
    ...
    if (req.method === 'GET' && url.pathname === '/api/account/summary') {
      sendJson(res, 200, {
        ok: true,
        user: {
          id: 'local-smart-vision',
          nickname: 'Local Smart Vision',
          email: 'local@smart-vision.invalid'
        },
        ...
        pricingBenefit: { discountActive: true, discountLabel: '本地联调' },
        recentRecharges: []
      });
      return;
    }
    ...
    if (req.method === 'GET' && url.pathname === '/api/canvas/nodes') {
      sendJson(res, 200, { ok: true, items: [], disabledNodeTypes: [] });
      return;
    }

    if (req.method === 'GET' && url.pathname === '/api/data-packs') {
      const registry = await readDataPackRegistry();
    ...
    if (req.method === 'GET' && url.pathname === '/api/workbench/image-studio/wordlist') {
      try {
        sendJson(res, 200, await readImageStudioWordlist(url.searchParams.get('id')));
      } catch (error) {
    ...
    if (req.method === 'GET' && url.pathname === '/api/health') {
      sendJson(res, 200, { ok: true, service: 'smart-vision-bridge', generationTasks: localGenerationTasks.size });
      return;
    }

    if (req.method === 'POST' && url.pathname === '/api/generation/tasks') {
      const body = await readRequestBody(req);
      const task = await submitLocalGenerationTask(body);
      sendJson(res, 200, { ok: true, task, ...task });
      return;
    }

    if (req.method === 'GET' && url.pathname === '/api/generation/tasks') {
      const items = listLocalGenerationTasks(url);
      sendJson(res, 200, { ok: true, items, tasks: items, total: items.length });
      return;
    }

    if (req.method === 'GET' && /^\/api\/generation\/tasks\/[^/]+$/.test(url.pathname)) {
      const taskId = decodeURIComponent(url.pathname.split('/').pop() || '');
      const task = localGenerationTasks.get(taskId);
      if (!task) {
        sendJson(res, 404, { ok: false, error: `Generation task not found: ${taskId}` });
        return;
      }
      sendJson(res, 200, { ok: true, task, ...task });
      return;
    ...
    if (req.method === 'POST' && /^\/api\/generation\/tasks\/[^/]+\/query$/.test(url.pathname)) {
    ...
    if (url.pathname.startsWith('/api/workbench/image-studio/') || url.pathname.startsWith('/v1/images/')) {
      await proxyToLocalWorkbench(req, res, url);
      return;
    }

    if (req.method === 'POST' && url.pathname === '/api/generate/llm/chat') {
      const body = await readRequestBody(req);
      const result = await callBridgeLlmChat(body, req.headers);
