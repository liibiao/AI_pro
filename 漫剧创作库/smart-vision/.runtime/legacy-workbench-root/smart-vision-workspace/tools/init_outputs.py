...
OUTPUTS_DIR = STUDIO_DIR / "tools" / "workbench-web"
INVALID_CHARS = r'[<>:"/\\|?*\u0000-\u001f]'
...
<head>
  ...
  <title>{html.escape(project_name)} - 漫剧创作库输出目录</title>
  <style>
...
<body>
  <div class="wrap">
    <section class="hero">
      ...
      <p>这是漫剧创作库工作流为当前输入内容建立的标准输出目录。你可以把不同版本的产物分别落到三个子目录里，保持结构稳定、检索清晰。</p>
      <p>创建时间：{created_at}</p>
    </section>
    <div class="grid">
{links}
...
<body>
  <div class="wrap">
    <h1>漫剧创作库输出目录</h1>
    <p>这里统一收纳由漫剧创作库工作流生成的项目结果。标准结构为 <code>项目目录 / seedance / 即梦 / 叙事 / index.html</code>。</p>
...
def main() -> None:
    parser = argparse.ArgumentParser(description="初始化漫剧创作库 outputs 项目目录")
