...
BACKUP_DIR="$APP_ROOT/.deploy-backups/canvas-generation-mode-new-node-20260624234741-$STAMP"
WORK_DIR="$(mktemp -d)"
...
require_marker "createGenerationRunClone" "$HTML_SRC"
require_marker "runGenerationOnNewNode" "$HTML_SRC"
...
echo "Installed canvas generation mode new-node flow."
