...
PKG="canvas-unified-progress-derive-pill-20260604134613"
...
verify_markers(){
  ...
  grep -Fq -- "updateUnifiedProgressText(text,progress,lbl)" "$dest"
  grep -Fq -- "display:grid;--gen-progress" "$dest"
  grep -Fq -- "unified node progress overlay" "$dest"
  grep -Fq -- "--unified-progress:var(--gen-progress,0%)" "$dest"
  grep -Fq -- "--unified-progress:var(--shot-progress,0%)" "$dest"
  grep -Fq -- "--unified-progress:var(--asset-card-derive-progress,0%)" "$dest"
  grep -Fq -- "mask.style.display='grid';mask.style.setProperty('--gen-progress','1%')" "$dest"
  grep -Fq -- "updateUnifiedProgressText(txt,progress,img.progressLabel||'上传参考图')" "$dest"
  grep -Fq -- '<span>推演资产</span>${renderAssetCardDeriveCreditPill(n)}</button>' "$dest"
