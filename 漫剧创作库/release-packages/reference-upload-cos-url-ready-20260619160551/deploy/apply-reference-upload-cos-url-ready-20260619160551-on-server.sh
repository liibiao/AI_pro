...
PKG="reference-upload-cos-url-ready-20260619160551"
...
verify_html(){
  local file="$1"
  grep -Fq "workbench-engine.js?v=20260619-reference-upload-url-ready-v1" "$file"
  grep -Fq "function referenceUploadEventPublicUrl" "$file"
  grep -Fq "function markReferenceObjectStorageReady" "$file"
  grep -Fq "markReferenceObjectStorageReady(img,publicUrl,evt)" "$file"
  grep -Fq "normalizeUploadedReferenceImage(img,publicUrl,'object_storage')" "$file"
  grep -Fq "recovered:true" "$file"
}

verify_engine(){
  local file="$1"
  grep -Fq "label: '确认公网 URL'" "$file"
  grep -Fq "remoteUrl: target.publicUrl" "$file"
  grep -Fq "uploaded: true" "$file"
  grep -Fq "provider: target.provider || 'object_storage'" "$file"
}
...
if run_sudo test -f "$WORKBENCH_DIR/image-studio-canvas-next.html"; then
  run_sudo grep -Fq "workbench-engine.js?v=20260619-reference-upload-url-ready-v1" "$WORKBENCH_DIR/image-studio-canvas-next.html"
fi
if run_sudo test -f "$WORKBENCH_DIR/workbench-engine.js"; then
  run_sudo grep -Fq "remoteUrl: target.publicUrl" "$WORKBENCH_DIR/workbench-engine.js"
fi
