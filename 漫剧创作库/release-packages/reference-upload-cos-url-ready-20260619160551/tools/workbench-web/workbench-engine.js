...
;(function (root) {
  ...
  async function requestObjectStorageUploadTarget(file) {
    var resp = await fetch(OBJECT_STORAGE_UPLOAD_TARGET_API, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
  ...
  async function uploadReferenceImage(file, options) {
    ...
    if (mode === 'object_storage' && typeof XMLHttpRequest === 'function') {
      ...
      try {
        var target = await requestObjectStorageUploadTarget(file);
