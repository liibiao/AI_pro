...
PKG="canvas-next-submit-recovery-ref-unlimited-20260604211525"
...
verify_canvas_markers(){
  local file="$1"
  grep -Fq "recoverUnifiedGenerationTaskByClientRequestId(clientRequestId,{delay:1500,maxMs:45000" "$file"
  grep -Fq "已按 clientRequestId 等待 45 秒仍未找到入库任务" "$file"
...
verify_service_markers(){
  local file="$1"
  grep -Fq "recoverBackendGenerationTaskByClientRequestId(clientRequestId,{maxMs:45000})" "$file"
  grep -Fq "已按 clientRequestId 等待 45 秒仍未找到入库任务" "$file"
  grep -Fq "maxMs=45000" "$file"
}
verify_canvas_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "recoverUnifiedGenerationTaskByClientRequestId(clientRequestId,{delay:1500,maxMs:45000" "$file"
  run_sudo grep -Fq "已按 clientRequestId 等待 45 秒仍未找到入库任务" "$file"
  run_sudo grep -Fq "function promptReferenceUsesUnlimitedLimit" "$file"
  run_sudo grep -Fq "function limitPromptReferenceItemsForNode" "$file"
  run_sudo grep -Fq "shouldSuppressVn2ChipHoverPreview" "$file"
  run_sudo grep -Fq "function resolveReferenceImagePayloadLimit" "$file"
  run_sudo grep -Fq "params.maxReferenceImages=0" "$file"
}
verify_service_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "recoverBackendGenerationTaskByClientRequestId(clientRequestId,{maxMs:45000})" "$file"
  run_sudo grep -Fq "已按 clientRequestId 等待 45 秒仍未找到入库任务" "$file"
  run_sudo grep -Fq "maxMs=45000" "$file"
}
