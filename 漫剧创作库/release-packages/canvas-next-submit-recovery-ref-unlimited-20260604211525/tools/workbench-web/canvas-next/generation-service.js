...
async function submitBackendGeneration({model,type,mode,prompt,inputFiles=[],params={},onProgress}){
  ...
  try{
    ...
    if(isTransientGenerationError(err)){
      const recovered=await recoverBackendGenerationTaskByClientRequestId(clientRequestId,{maxMs:45000});
      if(!recovered){
        throw new Error(`任务提交中断：后台生成接口连接中断，且已按 clientRequestId 等待 45 秒仍未找到入库任务。请确认后台 API / 生成队列进程仍在运行后重试；若资产库稍后出现该任务，可直接查看结果。原始错误：${err?.message||err}`);
      }
      data={task:recovered};
    }else{
      throw err;
