...
PKG="canvas-adaptive-preview-thumbnails-20260607162509"
...
verify_canvas_markers(){
  ...
  grep -Fq "function isPreviewImageTransformUrl" "$file"
  grep -Fq "function isLocalResizableImageUrl" "$file"
  grep -Fq "function scheduleAdaptiveImageHydrate" "$file"
  grep -Fq "data-adaptive-image" "$file"
  grep -Fq "previewImageSrc(src,w,h)" "$file"
  grep -Fq "imgFallbackProxy(img.dataset.rawSrc||img.src||'',240,240)" "$file"
...
verify_server_markers(){
  ...
  grep -Fq "def resize_image_for_preview" "$file"
  grep -Fq "max_w, max_h = preview_size_params(query)" "$file"
  grep -Fq "resize_image_for_preview(binary, content_type, max_w, max_h)" "$file"
}

verify_canvas_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "ADAPTIVE_IMAGE_PLACEHOLDER" "$file"
  run_sudo grep -Fq "function isLocalResizableImageUrl" "$file"
  run_sudo grep -Fq "function scheduleAdaptiveImageHydrate" "$file"
...
verify_server_markers_sudo(){
  ...
  run_sudo grep -Fq "def resize_image_for_preview" "$file"
  run_sudo grep -Fq "max_w, max_h = preview_size_params(query)" "$file"
}
