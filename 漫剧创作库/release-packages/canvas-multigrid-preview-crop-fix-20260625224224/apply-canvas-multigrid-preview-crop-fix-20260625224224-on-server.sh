...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/canvas-multigrid-preview-crop-fix-20260625224224-${STAMP}"
...
validate_html(){
  ...
  require_marker "function multiGridCroppedCellUrl(entry)" "$file"
  require_marker "function materializeMultiGridGeneratedEntry(entry)" "$file"
  require_marker "entry.gridCells=cells;" "$file"
  require_marker ".multi-grid-cell-caption{display:none!important}" "$file"
  require_marker ".preview-card-item.multi-grid-cell-item img" "$file"
...
echo "deployed canvas-multigrid-preview-crop-fix-20260625224224"
