...
PKG="canvas-image-async-cos-preview-fix-20260612110440"
...
verify_canvas_markers(){
  local file="$1"
  grep -Fq "function taskAsyncStoragePreviewUrls(task)" "$file"
  grep -Fq "return uniqueAssetResultUrls(values).filter(url=>!isObjectStorageReferenceUrl(url))" "$file"
  grep -Fq "const storagePending=['pending','uploading','failed','expired'].includes(storageStatus)" "$file"
  grep -Fq "const temporaryUrls=storagePending?uniqueAssetResultUrls([...rawTemporaryUrls,...taskAsyncStoragePreviewUrls(task)]):rawTemporaryUrls" "$file"
  grep -Fq "const storagePending=['uploading','failed','expired'].includes(storage.status)" "$file"
  grep -Fq "...(storagePending?taskAsyncStoragePreviewUrls(task):[])" "$file"
}
