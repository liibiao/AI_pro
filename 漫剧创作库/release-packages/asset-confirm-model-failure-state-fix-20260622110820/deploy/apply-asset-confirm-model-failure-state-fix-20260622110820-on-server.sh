...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/asset-confirm-model-failure-state-fix-20260622110820-${STAMP}"
...
check_markers(){
  local file="$1"
  test -f "$file"
  grep -Fq "function modelLooksLikeSeedanceSd2Video" "$file"
  grep -Fq "modelLooksLikeSeedanceSd2Video(model)" "$file"
  grep -Fq "case 'assetConfirm':" "$file"
  grep -Fq "const type=normalizeModelType(raw);" "$file"
  grep -Fq "function assetConfirmTaskIsFailedImageTask" "$file"
  grep -Fq "function applyFailedAssetConfirmCardTask" "$file"
  grep -Fq "if(!usable.length&&!failedTasks.length)" "$file"
  grep -Fq "n.status='failed';" "$file"
  if grep -Fq "n.includes('sd')||n.includes('stable.')" "$file"; then
    echo "old broad sd=>image fallback still present in $file" >&2
    return 1
  fi
}
...
echo "deployed asset-confirm-model-failure-state-fix-20260622110820"
