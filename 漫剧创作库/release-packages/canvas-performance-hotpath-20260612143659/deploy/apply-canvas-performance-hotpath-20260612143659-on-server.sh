...
PKG="canvas-performance-hotpath-20260612143659"
...
verify_html(){
  local file="$1"
  grep -Fq 'nodeCount>=32||connCount>=48' "$file"
  grep -Fq 'progressOnly:prevStatus===status&&status' "$file"
  grep -Fq 'if(!opts.progressOnly)syncNodeUiScale(el,n)' "$file"
  grep -Fq 'function updateCanvasPerformanceMode' "$file"
}
verify_html_sudo(){
  local file="$1"
  run_sudo grep -Fq 'nodeCount>=32||connCount>=48' "$file"
  run_sudo grep -Fq 'progressOnly:prevStatus===status&&status' "$file"
  run_sudo grep -Fq 'if(!opts.progressOnly)syncNodeUiScale(el,n)' "$file"
  run_sudo grep -Fq 'function updateCanvasPerformanceMode' "$file"
}
