...
PKG="node-interaction-libtv-20260602004221"
...
verify_markers(){
  local dest="$1"
  grep -Fq "function focusNodeLarge" "$dest"
  grep -Fq "function duplicateNodesForDrag" "$dest"
  grep -Fq "function ensureAltDragCopy" "$dest"
  grep -Fq "viewport-focus-animating" "$dest"
  grep -Fq "windowsDirectWheelZoom" "$dest"
  grep -Fq "const allowSingleClickPreview=!(n.type==='singleImage'||n.type==='singleVideo')" "$dest"
  grep -Fq "if(e.altKey)" "$dest"
  grep -Fq "data-action=\"openPreview\" data-tip=\"查看大图\"" "$dest"
  if grep -Fq "data-video-inline-more" "$dest"; then
    echo "video inline large-preview button was not removed" >&2
    return 1
  fi
  if grep -Fq "if(e.ctrlKey){" "$dest"; then
    echo "canvas wheel zoom still requires ctrl only" >&2
    return 1
  fi
}
