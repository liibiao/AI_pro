...
async function findAdminGenerationTasks(where, take, predicate) {
    const items = [];
    const chunkSize = 500;
    ...
    while (items.length < take && skip < maxScanned) {
        ...
        if (!tasks.length)
            break;
        items.push(...tasks.filter(predicate));
        skip += tasks.length;
    }
...
function resolveGenerationAdapterName(provider, model) {
    ...
    if (configuredLower === 'seedance2-vip' || configuredLower === 'seedance2.0-vip')
        return 'seedance2-vip';
    if (['gemini-image-generate', 'gemini-image-edit', 'gemini-video', 'veo-video', 'gemini-chat', 'gemini-llm', 'veo-chat', 'veo-3.1', 'grok-video', 'grok-image', 'grok-image-edit', 'grok-chat', 'grok-llm'].includes(configuredLower))
...
function isGptImage2GenerationHint(modelHint, hint, configuredLower) {
    ...
    if (!configuredLower)
        return true;
    return ['openai-edits', 'openai-image', 'openai-generations', 'gpt-image', 'gpt-image-v2'].includes(configuredLower)
...
async function closeInterruptedGenerationTask(task, reason) {
    const completed = await completeTaskFromStoredResultUrls(task);
    if (completed)
        return { task: completed };
...
function sanitizeGenerationTaskListItem(task) {
    return {
        ...task,
...
function sanitizeGenerationListJson(value) {
    if (value == null)
        return value;
    if (typeof value === 'string')
        return sanitizeGenerationListString(value);
    if (Array.isArray(value))
        return value.map(item => sanitizeGenerationListJson(item));
    if (!isRecord(value))
        return value;
    const next = {};
...
function sanitizeGenerationListString(value) {
...
function isLargeInlineGenerationField(key, value) {
    ...
    if (!/(b64|base64|data|inline)/.test(name))
        return false;
