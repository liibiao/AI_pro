...
async function main() {
  ...
  for (const provider of targetProviders) {
    ...
    console.log(`[fix] provider ${provider.providerKey} -> openai-edits /images/edits files`);
  }
  ...
  for (const model of targetModels.values()) {
    ...
    console.log(`[fix] model ${model.id}/${model.displayName} -> gpt-image-2 openai-edits /images/edits files`);
  }
