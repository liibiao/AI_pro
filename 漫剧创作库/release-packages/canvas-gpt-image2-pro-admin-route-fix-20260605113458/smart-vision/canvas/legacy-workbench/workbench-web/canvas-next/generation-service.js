...
function forceGptImage2ProModelConfig(model={}){
  ...
  const protocol={...(model.protocol||{}),adapter:'openai-edits',endpointPath:'/images/edits',uploadMode:'files',upload_mode:'files',method:model.protocol?.method||'sync'};
  return {
    ...model,
    model:'gpt-image-2',
    ...
    uploadMode:'files',
    uploadStrategy:'files',
    protocol,
    provider:{...(model.provider||{}),adapter:'openai-edits',endpointPath:'/images/edits',uploadMode:'files'}
  };
}
...
async function submitBackendGeneration({model,type,mode,prompt,inputFiles=[],params={},onProgress}){
  ...
  const protocolOverrides=isGptImage2ProModel(model)?{
    ...
    upload_mode:'files',
    uploadStrategy:'files',
    provider:{adapter:'openai-edits',endpointPath:'/images/edits',uploadMode:'files'},
    modelConfig:{model:'gpt-image-2',adapter:'openai-edits',endpointPath:'/images/edits',uploadMode:'files'},
    modelConfigOverride:{model:'gpt-image-2',adapter:'openai-edits',endpointPath:'/images/edits',uploadMode:'files'},
    protocol:{adapter:'openai-edits',endpointPath:'/images/edits',uploadMode:'files',upload_mode:'files',method:'sync'}
  }:{};
  const taskParams=isGptImage2ProModel(model)?{
    ...
    modelProtocol:protocolOverrides.protocol,
...
export async function ensureUploadedImages(images,model,{forVideo=false,onProgress}={}){
  ...
  const uploadMode=forVideo?'object_storage':normalizeUploadMode(protocol.uploadMode||model?.uploadMode||'files');
  const refs=[];
