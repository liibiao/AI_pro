...
PKG="canvas-gpt-image2-pro-admin-route-fix-20260605113458"
...
verify_package_markers(){
  ...
  grep -Fq "function isGptImage2ProModel" "$SRC/workbench-web/image-studio-canvas-next.html"
  grep -Fq "GPT-Image-2-pro 使用 /v1/files 上传参考图" "$SRC/workbench-web/image-studio-canvas-next.html"
  grep -Fq "filter(item=>String(item?.kind||'file')!=='text')" "$SRC/workbench-web/image-studio-canvas-next.html"
  grep -Fq "textPrompt:{cat:'input',name:'文本节点'" "$SRC/workbench-web/image-studio-canvas-next.html"
  grep -Fq "function forceGptImage2ProModelConfig" "$SRC/workbench-web/canvas-next/generation-service.js"
  grep -Fq "参考图上传后未返回 file-xxx" "$SRC/workbench-web/canvas-next/generation-service.js"
  grep -Fq "def is_gpt_image_2_pro_payload" "$SRC/tools/image_studio_backend.py"
  grep -Fq "def coerce_gpt_image_2_pro_model_config" "$SRC/tools/workbench_server.py"
  grep -Fq "function forceGptImage2ProBridgeModelConfig" "$SRC/smart-vision/app/scripts/bridge-server.mjs"
  grep -Fq "isCanvasGptImage2ProGenerationHint" "$SRC/api-server/dist/modules/generation/routes.js"
  grep -Fq "return 'openai-edits'" "$SRC/api-server/src/modules/generation/routes.ts"
  grep -Fq "resolveOpenAiEditsEndpoint" "$SRC/api-server/dist/modules/generation/adapters/registry.js"
  grep -Fq "provider \${provider.providerKey} -> openai-edits /images/edits files" "$SRC/api-server/scripts/fix-canvas-gpt-image2-pro-route.mjs"
}
...
verify_api_markers(){
  ...
  run_sudo grep -Fq "return 'openai-edits'" "$api_dir/src/modules/generation/routes.ts"
  run_sudo grep -Fq "provider \${provider.providerKey} -> openai-edits /images/edits files" "$api_dir/scripts/fix-canvas-gpt-image2-pro-route.mjs"
}

verify_workbench_marker_if_present(){
...
build_api_if_possible(){
  ...
  if command -v node >/dev/null 2>&1 && [ -f "$api_dir/node_modules/typescript/bin/tsc" ]; then
    ...
    if (cd "$api_dir" && node node_modules/typescript/bin/tsc -p tsconfig.json); then
      return 0
    fi
...
run_db_route_fix(){
  ...
  log "fix GPT-Image2 Pro provider/model route in database"
...
verify_workbench_marker_if_present "$WORKBENCH_DIR/image-studio-canvas-next.html" "GPT-Image-2-pro 使用 /v1/files 上传参考图"
verify_workbench_marker_if_present "$WORKBENCH_DIR/image-studio-canvas-next.html" "filter(item=>String(item?.kind||'file')!=='text')"
verify_workbench_marker_if_present "$WORKBENCH_DIR/canvas-next/generation-service.js" "function forceGptImage2ProModelConfig"
verify_workbench_marker_if_present "$WORKBENCH_DIR/canvas-next/generation-service.js" "参考图上传后未返回 file-xxx"
