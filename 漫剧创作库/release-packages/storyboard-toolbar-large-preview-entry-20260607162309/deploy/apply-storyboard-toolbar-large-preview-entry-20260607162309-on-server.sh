...
PKG="storyboard-toolbar-large-preview-entry-20260607162309"
...
verify_html_markers(){
  local file="$1"
  grep -Fq "let storyboardHasPreview=false;" "$file"
  grep -Fq "getPreviewEntriesForNode(n.id).filter(Boolean).some(entry=>!!imageEntryUrl(entry));" "$file"
  grep -Fq "const storyboardLargeTools=storyboardHasPreview?" "$file"
  grep -Fq "data-action=\"openPreview\" data-tip=\"查看大图\"" "$file"
  grep -Fq '${previewRefreshTool}${singleImageTools}${storyboardLargeTools}${assetDesignTools}' "$file"
  grep -Fq "20260607: screenshot toolbar sizing pass." "$file"
}
verify_html_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "let storyboardHasPreview=false;" "$file"
  run_sudo grep -Fq "getPreviewEntriesForNode(n.id).filter(Boolean).some(entry=>!!imageEntryUrl(entry));" "$file"
  run_sudo grep -Fq "const storyboardLargeTools=storyboardHasPreview?" "$file"
  run_sudo grep -Fq "data-action=\"openPreview\" data-tip=\"查看大图\"" "$file"
  run_sudo grep -Fq '${previewRefreshTool}${singleImageTools}${storyboardLargeTools}${assetDesignTools}' "$file"
