...
PKG="canvas-generated-image-preview-render-fix-20260612113420"
...
verify_canvas_markers(){
  local file="$1"
  grep -Fq "function renderGeneratedResultImg(src,attrs='')" "$file"
  grep -Fq "renderGeneratedResultImg(resultImg,'class=\"aio-preview-img\"')" "$file"
  grep -Fq "renderGeneratedResultImg(generatedUrl,'class=\"asset-preview-image\"')" "$file"
  grep -Fq "if(/\\.(png|jpe?g|webp|gif|avif|bmp|svg)(?:$|[?#])/i.test(path))return false" "$file"
  grep -Fq "function isAsyncStoragePendingStatus(value)" "$file"
  grep -Fq "const asyncStorageHint=storagePending||taskHasAsyncStorageHint(task)||taskHasObjectStorageResultUrl(task)" "$file"
}
