...
```json
{
  ...
  "params": "--ar 16:9 --v 8.1 --raw --s 100 --c 6 --hd",
  "aspectRatio": "16:9",
  "qualityProfile": "high",
...
版本策略：
- `--v 8.1`：官方不支持 `--cref --cw`，也不使用 V7 的 `--oref --ow`；当前智能模式也不输出 `--q`。角色图由程序转成 prompt 最前面的 image prompt URL，并自动使用高 `--iw` 保持形象；英文 prompt 必须写明 preserve the exact same character identity, facial features, hairstyle, outfit, clothing colors, body proportions and silhouette。
...
- `--niji 7` 或不支持角色引用的版本：不输出 `--raw --hd --cref --cw --oref --ow`，角色图转成 image prompt URL，并把 `--iw` 提到当前版本上限。
...
- 画质优先但必须合法：当前所选模型版本支持 Q4 且没有使用不兼容参数时优先 `--q 4`；V7 如果使用 `--oref`，不能使用 `--q 4`，必须用 `--q 2`；V8.1 不输出 `--q`；不支持 Q4 的旧版本回退 `--q 2`。
- 官方版本兼容硬规则：`--v 8.1` 不输出 `--q --cref --cw --oref --ow`；`--v 7` 角色一致用 `--oref --ow`，不用 `--cref --cw`；`--v 6` / `--v 6.1` / `--niji 6` 才用 `--cref --cw`；`--niji 7` 不输出 `--raw --hd` 和角色引用参数。
