...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/canvas-multigrid-cell-upscale-source-fix-20260626050759-${STAMP}"
...
validate_html(){
  local file="$1"
  test -f "$file"
  require_marker "const forceFreshUpload=!!(img._forceUploadDataUrlRef||img._fromMultiGridCell)" "$file"
  require_marker "const existing=forceFreshUpload?'':providerImageUrl(img,uploadMode)" "$file"
  require_marker "_forceUploadDataUrlRef:true" "$file"
  require_marker "staleRefKeys" "$file"
  require_marker "delete savedRest[key]" "$file"
  require_marker "plainMultiGridCellImageEntry(cropped,src)" "$file"
  require_marker "node.values._forceOwnImageRefine=true" "$file"
  require_marker "function completeMultiGridUpscaleNodeAsync(nodeId,source,state)" "$file"
}
...
echo "deployed canvas-multigrid-cell-upscale-source-fix-20260626050759"
