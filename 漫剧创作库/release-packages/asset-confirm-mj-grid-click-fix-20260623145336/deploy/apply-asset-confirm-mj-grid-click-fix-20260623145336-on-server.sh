...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/asset-confirm-mj-grid-click-fix-20260623145336-${STAMP}"
...
check_markers(){
  local file="$1"
  test -f "$file"
  grep -Fq "function assetConfirmMidjourneyDetailKeyFromTarget" "$file"
  grep -Fq "function openAssetConfirmMidjourneyScopedTarget" "$file"
  grep -Fq "openAssetConfirmMidjourneyScopedTarget(n,cardId,mjTarget)" "$file"
  grep -Fq "data-asset-confirm-mj-cell" "$file"
...
echo "deployed asset-confirm-mj-grid-click-fix-20260623145336"
