...
PKG="text-prompt-own-content-label-20260602021812"
ARCHIVE="${1:-}"
...
grep -Fq "['passthrough','自己编写内容']" "$DEST"
grep -Fq "function promptModeLabel(mode)" "$DEST"
if grep -Fq "默认透传" "$DEST"; then
  fail "old passthrough label still exists"
fi
