...
ARCHIVE="${1:-}"
PKG="canvas-storyboard-compose-mode-20260612144246"
...
verify_file() {
  local file="$1"
  grep -Fq "selectedShots:[],composeMode:true" "$file"
  grep -Fq "function storyboardImageComposeMode(n)" "$file"
  grep -Fq "data-prompt-action=\"toggleStoryboardComposeMode\"" "$file"
  grep -Fq "执行按钮将合成选中镜号" "$file"
  grep -Fq "已切换为合成执行" "$file"
  grep -Fq "合成分镜图不消耗模型积分" "$file"
  grep -Fq "storyboardImageComposeMode(n)?await runStoryboardImageRecomposeAction(id,n):await runStoryboardImageNode(id)" "$file"
}
