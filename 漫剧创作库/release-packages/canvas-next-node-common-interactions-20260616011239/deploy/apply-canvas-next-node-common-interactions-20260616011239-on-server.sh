...
PKG="canvas-next-node-common-interactions-20260616011239"
...
for marker in \
  "focusNodeLarge(n.id)" \
  ".asset-confirm-card,.asset-confirm-detail,.asset-confirm-side,.asset-confirm-bar" \
  "if(S.selected){e.preventDefault();removeNode(S.selected);return;}" \
  "function bindAssetConfirmNode"
do
...
if sed -n '22100,22155p' "$SRC" | grep -Fq ".asset-confirm-node"; then
  echo "[deploy] asset-confirm outer node is still blocking common interactions" >&2
  exit 5
fi
