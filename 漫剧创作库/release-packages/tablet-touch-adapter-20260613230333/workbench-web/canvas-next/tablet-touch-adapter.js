export function installCanvasTabletTouchAdapter(opts){
  ...
  const isTabletViewport=()=>{
    ...
    return (navigator.maxTouchPoints||0)>0&&Math.min(w,h)>=600;
  };
  const syncClass=()=>document.documentElement.classList.toggle('canvas-tablet-touch',isTabletViewport());
  const isTouch=e=>e.pointerType==='touch'&&isTabletViewport();
  const isEditable=t=>!!t?.closest?.('input,textarea,select,[contenteditable="true"]');
  const isBlank=t=>t===stage||t===world||t===svg;
