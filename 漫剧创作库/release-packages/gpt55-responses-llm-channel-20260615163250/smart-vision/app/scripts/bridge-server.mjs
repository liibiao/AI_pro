...
async function callBridgeLlmChat(input = {}) {
  ...
  if (isOpenAiResponsesLlmModel(model, endpointPath)) {
    ...
    const { response, payload } = await postBridgeChatCompletion(endpoint, headers, body, timeoutMs);
  ...
  Object.keys(body).forEach((key) => body[key] === undefined && delete body[key]);
  let { response, payload } = await postBridgeChatCompletion(endpoint, headers, body, timeoutMs);
  ...
  if (!response.ok && chatContentHasArray(messages) && chatContentArraySchemaError(payload)) {
    ...
    ({ response, payload } = await postBridgeChatCompletion(endpoint, headers, retryBody, timeoutMs));
  }
...
async function postBridgeChatCompletion(endpoint, headers, body, timeoutMs) {
  let response = null;
  try {
...
function sendJson(res, statusCode, payload) {
  ...
  res.writeHead(statusCode, {
    ...
    'access-control-allow-headers': 'content-type, authorization, x-request-timeout-ms, x-generation-timeout-ms, x-client-request-id, x-timeout-ms',
...
function generationTaskPayloadToWorkbenchPayload(input = {}, resolvedModel = null) {
  ...
  const merged = {
    ...
    apiKey: input.apiKey || params.apiKey || params.key || provider.apiKey || modelConfigOverride.apiKey || modelConfig.apiKey || modelConfigOverride.key || modelConfig.key || model.key || model.apiKey || modelProvider.apiKey || '',
