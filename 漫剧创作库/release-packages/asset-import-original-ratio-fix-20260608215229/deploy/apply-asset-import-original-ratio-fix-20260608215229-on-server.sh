...
PKG="asset-import-original-ratio-fix-20260608215229"
...
verify_html_markers(){
  local file="$1"
  grep -Fq "function imageOriginalDimensionProbeSrc(src)" "$file"
  grep -Fq "if(img.dataset.adaptiveImage==='1')ensureImageNodeOriginalSize(id,img);" "$file"
  grep -Fq "const fallback=previewImageSrc(src,4096,4096);" "$file"
  grep -Fq "function applyImageNodeMeasuredSize(id,w,h)" "$file"
  grep -Fq "function ensureImageNodeOriginalSize(id,img=null)" "$file"
  grep -Fq "return w>2&&h>2?w/h:null;" "$file"
  grep -Fq "ensureImageNodeOriginalSize(nid);" "$file"
...
verify_html_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "function imageOriginalDimensionProbeSrc(src)" "$file"
  run_sudo grep -Fq "if(img.dataset.adaptiveImage==='1')ensureImageNodeOriginalSize(id,img);" "$file"
  run_sudo grep -Fq "const fallback=previewImageSrc(src,4096,4096);" "$file"
  run_sudo grep -Fq "function applyImageNodeMeasuredSize(id,w,h)" "$file"
  run_sudo grep -Fq "function ensureImageNodeOriginalSize(id,img=null)" "$file"
  run_sudo grep -Fq "return w>2&&h>2?w/h:null;" "$file"
  run_sudo grep -Fq "ensureImageNodeOriginalSize(nid);" "$file"
