...
PKG="model-display-group-order-20260620031523"
...
verify_canvas_html(){
  local file="$1"
  grep -Fq "MODEL_DISPLAY_GROUPS" "$file"
  grep -Fq "function sortModelsForDisplay" "$file"
  grep -Fq "return sortModelsForDisplay" "$file"
  grep -Fq "midjourney|mj" "$file"
}
...
if run_sudo test -f "$WORKBENCH_DIR/image-studio-canvas-next.html"; then
  run_sudo grep -Fq "MODEL_DISPLAY_GROUPS" "$WORKBENCH_DIR/image-studio-canvas-next.html"
  run_sudo grep -Fq "function sortModelsForDisplay" "$WORKBENCH_DIR/image-studio-canvas-next.html"
fi
if run_sudo test -f "$WORKBENCH_DIR/image-studio-canvas.html"; then
  run_sudo grep -Fq "MODEL_DISPLAY_GROUPS" "$WORKBENCH_DIR/image-studio-canvas.html"
  run_sudo grep -Fq "function sortModelsForDisplay" "$WORKBENCH_DIR/image-studio-canvas.html"
fi
