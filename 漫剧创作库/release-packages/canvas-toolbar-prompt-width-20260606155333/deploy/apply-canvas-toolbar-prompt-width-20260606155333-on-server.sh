...
PKG="canvas-toolbar-prompt-width-20260606155333"
...
verify_html(){
  local file="$1"
  grep -Fq "tapnow-rewrite.css?v=20260606-toolbar-prompt-width" "$file"
  grep -Fq "NODE_BOTTOM_TOOLBAR_SELECTOR" "$file"
...
verify_css(){
  ...
  grep -Fq "position:static!important" "$file"
  grep -Fq -- "--node-toolbar-min-width" "$file"
}
verify_html_sudo(){
  local file="$1"
  run_sudo grep -Fq "tapnow-rewrite.css?v=20260606-toolbar-prompt-width" "$file"
  run_sudo grep -Fq "NODE_BOTTOM_TOOLBAR_SELECTOR" "$file"
...
verify_css_sudo(){
  ...
  run_sudo grep -Fq "position:static!important" "$file"
  run_sudo grep -Fq -- "--node-toolbar-min-width" "$file"
}
