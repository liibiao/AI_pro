...
PKG="prompt-border-clip-fix-20260601113104"
...
log "install prompt border and clipping fix"
install -m "$DEST_MODE" "$SRC" "$DEST"
chown "$DEST_OWNER" "$DEST"

log "verify markers"
grep -q "storygrid-note-editor .i2i-input-box{border:0" "$DEST"
grep -q "prompt-zoom-large-group.prompt-zoom-unified-group :is(.i2i-input-box,.vn2-input-row){padding:5px 0 14px" "$DEST"
grep -q "textarea:is(.aio-textarea,.prompt-node-inputbox,.vn2-prompt-text){height:100" "$DEST"
