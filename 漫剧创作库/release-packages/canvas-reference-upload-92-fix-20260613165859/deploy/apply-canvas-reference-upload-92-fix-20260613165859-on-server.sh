...
PKG="canvas-reference-upload-92-fix-20260613165859"
...
verify_canvas_markers(){
  local file="$1"
  grep -Fq "function markReferenceUploadComplete(entry,{label='上传完成',clearPromise=false}={})" "$file"
  grep -Fq "if(done)markReferenceUploadComplete(img);" "$file"
  grep -Fq "if(done)markReferenceUploadComplete(media);" "$file"
  grep -Fq "markReferenceUploadComplete(img,{clearPromise:true})" "$file"
  grep -Fq "markReferenceUploadComplete(img);" "$file"
  if grep -Fq "done?92" "$file" || grep -Fq "progress:92" "$file"; then
    fail "stale 92 percent upload marker still exists in $file"
  fi
}
verify_canvas_markers_sudo(){
  local file="$1"
  run_sudo bash -c "$(declare -f fail); $(declare -f verify_canvas_markers); verify_canvas_markers \"\$0\"" "$file"
}
