...
PKG="seedance-image-reference-label-tdz-fix-20260606235651"
...
verify_markers(){
  local file="$1"
  grep -Fq "const imageReferenceLabel=idx=>isGrok15Preview?'起始图':\`图片\${idx+1}\`;" "$file"
  grep -Fq "const imageReferenceLabel=idx=>isGrokVideo15PreviewModelEntry(getVideoActiveModel(n.values))?'起始图':\`图片\${idx+1}\`;" "$file"
  grep -Fq "...allImages.map((img,idx)=>({type:'image',entry:img,label:imageReferenceLabel(idx),idx}))" "$file"
  grep -Fq "...imgs.map((img,idx)=>({type:'image',entry:img,label:imageReferenceLabel(idx),idx}))" "$file"
}
verify_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "const imageReferenceLabel=idx=>isGrok15Preview?'起始图':\`图片\${idx+1}\`;" "$file"
  run_sudo grep -Fq "const imageReferenceLabel=idx=>isGrokVideo15PreviewModelEntry(getVideoActiveModel(n.values))?'起始图':\`图片\${idx+1}\`;" "$file"
  run_sudo grep -Fq "...allImages.map((img,idx)=>({type:'image',entry:img,label:imageReferenceLabel(idx),idx}))" "$file"
  run_sudo grep -Fq "...imgs.map((img,idx)=>({type:'image',entry:img,label:imageReferenceLabel(idx),idx}))" "$file"
}
