...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/pano-shot-worker-blob-export-20260625131016-${STAMP}"
...
validate_html(){
  local file="$1"
  test -f "$file"
  require_marker "pano-shot-mask" "$file"
  require_marker "async function encodeCanvasPngBlobInWorker(canvas)" "$file"
  require_marker "function canUseWorkerCanvasPngEncoder(canvas)" "$file"
  require_marker "async function canvasToPngBlobAsync(canvas)" "$file"
  require_marker "async function createSingleImageFromBlob(blob,sourceId,ratio='free',opts={})" "$file"
  require_marker "await createSingleImageFromBlob(blob,id,mode" "$file"
}
...
echo "deployed pano-shot-worker-blob-export-20260625131016"
