...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/mj-empty-preview-mask-fix-20260625105110-${STAMP}"
...
validate_html(){
  local file="$1"
  test -f "$file"
  require_marker "function midjourneySessionDisplayUrl(session={},fallbackUrl='')" "$file"
  require_marker "if(!sessionUrl&&!pages.length)return null" "$file"
  require_marker "function displayableImageResultListFromData(data)" "$file"
  require_marker "const resultList=displayableImageResultListFromData(n.data)" "$file"
  require_marker "function midjourneyCssImage(url)" "$file"
  require_marker ":'none';" "$file"
}
...
echo "deployed mj-empty-preview-mask-fix-20260625105110"
