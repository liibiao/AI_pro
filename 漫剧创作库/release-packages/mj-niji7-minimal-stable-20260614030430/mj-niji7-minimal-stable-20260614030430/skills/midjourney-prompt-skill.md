...
- `--niji 7`：官方没有角色引用参数。当前画布 MJ 通道按最小稳定参数集提交：不输出 `--raw --c --chaos --hd --cref --cw --oref --ow`；只保留 `--niji 7`、`--s`、安全 `--q 1`，有参考图时再由程序追加合法 `--iw` / `--sref`。
...
- 官方版本兼容硬规则：`--v 8.1` 不输出 `--q --cref --cw --oref --ow`；`--v 7` 角色一致用 `--oref --ow`，不用 `--cref --cw`；`--v 6` / `--v 6.1` / `--niji 6` 才用 `--cref --cw`；`--niji 7` 当前通道不自动输出 `--raw --c --chaos --hd` 和角色引用参数，质量固定 `--q 1`。
