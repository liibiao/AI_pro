...
PKG="mj-smart-version-preview-fix-20260613011233"
...
verify_canvas_markers(){
  local file="$1"
  grep -Fq "function midjourneyVersionLabel(version)" "$file"
  grep -Fq "function renderSmartMidjourneyPreviewIfActive(n,isMj)" "$file"
  grep -Fq "prepareSmartMidjourneyPromptForNode(n,model,opts={})" "$file"
  grep -Fq "不得自行把 --v 8.1 改成 --v 7" "$file"
  grep -Fq "智能生成MJ分镜提示词" "$file"
  grep -Fq "renderSmartMidjourneyPreviewIfActive(n,isMj)" "$file"
  grep -Fq "targetMidjourneyParams:runtime?.command||''" "$file"
}
...
verify_server_markers(){
  local file="$1"
  grep -Fq "def _normalize_midjourney_aspect_ratio" "$file"
  grep -Fq "def _strip_managed_midjourney_params" "$file"
  grep -Fq "Selected MJ runtime parameters" "$file"
  grep -Fq "runtime_command = re.sub" "$file"
}
...
verify_skill_markers(){
  local file="$1"
  grep -Fq "当前所选 Midjourney" "$file"
  grep -Fq "用户选择 V8.1 时必须使用" "$file"
  grep -Fq "模型版本参数必须来自画布当前选择" "$file"
}
