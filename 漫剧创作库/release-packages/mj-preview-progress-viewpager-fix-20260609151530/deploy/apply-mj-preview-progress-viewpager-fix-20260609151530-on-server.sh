...
PKG="mj-preview-progress-viewpager-fix-20260609151530"
...
verify_html_markers(){
  local file="$1"
  grep -Fq 'openMidjourneyPreviewForNode' "$file"
  grep -Fq 'suppressGenericPreview' "$file"
  grep -Fq 'if(n?.data?.mjSession)' "$file"
  grep -Fq 'setMidjourneyActivePage(n,e.deltaX>0?1:-1)' "$file"
  grep -Fq 'propagateImage(node.id,node.data)' "$file"
  grep -Fq 'sourceEntries=Array.isArray(img.images)' "$file"
  grep -Fq -- '--gen-progress-ratio' "$file"
  grep -Fq 'transform:scaleX(var(--gen-progress-ratio' "$file"
  grep -Fq 'genMask.style.setProperty('\''--gen-progress-ratio'\''' "$file"
}
verify_html_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq 'openMidjourneyPreviewForNode' "$file"
  run_sudo grep -Fq 'suppressGenericPreview' "$file"
  run_sudo grep -Fq 'if(n?.data?.mjSession)' "$file"
  run_sudo grep -Fq 'setMidjourneyActivePage(n,e.deltaX>0?1:-1)' "$file"
  run_sudo grep -Fq 'propagateImage(node.id,node.data)' "$file"
  run_sudo grep -Fq 'sourceEntries=Array.isArray(img.images)' "$file"
  run_sudo grep -Fq -- '--gen-progress-ratio' "$file"
  run_sudo grep -Fq 'transform:scaleX(var(--gen-progress-ratio' "$file"
  run_sudo grep -Fq 'genMask.style.setProperty('\''--gen-progress-ratio'\''' "$file"
}
