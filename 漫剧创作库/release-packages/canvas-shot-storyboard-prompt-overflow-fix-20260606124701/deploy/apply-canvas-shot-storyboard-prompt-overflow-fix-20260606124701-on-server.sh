...
PKG="canvas-shot-storyboard-prompt-overflow-fix-20260606124701"
...
verify_markers(){
  local file="$1"
  grep -Fq ".node.node-type-shotStoryboard .aio-editor-wrap:not(.prompt-zoomed-item)" "$file"
  grep -Fq ".node.node-type-shotStoryboard .aio-editor-wrap:not(.prompt-zoomed-item) .prompt-node-rich-input" "$file"
  grep -Fq ".node.node-type-shotStoryboard .shot-storyboard-edit-panel .storygrid-note-editor .prompt-node-inputbox" "$file"
  grep -Fq "overflow-wrap:anywhere!important;word-break:break-word!important;overflow:auto!important;resize:none!important" "$file"
}
verify_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq ".node.node-type-shotStoryboard .aio-editor-wrap:not(.prompt-zoomed-item)" "$file"
  run_sudo grep -Fq ".node.node-type-shotStoryboard .aio-editor-wrap:not(.prompt-zoomed-item) .prompt-node-rich-input" "$file"
  run_sudo grep -Fq ".node.node-type-shotStoryboard .shot-storyboard-edit-panel .storygrid-note-editor .prompt-node-inputbox" "$file"
  run_sudo grep -Fq "overflow-wrap:anywhere!important;word-break:break-word!important;overflow:auto!important;resize:none!important" "$file"
}
