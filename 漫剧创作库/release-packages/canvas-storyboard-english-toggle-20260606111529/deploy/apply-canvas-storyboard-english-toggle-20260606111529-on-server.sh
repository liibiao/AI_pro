...
PKG="canvas-storyboard-english-toggle-20260606111529"
...
verify_markers(){
  local file="$1"
  grep -Fq "storyboard-language-toggle" "$file"
  grep -Fq "storyboard-ref-column" "$file"
  grep -Fq "englishVersion:false" "$file"
  grep -Fq "语言版本硬规则：英文版" "$file"
  grep -Fq "n.type==='storyboard'&&inp.dataset.field==='englishVersion'" "$file"
}
verify_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "storyboard-language-toggle" "$file"
  run_sudo grep -Fq "storyboard-ref-column" "$file"
  run_sudo grep -Fq "englishVersion:false" "$file"
  run_sudo grep -Fq "语言版本硬规则：英文版" "$file"
  run_sudo grep -Fq "n.type==='storyboard'&&inp.dataset.field==='englishVersion'" "$file"
}
