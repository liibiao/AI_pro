...
PKG="mj-action-context-preview-fix-20260609122621"
...
verify_html_markers(){
  local file="$1"
  grep -Fq 'function hydrateMidjourneyNodeContext' "$file"
  grep -Fq 'findMidjourneyModelForContext' "$file"
  grep -Fq 'function refreshMidjourneyActionContextIfNeeded' "$file"
  grep -Fq 'payload.parentTaskId=taskId' "$file"
  grep -Fq 'payload.custom_id=payload.customId' "$file"
  grep -Fq 'data-mj-detail-copy-prompt' "$file"
  grep -Fq 'data-mj-detail-download' "$file"
  grep -Fq 'downloadMidjourneyDetailPage' "$file"
  grep -Fq 'cell._mjPointerDown' "$file"
  grep -Fq '.node .mj-preview-wrap .mj-preview-foot{display:none!important}' "$file"
  grep -Fq '.node .mj-preview-wrap .mj-action-chip' "$file"
}
verify_html_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq 'function hydrateMidjourneyNodeContext' "$file"
  run_sudo grep -Fq 'findMidjourneyModelForContext' "$file"
  run_sudo grep -Fq 'function refreshMidjourneyActionContextIfNeeded' "$file"
  run_sudo grep -Fq 'payload.parentTaskId=taskId' "$file"
  run_sudo grep -Fq 'payload.custom_id=payload.customId' "$file"
  run_sudo grep -Fq 'data-mj-detail-copy-prompt' "$file"
  run_sudo grep -Fq 'data-mj-detail-download' "$file"
  run_sudo grep -Fq 'downloadMidjourneyDetailPage' "$file"
  run_sudo grep -Fq 'cell._mjPointerDown' "$file"
  run_sudo grep -Fq '.node .mj-preview-wrap .mj-preview-foot{display:none!important}' "$file"
  run_sudo grep -Fq '.node .mj-preview-wrap .mj-action-chip' "$file"
}
