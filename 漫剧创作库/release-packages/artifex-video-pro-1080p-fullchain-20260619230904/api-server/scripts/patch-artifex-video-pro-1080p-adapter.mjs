...
function backup(file) {
  fs.copyFileSync(file, `${file}.bak-artifex-video-pro-1080p-${stamp}`);
}
...
function replaceFunction(text, functionName, nextFunctionName, replacement, file) {
  ...
  if (
    current.includes('video-pro-1080p') &&
    current.includes("item === '1080p'") &&
    current.includes("quality !== 'pro' && requested === '1080p'")
  ) {
    return text;
  }
...
function resolveSeedance2ModelName(ctx: AdapterContext) {
  ...
  const allowed = ['video-fast-480p', 'video-fast-720p', 'video-pro-480p', 'video-pro-720p', 'video-pro-1080p'];
  ...
  const legacyRequestedResolution = [
    ...
    ctx.params.videoResolution,
    ctx.params.requestedResolution,
  ].map(item => String(item || '').trim().toLowerCase()).find(item => item === '480p' || item === '720p' || item === '1080p');
  ...
  const quality = (exact || raw).includes('pro') || /^video-pro$/i.test(raw) || key.includes('pro') || key.includes('高质量') ? 'pro' : 'fast';
  const requested = legacyRequestedResolution || (key.includes('1080p') ? '1080p' : (key.includes('480p') ? '480p' : '720p'));
  const resolution = quality !== 'pro' && requested === '1080p' ? '720p' : requested;
...
const files = [
  { rel: 'src/modules/generation/adapters/registry.ts', replacement: tsResolveSeedance2ModelName },
  { rel: 'dist/modules/generation/adapters/registry.js', replacement: jsResolveSeedance2ModelName },
];
...
for (const item of files) {
  ...
  if (after !== before) {
    ...
    changed += 1;
    console.log(`[patch-artifex-video-pro-1080p] patched ${item.rel}`);
  } else {
    console.log(`[patch-artifex-video-pro-1080p] unchanged ${item.rel}`);
  }
}

console.log(`[patch-artifex-video-pro-1080p] changed=${changed}`);
