...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/asset-confirm-select-detail-fast-20260622213918-${STAMP}"
...
check_markers(){
  local file="$1"
  test -f "$file"
  grep -Fq "function assetConfirmDetailPreviewContent" "$file"
  grep -Fq "asset-confirm-detail-loading" "$file"
  grep -Fq "function scheduleAssetConfirmDeferredDetailPreview" "$file"
  grep -Fq "function updateAssetConfirmSelectedCardDom" "$file"
  grep -Fq "function selectAssetConfirmCard" "$file"
  grep -Fq "function bindAssetConfirmDetailInteractions" "$file"
  grep -Fq "selectAssetConfirmCard(n,cardId,{rerender:false})" "$file"
  grep -Fq "requestAnimationFrame(()=>setTimeout(render,0))" "$file"
}
...
echo "deployed asset-confirm-select-detail-fast-20260622213918"
