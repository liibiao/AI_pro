...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/canvas-multigrid-nonfatal-crop-fix-20260626034738-${STAMP}"
...
validate_html(){
  local file="$1"
  test -f "$file"
  require_marker "source?.assetPreviewUrl" "$file"
  require_marker "saved.assetPreviewUrl" "$file"
  require_marker "multi-grid materialize skipped; keep generation success" "$file"
  require_marker "generatedEntry._multiGridMaterializeError=err?.message||String(err||'')" "$file"
  require_marker "scheduleMultiGridGeneratedEntryMaterialize(generatedEntry,nd.id)" "$file"
}
...
echo "deployed canvas-multigrid-nonfatal-crop-fix-20260626034738"
