...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/canvas-multigrid-ready-cells-async-import-fix-20260626032248-${STAMP}"
...
validate_html(){
  local file="$1"
  test -f "$file"
  require_marker "function materializedMultiGridCellsFromEntry(entry" "$file"
  require_marker "const readyCells=materializedMultiGridCellsFromEntry(entry,meta)" "$file"
  require_marker "await ensureLocalImageForProvider(generatedEntry,'宫格源图预览')" "$file"
  require_marker "function completeImportedMultiGridCellNodeAsync(nodeId,source)" "$file"
  require_marker "createDerivedSingleImageNode(LB.nodeId,entry,{status:'running',progress:8,stageLabel:'裁切宫格中'})" "$file"
  require_marker "Math.max(pointerDx,pointerDy)>6" "$file"
}
...
echo "deployed canvas-multigrid-ready-cells-async-import-fix-20260626032248"
