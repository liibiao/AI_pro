...
PKG="video-node-reference-refresh-preserve-fix-20260608203802"
...
verify_html_markers(){
  ...
  grep -Fq 'if(pruneMedia){' "$file"
  grep -Fq 'normalizeMediaReferenceList(v,'\''video'\'');' "$file"
  grep -Fq 'normalizeMediaReferenceList(v,'\''audio'\'');' "$file"
  grep -Fq 'function syncUpstreamMediaToVideo(n)' "$file"
  grep -Fq 'const supported=kind==='\''video'\''?caps.supportsVideo:caps.supportsAudio;' "$file"
...
verify_html_markers_sudo(){
  ...
  run_sudo grep -Fq 'if(pruneMedia){' "$file"
  run_sudo grep -Fq 'normalizeMediaReferenceList(v,'\''video'\'');' "$file"
  run_sudo grep -Fq 'normalizeMediaReferenceList(v,'\''audio'\'');' "$file"
  run_sudo grep -Fq 'function syncUpstreamMediaToVideo(n)' "$file"
  run_sudo grep -Fq 'const supported=kind==='\''video'\''?caps.supportsVideo:caps.supportsAudio;' "$file"
