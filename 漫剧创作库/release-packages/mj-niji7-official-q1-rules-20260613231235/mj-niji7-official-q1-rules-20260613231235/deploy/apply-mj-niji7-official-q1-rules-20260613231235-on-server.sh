...
PKG="mj-niji7-official-q1-rules-20260613231235"
...
verify_html_markers(){
  ...
  grep -Fq "if(midjourneyVersionIsNiji(raw))return false" "$file"
  grep -Fq "Niji7 质量固定 --q 1" "$file"
}
...
verify_backend_markers_sudo(){ local file="$1"; run_sudo grep -Fq "_sanitize_midjourney_prompt_for_version" "$file"; run_sudo grep -Fq "_strip_midjourney_flag_param" "$file"
  grep -Fq -- "--q 1" "$file"; run_sudo grep -Fq "prompt_urls_to_prefix" "$file"; run_sudo grep -Fq -- "--oref {cref_url} --ow 100" "$file"; }
