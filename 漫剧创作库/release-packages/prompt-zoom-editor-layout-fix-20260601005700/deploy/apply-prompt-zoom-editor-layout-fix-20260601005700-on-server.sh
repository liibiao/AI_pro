...
ARCHIVE="${1:-/tmp/prompt-zoom-editor-layout-fix-20260601005700.tar.gz}"
TARGET="${2:-/home/ubuntu/漫剧创作库}"
STAMP="$(date +%Y%m%d%H%M%S)"
WORK="$(mktemp -d /tmp/prompt-zoom-editor-layout-fix-20260601005700-XXXXXX)"
...
SRC="$WORK/prompt-zoom-editor-layout-fix-20260601005700"
test -f "$SRC/tools/workbench-web/image-studio-canvas-next.html" || { echo "部署包缺少 canvas-next HTML" >&2; exit 1; }

BACKUP_DIR="$TARGET/.deploy-backups/prompt-zoom-editor-layout-fix-20260601005700-$STAMP"
...
log "verify markers"
grep -q "tn-src-prompt-node-model" "$TARGET/tools/workbench-web/image-studio-canvas-next.html"
grep -q "select.tn-native-control" "$TARGET/tools/workbench-web/image-studio-canvas-next.html"
