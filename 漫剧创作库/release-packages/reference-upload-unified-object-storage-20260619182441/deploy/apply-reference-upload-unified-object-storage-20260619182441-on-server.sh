...
PKG="reference-upload-unified-object-storage-20260619182441"
...
verify_html(){
  local file="$1"
  grep -Fq "workbench-engine.js?v=20260619-reference-upload-unified-v1" "$file"
  grep -Fq "function applyObjectStorageUploadProgress" "$file"
  grep -Fq "async function uploadObjectStorageReferenceFile" "$file"
  grep -Fq "objectStorageUploadResultUrl(uploaded)" "$file"
  grep -Fq "uploadObjectStorageReferenceFile(file,media" "$file"
}
...
verify_engine(){
  ...
  grep -Fq "获取 COS 直传地址超时" "$file"
  grep -Fq "COS 直传启动超时" "$file"
...
if run_sudo test -f "$WORKBENCH_DIR/image-studio-canvas-next.html"; then
  run_sudo grep -Fq "workbench-engine.js?v=20260619-reference-upload-unified-v1" "$WORKBENCH_DIR/image-studio-canvas-next.html"
  run_sudo grep -Fq "uploadObjectStorageReferenceFile(file,media" "$WORKBENCH_DIR/image-studio-canvas-next.html"
fi
