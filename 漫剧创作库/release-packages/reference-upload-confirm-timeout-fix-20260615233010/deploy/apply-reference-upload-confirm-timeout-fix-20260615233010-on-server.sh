...
PKG="reference-upload-confirm-timeout-fix-20260615233010"
...
verify_html_markers(){
  local file="$1"
  grep -Fq 'const REFERENCE_IMAGE_CONFIRM_TIMEOUT_MS=90000;' "$file"
  grep -Fq 'function markReferenceUploadConfirming(entry)' "$file"
  grep -Fq 'function isReferenceUploadConfirmStale(entry' "$file"
  grep -Fq 'confirmTimeoutMs:mode===' "$file"
  grep -Fq 'backendPostUploadTimeoutMs:REFERENCE_IMAGE_BACKEND_CONFIRM_TIMEOUT_MS' "$file"
}
verify_engine_markers(){
  local file="$1"
  grep -Fq 'COS 直传已上传 100%，但服务端确认超时' "$file"
  grep -Fq '后端 COS 上传已接收文件，但转存确认超时' "$file"
  grep -Fq '参考图已上传 100%，但 /v1/files 云端确认超时' "$file"
  grep -Fq 'function rejectOnce(err, abortXhr)' "$file"
}
verify_html_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq 'const REFERENCE_IMAGE_CONFIRM_TIMEOUT_MS=90000;' "$file"
  run_sudo grep -Fq 'function markReferenceUploadConfirming(entry)' "$file"
  run_sudo grep -Fq 'function isReferenceUploadConfirmStale(entry' "$file"
  run_sudo grep -Fq 'confirmTimeoutMs:mode===' "$file"
  run_sudo grep -Fq 'backendPostUploadTimeoutMs:REFERENCE_IMAGE_BACKEND_CONFIRM_TIMEOUT_MS' "$file"
}
verify_engine_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq 'COS 直传已上传 100%，但服务端确认超时' "$file"
  run_sudo grep -Fq '后端 COS 上传已接收文件，但转存确认超时' "$file"
  run_sudo grep -Fq '参考图已上传 100%，但 /v1/files 云端确认超时' "$file"
  run_sudo grep -Fq 'function rejectOnce(err, abortXhr)' "$file"
}
...
if [ -f "$WORKBENCH_DIR/workbench-engine.js" ]; then
...
if [ -f "$MIRROR_TARGET/tools/workbench-web/workbench-engine.js" ]; then
  verify_engine_markers_sudo "$MIRROR_TARGET/tools/workbench-web/workbench-engine.js"
fi
