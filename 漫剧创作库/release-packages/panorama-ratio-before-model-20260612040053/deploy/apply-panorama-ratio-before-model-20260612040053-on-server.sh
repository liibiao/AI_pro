...
PKG="panorama-ratio-before-model-20260612040053"
...
run_sudo(){
  "$@" && return 0
  local status=$?
  if [ "$(id -u)" -eq 0 ] || [ "${DEPLOY_USE_SUDO:-auto}" = "never" ] || [ "${1:-}" = "test" ]; then return "$status"; fi
...
verify_html(){
  local file="$1"
  grep -Fq 'function renderPanoramaGenerateControls(n)' "$file"
  grep -Fq 'return `<select data-field="size" class="aio-sel pano-ratio-sel"' "$file"
  grep -Fq '</select><select data-field="_modelIdx" class="aio-sel pano-model-select"' "$file"
  grep -Fq 'class="aio-sel pano-res-sel"' "$file"
}
verify_html_sudo(){
  local file="$1"
  run_sudo grep -Fq 'function renderPanoramaGenerateControls(n)' "$file"
  run_sudo grep -Fq 'return `<select data-field="size" class="aio-sel pano-ratio-sel"' "$file"
  run_sudo grep -Fq '</select><select data-field="_modelIdx" class="aio-sel pano-model-select"' "$file"
  run_sudo grep -Fq 'class="aio-sel pano-res-sel"' "$file"
}
...
if [ -f "$WORKBENCH_DIR/image-studio-canvas-next.html" ]; then verify_html_sudo "$WORKBENCH_DIR/image-studio-canvas-next.html"; fi
if [ -f "$MIRROR_TARGET/tools/workbench-web/image-studio-canvas-next.html" ]; then verify_html_sudo "$MIRROR_TARGET/tools/workbench-web/image-studio-canvas-next.html"; fi
log "done"
