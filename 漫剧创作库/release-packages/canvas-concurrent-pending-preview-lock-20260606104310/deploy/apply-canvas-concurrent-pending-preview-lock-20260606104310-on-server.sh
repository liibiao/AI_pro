...
PKG="canvas-concurrent-pending-preview-lock-20260606104310"
...
verify_canvas_markers(){
  local file="$1"
  grep -Fq "function markGeneratedImagePreviewPending" "$file"
  grep -Fq "function markGeneratedVideoPreviewPending" "$file"
  grep -Fq "function isGenerationPreviewUploadLocked" "$file"
  grep -Fq "function renderGeneratedPreviewPending" "$file"
  grep -Fq "generated-output-pending" "$file"
  grep -Fq "data-generation-pending" "$file"
  grep -Fq "markGeneratedImagePreviewPending(targetId,concurrentCtx.id,'图片生成中')" "$file"
  grep -Fq "markGeneratedVideoPreviewPending(targetId,concurrentCtx.id,'视频生成中')" "$file"
  grep -Fq "生成中，预览区暂不可上传" "$file"
  grep -Fq "clearGeneratedPreviewPending(target)" "$file"
  grep -Fq "clearGeneratedPreviewPending(n)" "$file"
}
