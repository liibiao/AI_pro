...
def _midjourney_major_version_from_params(params: str, image_model: object = None, label: str = "") -> float:
    ...
    match = re.search(r"--(?:v|version)\s+(\d+(?:\.\d+)?)", raw, re.I) or re.search(r"--niji\s+(\d+(?:\.\d+)?)", raw, re.I)
    if match:
        try:
...
def _midjourney_smart_quality_for_params(params: str, image_model: object = None, label: str = "") -> str:
    raw = str(params or "").lower()
    if re.search(r"--niji\b", raw, re.I) or _is_niji_midjourney_model(image_model or {}, label):
        return "1"
...
def _normalize_midjourney_params(params: str, aspect_ratio: str, image_model: object, image_model_label: str, runtime: object = None) -> str:
    ...
    if runtime_command:
        ...
        return re.sub(r"\s+", " ", f"{extras} --ar {aspect} {runtime_command}").strip()
    ...
    if not re.search(r"--q(?:uality)?\s+\S+", raw, re.I):
        raw = f"{raw} --q {_midjourney_smart_quality_for_params(raw, image_model, image_model_label)}".strip()
    return raw
...
def run_smart_midjourney_prompt(payload: dict[str, object]) -> dict[str, object]:
    ...
    default_params = "--niji 7 --q 1" if _is_niji_midjourney_model(image_model, image_model_label) else "--v 7 --style raw --q 4"
