...
PKG="canvas-text-prompt-editable-autofill-20260604162705"
...
verify_markers(){
  local dest="$1"
  grep -Fq "TEXT_PROMPT_EDITABLE_TARGET_TYPES" "$dest"
  grep -Fq "maybeAutofillEditablePromptFromTextPrompt" "$dest"
  grep -Fq "syncTextPromptOutputsToEditableDownstreams" "$dest"
  grep -Fq "_promptAutofillSourceId" "$dest"
  grep -Fq "maybeAutofillEditablePromptFromTextPrompt(fromId,toId,{rerender:false,forceEmpty:true})" "$dest"
}
