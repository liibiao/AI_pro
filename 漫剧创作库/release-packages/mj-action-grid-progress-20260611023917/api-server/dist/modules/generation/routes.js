...
router.get('/admin/tasks', requireAuth, requireRole(adminRoles), asyncHandler(async (req, res) => {
    ...
    if (requestedCategory) {
        if (requestedCategory === 'IMAGE' || requestedCategory === 'VIDEO') {
            ...
            ok(res, {
                items: tasks.map(task => sanitizeGenerationTaskListItem(task, requestedCategory)),
...
function resolveGenerationAdapterName(provider, model) {
    ...
    if (isCanvasGptImage2ProGenerationHint(modelHint, hint))
        return 'openai-edits';
    ...
    if (configuredLower === 'seedance2-vip' || configuredLower === 'seedance2.0-vip')
        return 'seedance2-vip';
    if (configuredLower === 'seedance2-sd' || configuredLower === 'sd-seedance2' || configuredLower === 'sd-video' || /\bsd2-(?:720p|1080p)(?:-fast)?\b/.test(hint) || hint.includes('aiyunzhi.top'))
        return 'seedance2-sd';
...
function canRecoverRefundedQueryFailure(task) {
    if (task.status !== 'REFUNDED' || !task.upstreamTaskId)
        return false;
...
async function materializeServerObjectStorageTask(taskId) {
    ...
    await prisma.generationTask.update({
        where: { id: task.id },
        data: { resultJson, resultUrlsJson: sourceUrls },
    });
}
function sleep(ms) {
...
function normalizeStoredGenerationResultJson(resultJson, urls) {
    ...
    return { url: urls[0], outputs: urls };
}
