...
def _normalize_midjourney_params(params: str, aspect_ratio: str, image_model: object, image_model_label: str) -> str:
    raw = re.sub(r"\s+", " ", str(params or "").strip())
    if not re.search(r"--(?:ar|aspect)\s+\S+", raw, re.I):
        raw = f"{raw} --ar {aspect_ratio}".strip()
    if not re.search(r"--(?:v|version|niji)\s+\S+", raw, re.I):
        raw = f"{raw} {'--niji 7' if _is_niji_midjourney_model(image_model, image_model_label) else '--v 7'}".strip()
    if not _is_niji_midjourney_model(image_model, image_model_label) and not re.search(r"--style\s+\S+", raw, re.I):
        raw = f"{raw} --style raw".strip()
...
def _fallback_midjourney_prompt(payload: dict[str, object], reason: str = "") -> dict[str, object]:
    ...
    aspect_ratio = _infer_midjourney_aspect_ratio(f"{prompt}\n{style_text}")
    ...
    english_prompt = f"{base}: {prompt}. Clean composition, clear subject, cinematic lighting, high detail, sharp focus, professional color grading"
    params = _normalize_midjourney_params("", aspect_ratio, payload.get("imageModel"), str(payload.get("imageModelLabel") or ""))
    warnings = ["LLM unavailable; used conservative fallback prompt."]
    if reason:
...
def run_smart_midjourney_prompt(payload: dict[str, object]) -> dict[str, object]:
    ...
    messages = [
        {
            ...
            "content": (
                ...
                "Use the supplied skill and wordlist candidates only when they match the user's intent. "
                "Never change the user's subject, action, setting, identity, style, emotion, or story meaning. "
                "Return JSON only."
            ),
        },
        {
            ...
            "content": (
                ...
                '{"prompt":"English MJ prompt without URLs","params":"--v 7 --ar 16:9 --style raw --q 2","aspectRatio":"16:9","qualityProfile":"high","fidelityCheck":"中文一句话说明如何保持原意","warnings":[]}'
            ),
        },
    ]
    request_payload = {
    ...
    aspect_ratio = str(data.get("aspectRatio") or data.get("aspect_ratio") or "").strip() or _infer_midjourney_aspect_ratio(f"{prompt}\n{style_text}")
    data["aspectRatio"] = aspect_ratio
    data["params"] = _normalize_midjourney_params(str(data.get("params") or data.get("midjourneyParams") or ""), aspect_ratio, image_model, image_model_label)
...
class WorkbenchRequestHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/workbench/url/extract":
            ...
            return
        if parsed.path == "/":
            self.path = "/漫剧创作库/tools/workbench-web/index.html"
        elif parsed.path == "/workbench.html":
            self.path = "/漫剧创作库/tools/workbench-web/workbench.html"
        elif parsed.path == "/image-studio.html":
            self.path = "/漫剧创作库/tools/workbench-web/image-studio.html"
        elif parsed.path == "/workbench-run.html":
            self.path = "/漫剧创作库/tools/workbench-web/workbench-run.html"
        elif parsed.path == "/image-studio-canvas.html":
            self.path = "/漫剧创作库/tools/workbench-web/image-studio-canvas-next.html"
        elif parsed.path == "/image-studio-canvas-next.html":
            self.path = "/漫剧创作库/tools/workbench-web/image-studio-canvas-next.html"
        elif parsed.path == "/workbench-engine.js":
            self.path = "/漫剧创作库/tools/workbench-web/workbench-engine.js"
        elif parsed.path.startswith("/canvas-next/"):
            self.path = "/漫剧创作库/tools/workbench-web" + parsed.path
