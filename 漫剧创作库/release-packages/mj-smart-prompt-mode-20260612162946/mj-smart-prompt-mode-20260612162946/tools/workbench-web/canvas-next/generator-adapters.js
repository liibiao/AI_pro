...
export function normalizeModelType(model){
  ...
  if(['lingdong-sd-2-vip','sd-2-vip','zaomeng-seedance2','zaomeng-seedance2-svip','zaomeng-seedance2-fast'].includes(adapter)||/\/(?:v1\/)?video\/generations(?:[/?#]|$)/i.test(endpoint)||/\/videos(?:[/?#]|$)/i.test(endpoint))return 'video';
