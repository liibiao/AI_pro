...
const smartMidjourneyPromptSchema = z.object({
    prompt: z.string().min(1),
...
router.post('/workbench/image-studio/smart-midjourney-prompt', requireAuth, asyncHandler(async (req, res) => {
    const body = smartMidjourneyPromptSchema.parse(req.body || {});
    const proxied = await proxySmartMidjourneyPrompt(body);
    ok(res, proxied || fallbackSmartMidjourneyPrompt(body));
}));
router.post('/workbench/image-studio/generate', requireAuth, asyncHandler(async (req, res) => {
    ...
    try {
        let upstream;
        let requestJsonForRecord = requestPayload;
        let saved;
        let resultB64 = '';
        ...
        if (runtime.generationMockMode) {
            ...
            if (submit.status === 'FAILED')
                fail(502, submit.errorMessage || '上游生图失败', submit.errorCode || 'UPSTREAM_IMAGE_FAILED');
            upstream = submit.responseJson;
            requestJsonForRecord = submit.requestJson;
            const resultUrl = submit.resultUrls?.[0] || String(submit.resultJson?.url || '');
        ...
        const record = await prisma.$transaction(async (tx) => {
            const usageRecord = await recordUsageAndCharge(tx, {
                ...
                params: pickCompatParams(body),
                prompt: body.prompt,
                requestJson: requestJsonForRecord,
            ...
            await tx.generationTask.create({
                data: {
                    ...
                    progress: 100,
                    prompt: body.prompt,
                    inputFilesJson: extractCompatInputLinks(body),
                    paramsJson: pickCompatParams(body),
                    requestJson: requestJsonForRecord,
                    responseJson: upstream,
                    upstreamTaskId: String(upstream?.id || ''),
                    resultJson: { url: resultUrl },
...
router.get('/workbench/image-studio/image/status', requireAuth, asyncHandler(async (req, res) => {
    ...
    if (!taskId)
        fail(400, '缺少 taskId', 'TASK_ID_REQUIRED');
    ok(res, {
        taskId,
        status: 'completed',
        progress: 100,
...
router.post('/workbench/image-studio/video/start', requireAuth, asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (runtime.generationMockMode) {
            ...
            status = normalizeVideoStatus(upstream, resultUrl, taskId);
        }
            ...
            if (submit.status === 'FAILED')
                fail(502, submit.errorMessage || '上游视频生成失败', submit.errorCode || 'UPSTREAM_VIDEO_FAILED');
            upstream = submit.responseJson;
            requestJsonForRecord = submit.requestJson;
            ...
            resultUrl = submit.resultUrls?.[0] || String(submit.resultJson?.url || '');
            ...
            taskId = extractVideoTask(upstream);
            if (!taskId)
                fail(502, '视频接口未返回 taskId', 'UPSTREAM_TASK_ID_MISSING');
            resultUrl = extractVideoResultUrl(upstream);
            status = normalizeVideoStatus(upstream, resultUrl, taskId);
        }
        if (!taskId)
        ...
        const record = await prisma.$transaction(async (tx) => {
            ...
            const generationTask = await tx.generationTask.create({
                data: {
                    ...
                    responseJson: upstream,
                    upstreamTaskId: taskId,
                    resultJson: resultUrl ? { url: resultUrl } : undefined,
                    resultUrlsJson: resultUrl ? [resultUrl] : [],
                    chargedCredits: usageRecord.chargedCredits,
                    costAmount: usageRecord.costAmount,
                    costUsd: new Prisma.Decimal(usageRecord.costUsd),
...
router.get('/workbench/image-studio/video/status', requireAuth, asyncHandler(async (req, res) => {
    ...
    if (!model || model.status !== 'ACTIVE' || model.type !== 'VIDEO')
        fail(404, '模型不可用', 'MODEL_NOT_FOUND');
    ...
    if (status === 'failed') {
        ...
        await prisma.$transaction(async (tx) => {
            await tx.modelUsage.updateMany({
                ...
                data: { responseJson: upstream, resultUrl },
            });
...
router.get('/workbench/image-studio/video/proxy', requireAuth, asyncHandler(async (req, res) => {
    ...
    if (!/^https?:\/\//i.test(remoteUrl))
        fail(400, '缺少可代理的视频地址', 'VIDEO_PROXY_URL_MISSING');
...
router.post('/workbench/image-studio/video/extract-audio', requireAuth, asyncHandler(async (req, res) => {
    ...
    if (!/^https?:\/\//i.test(remoteUrl))
        fail(400, '缺少可分离音频的视频地址', 'VIDEO_AUDIO_SOURCE_MISSING');
...
async function handleAudioToMp4(req, res) {
    ...
    if (dataUrl) {
        ...
        if (source.buffer.length > maxBytes)
            fail(413, '音频文件过大，无法转换为 mp4', 'AUDIO_TO_MP4_TOO_LARGE');
...
router.post('/workbench/image-studio/video/frame', requireAuth, asyncHandler(async (req, res) => {
    const remoteUrl = String(req.body?.remoteUrl || req.body?.url || '').trim();
    if (!/^https?:\/\//i.test(remoteUrl))
        fail(400, '缺少可截帧的视频地址', 'VIDEO_FRAME_SOURCE_MISSING');
...
router.get('/workbench/image-studio/proxy-image', requireAuth, asyncHandler(async (req, res) => {
    const remoteUrl = String(req.query.remoteUrl || req.query.url || '').trim();
    if (!/^https?:\/\//i.test(remoteUrl))
        fail(400, '缺少可代理的图片地址', 'REMOTE_IMAGE_PROXY_URL_MISSING');
...
router.post('/workbench/image-studio/upload-local-file-to-object-storage', requireAuth, asyncHandler(async (req, res) => {
    ...
    if (!filePath)
        fail(400, '本地图片不存在或不可读', 'LOCAL_IMAGE_NOT_FOUND');
...
router.post('/workbench/image-studio/upload-provider-file', requireAuth, asyncHandler(async (req, res) => {
    ...
    const fileBody = file.buffer.buffer.slice(file.buffer.byteOffset, file.buffer.byteOffset + file.buffer.byteLength);
    ...
    if (!/^file-[\w-]+$/i.test(fileId))
        fail(502, '中转站 /v1/files 未返回 file-xxx', 'PROVIDER_FILE_ID_MISSING');
...
async function getCompatModel(modelCode, type) {
...
async function getProviderFileUploadTarget(modelCode, channelKey) {
    const code = String(modelCode || '').trim();
...
function extractProviderFileId(upstream) {
...
async function proxySmartMidjourneyPrompt(body) {
    const base = String(process.env.WORKBENCH_COMPAT_BASE_URL || process.env.WORKBENCH_SERVER_BASE_URL || 'http://127.0.0.1:8766').replace(/\/+$/, '');
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), Number(process.env.WORKBENCH_SMART_MJ_PROXY_TIMEOUT_MS || 0) || 45000);
    try {
        const resp = await fetch(`${base}/api/workbench/image-studio/smart-midjourney-prompt`, {
            method: 'POST',
        ...
        if (!resp.ok || !data)
            return null;
        return data.ok === true ? data : { ok: true, ...asRecord(data) };
    }
    catch {
        return null;
    }
...
function fallbackSmartMidjourneyPrompt(body) {
    const source = String(body.prompt || body.naturalLanguage || '').trim();
    const aspectRatio = inferSmartMidjourneyAspectRatio(body.aspectRatio || body.size || source);
    const prompt = [
        source,
        'faithful to the user description',
    ...
    const params = `--ar ${aspectRatio} --v 7 --style raw --q 2`;
    return {
        ok: true,
        usedFallback: true,
        prompt,
        params,
        midjourneyParams: { version: '7', quality: '2', style: 'raw', aspectRatio },
        aspectRatio,
        qualityProfile: 'high',
        fidelityCheck: 'Fallback preserved the original user description and added only conservative execution/quality terms.',
        warnings: ['GPT-5.5 smart prompt service was unavailable; used conservative Midjourney V7 fallback.'],
    };
}
function inferSmartMidjourneyAspectRatio(...values) {
    const text = values.map(value => String(value || '')).join(' ').toLowerCase();
    const explicit = text.match(/\b(\d{1,2})\s*[:x×]\s*(\d{1,2})\b/);
    if (explicit)
        return `${explicit[1]}:${explicit[2]}`;
    if (/竖屏|纵向|portrait|vertical|手机|短视频|9:16/.test(text))
        return '9:16';
    if (/方图|头像|square|1:1/.test(text))
        return '1:1';
    if (/海报|poster|3:4|4:5/.test(text))
        return '3:4';
    if (/横版|宽屏|电影|cinematic|landscape|16:9/.test(text))
        return '16:9';
    return '16:9';
}
function getEndpointPath(_protocol, model, fallback) {
...
function compatAdapterName(model) {
    const modelProtocol = asRecord(model.protocol);
    return String(model.adapter || model.provider.adapter || modelProtocol.adapter || '').trim();
}
function isCompatGenerationAdapter(adapter, type) {
    const value = String(adapter || '').trim();
    if (type === 'IMAGE')
        return ['openai-edits', 'openai-responses-image', 'openai-chat-image', 'gpt-image-v2', 'grok_image', 'grok-image-unified', 'grok-image', 'grok-image-edit', 'gemini-image-generate', 'gemini-image-edit'].includes(value);
    if (type === 'VIDEO')
        return ['sora-video', 'sora-video-pro', 'seedance-task', 'seedance2', 'seedance2.0', 'seedance2-sd', 'sd-seedance2', 'sd-video', 'seedance2-vip', 'seedance2.0-vip', 'lingdong-sd-2-vip', 'zaomeng-seedance2', 'notevideo', 'grok-video', 'gemini-video', 'veo-video'].includes(value);
    return false;
}
function defaultEndpointPath(adapter, type) {
    switch (adapter) {
        ...
        case 'grok_image':
        case 'grok-image-unified': return '/images/generations';
        ...
        case 'seedance-task': return '/api/v3/contents/generations/tasks';
...
function resolveEndpointPath(endpointPath, tokens) {
...
function resolveUpstreamModelName(model, requested) {
    const value = String(requested || '').trim();
    const exact = [model.id, model.name, model.displayName].includes(value);
    if (model.type === 'VIDEO' && value && !exact)
        return value;
    return model.name;
}
function matchesModelAssembly(modelAssembly, modelCode) {
...
function buildImageSaved(result, upstream) {
    const filename = `img-${new Date().toISOString().replace(/[-:T]/g, '').slice(0, 14)}.png`;
    const upstreamId = String(upstream?.id || '');
    if (result.b64) {
        return {
...
function resolveCompatImageMode(body) {
    const text = [body.mode, body.taskType, body.template].filter(Boolean).join(' ').toLowerCase();
    if (/storyboard|故事板/.test(text))
        return 'storyboard';
    return body.image || body.images ? 'image-to-image' : 'text-to-image';
}
function resolveCompatVideoMode(body) {
    return body.images || body.image || body.image_url || body.imageUrl || body.extra_images || body.extraImages || body.reference_image_urls || body.referenceImageUrls || body.referenceImages || body.reference_images ? 'image-to-video' : 'text-to-video';
}
function extractCompatInputLinks(body) {
    const values = [
        body.image,
        body.images,
        body.image_url,
...
function pickCompatParams(body) {
    const params = {};
    [
        ...
        'image', 'images', 'image_url', 'imageUrl', 'extra_images', 'extraImages', 'reference_image_urls', 'referenceImageUrls', 'reference_images', 'referenceImages',
        'video_url', 'videoUrl', 'extra_videos', 'extraVideos', 'reference_video_urls', 'referenceVideoUrls', 'reference_video', 'referenceVideo', 'reference_videos', 'referenceVideos', 'refVideo', 'video',
        'audio_url', 'audioUrl', 'extra_audios', 'extraAudios', 'reference_audio_urls', 'referenceAudioUrls', 'reference_audio', 'referenceAudio', 'reference_audios', 'referenceAudios', 'refAudio', 'audio', 'soraV3ReferenceVideoUrl', 'referenceVideoUrl', 'audioReferenceVideoUrl',
...
function normalizeVideoStatus(upstream, resultUrl = '', taskId = '') {
    const raw = extractVideoTaskStatus(upstream);
    if (resultUrl)
        return 'completed';
    if (isCompatVideoDoneStatus(raw))
        return 'completed';
    if (taskId && isCompatQueuedOrBusyPayload(upstream))
        return 'processing';
    if (isCompatVideoFailedStatus(raw))
        return 'failed';
    return raw || 'processing';
}
function extractVideoFailureMessage(upstream) {
    const recursive = findCompatVideoErrorMessage(upstream, 0);
    return recursive || '上游视频任务失败';
}
function extractVideoTaskStatus(payload) {
    const data = payload;
    const candidates = [
        data?.data?.status,
        data?.data?.state,
...
function findCompatVideoStatus(value, depth) {
...
function normalizeCompatVideoStatusValue(value) {
...
function isCompatVideoFailedStatus(status) {
    return normalizeCompatVideoStatusValue(status) === 'failed';
}
function isCompatVideoDoneStatus(status) {
    return normalizeCompatVideoStatusValue(status) === 'completed';
}
function isCompatQueuedOrBusyPayload(upstream) {
    const raw = extractVideoTaskStatus(upstream);
    if (isCompatQueuedOrRunningStatus(raw))
        return true;
    return isCompatQueuedOrBusyMessage(extractVideoFailureMessage(upstream));
}
function isCompatQueuedOrRunningStatus(status) {
    const normalized = normalizeCompatVideoStatusValue(status);
...
function isCompatQueuedOrBusyMessage(message) {
...
function findCompatVideoErrorMessage(value, depth) {
    if (!value || depth > 6)
        return '';
...
function formatCompatVideoErrorValue(value) {
    if (value === undefined || value === null || value === false)
        return '';
    if (typeof value === 'string' || typeof value === 'number') {
        const raw = String(value).trim();
        if (!raw || /^(failed|failure|fail|error|cancelled|canceled)$/i.test(raw))
            return '';
...
function isLikelyVideoResultUrl(value) {
    const raw = String(value || '').trim();
    if (!raw)
        return false;
...
function collectGenerationVideoResultUrls(value, urls = []) {
    const push = (item) => {
        const raw = String(item || '').trim();
        if (!raw)
            return;
        if (!/^data:video\//i.test(raw) && !/^https?:\/\//i.test(raw) && !/^\/api\//i.test(raw) && !/^\/v1\/(?:files|videos)\//i.test(raw))
            return;
    ...
    const visit = (item, key = '') => {
        ...
        if (typeof item === 'string') {
            if (/url|uri|output|video|result|content|download/i.test(key) || /^data:video\//i.test(item) || /^https?:\/\//i.test(item) || /^\/api\//i.test(item) || /^\/v1\//i.test(item))
                push(item);
            return;
        }
        ...
        if (typeof item === 'object') {
            ...
            Object.entries(record).forEach(([itemKey, child]) => {
                if (/url|uri|output|video|result|content|download/i.test(itemKey))
                    visit(child, itemKey);
...
function firstGenerationVideoResultUrl(task) {
    const urls = [];
...
function resolveExistingLocalFile(value) {
    const raw = String(value || '').trim();
    if (!raw)
        return '';
    const candidates = [
...
function requireFsStat(filePath) {
    try {
...
function pathFromWorkbenchFileUrl(value) {
    const raw = String(value || '').trim();
    if (!raw)
        return '';
    try {
        ...
        if (url.pathname !== '/api/workbench/image-studio/file')
            return '';
...
function decodeImageDataUrl(value) {
    ...
    if (!match)
        fail(400, '图片数据格式不正确，无法落盘', 'IMAGE_DATA_URL_INVALID');
    const contentType = match[1].toLowerCase();
    const buffer = Buffer.from(match[2].replace(/\s+/g, ''), 'base64');
    if (!buffer.length)
        fail(400, '图片数据为空，无法落盘', 'IMAGE_DATA_EMPTY');
    return { buffer, contentType, ext: extFromContentType(contentType) };
}
async function writeCanvasLocalAsset(buffer, ext = '.png') {
...
function buildLocalImageResponse(filePath, remoteUrl = '', sourceType = 'local_image') {
...
async function downloadImageToBuffer(remoteUrl) {
    const resp = await fetch(remoteUrl, { headers: { Accept: 'image/*,*/*' } });
    if (!resp.ok)
        fail(resp.status >= 500 ? 502 : 400, `在线图片下载失败 HTTP ${resp.status}`, 'REMOTE_IMAGE_DOWNLOAD_FAILED');
    const contentType = String(resp.headers.get('content-type') || '').split(';')[0].trim().toLowerCase();
    if (contentType && !contentType.startsWith('image/'))
        fail(400, `在线地址不是图片：${contentType}`, 'REMOTE_IMAGE_CONTENT_TYPE_INVALID');
    const buffer = Buffer.from(await resp.arrayBuffer());
    if (!buffer.length)
        fail(400, '在线图片内容为空', 'REMOTE_IMAGE_EMPTY');
    return { buffer, contentType, ext: extFromContentType(contentType || 'image/png') };
}
async function sendRemoteImage(res, remoteUrl) {
    ...
    if (buffer.length > maxBytes)
        fail(413, '在线图片过大，无法代理加载全景图', 'REMOTE_IMAGE_TOO_LARGE');
    res.setHeader('Content-Type', contentType || inferLocalContentType(remoteUrl));
    res.setHeader('Content-Length', String(buffer.length));
    res.setHeader('Cache-Control', 'private, max-age=300');
    res.end(buffer);
}
async function sendVideoResultUrl(req, res, resultUrl) {
...
async function sendRemoteVideo(req, res, remoteUrl) {
    const range = String(req.headers.range || '').trim();
    const headers = { Accept: 'video/*,*/*', 'User-Agent': 'Mozilla/5.0 CanvasVideoProxy/1.0' };
    if (range)
        headers.Range = range;
    const controller = new AbortController();
    ...
    if (!resp)
        fail(502, '在线视频代理读取失败', 'REMOTE_VIDEO_PROXY_FETCH_FAILED');
    if (!resp.ok)
        fail(resp.status >= 500 ? 502 : 400, `在线视频代理失败 HTTP ${resp.status}`, 'REMOTE_VIDEO_PROXY_FAILED');
    const contentType = String(resp.headers.get('content-type') || '').split(';')[0].trim().toLowerCase();
    ...
    if (!range && contentLength > maxBytes)
        fail(413, '在线视频过大，无法代理截帧', 'REMOTE_VIDEO_TOO_LARGE');
    ...
    for (const key of ['content-range', 'content-length', 'etag', 'last-modified']) {
        ...
        if (value)
            res.setHeader(key.replace(/(^|-)([a-z])/g, (_, p, c) => p + c.toUpperCase()), value);
    }
    ...
    const buffer = Buffer.from(await resp.arrayBuffer());
    if (!buffer.length)
        fail(400, '在线视频内容为空', 'REMOTE_VIDEO_EMPTY');
    if (!range && buffer.length > maxBytes)
        fail(413, '在线视频过大，无法代理截帧', 'REMOTE_VIDEO_TOO_LARGE');
    if (!res.getHeader('Content-Length'))
        res.setHeader('Content-Length', String(buffer.length));
    res.end(buffer);
}
async function downloadVideoToTempFile(remoteUrl) {
    const controller = new AbortController();
    ...
    try {
        ...
        if (!resp.ok)
            fail(resp.status >= 500 ? 502 : 400, `在线视频下载失败 HTTP ${resp.status}`, 'REMOTE_VIDEO_DOWNLOAD_FAILED');
        const contentType = String(resp.headers.get('content-type') || '').split(';')[0].trim().toLowerCase();
        if (contentType && !contentType.startsWith('video/') && contentType !== 'application/octet-stream') {
            fail(400, `在线地址不是视频：${contentType}`, 'REMOTE_VIDEO_CONTENT_TYPE_INVALID');
        }
        ...
        if (contentLength > maxBytes)
            fail(413, '在线视频过大，无法在线分离音频', 'REMOTE_VIDEO_TOO_LARGE_FOR_AUDIO_EXTRACT');
        const buffer = Buffer.from(await resp.arrayBuffer());
        if (!buffer.length)
            fail(400, '在线视频内容为空', 'REMOTE_VIDEO_EMPTY');
        if (buffer.length > maxBytes)
...
function runFfmpegExtractAudio(input, output) {
    return new Promise((resolve, reject) => {
        ...
        child.on('close', code => {
            if (code === 0)
                ...
                reject(new Error(stderr.trim() || `ffmpeg 分离音频失败，退出码 ${code}`));
        });
    });
}
function runFfmpegExtractFrame(input, output, at, maxWidth = 0, remote = false) {
    return new Promise((resolve, reject) => {
        const ffmpeg = process.env.FFMPEG_BIN || 'ffmpeg';
        ...
        if (maxWidth > 0)
            args.push('-vf', `scale='min(${Math.round(maxWidth)},iw)':-2`);
        ...
        child.on('close', code => {
            if (code === 0)
                ...
                reject(new Error(stderr.trim() || `ffmpeg 截取视频帧失败，退出码 ${code}`));
        });
    });
}
async function extractAudioFromUploadedVideo(buffer, filename, contentType, name) {
    ...
    try {
        ...
        if (!audio.length)
            fail(400, '视频没有可分离的音轨', 'VIDEO_AUDIO_TRACK_EMPTY');
...
async function captureFrameFromRemoteVideo(remoteUrl, at, maxWidth = 0) {
    ...
    try {
        ...
        if (!image.length)
            fail(400, '视频当前帧为空', 'VIDEO_FRAME_EMPTY');
        return {
...
async function extractAudioFromRemoteVideo(remoteUrl, name) {
    let tempDir = '';
    try {
        const downloaded = await downloadVideoToTempFile(remoteUrl);
...
function audioDataUrlToBuffer(dataUrl) {
    ...
    if (!match)
        fail(400, '音频 dataUrl 格式无效', 'AUDIO_DATA_URL_INVALID');
    const contentType = match[1].toLowerCase();
    const buffer = Buffer.from(match[2].replace(/\s+/g, ''), 'base64');
    if (!buffer.length)
        fail(400, '音频 dataUrl 内容为空', 'AUDIO_DATA_URL_EMPTY');
    return { buffer, contentType };
}
async function downloadAudioToTempFile(remoteUrl) {
    const controller = new AbortController();
    ...
    try {
        ...
        if (!resp.ok)
            fail(resp.status >= 500 ? 502 : 400, `在线音频下载失败 HTTP ${resp.status}`, 'REMOTE_AUDIO_DOWNLOAD_FAILED');
        const contentType = String(resp.headers.get('content-type') || '').split(';')[0].trim().toLowerCase();
        ...
        const contentLength = Number(resp.headers.get('content-length') || 0);
        if (contentLength > maxBytes)
            fail(413, '在线音频过大，无法转换为 mp4', 'REMOTE_AUDIO_TOO_LARGE_FOR_MP4');
        const buffer = Buffer.from(await resp.arrayBuffer());
        if (!buffer.length)
            fail(400, '在线音频内容为空', 'REMOTE_AUDIO_EMPTY');
        if (buffer.length > maxBytes)
            fail(413, '在线音频过大，无法转换为 mp4', 'REMOTE_AUDIO_TOO_LARGE_FOR_MP4');
...
async function convertAudioBufferToMp4(buffer, name, contentType = 'audio/mpeg') {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'canvas-audio-to-mp4-'));
    try {
...
async function convertAudioFileToMp4(input, name) {
    ...
    try {
        ...
        if (!video.length)
            fail(400, '音频转 mp4 输出为空', 'AUDIO_TO_MP4_EMPTY');
...
function runFfmpegAudioToMp4(input, output) {
    return new Promise((resolve, reject) => {
        const ffmpeg = process.env.FFMPEG_BIN || 'ffmpeg';
        const args = [
            '-y',
        ...
        child.on('close', code => {
            if (code === 0)
                ...
                reject(new Error(stderr.trim() || `ffmpeg 音频转 mp4 失败，退出码 ${code}`));
        });
    });
}
function inferLocalContentType(filePath) {
    return {
        '.jpg': 'image/jpeg',
        '.jpeg': 'image/jpeg',
        '.png': 'image/png',
...
function inferRemoteVideoContentType(remoteUrl) {
    const pathname = (() => {
        try {
    ...
    return {
        ...
        '.mov': 'video/quicktime',
        '.m4v': 'video/mp4',
    }[path.extname(pathname).toLowerCase()] || 'video/mp4';
}
function inferRemoteAudioContentType(remoteUrl) {
    const pathname = (() => {
        try {
            return new URL(remoteUrl).pathname;
        }
    ...
    return {
        ...
        '.webm': 'audio/webm',
        '.flac': 'audio/flac',
    }[path.extname(pathname).toLowerCase()] || 'audio/mpeg';
}
function audioExtFromContentType(contentType) {
    ...
    }[String(contentType || '').toLowerCase()] || '.mp3';
}
...
function safeLocalExt(ext) {
    const value = String(ext || '').toLowerCase();
    return /^\.[a-z0-9]{1,8}$/.test(value) ? value : '.png';
}
async function recordFailedUsage(userId, model, prompt, requestPayload, err) {
