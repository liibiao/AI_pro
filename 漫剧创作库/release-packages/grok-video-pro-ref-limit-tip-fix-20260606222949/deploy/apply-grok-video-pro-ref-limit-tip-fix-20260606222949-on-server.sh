...
PKG="grok-video-pro-ref-limit-tip-fix-20260606222949"
...
verify_markers(){
  local file="$1"
  grep -Fq "const GROK_VIDEO_REFERENCE_MAX_IMAGES=7;" "$file"
  grep -Fq "const GROK_VIDEO_IMAGE_TO_VIDEO_MAX_IMAGES=1;" "$file"
  grep -Fq "function grokVideoMaxImages" "$file"
  grep -Fq "key.includes('grok-video-pro')" "$file"
  grep -Fq "const refText=isGrok15Preview?'1张起始图'" "$file"
  grep -Fq "const refImageInputMultipleAttr=Number(caps.maxImages||0)>1?' multiple':'';" "$file"
  grep -Fq "Grok-video-pro / 1.5 预览版仅支持图生视频，请先上传 1 张起始图" "$file"
}
verify_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "const GROK_VIDEO_REFERENCE_MAX_IMAGES=7;" "$file"
  run_sudo grep -Fq "const GROK_VIDEO_IMAGE_TO_VIDEO_MAX_IMAGES=1;" "$file"
  run_sudo grep -Fq "function grokVideoMaxImages" "$file"
  run_sudo grep -Fq "key.includes('grok-video-pro')" "$file"
  run_sudo grep -Fq "const refText=isGrok15Preview?'1张起始图'" "$file"
  run_sudo grep -Fq "const refImageInputMultipleAttr=Number(caps.maxImages||0)>1?' multiple':'';" "$file"
  run_sudo grep -Fq "Grok-video-pro / 1.5 预览版仅支持图生视频，请先上传 1 张起始图" "$file"
}
