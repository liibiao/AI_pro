...
PKG="canvas-panorama-image-toolbar-compact-20260607002835"
...
verify_markers(){
  ...
  grep -Fq ".node.node-type-singleImage:has(.image-action-toolbar,.refine-toolbar)" "$file"
  grep -Fq ".node.node-type-imageToPanorama:has(.pano-gen-bar)" "$file"
  ...
  grep -Fq ".node.node-type-singleImage.node-auto-fold:not(.lower-panel-open):not(.lower-panel-pinned) :is(.image-action-toolbar,.refine-toolbar):not(.prompt-zoomed-item)" "$file"
}
verify_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "20260607: compact panorama generation/action toolbars" "$file"
  run_sudo grep -Fq ".node.node-type-singleImage:has(.image-action-toolbar,.refine-toolbar)" "$file"
  run_sudo grep -Fq ".node.node-type-imageToPanorama:has(.pano-gen-bar)" "$file"
  ...
  run_sudo grep -Fq ".node.node-type-singleImage.node-auto-fold:not(.lower-panel-open):not(.lower-panel-pinned) :is(.image-action-toolbar,.refine-toolbar):not(.prompt-zoomed-item)" "$file"
}
