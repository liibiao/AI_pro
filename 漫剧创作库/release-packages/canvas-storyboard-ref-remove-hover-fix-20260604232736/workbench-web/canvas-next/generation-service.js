...
const GENERATION_TASK_SUBMIT_TIMEOUT_MS=600000;
const GENERATION_TASK_RECOVERY_MAX_MS=600000;
...
async function submitBackendGeneration({model,type,mode,prompt,inputFiles=[],params={},onProgress}){
  ...
  try{
    data=await window.CanvasAccountGate.request('/api/generation/tasks',{
      ...
      headers:{'X-Upstream-Timeout-Ms':String(GENERATION_TASK_SUBMIT_TIMEOUT_MS)},
      body:JSON.stringify({
        ...
        clientRequestId,
        timeoutMs:GENERATION_TASK_SUBMIT_TIMEOUT_MS,
        upstreamTimeoutMs:GENERATION_TASK_SUBMIT_TIMEOUT_MS,
        requestTimeoutMs:GENERATION_TASK_SUBMIT_TIMEOUT_MS
      })
    });
  }catch(err){
