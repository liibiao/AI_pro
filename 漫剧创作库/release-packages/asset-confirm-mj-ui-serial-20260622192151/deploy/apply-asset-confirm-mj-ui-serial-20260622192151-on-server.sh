...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/asset-confirm-mj-ui-serial-20260622192151-${STAMP}"
...
check_markers(){
  local file="$1"
  test -f "$file"
  grep -Fq "20260622-asset-confirm-mj-serial" "$file"
  grep -Fq "const ASSET_CONFIRM_BATCH_MJ_MAX_CONCURRENCY=1" "$file"
  grep -Fq "function assetConfirmCardUsesMidjourney" "$file"
  grep -Fq "runWithImmediateFeedback" "$file"
  grep -Fq "sharedPreviewBackgroundStyle" "$file"
  grep -Fq ".asset-confirm-field.asset-confirm-pano-panel .asset-confirm-pano-row.model-row" "$file"
  grep -Fq "class=\"asset-confirm-pano-row model-row\"" "$file"
  grep -Fq ">4格</button>" "$file"
}
...
echo "deployed asset-confirm-mj-ui-serial-20260622192151"
