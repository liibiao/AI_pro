...
PKG="canvas-image-video-asset-sync-fix-20260612021930"
...
verify_canvas_markers(){
  local file="$1"
  grep -Fq "function syncRunningImageNodesFromAssetTasks" "$file"
  grep -Fq "function generatedImageTaskIdentityKeys" "$file"
  grep -Fq "function assetIsImageTask" "$file"
  grep -Fq "const syncedImageNodes=syncRunningImageNodesFromAssetTasks" "$file"
  grep -Fq "n.values.clientRequestId=startTask.clientRequestId" "$file"
  grep -Fq "function syncRunningVideoNodesFromAssetTasks" "$file"
  grep -Fq "function generatedVideoTaskIdentityKeys" "$file"
  grep -Fq "clientRequestId:task.clientRequestId||clientRequestId" "$file"
}
