...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/shot-video-prompt-ui-thread-fix-20260623204156-${STAMP}"
...
check_markers(){
  local file="$1"
  test -f "$file"
  grep -Fq "function makeInternalBackingNode" "$file"
  grep -Fq "function renderShotVideoPromptLightResourceClips" "$file"
  grep -Fq "function scheduleShotVideoPromptInitialHydration" "$file"
  grep -Fq "makeInternalBackingNode('shotStoryboard'" "$file"
  grep -Fq "runShotStoryboardImageGeneration(storyId,tableText,tableText)" "$file"
  grep -Fq "opts.deferRender" "$file"
  grep -Fq "ASSET_CONFIRM_BATCH_MJ_MAX_CONCURRENCY=1" "$file"
  grep -Fq "values._internalBacking" "$file"
  grep -Fq "function openAssetConfirmMidjourneyScopedTarget" "$file"
}
...
echo "deployed shot-video-prompt-ui-thread-fix-20260623204156"
