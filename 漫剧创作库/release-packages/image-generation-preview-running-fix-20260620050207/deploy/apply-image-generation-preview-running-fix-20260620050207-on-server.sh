...
PKG="image-generation-preview-running-fix-20260620050207"
...
verify_canvas_next_html(){
  local file="$1"
  grep -Fq "IMAGE_GENERATION_RUNNING_STATUS_PRESERVE" "$file"
  grep -Fq "IMAGE_PREVIEW_CLEAR_STALE_CONCURRENT_TARGET" "$file"
  grep -Fq "generatedImageResultSnapshot" "$file"
  grep -Fq "const imagePreviewSource=autoPreviewAllowed" "$file"
}
...
if run_sudo test -f "$WORKBENCH_DIR/image-studio-canvas-next.html"; then
  run_sudo grep -Fq "IMAGE_GENERATION_RUNNING_STATUS_PRESERVE" "$WORKBENCH_DIR/image-studio-canvas-next.html"
  run_sudo grep -Fq "generatedImageResultSnapshot" "$WORKBENCH_DIR/image-studio-canvas-next.html"
fi
