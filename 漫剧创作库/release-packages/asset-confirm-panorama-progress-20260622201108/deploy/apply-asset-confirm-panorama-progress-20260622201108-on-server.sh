...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/asset-confirm-panorama-progress-20260622201108-${STAMP}"
...
check_markers(){
  local file="$1"
  test -f "$file"
  grep -Fq "function assetConfirmCardPanoramaStatusInfo" "$file"
  grep -Fq "function updateAssetConfirmPanoramaStatusElement" "$file"
  grep -Fq "data-asset-confirm-pano-status" "$file"
  grep -Fq -- "--pano-progress" "$file"
  grep -Fq "modal.dataset.assetConfirmNodeId=n.id" "$file"
  grep -Fq "panoState.active" "$file"
  grep -Fq "准备生成全景图" "$file"
...
echo "deployed asset-confirm-panorama-progress-20260622201108"
