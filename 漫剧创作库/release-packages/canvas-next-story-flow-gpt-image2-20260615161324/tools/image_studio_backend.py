...
def build_config(payload: dict[str, Any], require_api_key: bool = True) -> ImageStudioConfig:
    ...
    if isinstance(protocol, dict):
        ...
        endpoint_path = str(payload.get("endpointPath") or payload.get("endpoint_path") or payload.get("endpoint") or "").strip()
        status_endpoint_path = str(payload.get("statusEndpointPath") or payload.get("status_endpoint_path") or "").strip()
...
def _preview_response_body(raw: str, limit: int = 800) -> str:
...
def _parse_json_response(raw: str, *, url: str, status: int | None, content_type: str | None, debug_text: str) -> dict[str, Any]:
    if not raw:
        return {}
    try:
        data = json.loads(raw)
        ...
        details = [
            f"模型接口返回的不是有效 JSON：{exc.msg} (line {exc.lineno}, column {exc.colno})",
            f"endpoint: {url}",
        ]
        if status is not None:
            details.append(f"HTTP status: {status}")
        if content_type:
            details.append(f"Content-Type: {content_type}")
        details.append(f"响应片段：{_preview_response_body(raw)}")
        details.append(f"请求调试信息：{debug_text}")
        raise ImageStudioError("\n".join(details)) from exc
    if isinstance(data, dict):
        return data
    raise ImageStudioError(
        "模型接口 JSON 响应格式不符合预期：期望 object。"
        f"\nendpoint: {url}"
        f"\nHTTP status: {status if status is not None else 'unknown'}"
        f"\nContent-Type: {content_type or ''}"
        f"\n响应片段：{_preview_response_body(raw)}"
        f"\n请求调试信息：{debug_text}"
    )
...
def build_aiyunzhi_firefly_gpt_image_payload(payload: dict[str, Any], images: list[str] | None = None) -> tuple[ImageStudioConfig, dict[str, Any]]:
    ...
    model_name = f"{re.sub(r'-(?:1|2|4)k-[0-9]+x[0-9]+$', '', config.model, flags=re.I)}-{image_size.lower()}-{_aspect_ratio_slug(aspect_ratio)}"
...
def _normalize_aiyunzhi_firefly_response(raw_text: str) -> dict[str, Any]:
    ...
    try:
        ...
        image_url = _extract_image_url_from_text(json.dumps(payload, ensure_ascii=False))
        if image_url:
    ...
    image_url = _extract_image_url_from_text("\n".join(chunks) or text)
    if image_url:
...
def _request_image_generation_once(config: ImageStudioConfig, request_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if config.adapter == "midjourney-imagine":
        ...
        attempts = [request_payload, *_midjourney_niji_retry_payloads(request_payload)]
        for index, attempt_payload in enumerate(attempts):
            try:
                ...
                if index < len(attempts) - 1 and _midjourney_is_prompt_format_error(exc):
                    print(f"[Midjourney] Niji prompt format fallback {index + 1}/{len(attempts) - 1}: {exc}", flush=True)
                    continue
                raise
...
def generate_panorama(payload: dict[str, Any], reference_images: list[str], output_dir: Path) -> dict[str, Any]:
    ...
    config, request_payload = build_image_payload(payload, images=reference_values)
    prompt = str(request_payload.get("prompt") or "").strip()
    if config.adapter not in {"gpt-image-v2", "gemini-image", "midjourney-imagine", "aiyunzhi-firefly-gpt-image", "aiyunzhi-gpt-image-2", "aiyunzhi-gemini-image"}:
