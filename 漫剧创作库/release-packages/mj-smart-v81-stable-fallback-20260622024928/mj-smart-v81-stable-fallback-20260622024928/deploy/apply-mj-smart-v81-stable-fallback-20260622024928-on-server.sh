...
PKG="mj-smart-v81-stable-fallback-20260622024928"
...
verify_html_markers(){
  local file="$1"
  grep -Fq "v.mjHd=false" "$file"
  grep -Fq "LLM 智能提示词通道暂不可用" "$file"
  grep -Fq "V8.1 智能模式不输出 --q、--hd" "$file"
  grep -Fq "function midjourneyActiveDownloadEntryForNode" "$file"
}

verify_server_markers(){
  local file="$1"
  grep -Fq "def _midjourney_version_key_from_params" "$file"
  grep -Fq "def _default_midjourney_params_for_model" "$file"
  grep -Fq "GPT-5.5 smart prompt service unavailable" "$file"
  grep -Fq "used conservative Midjourney {version_label} fallback" "$file"
}

verify_image_backend_markers(){
  local file="$1"
  grep -Fq "def _midjourney_stable_submit_retry_payloads" "$file"
  grep -Fq "no hd fallback" "$file"
  grep -Fq "quality q2 without accountFilter fallback" "$file"
}

verify_skill_markers(){
  local file="$1"
  grep -Fq '"params": "--ar 16:9 --v 8.1 --raw --s 100 --c 6"' "$file"
  grep -Fq '智能模式不输出 `--q --hd' "$file"
}

verify_html_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "v.mjHd=false" "$file"
  run_sudo grep -Fq "LLM 智能提示词通道暂不可用" "$file"
  run_sudo grep -Fq "V8.1 智能模式不输出 --q、--hd" "$file"
  run_sudo grep -Fq "function midjourneyActiveDownloadEntryForNode" "$file"
}

verify_server_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "def _midjourney_version_key_from_params" "$file"
  run_sudo grep -Fq "def _default_midjourney_params_for_model" "$file"
  run_sudo grep -Fq "GPT-5.5 smart prompt service unavailable" "$file"
  run_sudo grep -Fq "used conservative Midjourney {version_label} fallback" "$file"
}

verify_image_backend_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "def _midjourney_stable_submit_retry_payloads" "$file"
  run_sudo grep -Fq "no hd fallback" "$file"
  run_sudo grep -Fq "quality q2 without accountFilter fallback" "$file"
}

verify_skill_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq '"params": "--ar 16:9 --v 8.1 --raw --s 100 --c 6"' "$file"
  run_sudo grep -Fq '智能模式不输出 `--q --hd' "$file"
}
...
install_if_parent_exists "$SRC/tools/workbench_server.py" "$MIRROR_ROOT/tools/workbench_server.py" "mirror workbench server"
...
install_if_parent_exists "$SRC/smart-vision/services/workbench/workbench_server.py" "$MIRROR_ROOT/smart-vision/services/workbench/workbench_server.py" "smart-vision workbench server"
...
install_if_parent_exists "$SRC/skills/midjourney-prompt-skill.md" "$MIRROR_ROOT/skills/midjourney-prompt-skill.md" "midjourney prompt skill"
...
install_if_parent_exists "$SRC/tools/workbench_server.py" "$RUNTIME_ROOT/tools/workbench_server.py" "runtime workbench server"
...
install_if_parent_exists "$SRC/smart-vision/services/workbench/workbench_server.py" "$RUNTIME_ROOT/smart-vision/services/workbench/workbench_server.py" "runtime smart-vision workbench server"
...
install_if_parent_exists "$SRC/skills/midjourney-prompt-skill.md" "$RUNTIME_ROOT/skills/midjourney-prompt-skill.md" "runtime midjourney prompt skill"
...
if run_sudo test -f "$MIRROR_ROOT/tools/workbench_server.py"; then
  ...
  compile_python_if_present "$MIRROR_ROOT/tools/workbench_server.py" "mirror workbench server"
fi
if run_sudo test -f "$MIRROR_ROOT/tools/image_studio_backend.py"; then
  verify_image_backend_markers_sudo "$MIRROR_ROOT/tools/image_studio_backend.py"
...
if run_sudo test -f "$MIRROR_ROOT/smart-vision/services/workbench/workbench_server.py"; then
  ...
  compile_python_if_present "$MIRROR_ROOT/smart-vision/services/workbench/workbench_server.py" "smart-vision workbench server"
fi
if run_sudo test -f "$MIRROR_ROOT/smart-vision/services/workbench/image_studio_backend.py"; then
  verify_image_backend_markers_sudo "$MIRROR_ROOT/smart-vision/services/workbench/image_studio_backend.py"
