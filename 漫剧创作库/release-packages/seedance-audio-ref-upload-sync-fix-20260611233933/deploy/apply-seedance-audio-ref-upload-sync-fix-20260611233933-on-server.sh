...
PKG="seedance-audio-ref-upload-sync-fix-20260611233933"
...
verify_html_markers(){
  local file="$1"
  grep -Fq 'function connectedMediaSyncSignature(entry,kind=' "$file"
  grep -Fq 'const nextSignature=connectedMediaSyncSignature(normalized,kind,sourceKey);' "$file"
  grep -Fq 'const curSignature=connectedMediaSyncSignature(existing,kind,existing?.[sourceProp]||sourceKey);' "$file"
  grep -Fq 'curSignature!==nextSignature' "$file"
  grep -Fq 'refreshDownstreamAfterConnectionChange(downstreamNodeIds(id));toast' "$file"
}
verify_html_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq 'function connectedMediaSyncSignature(entry,kind=' "$file"
  run_sudo grep -Fq 'const nextSignature=connectedMediaSyncSignature(normalized,kind,sourceKey);' "$file"
  run_sudo grep -Fq 'const curSignature=connectedMediaSyncSignature(existing,kind,existing?.[sourceProp]||sourceKey);' "$file"
  run_sudo grep -Fq 'curSignature!==nextSignature' "$file"
  run_sudo grep -Fq 'refreshDownstreamAfterConnectionChange(downstreamNodeIds(id));toast' "$file"
}
