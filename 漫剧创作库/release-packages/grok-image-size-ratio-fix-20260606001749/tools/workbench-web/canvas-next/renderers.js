...
function renderModeControls(node,mode,values,model){
  ...
  if(isGrokImageModel(model)){
    ...
    const geometryControls=`${selectPill('▭','aspectRatio',['1:1','16:9','9:16','3:2','2:3'],ratio,node,mode)}
      ${selectPill('1K','resolution',['1K'],resolution,node,mode)}`;
    if(mode==='storyboard'){
...
function isGrokImageModel(model={}){
  ...
  return /grok[-_\s]?image|grok[-_\s]?imagine|cliproxy_xai_media|xai/.test(text);
}
