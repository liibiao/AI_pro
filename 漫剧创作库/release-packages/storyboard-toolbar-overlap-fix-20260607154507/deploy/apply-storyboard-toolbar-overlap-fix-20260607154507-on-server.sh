...
PKG="storyboard-toolbar-overlap-fix-20260607154507"
...
verify_markers(){
  local file="$1"
  grep -Fq "storyboard toolbar no-overlap final pass" "$file"
  grep -Fq -- "--storyboard-toolbar-right-reserve:146px" "$file"
  grep -Fq "select[data-field=\"_modelIdx\"] + .tn-custom-control" "$file"
  grep -Fq "node?.classList?.contains('node-type-storyboard')&&bar?.matches?.('.aio-bar'))return 1;" "$file"
  grep -Fq "auth-cursor-field" "$file"
  ...
  if grep -Fq "syncStageCursorFieldActive" "$file"; then fail "stage cursor init hook still exists in $file"; fi
}
verify_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "storyboard toolbar no-overlap final pass" "$file"
  run_sudo grep -Fq -- "--storyboard-toolbar-right-reserve:146px" "$file"
  run_sudo grep -Fq "select[data-field=\"_modelIdx\"] + .tn-custom-control" "$file"
  run_sudo grep -Fq "node?.classList?.contains('node-type-storyboard')&&bar?.matches?.('.aio-bar'))return 1;" "$file"
  run_sudo grep -Fq "auth-cursor-field" "$file"
  run_sudo grep -Fq "settingsCursorEffectToggle" "$file"
  run_sudo grep -Fq "登录页" "$file"
  ...
  if run_sudo grep -Fq "syncStageCursorFieldActive" "$file"; then fail "stage cursor init hook still exists in $file"; fi
}
