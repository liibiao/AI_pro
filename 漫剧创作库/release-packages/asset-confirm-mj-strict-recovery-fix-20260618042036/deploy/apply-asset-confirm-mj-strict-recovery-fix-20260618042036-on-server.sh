...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/asset-confirm-mj-strict-recovery-fix-20260618042036-${STAMP}"
...
check_markers(){
  local file="$1"
  test -f "$file"
  grep -q "function continueAssetConfirmSubmittedGeneration" "$file"
  grep -q "assetConfirmTaskStrictlyMatchesCard" "$file"
  grep -q "assetConfirmTaskCanRecoverCard" "$file"
  grep -q "skipAssetConfirmSync" "$file"
  grep -q "assetConfirmMjSubmitGapMs" "$file"
  grep -q "refreshAssetLibrary:false" "$file"
  grep -q "isGenerationFailureStatus(generationTaskStatus(task))" "$file"
  if grep -q "mjConcurrency=Math.min(4" "$file"; then
    echo "old burst MJ submit queue is still present in $file" >&2
    exit 1
  fi
}
...
echo "deployed asset-confirm-mj-strict-recovery-fix-20260618042036"
