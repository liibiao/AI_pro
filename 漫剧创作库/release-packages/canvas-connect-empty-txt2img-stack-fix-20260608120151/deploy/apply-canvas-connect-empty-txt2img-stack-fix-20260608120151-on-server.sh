...
PKG="canvas-connect-empty-txt2img-stack-fix-20260608120151"
...
verify_html_markers(){
  ...
  grep -Fq "function _doSyncUpstreamImages(n,visited=null)" "$file"
  grep -Fq "if(!srcNode.data)computeNode(c.from,visited);" "$file"
  grep -Fq "const hadSyncAio=syncUpstreamImagesToAio(n,visited);" "$file"
  grep -Fq "const hadSyncStoryboard=syncUpstreamImagesToAio(n,visited);" "$file"
  grep -Fq "out.push({...entry,_sourceNodeId:src.id,_sourceLabel:sourceNodeLabel(src),_sourceIndex:idx});" "$file"
...
verify_html_markers_sudo(){
  ...
  run_sudo grep -Fq "function _doSyncUpstreamImages(n,visited=null)" "$file"
  run_sudo grep -Fq "if(!srcNode.data)computeNode(c.from,visited);" "$file"
  run_sudo grep -Fq "const hadSyncAio=syncUpstreamImagesToAio(n,visited);" "$file"
  run_sudo grep -Fq "const hadSyncStoryboard=syncUpstreamImagesToAio(n,visited);" "$file"
  run_sudo grep -Fq "out.push({...entry,_sourceNodeId:src.id,_sourceLabel:sourceNodeLabel(src),_sourceIndex:idx});" "$file"
