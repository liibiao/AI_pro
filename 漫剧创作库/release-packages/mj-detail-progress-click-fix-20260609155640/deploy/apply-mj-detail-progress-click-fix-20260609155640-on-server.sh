...
STAMP="20260609155640"
PKG="mj-detail-progress-click-fix-${STAMP}"
TMP="/tmp/${PKG}-extract"
...
for marker in \
  "function closeMidjourneyDetailViewer(opts={})" \
  "closeMidjourneyDetailViewer({animate:true})" \
  "const mjOpenTargetFromEvent=e=>" \
  ".mj-detail-modal.closing .mj-detail-panel" \
  "width:var(--gen-progress,0%)!important" \
  "openMidjourneyDetailViewer(n,Number(target.dataset.mjCell)||1,target)"; do
