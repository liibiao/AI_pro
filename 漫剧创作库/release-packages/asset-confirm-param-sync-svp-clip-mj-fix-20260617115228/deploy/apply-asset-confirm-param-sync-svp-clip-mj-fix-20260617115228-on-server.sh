...
BACKUP_ROOT="${APP_ROOT}/.deploy-backups/asset-confirm-param-sync-svp-clip-mj-fix-20260617115228-${STAMP}"
TMP_DIR="$(mktemp -d)"
...
check_file(){
  local file="$1"
  test -s "${file}"
  must_have "${file}" "function invalidateAssetConfirmCardsCache"
  must_have "${file}" "资产参数异常：当前卡片是"
  must_have "${file}" "requestedCardId:card.id"
  must_have "${file}" "const assetNodeIds=[]"
  must_have "${file}" "assetConfirmPanoramaImageModelCandidates"
  must_have "${file}" "assetConfirmPanoramaMidjourneyVersionOptionsHtml"
  must_have "${file}" "当前生图参数"
  must_have "${file}" "width:max-content!important"
  must_have "${file}" "svp-config-dock"
}
...
echo "deployed=asset-confirm-param-sync-svp-clip-mj-fix-20260617115228"
