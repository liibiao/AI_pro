...
PKG="canvas-toolbar-gap8-20260606235957"
...
verify_html(){
  local file="$1"
  grep -Fq "tapnow-rewrite.css?v=20260606-toolbar-center-gap8" "$file"
  grep -Fq "generation node bottom bars are centered as row boxes" "$file"
  grep -Fq "parentLocalWidth" "$file"
  grep -Fq "currentShift+delta" "$file"
  grep -Fq "gap:8px!important" "$file"
...
verify_html_sudo(){
  local file="$1"
  run_sudo grep -Fq "tapnow-rewrite.css?v=20260606-toolbar-center-gap8" "$file"
  run_sudo grep -Fq "generation node bottom bars are centered as row boxes" "$file"
  run_sudo grep -Fq "parentLocalWidth" "$file"
  run_sudo grep -Fq "currentShift+delta" "$file"
  run_sudo grep -Fq "gap:8px!important" "$file"
