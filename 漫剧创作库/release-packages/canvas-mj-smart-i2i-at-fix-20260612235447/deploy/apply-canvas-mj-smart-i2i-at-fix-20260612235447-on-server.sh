...
PKG="canvas-mj-smart-i2i-at-fix-20260612235447"
...
verify_canvas_markers(){
  local file="$1"
  grep -Fq "function smartMidjourneyNaturalLanguageSource(n)" "$file"
  grep -Fq "promptTranslateI2iStateText(n)||String(n.values?.story||'')" "$file"
  grep -Fq "const source=smartMidjourneyNaturalLanguageSource(n);" "$file"
  grep -Fq "const mjPromptNoAt=false;" "$file"
  grep -Fq "输入 @ 引用参考图，或描述主体 / 场景 / 风格" "$file"
}
