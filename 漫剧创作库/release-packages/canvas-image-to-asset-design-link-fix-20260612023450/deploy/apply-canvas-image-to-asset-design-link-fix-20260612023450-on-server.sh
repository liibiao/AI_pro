...
PKG="canvas-image-to-asset-design-link-fix-20260612023450"
...
verify_canvas_markers(){
  local file="$1"
  grep -Fq "const choices=['assetDesign','imageToPanorama','img2imgAll','storyboard','seedanceVideo','singleImage']" "$file"
  grep -Fq "if(first.kind==='image')return showImageDownstreamPopup(id,first,opts)" "$file"
  grep -Fq "label:'资产设计',desc:\`使用 \${imageCount} 张图片作为资产参考\`" "$file"
  grep -Fq "assetDesign:{cat:'generate',name:'资产设计'" "$file"
}
