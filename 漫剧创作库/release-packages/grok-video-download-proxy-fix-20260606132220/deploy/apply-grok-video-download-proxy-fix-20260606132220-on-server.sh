...
PKG="grok-video-download-proxy-fix-20260606132220"
...
verify_markers(){
  local file="$1"
  grep -Fq "function videoDownloadFetchUrl(src)" "$file"
  grep -Fq "function downloadMediaFetchUrl(src,kind='image')" "$file"
  grep -Fq "const kind=opts.kind||'image';" "$file"
  grep -Fq "const fetchUrl=downloadMediaFetchUrl(rawUrl,kind);" "$file"
  grep -Fq "if(kind!=='video'){" "$file"
}
verify_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "function videoDownloadFetchUrl(src)" "$file"
  run_sudo grep -Fq "function downloadMediaFetchUrl(src,kind='image')" "$file"
  run_sudo grep -Fq "const kind=opts.kind||'image';" "$file"
  run_sudo grep -Fq "const fetchUrl=downloadMediaFetchUrl(rawUrl,kind);" "$file"
  run_sudo grep -Fq "if(kind!=='video'){" "$file"
}
