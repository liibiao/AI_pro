...
PKG="gpt55-llm-timeout-responses-20260615203435"
...
verify_html_markers(){
  local file="$1"
  grep -Fq "TEXT_PROMPT_LLM_CHAT_TIMEOUT_MS" "$file"
  grep -Fq "generationTaskTimeoutHeaders(TEXT_PROMPT_LLM_CHAT_TIMEOUT_MS)" "$file"
  grep -Fq "withLlmChatTimeout({modelId,messages" "$file"
}

verify_html_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "TEXT_PROMPT_LLM_CHAT_TIMEOUT_MS" "$file"
  run_sudo grep -Fq "generationTaskTimeoutHeaders(TEXT_PROMPT_LLM_CHAT_TIMEOUT_MS)" "$file"
  run_sudo grep -Fq "withLlmChatTimeout({modelId,messages" "$file"
}
...
[ -f "$SRC/api-server/scripts/apply-gpt55-llm-timeout-responses.mjs" ] || fail "package missing api patch script"
grep -Fq "resolveTextAgentTimeoutMs" "$SRC/api-server/scripts/apply-gpt55-llm-timeout-responses.mjs"
grep -Fq "resolveLlmRequestTimeoutMs" "$SRC/api-server/scripts/apply-gpt55-llm-timeout-responses.mjs"
...
run_sudo cp -p "$API_DIR/dist/modules/generate/routes.js" "$BACKUP_DIR/generate-routes.js.prev" 2>/dev/null || true
...
run_sudo cp -p "$SRC/api-server/scripts/apply-gpt55-llm-timeout-responses.mjs" "$API_DIR/scripts/apply-gpt55-llm-timeout-responses.mjs"
run_sudo chmod 644 "$API_DIR/scripts/apply-gpt55-llm-timeout-responses.mjs"

log "patch api timeout and responses routes"
(cd "$API_DIR" && node scripts/apply-gpt55-llm-timeout-responses.mjs)
...
run_sudo grep -Fq "function resolveLlmRequestTimeoutMs" "$API_DIR/dist/modules/generate/routes.js"
run_sudo grep -Fq "resolveLlmRequestTimeoutMs(body, runtime.upstreamTimeoutMs)" "$API_DIR/dist/modules/generate/routes.js"
run_sudo grep -Fq "function resolveTextAgentTimeoutMs" "$API_DIR/dist/modules/workbench/text-agent-routes.js"
run_sudo grep -Fq "buildResponsesTextAgentPayload" "$API_DIR/dist/modules/workbench/text-agent-routes.js"
