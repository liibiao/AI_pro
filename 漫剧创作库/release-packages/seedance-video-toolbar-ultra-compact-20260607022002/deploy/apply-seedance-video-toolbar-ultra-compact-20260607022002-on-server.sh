...
PKG="seedance-video-toolbar-ultra-compact-20260607022002"
...
verify_markers(){
  local file="$1"
  grep -Fq "20260607: keep video generation bottom toolbar ultra compact and separated from the prompt panel." "$file"
  grep -Fq ".node.node-type-seedanceVideo .vn2-layout>.vn2-bar:not(.prompt-zoomed-item){" "$file"
  grep -Fq "height:30px!important;" "$file"
  grep -Fq "gap:1px!important;" "$file"
  grep -Fq "height:18px!important;" "$file"
  grep -Fq "width:90px!important;" "$file"
  grep -Fq "width:34px!important;" "$file"
  grep -Fq "width:24px!important;" "$file"
  grep -Fq "transform:translateX(8px)!important;" "$file"
}
verify_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "20260607: keep video generation bottom toolbar ultra compact and separated from the prompt panel." "$file"
  run_sudo grep -Fq ".node.node-type-seedanceVideo .vn2-layout>.vn2-bar:not(.prompt-zoomed-item){" "$file"
  run_sudo grep -Fq "height:30px!important;" "$file"
  run_sudo grep -Fq "gap:1px!important;" "$file"
  run_sudo grep -Fq "height:18px!important;" "$file"
  run_sudo grep -Fq "width:90px!important;" "$file"
  run_sudo grep -Fq "width:34px!important;" "$file"
  run_sudo grep -Fq "width:24px!important;" "$file"
