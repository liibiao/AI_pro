...
PKG="canvas-login-fast-20260612213100"
...
verify_html(){
  local file="$1"
  grep -Fq "localStorage.getItem('canvas_platform_token')" "$file"
  grep -Fq "hideAuthGate();" "$file"
  grep -Fq "refreshSummary().catch(()=>{setToken('');showAuthGate('login');});" "$file"
  grep -Fq "Promise.allSettled([loadDataPacks(),loadCanvasNodeConfig()])" "$file"
}
verify_html_sudo(){
  local file="$1"
  run_sudo grep -Fq "localStorage.getItem('canvas_platform_token')" "$file"
  run_sudo grep -Fq "hideAuthGate();" "$file"
  run_sudo grep -Fq "refreshSummary().catch(()=>{setToken('');showAuthGate('login');});" "$file"
  run_sudo grep -Fq "Promise.allSettled([loadDataPacks(),loadCanvasNodeConfig()])" "$file"
}
