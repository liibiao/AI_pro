...
PKG="canvas-focus-connection-geometry-fix-20260603215901"
...
verify_markers(){
  local file="$1"
  grep -Fq "function nodeSurfaceLayoutWorldRect" "$file"
  grep -Fq "const layoutRect=nodeSurfaceLayoutWorldRect(nid,rectCache)" "$file"
  grep -Fq "const stable=nodeSurfaceLayoutWorldRect(nid,rectCache)" "$file"
  grep -Fq "drawConns(false,id);" "$file"
  grep -Fq "flushViewportAux();" "$file"
}
verify_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "function nodeSurfaceLayoutWorldRect" "$file"
  run_sudo grep -Fq "const layoutRect=nodeSurfaceLayoutWorldRect(nid,rectCache)" "$file"
  run_sudo grep -Fq "const stable=nodeSurfaceLayoutWorldRect(nid,rectCache)" "$file"
  run_sudo grep -Fq "drawConns(false,id);" "$file"
  run_sudo grep -Fq "flushViewportAux();" "$file"
}
