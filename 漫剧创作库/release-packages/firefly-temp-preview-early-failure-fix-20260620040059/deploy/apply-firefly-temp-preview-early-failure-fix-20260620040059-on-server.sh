...
ARCHIVE="${1:-}"
PKG="firefly-temp-preview-early-failure-fix-20260620040059"
...
verify_file() {
  ...
  grep -Fq "function taskFireflyTemporaryPreviewUrls(task)" "$file"
  grep -Fq "function taskHasFireflyTemporaryPreviewUrl(task)" "$file"
  grep -Fq "taskImagePreviewUrlBundle(data.task).urls[0]" "$file"
  grep -Fq "fireflyTemporaryUrls.length>0" "$file"
}
