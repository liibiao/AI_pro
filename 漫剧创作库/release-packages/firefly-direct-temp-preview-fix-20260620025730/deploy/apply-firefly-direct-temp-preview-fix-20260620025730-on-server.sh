...
ARCHIVE="${1:-}"
PKG="firefly-direct-temp-preview-fix-20260620025730"
...
verify_file() {
  ...
  grep -Fq "if(isFireflyPresignedImageUrl(raw))return false;" "$file"
  grep -Fq "function imgFallbackProxy(src,maxW=COS_PREVIEW_THUMB_MAX,maxH=COS_PREVIEW_THUMB_MAX)" "$file"
  grep -Fq "return remoteImageProxyUrl(src,maxW,maxH);" "$file"
