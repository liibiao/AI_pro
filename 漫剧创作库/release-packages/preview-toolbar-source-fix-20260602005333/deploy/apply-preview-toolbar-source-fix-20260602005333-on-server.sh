...
PKG="preview-toolbar-source-fix-20260602005333"
...
verify_markers(){
  ...
  grep -Fq "if(sourceEl?.closest?.('.node-tools'))return null;" "$dest"
  grep -Fq "openPreview(id,actionPreviewSource(id,sourceEl))" "$dest"
