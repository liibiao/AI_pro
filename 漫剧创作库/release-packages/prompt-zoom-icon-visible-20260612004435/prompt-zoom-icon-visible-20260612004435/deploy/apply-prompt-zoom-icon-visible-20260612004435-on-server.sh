...
PKG="prompt-zoom-icon-visible-20260612004435"
...
verify_canvas_next(){
  local file="$1"
  grep -Fq "prompt-zoom-btn{position:absolute!important;right:10px" "$file"
  grep -Fq "bottom:10px!important" "$file"
  grep -Fq "background:transparent!important" "$file"
  grep -Fq "font-size:20px!important" "$file"
  grep -Fq "btn.textContent='⛶'" "$file"
}
...
if [ -f "$WORKBENCH_DIR/image-studio-canvas-next.html" ]; then
  run_sudo grep -Fq "font-size:20px!important" "$WORKBENCH_DIR/image-studio-canvas-next.html"
fi
if [ -f "$MIRROR_TARGET/tools/workbench-web/image-studio-canvas-next.html" ]; then
  run_sudo grep -Fq "font-size:20px!important" "$MIRROR_TARGET/tools/workbench-web/image-studio-canvas-next.html"
fi
log "done"
