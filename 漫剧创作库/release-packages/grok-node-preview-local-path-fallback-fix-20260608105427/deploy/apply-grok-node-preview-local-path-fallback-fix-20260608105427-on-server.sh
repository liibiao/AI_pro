...
PKG="grok-node-preview-local-path-fallback-fix-20260608105427"
...
verify_html_markers(){
  ...
  grep -Fq "function imageEntryUrl(entry){return firstDisplayImageUrl(entry?.objectStorageUrl,entry?.saved?.objectStorageUrl,...generatedAssetPreviewUrls(entry)," "$file"
  ...
  grep -Fq "function buildImagePreviewDebug(n)" "$file"
  grep -Fq "display.canvasPreviewDebug=buildImagePreviewDebug(n);" "$file"
  grep -Fq "resolvedLocalPreviewUrl:resolveLocalPreviewUrl(saved)" "$file"
  ...
  if grep -Fq "function imagePreviewUrl(entry){return firstDisplayImageUrl(entry?.objectStorageUrl,entry?.saved?.objectStorageUrl,entry?.localUrl,resolveLocalPreviewUrl" "$file"; then
    fail "local file preview still has priority over generated asset previews in $file"
  fi
}
verify_html_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "function generatedAssetPreviewUrls(entry)" "$file"
  run_sudo grep -Fq "function imageEntryUrl(entry){return firstDisplayImageUrl(entry?.objectStorageUrl,entry?.saved?.objectStorageUrl,...generatedAssetPreviewUrls(entry)," "$file"
  ...
  run_sudo grep -Fq "function buildImagePreviewDebug(n)" "$file"
  run_sudo grep -Fq "display.canvasPreviewDebug=buildImagePreviewDebug(n);" "$file"
  run_sudo grep -Fq "resolvedLocalPreviewUrl:resolveLocalPreviewUrl(saved)" "$file"
  ...
  if run_sudo grep -Fq "function imagePreviewUrl(entry){return firstDisplayImageUrl(entry?.objectStorageUrl,entry?.saved?.objectStorageUrl,entry?.localUrl,resolveLocalPreviewUrl" "$file"; then
    fail "local file preview still has priority over generated asset previews in $file"
  fi
}
