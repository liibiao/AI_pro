...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/asset-confirm-async-svp-conn-stable-20260622202643-${STAMP}"
...
check_markers(){
  ...
  grep -Fq "async function buildShotVideoPromptSegmentsAsync" "$file"
  grep -Fq "function rebuildShotVideoPromptSegmentsWithFeedback" "$file"
  grep -Fq "function shotVideoPromptLightweightResourceEntry" "$file"
  grep -Fq "function shouldUseStableConnectionRect" "$file"
  grep -Fq "asset-confirm-create-status" "$file"
  grep -Fq "svp-build-loading" "$file"
  grep -Fq "const ASSET_CONFIRM_BATCH_MJ_MAX_CONCURRENCY=1" "$file"
  grep -Fq "function updateAssetConfirmPanoramaStatusElement" "$file"
}
...
echo "deployed asset-confirm-async-svp-conn-stable-20260622202643"
