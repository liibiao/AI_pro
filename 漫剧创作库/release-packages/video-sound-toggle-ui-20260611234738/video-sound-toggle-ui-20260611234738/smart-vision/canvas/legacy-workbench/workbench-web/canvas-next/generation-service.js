...
function assembleGptImageV2ModelName(baseModel,resolution){
  ...
  const res=normalizeGptImageV2Resolution(resolution).toLowerCase();
  return `${base}-${res}`;
}
...
export async function runPanorama(node,inputs,models,{onProgress}={}){
  ...
  onProgress?.('提交全景图任务');
  const resolution=normalizeGptImageV2Resolution(node.values.resolution||'2K');
  ...
  if(modelHasBackendGateway(model)){
    const payload={mode:'panorama',prompt,size:node.values.size||'2048x1024',resolution,requestedResolution:resolution,model:requestModel,image:refs,images:refs,outputDir:OUTPUT_DIR};
  ...
  const data=await E.generateImageStandalone({
    ...
    model:requestModel,size:node.values.size||'2048x1024',resolution,requestedResolution:resolution,image:refs,images:refs,background:'auto',outputDir:OUTPUT_DIR
  });
...
async function runImage(node,inputs,models,{onProgress}={}){
  ...
  const resolution=normalizeGptImageV2Resolution(values.resolution||values.requestedResolution||model.defaults?.resolution||'1K');
  const requestModel=isGptImageV2Channel(model,modelInfo.protocol?.adapter)
    ? assembleGptImageV2ModelName(modelInfo.model,resolution)
    : modelInfo.model;
  onProgress?.('提交生图任务');
