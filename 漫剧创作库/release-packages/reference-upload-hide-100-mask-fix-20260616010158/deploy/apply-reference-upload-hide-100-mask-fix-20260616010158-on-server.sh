...
if [ -z "$ARCHIVE" ] || [ ! -f "$ARCHIVE" ]; then
  echo "usage: $0 /tmp/reference-upload-hide-100-mask-fix-20260616010158.tar.gz" >&2
  exit 2
fi

PKG_NAME="reference-upload-hide-100-mask-fix-20260616010158"
...
grep -Fq "return Math.round(referenceUploadProgressPercent(img))<100;" "$HTML_SRC"
grep -Fq "if(!shouldShowReferenceUploadMask(img))return '';" "$HTML_SRC"
grep -Fq "const active=shouldShowReferenceUploadMask(img);" "$HTML_SRC"
grep -Fq "const img=imgs.find(item=>shouldShowReferenceUploadMask(item))||null;" "$HTML_SRC"
...
for target in \
  ...
  grep -Fq "return Math.round(referenceUploadProgressPercent(img))<100;" "$target"
  grep -Fq "if(!shouldShowReferenceUploadMask(img))return '';" "$target"
  grep -Fq "const active=shouldShowReferenceUploadMask(img);" "$target"
done
