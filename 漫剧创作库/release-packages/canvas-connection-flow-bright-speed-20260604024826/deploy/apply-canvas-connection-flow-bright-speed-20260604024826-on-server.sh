...
PKG="canvas-connection-flow-bright-speed-20260604024826"
...
verify_markers(){
  local dest="$1"
  grep -Fq "conn-flow-pill{display:block!important;fill:url(#connMinimalFlowGradient)!important" "$dest"
  grep -Fq "animation:connMinimalPillOpacity 2.58s ease-in-out infinite!important" "$dest"
  grep -Fq "animation-duration:2.8s!important" "$dest"
  grep -Fq "motion.setAttribute('dur','2.58s')" "$dest"
  grep -Fq "fill=\"#aab8de\"" "$dest"
  grep -Fq "prompt:'#b4a8ff'" "$dest"
  grep -Fq "stop.setAttribute('stop-color','#f8fbff')" "$dest"
  grep -Fq "const sleep=ms=>new Promise(resolve=>setTimeout(resolve,Math.max(0,Number(ms)||0)));" "$dest"
  grep -Fq "function storyboardImageCanonicalCellRatio" "$dest"
  grep -Fq "data-shot-drop" "$dest"
}
