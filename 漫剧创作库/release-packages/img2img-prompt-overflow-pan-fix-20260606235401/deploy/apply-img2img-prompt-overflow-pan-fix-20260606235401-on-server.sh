...
PKG="img2img-prompt-overflow-pan-fix-20260606235401"
...
verify_markers(){
  local file="$1"
  grep -Fq "平移画布时外层可见，但真正的提示词框必须继续内部滚动" "$file"
  grep -Fq "图生图节点同样要在平移时保持提示词框裁剪" "$file"
  grep -Fq ".stage.panning .node.node-type-storyboard:has(.storyboard-rich-box) .storyboard-rich-box" "$file"
  grep -Fq ".stage.panning .node.node-type-img2imgAll:has(.i2i-input-box) .i2i-input-box:has([data-i2i-input])" "$file"
  grep -Fq "contain:paint!important;" "$file"
  grep -Fq ".i2i-input-box:has([data-i2i-input])>.i2i-text-input" "$file"
  grep -Fq "overflow-wrap:anywhere!important;" "$file"
}
verify_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "平移画布时外层可见，但真正的提示词框必须继续内部滚动" "$file"
  run_sudo grep -Fq "图生图节点同样要在平移时保持提示词框裁剪" "$file"
  run_sudo grep -Fq ".stage.panning .node.node-type-storyboard:has(.storyboard-rich-box) .storyboard-rich-box" "$file"
  run_sudo grep -Fq ".stage.panning .node.node-type-img2imgAll:has(.i2i-input-box) .i2i-input-box:has([data-i2i-input])" "$file"
  run_sudo grep -Fq "contain:paint!important;" "$file"
  run_sudo grep -Fq ".i2i-input-box:has([data-i2i-input])>.i2i-text-input" "$file"
  run_sudo grep -Fq "overflow-wrap:anywhere!important;" "$file"
}
