...
PKG="storyboard-english-toggle-align-fix-20260606140644"
...
verify_markers(){
  local file="$1"
  grep -Fq "concurrent-toggle storyboard-language-toggle" "$file"
  grep -Fq "data-field=\"englishVersion\" role=\"switch\" aria-label=\"英文版故事板\"" "$file"
  grep -Fq "<span class=\"concurrent-switch\" aria-hidden=\"true\"></span>" "$file"
  grep -Fq ".node-type-storyboard .storyboard-option-row:has(.storyboard-more[open])" "$file"
  grep -Fq ".node-type-storyboard .storyboard-language-toggle.concurrent-toggle:hover" "$file"
  grep -Fq ".node-type-storyboard .storyboard-more>summary.tiny" "$file"
}
verify_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "concurrent-toggle storyboard-language-toggle" "$file"
  run_sudo grep -Fq "data-field=\"englishVersion\" role=\"switch\" aria-label=\"英文版故事板\"" "$file"
  run_sudo grep -Fq "<span class=\"concurrent-switch\" aria-hidden=\"true\"></span>" "$file"
  run_sudo grep -Fq ".node-type-storyboard .storyboard-option-row:has(.storyboard-more[open])" "$file"
  run_sudo grep -Fq ".node-type-storyboard .storyboard-language-toggle.concurrent-toggle:hover" "$file"
  run_sudo grep -Fq ".node-type-storyboard .storyboard-more>summary.tiny" "$file"
}
