...
- `--niji 7`：官方没有角色引用参数，不输出 `--cref --cw --oref --ow`，也不输出 `--hd`；可用 `--raw`、`--s`、`--c`、`--iw`、`--sref`。质量只用安全 `--q 1`，不要输出 `--q 2` 或 `--q 4`。
...
- 官方版本兼容硬规则：`--v 8.1` 不输出 `--q --cref --cw --oref --ow`；`--v 7` 角色一致用 `--oref --ow`，不用 `--cref --cw`；`--v 6` / `--v 6.1` / `--niji 6` 才用 `--cref --cw`；`--niji 7` 可用 `--raw`，但不输出 `--hd` 和角色引用参数。
