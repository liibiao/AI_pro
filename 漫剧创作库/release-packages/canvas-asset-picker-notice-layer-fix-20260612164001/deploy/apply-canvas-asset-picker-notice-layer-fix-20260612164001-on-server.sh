...
ARCHIVE="${1:-}"
PKG="canvas-asset-picker-notice-layer-fix-20260612164001"
...
verify_file() {
  ...
  grep -Fq ".asset-picker-dialog{position:relative;" "$file"
  grep -Fq ".asset-picker-notice{position:absolute;left:50%;top:68px;z-index:20;" "$file"
  grep -Fq ".asset-picker-notice.show{opacity:1;transform:translate(-50%,0) scale(1)}" "$file"
  grep -Fq "function showAssetPickerNotice(message,type='warn')" "$file"
  grep -Fq "showAssetPickerNotice(check.message||'已超过可选数量','warn')" "$file"
  grep -Fq "data-asset-picker-notice" "$file"
