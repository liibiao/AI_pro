...
function normalizeBackendModel(item){
  ...
  return forceAiyunzhiFireflyGptImageModelConfig(forceGptImage2ProModelConfig({
    ...item,
    id:item.id,
    configId:item.id,
    modelKey:item.id,
...
function modelPayload(model){
  model=forceAiyunzhiFireflyGptImageModelConfig(forceGptImage2ProModelConfig(model||{}));
  return {
...
function assembleFireflyGptImageModelName(baseModel,resolution,aspectRatio){
  const base=String(baseModel||'firefly-gpt-image').trim().replace(/-(?:1|2|4)k-[0-9]+x[0-9]+$/i,'')||'firefly-gpt-image';
...
function imageRequestGeometry(model,values={}){
  ...
  const aspectRatio=fireflyImage
    ...
    : (grokImage?normalizeImageAspectRatio(rawRatio,fallbackSize):normalizeImageAspectRatio('',fallbackSize));
  ...
  const size=grokImage
    ...
    : (fireflyImage?aspectRatio:fallbackSize);
  const finalRatio=normalizeImageAspectRatio(aspectRatio,size);
  return {
    size,
    imageSize:fireflyImage?resolution:size,
    image_size:fireflyImage?resolution:size,
...
function assembleGptImageV2ModelName(baseModel,resolution){
  ...
  return ['1k','2k','3k','4k'].includes(res)?`${base}-${res}`:base;
}
...
async function runVideo(node,inputs,models,{onProgress}={}){
  ...
  const usesSeedance2=!usesFullbloodVideo&&isSeedance2VideoModel(model);
  ...
  const usesArtifexVideo=(usesSoraVideoPro||usesSeedance2)&&!usesSeedance2Sd&&!usesFullbloodVideo;
