...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/asset-confirm-ui-center-fix-20260622021351-${STAMP}"
...
check_markers(){
  local file="$1"
  test -f "$file"
  grep -Fq "asset-confirm-thumb-empty{box-sizing:border-box!important" "$file"
  grep -Fq "asset-confirm-gen-mask>.gen-progress-text" "$file"
  grep -Fq "node.node-type-assetDesign .asset-preview-empty{display:grid!important" "$file"
}
...
echo "deployed asset-confirm-ui-center-fix-20260622021351"
