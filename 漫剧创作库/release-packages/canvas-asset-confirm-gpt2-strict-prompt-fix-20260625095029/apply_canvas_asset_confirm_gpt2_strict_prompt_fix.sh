...
for pair in "${TARGETS[@]}"; do
  ...
  grep -q "applyAssetConfirmStrictProviderPromptPayload" "$target"
  grep -q "strict-provider-direct" "$target"
  grep -q "assetConfirmModelNeedsStrictPromptSafety" "$target"
  grep -q "assetConfirmSoftenStrictProviderPromptText" "$target"
