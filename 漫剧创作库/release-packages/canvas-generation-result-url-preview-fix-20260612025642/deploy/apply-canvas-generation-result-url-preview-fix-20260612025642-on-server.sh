...
PKG="canvas-generation-result-url-preview-fix-20260612025642"
...
verify_html_markers(){
  local file="$1"
  grep -Fq 'function taskResultUrlContainers(task)' "$file"
  grep -Fq 'function taskDirectResultUrlCandidates(task)' "$file"
  grep -Fq 'function taskTemporaryResultUrls(task)' "$file"
  grep -Fq 'function taskStorageStatusValue(task)' "$file"
  grep -Fq 'const taskPreview=data?.task?taskResultUrls(data.task)[0]' "$file"
  grep -Fq 'const raw=taskStorageStatusValue(task)||String(resultJson.storageStatus||resultJson.storage_status||' "$file"
  grep -Fq 'http://127.0.0.1:4000' "$file"
}
verify_html_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq 'function taskResultUrlContainers(task)' "$file"
  run_sudo grep -Fq 'function taskDirectResultUrlCandidates(task)' "$file"
  run_sudo grep -Fq 'function taskTemporaryResultUrls(task)' "$file"
  run_sudo grep -Fq 'function taskStorageStatusValue(task)' "$file"
  run_sudo grep -Fq 'const taskPreview=data?.task?taskResultUrls(data.task)[0]' "$file"
  run_sudo grep -Fq 'const raw=taskStorageStatusValue(task)||String(resultJson.storageStatus||resultJson.storage_status||' "$file"
  run_sudo grep -Fq 'http://127.0.0.1:4000' "$file"
}
