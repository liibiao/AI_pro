...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/asset-confirm-mj-queue-progress-fix-20260618030244-${STAMP}"
...
check_markers(){
  ...
  grep -q "assetConfirmLiveQueueKeys" "$file"
  grep -q "queuedHasSubmittedTask" "$file"
  grep -q "mjConcurrency=Math.min(4" "$file"
  grep -q "const concurrency=Math.min(4" "$file"
  grep -q "等待提交" "$file"
  if grep -q "const concurrency=Math.min(2,Math.max(1,queue.length));" "$file"; then
    echo "old two-task asset generation queue limit still present in $file" >&2
    exit 1
  fi
}
...
echo "deployed asset-confirm-mj-queue-progress-fix-20260618030244"
