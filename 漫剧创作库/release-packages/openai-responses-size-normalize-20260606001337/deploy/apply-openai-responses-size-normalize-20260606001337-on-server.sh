...
PKG="openai-responses-size-normalize-20260606001337"
...
verify_markers(){
  ...
  file_contains "const OPENAI_RESPONSES_IMAGE_SIZE_RATIOS=['auto','1:1','3:2','2:3'];" "$file"
  file_contains "'3:2|1k':'1536×1024'" "$file"
  file_contains "function nearestOpenAiResponsesImageRatio" "$file"
  file_contains "useNormalizedSizeMeta=!params.allowLegacySize&&isOpenAiResponsesImageAdapter(adapter)" "$file"
...
verify_markers_sudo(){
  ...
  run_sudo grep -Fq "const OPENAI_RESPONSES_IMAGE_SIZE_RATIOS=['auto','1:1','3:2','2:3'];" "$file"
  run_sudo grep -Fq "'3:2|1k':'1536×1024'" "$file"
  run_sudo grep -Fq "function nearestOpenAiResponsesImageRatio" "$file"
  run_sudo grep -Fq "useNormalizedSizeMeta=!params.allowLegacySize&&isOpenAiResponsesImageAdapter(adapter)" "$file"
