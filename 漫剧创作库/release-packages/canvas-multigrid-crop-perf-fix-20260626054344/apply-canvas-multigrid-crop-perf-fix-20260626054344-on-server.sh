...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/canvas-multigrid-crop-perf-fix-20260626054344-${STAMP}"
...
validate_html(){
  local file="$1"
  test -f "$file"
  require_marker "const MULTI_GRID_CROP_VERSION=4;" "$file"
  require_marker "const guardX=multiGridClampInt(cellW*.008,1,6);" "$file"
  require_marker "multiGridCropVersion:MULTI_GRID_CROP_VERSION" "$file"
  require_marker "cropVersion:MULTI_GRID_CROP_VERSION" "$file"
  require_marker "function isNodeProgressVisualPaused()" "$file"
  require_marker "function flushPendingNodeProgressVisuals()" "$file"
  require_marker "scheduleMultiGridGeneratedEntryMaterialize(entry,nodeId);" "$file"
  require_marker ".stage.node-dragging .node .preview-card>.gen-progress-mask:not([data-pano-upload-mask])" "$file"
}
...
echo "deployed canvas-multigrid-crop-perf-fix-20260626054344"
