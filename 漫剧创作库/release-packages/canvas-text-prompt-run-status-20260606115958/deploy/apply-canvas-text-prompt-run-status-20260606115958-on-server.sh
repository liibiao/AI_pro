...
PKG="canvas-text-prompt-run-status-20260606115958"
...
verify_canvas_markers(){
  local file="$1"
  grep -Fq 'class="prompt-run-status"' "$file"
  grep -Fq 'style="--prompt-run-progress:${(progress/100).toFixed(3)}"' "$file"
  grep -Fq '<span class="prompt-run-dot"></span>' "$file"
  grep -Fq '.prompt-run-status i{position:absolute;' "$file"
  grep -Fq '.node.node-type-textPrompt .aio-preview .gen-progress-mask{display:none!important}' "$file"
  grep -Fq 'keep in-node toolbars on one line and make capsules content-sized' "$file"
  if grep -Fq 'asset-card-derive-progress prompt-run-progress' "$file"; then
    fail "old text prompt overlay progress marker still exists in $file"
  fi
}
verify_canvas_markers_sudo(){ local file="$1"; run_sudo bash -c "$(declare -f fail); $(declare -f verify_canvas_markers); verify_canvas_markers \"\$0\"" "$file"; }
