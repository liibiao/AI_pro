...
PKG="video-model-billing-badges-inline-20260620041315"
...
verify_canvas_next_html(){
  local file="$1"
  grep -Fq ".tn-menu-item.has-billing-badge{min-height:32px;padding-left:10px}" "$file"
  grep -Fq ".tn-billing-badge{flex:0 0 auto" "$file"
  grep -Fq "videoModelBillingBadgeAttrs" "$file"
  grep -Fq "data-billing-badge" "$file"
}
...
if run_sudo test -f "$WORKBENCH_DIR/image-studio-canvas-next.html"; then
  run_sudo grep -Fq ".tn-billing-badge{flex:0 0 auto" "$WORKBENCH_DIR/image-studio-canvas-next.html"
  run_sudo grep -Fq "videoModelBillingBadgeAttrs" "$WORKBENCH_DIR/image-studio-canvas-next.html"
fi
