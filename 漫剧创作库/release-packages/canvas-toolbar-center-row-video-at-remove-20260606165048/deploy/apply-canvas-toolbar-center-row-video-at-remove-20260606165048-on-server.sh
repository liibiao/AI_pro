...
PKG="canvas-toolbar-center-row-video-at-remove-20260606165048"
...
verify_html(){
  local file="$1"
  grep -Fq "tapnow-rewrite.css?v=20260606-toolbar-center-row" "$file"
  grep -Fq "generation node bottom bars are centered as row boxes" "$file"
  grep -Fq "findNodeToolbarWidthAnchor" "$file"
  grep -Fq "syncNodeToolbarMinWidth" "$file"
  grep -Fq -- "--node-toolbar-min-width" "$file"
  ! grep -Fq '<span class="vn2-at-wrap"><button class="vn2-at-btn"' "$file"
}
verify_css(){
  local file="$1"
  ...
  grep -Fq "generation node bottom bars are centered as row boxes" "$file"
  grep -Fq ".node.node-type-storyboard .aio-bar:not(.prompt-zoomed-item)" "$file"
  grep -Fq -- "--node-toolbar-min-width" "$file"
}
verify_html_sudo(){
  local file="$1"
  run_sudo grep -Fq "tapnow-rewrite.css?v=20260606-toolbar-center-row" "$file"
  run_sudo grep -Fq "generation node bottom bars are centered as row boxes" "$file"
  run_sudo grep -Fq "findNodeToolbarWidthAnchor" "$file"
  run_sudo grep -Fq "syncNodeToolbarMinWidth" "$file"
  run_sudo grep -Fq -- "--node-toolbar-min-width" "$file"
  ! run_sudo grep -Fq '<span class="vn2-at-wrap"><button class="vn2-at-btn"' "$file"
}
verify_css_sudo(){
  local file="$1"
  ...
  run_sudo grep -Fq "generation node bottom bars are centered as row boxes" "$file"
  run_sudo grep -Fq ".node.node-type-storyboard .aio-bar:not(.prompt-zoomed-item)" "$file"
  run_sudo grep -Fq -- "--node-toolbar-min-width" "$file"
}
