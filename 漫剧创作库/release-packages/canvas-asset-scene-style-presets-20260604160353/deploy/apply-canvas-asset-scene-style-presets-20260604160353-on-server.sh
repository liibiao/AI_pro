...
PKG="canvas-asset-scene-style-presets-20260604160353"
...
verify_markers(){
  local dest="$1"
  grep -Fq "asset-film-style-select" "$dest"
  grep -Fq "data-asset-card-global-style" "$dest"
  grep -Fq "renderAssetCardTemplateOrScenePicker(card,idx)" "$dest"
  grep -Fq "templateId:isScene?'scene-preset'" "$dest"
  grep -Fq "assetGlobalStylePreset:S.assetGlobalStylePreset||'none'" "$dest"
  grep -Fq "stylePreset:type==='scene'?(card.scenePreset||'cinematic'):''" "$dest"
}
