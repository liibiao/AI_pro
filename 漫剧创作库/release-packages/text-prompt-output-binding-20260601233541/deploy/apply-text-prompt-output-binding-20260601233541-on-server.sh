...
PKG="text-prompt-output-binding-20260601233541"
...
verify_markers(){
  local dest="$1"
  grep -Fq "function boundTextPromptOutputKind" "$dest"
  grep -Fq "function syncTextPromptOutputKind" "$dest"
  grep -Fq "function shouldShowTextPromptOutputKindSelect" "$dest"
  grep -Fq "function renderTextPromptOutputKindControl" "$dest"
  grep -Fq "prompt-output-kind-control" "$dest"
  grep -Fq "prompt-output-kind-select" "$dest"
  grep -Fq "syncTextPromptOutputKind(n,{modeChanged:true})" "$dest"
  grep -Fq "select data-field=\"mode\" class=\"aio-sel prompt-node-mode\"" "$dest"
  if grep -Fq '<select data-field="outputKind" class="aio-sel prompt-node-output">${outputKindOptionsForTextPrompt(v)}</select>' "$dest"; then
    echo "text prompt output format select still exists in bottom toolbar" >&2
    return 1
  fi
}
