...
def _read_image_studio_model_registry_document() -> dict:
    fallback = {
        "version": 1,
        "description": "Image Studio 模型身份注册表。只管理稳定 identityKey 与 adapter 绑定，不存储 API Key，不接管显示名称/别名/真实模型名，不覆盖 models/*.json 渠道配置。",
        "models": [],
    }
    if not IMAGE_STUDIO_MODEL_REGISTRY_PATH.exists():
        return fallback
...
def _safe_model_config_id(value: str) -> str:
    text = re.sub(r"[^A-Za-z0-9_.-]+", "-", str(value or "").strip()).strip(".-").lower()
    return (text[:120].strip(".-") or f"newapi-{uuid.uuid4().hex[:8]}")


def _newapi_v1_base_url(base_url: str) -> str:
    raw = str(base_url or "").strip().rstrip("/")
    if not raw:
        raise ValueError("请填写 NewAPI baseURL")
    if not re.match(r"^https?://", raw, flags=re.I):
        raise ValueError("NewAPI baseURL 需要包含 http:// 或 https://")
    parsed = urlparse(raw)
    if not parsed.netloc:
        raise ValueError("NewAPI baseURL 格式不正确")
    path = parsed.path.rstrip("/")
    if re.search(r"/v\d+$", path, flags=re.I):
        return raw
    return f"{raw}/v1"


def _newapi_models_url(base_url: str) -> str:
    return f"{_newapi_v1_base_url(base_url).rstrip('/')}/models"


NEWAPI_VENDOR_LABELS = {
    "openai": "OpenAI",
    "grok": "Grok / xAI",
    "xai": "Grok / xAI",
    "google": "Google",
...
def _newapi_vendor_label(vendor: str) -> str:
    vendor = str(vendor or "unknown").strip().lower() or "unknown"
    return NEWAPI_VENDOR_LABELS.get(vendor, vendor[:1].upper() + vendor[1:])


def _newapi_type_label(model_type: str) -> str:
    return NEWAPI_TYPE_LABELS.get(str(model_type or "").strip().lower(), "模型")


def _newapi_model_text(item: dict) -> str:
    values = [
        item.get("id"),
        item.get("model"),
        item.get("name"),
...
def infer_newapi_vendor(item: dict) -> str:
    explicit = str(item.get("vendor") or item.get("provider") or item.get("owned_by") or item.get("ownedBy") or "").strip().lower()
    text = _newapi_model_text(item)
    combined = f"{explicit} {text}"
    vendor_rules = [
        ("grok", r"\b(?:grok|xai|x\.ai)\b"),
        ("openai", r"\b(?:openai|gpt-|gpt_|gpt\d|o1|o3|o4|dall[-_ ]?e|sora)\b"),
        ("gemini", r"\b(?:gemini|imagen|veo|google)\b"),
        ("anthropic", r"\b(?:anthropic|claude)\b"),
        ("deepseek", r"\bdeepseek\b"),
        ("qwen", r"\b(?:qwen|tongyi|aliyun|dashscope)\b"),
        ("moonshot", r"\b(?:moonshot|kimi)\b"),
        ("zhipu", r"\b(?:zhipu|glm|chatglm)\b"),
        ("minimax", r"\bminimax\b"),
        ("stability", r"\b(?:stability|stable[-_ ]?diffusion|sdxl)\b"),
        ("midjourney", r"\b(?:midjourney|mj|niji)\b"),
        ("runway", r"\brunway\b"),
        ("kling", r"\b(?:kling|可灵)\b"),
        ("hailuo", r"\b(?:hailuo|海螺)\b"),
        ("seedance", r"\b(?:seedance|doubao|volc|火山|即梦|jimeng)\b"),
    ]
    for vendor, pattern in vendor_rules:
        if re.search(pattern, combined, flags=re.I):
            return vendor
    if explicit:
        return re.sub(r"[^a-z0-9_-]+", "-", explicit).strip("-") or "unknown"
    return "unknown"


def infer_newapi_model_type(item: dict) -> str:
    raw = str(item.get("modelType") or item.get("model_type") or item.get("type") or item.get("category") or "").strip().lower()
    if raw in {"image", "img", "images", "image_generation", "text2image", "txt2img", "图片", "生图"}:
        return "image"
    if raw in {"video", "videos", "video_generation", "text2video", "txt2video", "image2video", "视频", "生视频"}:
        return "video"
    if raw in {"llm", "chat", "text", "language", "completion", "大语言", "语言"}:
        return "llm"
    text = _newapi_model_text(item)
    if re.search(r"\b(?:video|veo|sora|seedance|kling|runway|pika|hailuo|vidu|luma|wan|t2v|i2v)\b", text, flags=re.I):
        return "video"
    if re.search(r"\b(?:image|img|imagen|dall[-_ ]?e|gpt[-_ ]?image|midjourney|mj|niji|flux|sdxl|stable[-_ ]?diffusion|ideogram|jimeng|imagine)\b", text, flags=re.I):
        return "image"
    if re.search(r"\b(?:gpt|claude|deepseek|qwen|glm|grok|gemini|kimi|llama|mistral|yi|ernie|doubao|chat|llm)\b", text, flags=re.I):
        return "llm"
    return "llm"


def _newapi_adapter_for(model_type: str, vendor: str, model_id: str) -> str:
    text = f"{vendor} {model_id}".lower()
    if model_type == "llm":
    ...
    if model_type == "video":
        if "grok" in text or "xai" in text:
            return "grok-video"
        if "veo" in text or "gemini" in text:
            return "gemini-video"
        if "seedance" in text or "doubao" in text or "jimeng" in text:
            return "seedance2"
        if "sora" in text:
    ...
    if "midjourney" in text or re.search(r"\bmj\b", text):
        return "midjourney-imagine"
    return "openai-generations"


def _newapi_endpoint_for(model_type: str, adapter: str) -> str:
    if model_type == "llm":
        return "/responses" if adapter == "openai-responses" else "/chat/completions"
    if model_type == "video":
        return "/videos"
    if adapter == "midjourney-imagine":
        return "/mj/submit/imagine"
    return "/images/generations"


def _newapi_supports_template(model_type: str, model_id: str, adapter: str) -> dict:
    text = f"{model_id} {adapter}".lower()
    if model_type == "llm":
        return {"chat": True, "vision": bool(re.search(r"vision|gpt-4o|gemini|grok", text))}
    if model_type == "video":
        return {"txt2video": True, "img2video": True}
    supports_image_reference = bool(re.search(r"edit|image-to-image|img2img|grok|gemini", text))
    return {
        "txt2img": True,
        "img2img": supports_image_reference,
        "imageToImage": supports_image_reference,
        "storyboard": True,
        "panorama": False,
    }


def _newapi_capabilities_template(model_type: str) -> dict:
    if model_type == "video":
        return {
...
def _newapi_pricing_template(model_type: str) -> dict:
    if model_type == "llm":
        return {
            "unit": "token",
            "billingMode": "token",
            ...
            "chargedCreditsPerRequest": 0,
            "note": "NewAPI 导入后请在后台按平台定价规则填写。",
        }
    if model_type == "video":
        return {
            "unit": "generation",
            ...
            "note": "NewAPI 导入后请在后台按分辨率/时长配置定价。",
        }
    return {
        "unit": "generation",
        "billingMode": "per_generation",
        ...
        "chargedCreditsPerGeneration": 0,
        "costCreditsPerGeneration": 0,
        "note": "NewAPI 导入后请在后台按图片规格配置定价。",
    }


def fetch_newapi_models(payload: dict[str, object]) -> dict[str, object]:
    base_url = _newapi_v1_base_url(str(payload.get("baseUrl") or payload.get("baseURL") or payload.get("url") or ""))
    api_key = str(payload.get("apiKey") or payload.get("key") or payload.get("token") or "").strip()
    if not api_key:
        raise ValueError("请填写 NewAPI Key")
    models_url = _newapi_models_url(base_url)
    request = Request(
        models_url,
        headers={
            "Accept": "application/json",
            "Authorization": f"Bearer {api_key}",
            "User-Agent": "studio-workbench-newapi/1.0",
        },
        method="GET",
    )
    try:
        ...
        try:
            ...
            body = str(exc)
        raise ValueError(f"NewAPI /v1/models 请求失败（HTTP {exc.code}）：{body[:500]}") from exc
    except Exception as exc:
        raise ValueError(f"NewAPI /v1/models 请求失败：{exc}") from exc
    try:
        data = json.loads(raw.decode("utf-8", errors="replace"))
    except Exception as exc:
        raise ValueError("NewAPI /v1/models 返回的不是合法 JSON") from exc
    ...
    if not isinstance(raw_models, list):
        raise ValueError("NewAPI /v1/models 返回结构中未找到模型列表")
    ...
    for raw_item in raw_models:
        ...
        if not model_id:
            continue
        vendor = infer_newapi_vendor(item)
        model_type = infer_newapi_model_type(item)
        owned_by = str(item.get("owned_by") or item.get("ownedBy") or item.get("provider") or vendor).strip()
        label = str(item.get("label") or item.get("displayName") or item.get("name") or model_id).strip()
        adapter = _newapi_adapter_for(model_type, vendor, model_id)
        endpoint_path = _newapi_endpoint_for(model_type, adapter)
        models.append({
            "id": model_id,
            "model": model_id,
            "name": label,
            ...
            "displayName": label,
            "type": model_type,
            "typeLabel": _newapi_type_label(model_type),
            "vendor": vendor,
            "vendorLabel": _newapi_vendor_label(vendor),
            "ownedBy": owned_by,
            "adapter": adapter,
    ...
    for item in models:
        counts[str(item.get("type") or "llm")] = counts.get(str(item.get("type") or "llm"), 0) + 1
        vendor = str(item.get("vendor") or "unknown")
        vendor_counts[vendor] = vendor_counts.get(vendor, 0) + 1
    vendors = [
        {"key": key, "label": _newapi_vendor_label(key), "count": vendor_counts[key]}
        for key in sorted(vendor_counts.keys(), key=lambda value: (_newapi_vendor_label(value).lower(), value))
    ]
    return {
        "ok": True,
        "baseUrl": base_url,
...
def _build_newapi_model_config(item: dict, base_url: str, api_key: str) -> tuple[str, dict, dict]:
    model_id = str(item.get("model") or item.get("id") or item.get("name") or "").strip()
    if not model_id:
        raise ValueError("导入模型缺少 id/model")
    model_type = infer_newapi_model_type(item)
    vendor = infer_newapi_vendor(item)
    label = str(item.get("label") or item.get("displayName") or item.get("name") or model_id).strip()
    config_id = _safe_model_config_id(str(item.get("configId") or f"newapi-{vendor}-{model_type}-{model_id}"))
    adapter = str(item.get("adapter") or _newapi_adapter_for(model_type, vendor, model_id)).strip()
    endpoint_path = str(item.get("endpointPath") or _newapi_endpoint_for(model_type, adapter)).strip()
    status_endpoint_path = "/videos/{taskId}" if model_type == "video" else ""
    provider_key = f"newapi::{vendor}"
    path = IMAGE_STUDIO_MODELS_DIR / f"{config_id}.json"
    existing: dict = {}
    if path.exists():
        try:
    ...
    existing_protocol = existing.get("protocol") if isinstance(existing.get("protocol"), dict) else {}
    pricing = existing.get("pricing") if isinstance(existing.get("pricing"), dict) else _newapi_pricing_template(model_type)
    capabilities = existing.get("capabilities") if isinstance(existing.get("capabilities"), dict) else _newapi_capabilities_template(model_type)
    supports = existing.get("supports") if isinstance(existing.get("supports"), dict) else _newapi_supports_template(model_type, model_id, adapter)
    enabled = existing.get("enabled", True)
    status = str(existing.get("status") or ("enabled" if enabled is not False else "disabled")).strip()
    defaults = existing.get("defaults") if isinstance(existing.get("defaults"), dict) else {}
    if model_type == "image":
        defaults = {"size": "1024x1024", "aspectRatio": "1:1", "resolution": "1K", **defaults}
    elif model_type == "video":
        defaults = {"duration": 6, "resolution": "720p", "aspectRatio": "16:9", **defaults}
    config = {
        **existing,
        "id": config_id,
        "name": str(existing.get("name") or model_id),
        "type": model_type,
        "label": str(existing.get("label") or label),
        "displayName": str(existing.get("displayName") or existing.get("label") or label),
        "model": model_id,
        "baseUrl": base_url,
        "url": base_url,
        "key": api_key,
        ...
        "provider": {
            **existing_provider,
            "name": _newapi_vendor_label(vendor),
            "providerKey": provider_key,
            "baseUrl": base_url,
            "apiKey": api_key,
            "adapter": adapter,
            "endpointPath": endpoint_path,
            **({"statusEndpointPath": status_endpoint_path} if status_endpoint_path else {}),
        },
        "protocol": {
            **existing_protocol,
            "adapter": adapter,
            "endpointPath": endpoint_path,
            "method": "async-poll" if model_type == "video" else "sync",
            **({"statusEndpointPath": status_endpoint_path} if status_endpoint_path else {}),
        },
        "ui": {
            **(existing.get("ui") if isinstance(existing.get("ui"), dict) else {}),
            "label": str(existing.get("ui", {}).get("label") if isinstance(existing.get("ui"), dict) else "") or str(existing.get("label") or label),
            "badge": _newapi_vendor_label(vendor),
            "badgeColor": "#2563eb" if model_type == "image" else ("#7c3aed" if model_type == "video" else "#111827"),
        },
        "note": str(existing.get("note") or f"NewAPI 导入模型：{_newapi_vendor_label(vendor)} / {model_id}。"),
    }
    if status_endpoint_path:
        config["statusEndpointPath"] = status_endpoint_path
    registry_entry = {
        "configId": config_id,
        "identityKey": f"newapi::{vendor}::{model_type}::{model_id}",
        "adapter": adapter,
    }
    return config_id, config, registry_entry


def import_newapi_models(payload: dict[str, object]) -> dict[str, object]:
    base_url = _newapi_v1_base_url(str(payload.get("baseUrl") or payload.get("baseURL") or payload.get("url") or ""))
    api_key = str(payload.get("apiKey") or payload.get("key") or payload.get("token") or "").strip()
    if not api_key:
        raise ValueError("请填写 NewAPI Key")
    ...
    if not isinstance(raw_models, list) or not raw_models:
        raise ValueError("请选择要导入的 NewAPI 模型")
    registry_doc = _read_image_studio_model_registry_document()
    registry_items = registry_doc.get("models") if isinstance(registry_doc.get("models"), list) else []
    registry_by_config = {
        str(item.get("configId") or "").strip(): item
    ...
    for raw_item in raw_models:
        if isinstance(raw_item, str):
            ...
            continue
        config_id, config, registry_entry = _build_newapi_model_config(item, base_url, api_key)
        _write_json_atomic(IMAGE_STUDIO_MODELS_DIR / f"{config_id}.json", config)
        registry_by_config[config_id] = {**registry_by_config.get(config_id, {}), **registry_entry}
        imported.append({
            "configId": config_id,
            "id": config_id,
    ...
    registry_doc["models"] = list(registry_by_config.values())
    _write_json_atomic(IMAGE_STUDIO_MODEL_REGISTRY_PATH, registry_doc)
    return {
        "ok": True,
        "imported": imported,
        "models": list_image_studio_models(),
    }


def update_image_studio_model_config(payload: dict[str, object]) -> dict[str, object]:
    config = payload.get("config") if isinstance(payload.get("config"), dict) else payload
    raw_config_id = str(config.get("configId") or config.get("id") or payload.get("configId") or payload.get("id") or "").strip()
    if not raw_config_id:
        raise ValueError("缺少模型 configId")
    config_id = _safe_model_config_id(raw_config_id)
    path = IMAGE_STUDIO_MODELS_DIR / f"{config_id}.json"
    if not path.exists():
    ...
    try:
        ...
        data = {}
    allowed_scalar = {
        "name", "label", "displayName", "nick", "nickname", "model", "type",
        "baseUrl", "url", "key", "apiKey", "adapter", "endpointPath",
        "statusEndpointPath", "uploadMode", "uploadStrategy", "requestMethod",
        "enabled", "status", "source", "vendor", "note", "providerKey", "channelKey",
    }
    for key in allowed_scalar:
        if key in config:
            data[key] = config.get(key)
    for key in ("supports", "capabilities", "pricing", "defaults", "provider", "protocol", "ui", "modelAssembly"):
        if isinstance(config.get(key), dict):
            data[key] = config.get(key)
    base_url = str(data.get("baseUrl") or data.get("url") or "").strip()
    api_key = str(data.get("apiKey") or data.get("key") or "").strip()
    if base_url:
    ...
    adapter = str(data.get("adapter") or protocol.get("adapter") or provider.get("adapter") or "").strip()
    endpoint_path = str(data.get("endpointPath") or protocol.get("endpointPath") or provider.get("endpointPath") or "").strip()
    status_endpoint_path = str(data.get("statusEndpointPath") or protocol.get("statusEndpointPath") or provider.get("statusEndpointPath") or "").strip()
    if adapter:
        provider["adapter"] = adapter
        protocol["adapter"] = adapter
        data["adapter"] = adapter
    if endpoint_path:
        provider["endpointPath"] = endpoint_path
        protocol["endpointPath"] = endpoint_path
        data["endpointPath"] = endpoint_path
    if status_endpoint_path:
        provider["statusEndpointPath"] = status_endpoint_path
        protocol["statusEndpointPath"] = status_endpoint_path
        data["statusEndpointPath"] = status_endpoint_path
    if base_url:
    ...
    return {"ok": True, "model": data, "models": list_image_studio_models()}
...
def list_image_studio_models() -> list[dict[str, object]]:
    ...
    for path in model_paths:
        try:
            ...
            if real_model or alias:
                ...
                status = str(data.get("status") or ("enabled" if enabled is not False else "disabled")).strip()
                protocol = {**protocol}
                if adapter:
                    protocol["adapter"] = adapter
                if endpoint_path:
                ...
                models.append({
                    ...
                    "provider": provider,
                    "providerKey": str(data.get("providerKey") or data.get("channelKey") or provider.get("providerKey") or "").strip(),
                    "channelKey": str(data.get("channelKey") or data.get("providerKey") or provider.get("providerKey") or "").strip(),
                    "defaults": data.get("defaults") if isinstance(data.get("defaults"), dict) else {},
                    "pricing": pricing,
                    "protocol": protocol,
                    "ui": ui,
...
class WorkbenchRequestHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/":
            ...
            self.path = "/image-studio-canvas-next.html"
    ...
    def do_POST(self) -> None:
        ...
        if parsed.path in {"/v1/images/edits", "/v1/images/generations"}:
            ...
            return
        if parsed.path == "/api/workbench/image-studio/newapi/models":
            try:
                payload = self.read_json_body()
                self.respond_json(fetch_newapi_models(payload))
            except (ValueError, ImageStudioError) as exc:
                self.respond_json({"ok": False, "error": str(exc)}, status=HTTPStatus.BAD_REQUEST)
            except Exception as exc:  # noqa: BLE001
                self.respond_json({"ok": False, "error": str(exc)}, status=HTTPStatus.INTERNAL_SERVER_ERROR)
            return
        if parsed.path == "/api/workbench/image-studio/newapi/import":
            try:
                payload = self.read_json_body()
                self.respond_json(import_newapi_models(payload))
            except (ValueError, ImageStudioError) as exc:
                self.respond_json({"ok": False, "error": str(exc)}, status=HTTPStatus.BAD_REQUEST)
            except Exception as exc:  # noqa: BLE001
                self.respond_json({"ok": False, "error": str(exc)}, status=HTTPStatus.INTERNAL_SERVER_ERROR)
            return
        if parsed.path == "/api/workbench/image-studio/model/update":
            try:
                payload = self.read_json_body()
                self.respond_json(update_image_studio_model_config(payload))
            except (ValueError, ImageStudioError) as exc:
                self.respond_json({"ok": False, "error": str(exc)}, status=HTTPStatus.BAD_REQUEST)
            except Exception as exc:  # noqa: BLE001
                self.respond_json({"ok": False, "error": str(exc)}, status=HTTPStatus.INTERNAL_SERVER_ERROR)
