...
PKG="generic-model-prefix-order-20260620035651"
...
verify_canvas_html(){
  local file="$1"
  grep -Fq "function modelDisplayGroupMeta" "$file"
  grep -Fq "const chinesePrefix" "$file"
  grep -Fq "const latinPrefix" "$file"
  grep -Fq "const dynamicRanks=new Map()" "$file"
  grep -Fq "return sortModelsForDisplay" "$file"
}
...
verify_canvas_html "$SRC/workbench-web/image-studio-canvas-next.html"
verify_canvas_html "$SRC/workbench-web/image-studio-canvas.html"
verify_canvas_html "$SRC/tools/workbench-web/image-studio-canvas-next.html"
...
install_if_parent_exists "$SRC/tools/workbench-web/image-studio-canvas.html" "$MIRROR_ROOT/tools/workbench-web/image-studio-canvas.html" "mirror canvas"
...
if run_sudo test -f "$WORKBENCH_DIR/image-studio-canvas-next.html"; then
  run_sudo grep -Fq "function modelDisplayGroupMeta" "$WORKBENCH_DIR/image-studio-canvas-next.html"
  run_sudo grep -Fq "const dynamicRanks=new Map()" "$WORKBENCH_DIR/image-studio-canvas-next.html"
fi
if run_sudo test -f "$WORKBENCH_DIR/image-studio-canvas.html"; then
  run_sudo grep -Fq "function modelDisplayGroupMeta" "$WORKBENCH_DIR/image-studio-canvas.html"
  run_sudo grep -Fq "const dynamicRanks=new Map()" "$WORKBENCH_DIR/image-studio-canvas.html"
fi
