...
PKG="reference-upload-stall-fallback-20260619162001"
...
verify_html(){
  local file="$1"
  grep -Fq "workbench-engine.js?v=20260619-reference-upload-stall-fallback-v1" "$file"
}
...
if run_sudo test -f "$WORKBENCH_DIR/image-studio-canvas-next.html"; then
  run_sudo grep -Fq "workbench-engine.js?v=20260619-reference-upload-stall-fallback-v1" "$WORKBENCH_DIR/image-studio-canvas-next.html"
fi
