...
PKG="canvas-async-cos-refresh-preview-fix-20260608095238"
...
verify_registry_markers(){
  ...
  grep -Fq "persistInlineImageLocalPreview(inline" "$file"
  grep -Fq "persistInlineImageLocalPreview(directUrl" "$file"
}

verify_registry_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "server_base64_async_object_storage" "$file"
  run_sudo grep -Fq "persistInlineImageLocalPreview(inline" "$file"
  run_sudo grep -Fq "persistInlineImageLocalPreview(directUrl" "$file"
}

verify_routes_markers(){
  local file="$1"
  grep -Fq "GENERATION_TASK_TRANSACTION_OPTIONS" "$file"
  grep -Fq "writeProviderHealthLog" "$file"
  grep -Fq "storedUrls = resolved.filter(isLikelyPermanentStorageUrl)" "$file"
  grep -Fq "config.objectStorage.publicBaseUrl" "$file"
  if grep -Fq "tx.providerHealthLog.create" "$file"; then
    fail "providerHealthLog is still written through transaction client in $file"
  fi
}

verify_routes_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "GENERATION_TASK_TRANSACTION_OPTIONS" "$file"
  run_sudo grep -Fq "writeProviderHealthLog" "$file"
  run_sudo grep -Fq "storedUrls = resolved.filter(isLikelyPermanentStorageUrl)" "$file"
  run_sudo grep -Fq "config.objectStorage.publicBaseUrl" "$file"
  if run_sudo grep -Fq "tx.providerHealthLog.create" "$file"; then
    fail "providerHealthLog is still written through transaction client in $file"
  fi
}
...
verify_canvas_markers(){
  local file="$1"
  grep -Fq "function isUsableAssetResultUrl" "$file"
  grep -Fq "function uniqueAssetResultUrls" "$file"
  grep -Fq "tempUrls.length&&['uploading','failed','expired'].includes(storage.status)" "$file"
...
verify_canvas_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "function isUsableAssetResultUrl" "$file"
  run_sudo grep -Fq "function uniqueAssetResultUrls" "$file"
  run_sudo grep -Fq "tempUrls.length&&['uploading','failed','expired'].includes(storage.status)" "$file"
...
verify_routes_markers "$GEN_SRC"
verify_routes_markers "$GEN_DIST"
...
if run_sudo bash -lc "cd '$APP_DIR/api-server' && command -v node >/dev/null 2>&1 && [ -f node_modules/typescript/bin/tsc ]"; then
  if run_sudo bash -lc "cd '$APP_DIR/api-server' && node node_modules/typescript/bin/tsc -p tsconfig.json"; then
    ...
    log "api-server build failed; restoring packaged dist files"
    run_sudo install -m 0644 "$REGISTRY_DIST" "$APP_DIR/api-server/dist/modules/generation/adapters/registry.js"
    run_sudo install -m 0644 "$GEN_DIST" "$APP_DIR/api-server/dist/modules/generation/routes.js"
  fi
else
...
verify_routes_markers_sudo "$APP_DIR/api-server/src/modules/generation/routes.ts"
verify_routes_markers_sudo "$APP_DIR/api-server/dist/modules/generation/routes.js"
