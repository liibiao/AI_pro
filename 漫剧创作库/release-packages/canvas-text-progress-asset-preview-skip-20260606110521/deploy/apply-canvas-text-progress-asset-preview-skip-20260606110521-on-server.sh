...
PKG="canvas-text-progress-asset-preview-skip-20260606110521"
...
verify_canvas_markers(){
  local file="$1"
  grep -Fq ".node.node-type-textPrompt .aio-preview>.gen-progress-mask:not([data-pano-upload-mask])" "$file"
  grep -Fq "_skipAutoGeneratedImagePreview:true" "$file"
  grep -Fq "const autoPreviewAllowed=!(nd?.values?._skipAutoGeneratedImagePreview||nd?.data?._skipAutoGeneratedImagePreview)" "$file"
  grep -Fq "if(!concurrentCtx&&autoPreviewAllowed)ensureGeneratedImagePreviewDownstream(id)" "$file"
  grep -Fq "if(n.values?._skipAutoGeneratedImagePreview||n.data?._skipAutoGeneratedImagePreview)return ''" "$file"
