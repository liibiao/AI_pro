...
async function submitGrokVideo(ctx) {
    ...
    return {
        ...
        upstreamTaskId: String(upstream?.id || upstream?.task_id || ''),
        upstreamRequestId: String(upstream?.request_id || upstream?.id || ''),
        requestJson,
        responseJson: upstream,
        resultJson: { url: urls[0] || '', outputs: urls },
        resultUrls: urls,
...
function normalizeGrokVideoQuality(value) {
    const raw = String(value || '').trim().toLowerCase();
    if (['high', '720p', '1080p', '2k', '4k'].includes(raw))
        return 'high';
    return 'standard';
}
...
async function collectPublicImageReferenceUrls(ctx, timeoutMs = 15000) {
    ...
    for (const ref of refs) {
        if (/^data:image\//i.test(ref))
            ...
            resolved.push(await materializeGenerationResultReferenceUrl(ref));
    }
    return Array.from(new Set(resolved.filter(Boolean)));
}
...
async function uploadRemoteMediaUrlToObjectStorageBestEffort(provider, rawUrl, mediaType = 'video') {
    ...
    try {
        ...
        if (mediaType === 'video' && !/^video\//i.test(contentType))
            return '';
        if (mediaType === 'audio' && !/^audio\//i.test(contentType))
            return '';
        const contentLength = Number(resp.headers.get('content-length') || 0);
