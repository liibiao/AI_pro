...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/canvas-next-asset-confirm-grid4-perf-20260616170352-${STAMP}"
...
for rel in "${required_files[@]}"; do
  ...
  grep -q "scheduleAssetConfirmGenerationProgressDomUpdate" "$WORKDIR/$rel"
  grep -q "scheduleAssetConfirmNodeVisualUpdate" "$WORKDIR/$rel"
  grep -q "deferSave:true,toastMessage:false" "$WORKDIR/$rel"
  grep -q "资产确认批量生图结果" "$WORKDIR/$rel"
  grep -q "},1100);" "$WORKDIR/$rel"
...
grep -q "scheduleAssetConfirmGenerationProgressDomUpdate" "$PUBLIC_ROOT/image-studio-canvas-next.html"
grep -q "deferSave:true,toastMessage:false" "$PUBLIC_ROOT/image-studio-canvas-next.html"
! grep -q ">筛选状态<" "$PUBLIC_ROOT/image-studio-canvas-next.html"

echo "deployed canvas-next-asset-confirm-grid4-perf-20260616170352"
