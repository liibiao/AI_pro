...
PKG="canvas-toolbar-storyboard-compact-20260606134403"
...
verify_canvas_markers(){
  local file="$1"
  grep -Fq "keep in-node toolbars centered, compact, single-line, and content-sized" "$file"
  grep -Fq "storyboard-option-row" "$file"
  grep -Fq ".node-type-storyboard .storyboard-more>summary.tiny" "$file"
  grep -Fq ".node.node-type-shotStoryboard .prompt-node-bar.v3{align-items:center!important;gap:5px!important;row-gap:5px!important}" "$file"
  grep -Fq ".node.node-type-assetDesign .prompt-node-bar.v3,.node.node-type-shotStoryboard .prompt-node-bar.v3{flex-wrap:nowrap}" "$file"
  grep -Fq ".node.node-type-assetDesign .prompt-node-bar.v3 .prompt-node-go,.node.node-type-shotStoryboard .prompt-node-bar.v3 .prompt-node-go{margin-left:0}" "$file"
  grep -Fq "display:none!important;" "$file"
  grep -Fq "flex:0 0 0!important;" "$file"
  grep -Fq "function hideTapnowNativeControl(src)" "$file"
  grep -Fq "if(src.dataset.tnEnhanced){hideTapnowNativeControl(src);return;}" "$file"
  grep -Fq "hideTapnowNativeControl(src);" "$file"
  grep -Fq "transform:translateX(-50%)!important;" "$file"
  ! grep -Fq "flex:1 1 132px!important" "$file"
  ! grep -Fq "flex:1 1 142px!important" "$file"
  ! grep -Fq ".node.node-type-shotStoryboard .prompt-node-bar.v3{align-items:center!important;gap:6px!important" "$file"
}
