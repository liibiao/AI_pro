...
PKG="reference-upload-mask-clear-fix-20260615235820"
...
verify_markers(){
  local file="$1"
  grep -Fq 'function hasAnyUploadedReference(img)' "$file"
  grep -Fq "if(hasAnyUploadedReference(img))return '';" "$file"
  grep -Fq "const processing=['loading','compressing','uploading'].includes(img.status)&&!hasAnyUploadedReference(img);" "$file"
  grep -Fq "const active=['loading','compressing','uploading'].includes(img?.status)&&!hasAnyUploadedReference(img);" "$file"
  grep -Fq "function isReferenceUploadInProgress(img){return !!(img&&(img._uploadPromise||['loading','compressing','uploading'].includes(img.status))&&!hasAnyUploadedReference(img));}" "$file"
}
verify_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq 'function hasAnyUploadedReference(img)' "$file"
  run_sudo grep -Fq "if(hasAnyUploadedReference(img))return '';" "$file"
  run_sudo grep -Fq "const processing=['loading','compressing','uploading'].includes(img.status)&&!hasAnyUploadedReference(img);" "$file"
  run_sudo grep -Fq "const active=['loading','compressing','uploading'].includes(img?.status)&&!hasAnyUploadedReference(img);" "$file"
  run_sudo grep -Fq "function isReferenceUploadInProgress(img){return !!(img&&(img._uploadPromise||['loading','compressing','uploading'].includes(img.status))&&!hasAnyUploadedReference(img));}" "$file"
}
