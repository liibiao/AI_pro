...
PKG="canvas-viewport-credit-fix-20260604231221"
...
verify_canvas_markers(){
  ...
  grep -Fq "function nodeMeasuredWorldBounds" "$file"
  grep -Fq "function fitWorldBoundsToViewport" "$file"
  grep -Fq "fitWorldBoundsToViewport({left:minX,top:minY,right:maxX,bottom:maxY}" "$file"
  grep -Fq "const rects=ns.map(n=>nodeMeasuredWorldBounds(n))" "$file"
  ...
  grep -Fq "ASSET_CARD_DERIVE_MEMBER_CREDITS_FALLBACK=8" "$file"
  grep -Fq "function assetCardDeriveFallbackCredits" "$file"
  grep -Fq "keep the canvas chrome pinned to the visible browser viewport" "$file"
  grep -Fq "function minimapCanvasContext" "$file"
  grep -Fq "function syncCanvasViewportLayout" "$file"
  grep -Fq "window.visualViewport.addEventListener('resize',()=>syncCanvasViewportLayout()" "$file"
}
