...
ARCHIVE="${1:-/tmp/prompt-zoom-startrect-runtime-fix-20260601010331.tar.gz}"
TARGET="${2:-/home/ubuntu/漫剧创作库}"
STAMP="$(date +%Y%m%d%H%M%S)"
WORK="$(mktemp -d /tmp/prompt-zoom-startrect-runtime-fix-20260601010331-XXXXXX)"
...
SRC="$WORK/prompt-zoom-startrect-runtime-fix-20260601010331"
test -f "$SRC/tools/workbench-web/image-studio-canvas-next.html" || { echo "部署包缺少 canvas-next HTML" >&2; exit 1; }

BACKUP_DIR="$TARGET/.deploy-backups/prompt-zoom-startrect-runtime-fix-20260601010331-$STAMP"
...
log "verify markers"
grep -q "const elementRect=promptZoomUnionRect(elements)" "$TARGET/tools/workbench-web/image-studio-canvas-next.html"
grep -q "rect.left-elementRect.left" "$TARGET/tools/workbench-web/image-studio-canvas-next.html"
