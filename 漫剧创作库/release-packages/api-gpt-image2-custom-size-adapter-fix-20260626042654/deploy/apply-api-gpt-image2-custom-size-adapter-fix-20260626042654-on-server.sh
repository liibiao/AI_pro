...
PKG="api-gpt-image2-custom-size-adapter-fix-20260626042654"
...
log "verify GPT Image 2 custom adapter markers"
grep -Fq "gpt 2 稳定max" "$APP_DIR/dist/modules/generation/routes.js"
grep -Fq "gpt 2 优化plus" "$APP_DIR/dist/modules/generation/routes.js"
grep -Fq "normalizeAiyunzhiGptImage2PixelSize" "$APP_DIR/dist/modules/generation/adapters/registry.js"
grep -Fq "openAiImageSizeFromRatioResolution(ratio, resolution)" "$APP_DIR/dist/modules/generation/adapters/registry.js"
grep -Fq "ctx.params.endpointPath" "$APP_DIR/dist/modules/generation/adapters/registry.js"

if grep -Fq "gpt-image-2-fast" "$APP_DIR/dist/modules/generation/routes.js" "$APP_DIR/dist/modules/generation/adapters/registry.js"; then
  fail "unexpected GPT Image 2 fast marker found in this package"
fi
...
for (const marker of ['gpt-image-2-max', 'gpt-image-2-plus', 'model_gtp-2-max', 'model_gpt-2-plus']) {
  if (!routeBlock.includes(marker)) throw new Error(`missing route marker ${marker}`);
}

const sizeStart = registry.indexOf('function resolveAiyunzhiGptImage2Size');
const sizeEnd = registry.indexOf('function resolveAiyunzhiGptImage2Count', sizeStart);
if (sizeStart < 0 || sizeEnd < 0) throw new Error('Aiyunzhi size block not found');
const sizeBlock = registry.slice(sizeStart, sizeEnd);
if (sizeBlock.includes('return resolution.toLowerCase()')) throw new Error('old resolution-as-size behavior still present');
if (!sizeBlock.includes('openAiImageSizeFromRatioResolution(ratio, resolution)')) throw new Error('pixel-size mapping call missing');
if (!sizeBlock.includes('normalizeExplicitAiyunzhiGptImage2Resolution(explicitPixelSize) === resolution')) throw new Error('stale pixel-size guard missing');

const endpointStart = registry.indexOf('function resolveAiyunzhiGptImage2Endpoint');
const endpointEnd = registry.indexOf('async function callAiyunzhiFireflyJson', endpointStart);
if (endpointStart < 0 || endpointEnd < 0) throw new Error('Aiyunzhi endpoint block not found');
const endpointBlock = registry.slice(endpointStart, endpointEnd);
if (!endpointBlock.includes('protocol.endpointPath')) throw new Error('generic endpointPath fallback missing');

console.log('[verify] GPT Image 2 custom adapter route/size/endpoint markers ok');
NODE
