...
function resolveGenerationAdapterName(
  ...
  if (configuredLower === 'aiyunzhi-gpt-image-2' || isAiyunzhiGptImage2Hint(modelHint, hint, configuredLower)) return 'aiyunzhi-gpt-image-2';
  ...
  if (configuredLower === 'openai-image' || configuredLower === 'openai-generations' || configuredLower === 'openai-edits') return configuredLower;
  if (configuredLower === 'gpt-image' || configuredLower === 'gpt-image-2') return 'openai-edits';
  if (hint.includes('gpt-image-v2') || hint.includes('gpt image v2') || hint.includes('gpt-image- v2')) return 'gpt-image-v2';
  if (isGptImage2GenerationHint(modelHint, hint, configuredLower)) return 'openai-edits';
  if (configuredLower === 'aistartlab-video'
    ...
    || hint.includes('api.video.aistarslab.com')) return 'aistartlab-video';
  ...
  if (configuredLower === 'seedance2-sd' || configuredLower === 'sd-seedance2' || configuredLower === 'sd-video' || /\bsd2-(?:720p|1080p)(?:-fast)?\b/.test(hint) || (hint.includes('aiyunzhi.top') && /seedance|sd2|\/video\/generations/.test(hint))) return 'seedance2-sd';
...
function isAiyunzhiGptImage2Hint(modelHint: string, hint: string, configuredLower: string) {
  const text = `${modelHint} ${hint} ${configuredLower}`;
  const isExplicitVariant = text.includes('gpt-image-2-max')
  ...
  if (isExplicitVariant) return true;
  const isAiyunzhiProvider = text.includes('aiyunzhi.top')
    || text.includes('aiyunzhi-gpt-image-2-api')
    || text.includes('canvas-aiyunzhi-gpt-image-2-api')
    || text.includes('canvas_aiyunzhi-gpt-image-2-api');
  if (!isAiyunzhiProvider) return false;
  if (/seedance|sd2|\/video\//.test(text)) return false;
  if (isLegacyAiyunzhiGptImage2LowPriceHint(text)) return false;
  return /gpt[-_ ]?image[-_ ]?2/.test(text);
}

function isLegacyAiyunzhiGptImage2LowPriceHint(text: string) {
  return text.includes('gpt 2 低价')
...
async function queryAndUpdateGenerationTask(task: RuntimeGenerationTask) {
  ...
  try {
    ...
    return closeInterruptedGenerationTask(
      task,
      `上游任务状态查询失败，任务已自动关闭并退款：${message}`,
      'GENERATION_QUERY_FAILED',
    );
  }
...
function canRecoverRefundedQueryFailure(_task: Pick<RuntimeGenerationTask, 'status' | 'upstreamTaskId' | 'errorCode' | 'errorMessage' | 'refundReason'>) {
  return false;
}
