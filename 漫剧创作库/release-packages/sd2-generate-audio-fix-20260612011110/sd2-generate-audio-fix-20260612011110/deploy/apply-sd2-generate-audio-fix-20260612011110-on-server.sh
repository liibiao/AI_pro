...
PKG="sd2-generate-audio-fix-20260612011110"
...
verify_canvas_file(){
  local file="$1"
  grep -Fq "return'generate_audio'" "$file"
...
verify_engine_file(){
  local file="$1"
  grep -Fq "hasGenerateAudio || soundField" "$file"
}

verify_model_file(){
  local file="$1"
  grep -Fq '"soundField": "generate_audio"' "$file"
  ! grep -Fq "metadata.enableSound" "$file"
}

verify_backend_file(){
  local file="$1"
  grep -Fq 'if field == "metadata.enableSound" and default_field:' "$file"
  grep -Fq "enabled = _payload_bool(payload" "$file"
}
...
verify_canvas_file "$SRC/workbench-web/image-studio-canvas-next.html"
verify_canvas_file "$SRC/workbench-web/image-studio-canvas.html"
verify_engine_file "$SRC/workbench-web/workbench-engine.js"
verify_module_file "$SRC/workbench-web/canvas-next/renderers.js"
verify_module_file "$SRC/workbench-web/canvas-next/generation-service.js"
verify_model_file "$SRC/workbench-web/models/sd2-fast.json"
verify_model_file "$SRC/workbench-web/models/sd2-full.json"
...
if [ -d "$WORKBENCH_DIR" ]; then
  ...
  install_file "$SRC/workbench-web/models/sd2.json" "$WORKBENCH_DIR/models/sd2.json" "public sd2 model"
  install_file "$SRC/workbench-web/models/sd2-fast.json" "$WORKBENCH_DIR/models/sd2-fast.json" "public sd2 fast model"
  install_file "$SRC/workbench-web/models/sd2-full.json" "$WORKBENCH_DIR/models/sd2-full.json" "public sd2 full model"
...
if [ -d "$MIRROR_TARGET/tools/workbench-web" ]; then
  ...
  install_file "$SRC/tools/workbench-web/models/sd2.json" "$MIRROR_TARGET/tools/workbench-web/models/sd2.json" "mirror sd2 model"
  install_file "$SRC/tools/workbench-web/models/sd2-fast.json" "$MIRROR_TARGET/tools/workbench-web/models/sd2-fast.json" "mirror sd2 fast model"
  install_file "$SRC/tools/workbench-web/models/sd2-full.json" "$MIRROR_TARGET/tools/workbench-web/models/sd2-full.json" "mirror sd2 full model"
  install_file "$SRC/tools/image_studio_backend.py" "$MIRROR_TARGET/tools/image_studio_backend.py" "mirror backend"
  installed=$((installed+1))
fi
...
if [ -d "$MIRROR_TARGET/smart-vision/canvas/legacy-workbench/workbench-web" ]; then
  ...
  install_file "$SRC/smart-vision/canvas/legacy-workbench/workbench-web/canvas-next/renderers.js" "$MIRROR_TARGET/smart-vision/canvas/legacy-workbench/workbench-web/canvas-next/renderers.js" "smart legacy renderer"
...
if [ -d "$RUNTIME_ROOT/tools/workbench-web" ]; then
  ...
  install_file "$SRC/tools/workbench-web/models/sd2.json" "$RUNTIME_ROOT/tools/workbench-web/models/sd2.json" "runtime sd2 model"
  install_file "$SRC/tools/workbench-web/models/sd2-fast.json" "$RUNTIME_ROOT/tools/workbench-web/models/sd2-fast.json" "runtime sd2 fast model"
  install_file "$SRC/tools/workbench-web/models/sd2-full.json" "$RUNTIME_ROOT/tools/workbench-web/models/sd2-full.json" "runtime sd2 full model"
  install_file "$SRC/tools/image_studio_backend.py" "$RUNTIME_ROOT/tools/image_studio_backend.py" "runtime backend"
fi

[ "$installed" -gt 0 ] || fail "no public or mirror workbench target found"

log "verify installed markers"
if [ -f "$WORKBENCH_DIR/models/sd2-fast.json" ]; then
  run_sudo grep -Fq '"soundField": "generate_audio"' "$WORKBENCH_DIR/models/sd2-fast.json"
fi
if [ -f "$MIRROR_TARGET/tools/image_studio_backend.py" ]; then
  run_sudo grep -Fq 'if field == "metadata.enableSound" and default_field:' "$MIRROR_TARGET/tools/image_studio_backend.py"
fi
