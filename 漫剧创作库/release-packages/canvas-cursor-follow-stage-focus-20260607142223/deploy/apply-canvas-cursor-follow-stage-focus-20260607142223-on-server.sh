...
PKG="canvas-cursor-follow-stage-focus-20260607142223"
...
verify_markers(){
  local file="$1"
  grep -Fq "stage-cursor-field" "$file"
  grep -Fq ".stage-cursor-field.is-active{opacity:.82}" "$file"
  grep -Fq "Math.round((fx.w*fx.h)/5000),180,360" "$file"
  grep -Fq "clamp(Math.max(fx.w,fx.h)*.46,240,540)" "$file"
  grep -Fq "alpha:rand(.34,.86)" "$file"
  grep -Fq "clamp(p.alpha*edge*pulse,0,.92)" "$file"
  grep -Fq "settingsCursorEffectToggle" "$file"
  grep -Fq "mjb_canvas_cursor_effect_enabled" "$file"
}
verify_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "stage-cursor-field" "$file"
  run_sudo grep -Fq ".stage-cursor-field.is-active{opacity:.82}" "$file"
  run_sudo grep -Fq "Math.round((fx.w*fx.h)/5000),180,360" "$file"
  run_sudo grep -Fq "clamp(Math.max(fx.w,fx.h)*.46,240,540)" "$file"
  run_sudo grep -Fq "alpha:rand(.34,.86)" "$file"
  run_sudo grep -Fq "clamp(p.alpha*edge*pulse,0,.92)" "$file"
  run_sudo grep -Fq "settingsCursorEffectToggle" "$file"
  run_sudo grep -Fq "mjb_canvas_cursor_effect_enabled" "$file"
}
