...
PKG="canvas-storyboard-image-table-scroll-20260604112913"
...
verify_markers(){
  ...
  grep -Fq ".storyboard-image-table-panel .shot-table-preview.limited" "$dest"
  grep -Fq "overscroll-behavior:contain!important" "$dest"
  grep -Fq "table.addEventListener('wheel',e=>e.stopPropagation(),{passive:true});" "$dest"
  grep -Fq ".shot-table-preview,.shot-table-preview-wrap,.storyboard-image-table-panel" "$dest"
