...
PKG="mj-text-describe-runtime-fix-20260609100804"
...
verify_html_markers(){
  local file="$1"
  grep -Fq 'const TEXT_PROMPT_MJ_DESCRIBE_RUNTIME_STEPS=' "$file"
  grep -Fq "runtimeMode='midjourney-describe'" "$file"
  grep -Fq "const agentSelect=isMjDescribe?'':" "$file"
  grep -Fq "deriving||isMjDescribe?'':renderGenerateProgress" "$file"
  grep -Fq 'response.promptEn,response.prompt_en,response.finalPrompt' "$file"
  grep -Fq 'response.properties?.finalPrompt' "$file"
  grep -Fq "if(kind==='describe'){" "$file"
  grep -Fq 'ctx.onProgress' "$file"
  grep -Fq 'Describe 没有返回可用提示词' "$file"
}
verify_html_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq 'const TEXT_PROMPT_MJ_DESCRIBE_RUNTIME_STEPS=' "$file"
  run_sudo grep -Fq "runtimeMode='midjourney-describe'" "$file"
  run_sudo grep -Fq "const agentSelect=isMjDescribe?'':" "$file"
  run_sudo grep -Fq "deriving||isMjDescribe?'':renderGenerateProgress" "$file"
  run_sudo grep -Fq 'response.promptEn,response.prompt_en,response.finalPrompt' "$file"
  run_sudo grep -Fq 'response.properties?.finalPrompt' "$file"
  run_sudo grep -Fq "if(kind==='describe'){" "$file"
  run_sudo grep -Fq 'ctx.onProgress' "$file"
  run_sudo grep -Fq 'Describe 没有返回可用提示词' "$file"
}
