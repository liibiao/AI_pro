...
PKG="canvas-marquee-stage-local-position-fix-20260606105123"
...
verify_markers(){
  local file="$1"
  grep -Fq "_startLocalX:startLocalX" "$file"
  grep -Fq "const localX=e.clientX-sr.left,localY=e.clientY-sr.top;" "$file"
  grep -Fq "Math.min(startLocalX,localX)+'px'" "$file"
  grep -Fq "Math.abs(localX-startLocalX)+'px'" "$file"
  grep -Fq "S.mode!=='marquee'" "$file"
}
verify_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "_startLocalX:startLocalX" "$file"
  run_sudo grep -Fq "const localX=e.clientX-sr.left,localY=e.clientY-sr.top;" "$file"
  run_sudo grep -Fq "Math.min(startLocalX,localX)+'px'" "$file"
  run_sudo grep -Fq "Math.abs(localX-startLocalX)+'px'" "$file"
  run_sudo grep -Fq "S.mode!=='marquee'" "$file"
}
