...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/asset-confirm-refresh-preview-recovery-fix-20260622114424-${STAMP}"
...
check_markers(){
  local file="$1"
  test -f "$file"
  grep -Fq "function assetConfirmLegacyImageEntry" "$file"
  grep -Fq "const ASSET_CONFIRM_NORMALIZE_CACHE_VERSION='20260622-preview-recovery-v2';" "$file"
  grep -Fq "if(!assetConfirmCardNeedsRecoveredImage(card))return false;" "$file"
  grep -Fq "if(imagePreviewUrl(assetConfirmCardImageEntry(card)))" "$file"
  grep -Fq "let task=normalizedTasks.find(task=>assetConfirmTaskHasImageUrl(task)&&assetConfirmTaskCanRecoverCard(card,task));" "$file"
  grep -Fq "saved.objectStorageUrl" "$file"
  grep -Fq "saved.assetPreviewUrl" "$file"
  grep -Fq "assetPreviewUrl','asset_preview_url','objectStorageUrl','object_storage_url'" "$file"
}
...
echo "deployed asset-confirm-refresh-preview-recovery-fix-20260622114424"
