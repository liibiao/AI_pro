...
BACKUP_DIR="$APP_ROOT/.deploy-backups/canvas-single-image-preview-chain-fix-20260624214324-$STAMP"
WORK_DIR="$(mktemp -d)"
...
grep -q "imageNodePreviewDisplayUrl" "$HTML_SRC"
grep -q "bindImagePreviewFallbackChains" "$HTML_SRC"
grep -q "data-preview-fallback-chain" "$HTML_SRC"
...
echo "Installed canvas single-image preview fallback-chain fix."
