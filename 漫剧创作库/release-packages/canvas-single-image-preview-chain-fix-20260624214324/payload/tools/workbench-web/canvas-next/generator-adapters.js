...
export function modelKey(model,index){
  if(model?._selectionKey)return String(model._selectionKey);
  const provider=model?.provider&&typeof model.provider==='object'?model.provider:{};
  const primary=String(model?.identityKey||model?._identityKey||model?.id||model?.configId||model?.modelKey||'').trim();
  const scope=[
    model?.channelKey,model?.providerKey,provider.providerKey,provider.id,
    model?.baseUrl||model?.url,
    model?.adapter,model?.protocol?.adapter,provider.adapter,
    model?.endpointPath,model?.protocol?.endpointPath,provider.endpointPath,
    modelLabel(model)
  ].map(normalizeModelKeyPart).filter(Boolean).join('::');
  if(scope||primary)return ['model',scope,normalizeModelKeyPart(primary)].filter(Boolean).join('::');
  return `${normalizeModelType(model)}-${index}`;
}
