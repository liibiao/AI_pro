...
PKG="smooth-gradient-flow-fix-20260601123903"
...
log "install smooth gradient flow fix"
...
grep -q "\['50%','1'\]" "$DEST"
grep -q "\['93%','.01'\]" "$DEST"
grep -q "pill.setAttribute('width','48')" "$DEST"
grep -q "pill.setAttribute('height','1.5')" "$DEST"
grep -q "motion.setAttribute('dur','5.15s')" "$DEST"
