...
async function submitSeedanceTaskVideo(ctx) {
    ...
    const endpoint = resolveEndpointPath(ctx.model.endpointPath || ctx.provider.endpointPath || '/api/v3/contents/generations/tasks', ctx);
    const upstream = await callUpstreamJson(ctx.provider, endpoint, requestJson, ctx.timeoutMs);
    return normalizeSeedanceTaskSubmitResult(ctx, upstream, requestJson);
}
async function querySeedanceTaskVideo(ctx) {
    const endpoint = resolveEndpointPath(ctx.model.statusEndpointPath || ctx.provider.statusEndpointPath || '/api/v3/contents/generations/tasks/{taskId}', { ...ctx, taskId: ctx.upstreamTaskId });
...
async function buildSeedanceTaskRequestJson(ctx) {
    ...
    return compactJson({
        ...
        ...(imageUrls.length ? { image_urls: imageUrls, image_url: imageUrls[0] } : {}),
        ...(imageUrls.length >= 2 && mode === 'i2v_first_last' ? { end_image_url: imageUrls[1], last_image_url: imageUrls[1] } : {}),
...
async function normalizeSeedanceTaskResult(ctx, upstream, fallbackTaskId = '') {
    ...
    const failed = isSeedanceTaskFailed(statusText);
    ...
    return {
        ...
        errorMessage: failed ? extractSeedanceTaskError(upstream) : (!taskId && !urls.length ? 'Seedance Task 上游响应未包含任务 ID 或视频 URL' : undefined),
    };
}
