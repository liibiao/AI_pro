...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/canvas-multigrid-lightbox-click-persist-fix-20260626025325-${STAMP}"
...
validate_html(){
  local file="$1"
  test -f "$file"
  require_marker "function generatedImageResultMultiGridPatch(...sources)" "$file"
  require_marker "key==='gridCells'&&Array.isArray(val)" "$file"
  require_marker "bar.dataset.multiGridWorkbarBound" "$file"
  require_marker "openPreviewAt(n.id,Number.isInteger(idx)?idx:0,item)" "$file"
  require_marker "lb.shared-preview.animating.multi-grid-lightbox .lb-workbar" "$file"
  require_marker "function renderMultiGridLightboxWorkbar(bar)" "$file"
...
echo "deployed canvas-multigrid-lightbox-click-persist-fix-20260626025325"
