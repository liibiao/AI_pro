...
PKG="mj-smart-reference-actor-role-fix-20260613144104"
...
verify_html_markers(){
  local file="$1"
  grep -Fq "afterHasActorAction" "$file"
  grep -Fq "nearCompositionTarget" "$file"
  grep -Fq "payload.mjPromptImageUrls=imageRefs" "$file"
  grep -Fq "lockedAsCharacterOrStyle" "$file"
}
verify_backend_markers(){
  local file="$1"
  grep -Fq "planned_prompt_images_raw" "$file"
  grep -Fq "extra_params_value" "$file"
}
verify_server_markers(){
  local file="$1"
  grep -Fq "mjPromptImageUrls" "$file"
  grep -Fq "return \"4\" if _midjourney_major_version_from_params" "$file"
}
verify_skill_markers(){
  local file="$1"
  grep -Fq "同一张图默认只承担一个主控制职责" "$file"
  grep -Fq "A 是动作发起者" "$file"
  grep -Fq '角色身份锁定：`--cref`' "$file"
}
verify_html_markers_sudo(){ local file="$1"; run_sudo grep -Fq "afterHasActorAction" "$file"; run_sudo grep -Fq "nearCompositionTarget" "$file"; run_sudo grep -Fq "payload.mjPromptImageUrls=imageRefs" "$file"; run_sudo grep -Fq "lockedAsCharacterOrStyle" "$file"; }
verify_backend_markers_sudo(){ local file="$1"; run_sudo grep -Fq "planned_prompt_images_raw" "$file"; run_sudo grep -Fq "extra_params_value" "$file"; }
verify_server_markers_sudo(){ local file="$1"; run_sudo grep -Fq "mjPromptImageUrls" "$file"; run_sudo grep -Fq "return \"4\" if _midjourney_major_version_from_params" "$file"; }
verify_skill_markers_sudo(){ local file="$1"; run_sudo grep -Fq "同一张图默认只承担一个主控制职责" "$file"; run_sudo grep -Fq "A 是动作发起者" "$file"; run_sudo grep -Fq '角色身份锁定：`--cref`' "$file"; }
