...
function openAiImageGeometryOptions(params: Record<string, unknown>, ctx?: AdapterContext) {
...
function shouldUseGptImage2VariantSizeToken(ctx: AdapterContext) {
  ...
  if (hint.includes('aiyunzhi-gpt-image-2')) return false;
...
function withAiyunzhiGptImage2AspectRatioPrompt(prompt: string, ctx: AdapterContext) {
  const base = String(prompt || '').trim();
  const ratio = resolveAiyunzhiGptImage2AspectRatio(ctx);
  if (!base || !ratio) return base;
  const ratioPattern = escapeRegExp(ratio);
  if (new RegExp(`(?:图片比例|画面比例|比例)\\s*${ratioPattern}(?!\\d)`, 'i').test(base)) return base;
  return `${base}\n图片比例${ratio}`;
}

function resolveAiyunzhiGptImage2AspectRatio(ctx: AdapterContext) {
  const raw = String(firstDefined(
    ctx.params.aspect_ratio,
    ctx.params.aspectRatio,
    ctx.params.requestedRatio,
    ctx.params.requested_ratio,
    ctx.params.ratio,
    ctx.params.requestedPixelSize,
    ctx.params.pixelSize,
    ctx.params.size,
  ) || '').trim().toLowerCase().replace('×', 'x');
  if (!raw || raw === 'auto') return '';
  if (/^\d+(?:\.\d+)?\s*:\s*\d+(?:\.\d+)?$/.test(raw)) return ratioFromSize(raw);
...
function normalizeExplicitAiyunzhiGptImage2Resolution(value: unknown) {
  const raw = String(value || '').trim().toLowerCase().replace(/\s+/g, '').replace('×', 'x');
  if (!raw || raw === 'auto') return '';
  if (/^[1-4]k$/.test(raw)) return normalizeGptImageV2Resolution(raw);
  if (/^(?:uhd|4k|4096|3840|2160p|3k|3072|2880|1800p|qhd|2k|2048|1920|1440p)$/.test(raw)) {
    return normalizeGptImageV2Resolution(raw);
  }
  if (/^\d{3,5}x\d{3,5}$/.test(raw)) return normalizeGptImageV2Resolution(raw);
  return '';
}

function firstAiyunzhiGptImage2Resolution(...values: unknown[]) {
  for (const value of values) {
...
function normalizeAiyunzhiGptImage2PixelSize(value: unknown) {
  const raw = String(value || '').trim().toLowerCase().replace(/\s+/g, '').replace('×', 'x');
  return /^\d{3,5}x\d{3,5}$/.test(raw) ? raw : '';
}

function firstAiyunzhiGptImage2PixelSize(...values: unknown[]) {
  for (const value of values) {
...
function resolveAiyunzhiGptImage2Resolution(ctx: AdapterContext) {
  const defaults = isRecord(ctx.model.defaults) ? ctx.model.defaults : {};
  const providerDefaults = isRecord(ctx.provider.defaultParams) ? ctx.provider.defaultParams : {};
  const resolution = firstAiyunzhiGptImage2Resolution(
    ctx.params.resolution,
    ctx.params.requestedResolution,
    ctx.params.imageSize,
    ctx.params.image_size,
    ctx.params.requestedPixelSize,
    ctx.params.requested_pixel_size,
    ctx.params.pixelSize,
    ctx.params.pixel_size,
    ctx.params.size,
    defaults.resolution,
    defaults.imageSize,
    defaults.image_size,
...
function resolveAiyunzhiGptImage2Size(ctx: AdapterContext) {
  ...
  const explicitPixelSize = firstAiyunzhiGptImage2PixelSize(
    ctx.params.requestedPixelSize,
    ctx.params.requested_pixel_size,
    ctx.params.pixelSize,
    ctx.params.pixel_size,
    ctx.params.size,
    defaults.requestedPixelSize,
    defaults.requested_pixel_size,
    defaults.pixelSize,
    defaults.pixel_size,
    defaults.size,
    providerDefaults.requestedPixelSize,
    providerDefaults.requested_pixel_size,
    providerDefaults.pixelSize,
    providerDefaults.pixel_size,
    providerDefaults.size,
  );
  if (explicitPixelSize && normalizeExplicitAiyunzhiGptImage2Resolution(explicitPixelSize) === resolution) return explicitPixelSize;
...
function resolveGptImage2Resolution(ctx: AdapterContext, requestSize: unknown) {
  ...
  if (isGptImage2ProOfficialProxyRequest(ctx)) return resolution === '1K' ? '1K' : '2K';
  return resolution;
}
...
async function submitAistartLabVideo(ctx: AdapterContext): Promise<AdapterSubmitResult> {
...
async function queryAistartLabVideo(ctx: AdapterContext & { upstreamTaskId: string }): Promise<AdapterQueryResult> {
...
async function buildAistartLabVideoRequestJson(ctx: AdapterContext) {
  const prompt = String(ctx.prompt || '').trim();
  if (!prompt) fail(400, 'AIStartLab 视频生成 prompt 不能为空', 'AISTARTLAB_PROMPT_REQUIRED');
  if (prompt.length > 5000) fail(400, `AIStartLab 视频生成 prompt 最多 5000 字符，当前 ${prompt.length} 字符`, 'AISTARTLAB_PROMPT_TOO_LONG');
...
function resolveAistartLabSubmitEndpointPath(ctx: AdapterContext) {
  const protocol = isRecord(ctx.model.protocol) ? ctx.model.protocol : {};
  const providerDefaults = isRecord(ctx.provider.defaultParams) ? ctx.provider.defaultParams : {};
...
function resolveAistartLabStatusEndpointPath(ctx: AdapterContext) {
  const protocol = isRecord(ctx.model.protocol) ? ctx.model.protocol : {};
  const providerDefaults = isRecord(ctx.provider.defaultParams) ? ctx.provider.defaultParams : {};
  return String(firstDefined(
    ctx.params.statusEndpointPath,
    ...
    '/video/task/status',
  ) || '/video/task/status').trim();
}

function resolveAistartLabChannel(ctx: AdapterContext) {
  const protocol = isRecord(ctx.model.protocol) ? ctx.model.protocol : {};
  const defaults = isRecord(ctx.model.defaults) ? ctx.model.defaults : {};
  const providerDefaults = isRecord(ctx.provider.defaultParams) ? ctx.provider.defaultParams : {};
  const value = firstDefined(
    ctx.params.aistartlabChannel,
    ctx.params.aistartlab_channel,
    ctx.params.upstreamChannel,
    ctx.params.upstream_channel,
    ctx.params.channel,
    protocol.channel,
    protocol.upstreamChannel,
    protocol.upstream_channel,
    defaults.channel,
    defaults.upstreamChannel,
    defaults.upstream_channel,
    providerDefaults.channel,
    providerDefaults.upstreamChannel,
    providerDefaults.upstream_channel,
  );
  const channel = String(value || '').trim();
  if (!channel) fail(400, 'AIStartLab 视频渠道缺少上游 channel，请在模型 protocol.channel 或 defaults.channel 中配置', 'AISTARTLAB_CHANNEL_REQUIRED');
  return channel;
}

function resolveAistartLabModel(ctx: AdapterContext, resolution?: string) {
  const model = resolveAistartLabBaseModel(ctx);
  return assembleAistartLabModelName(ctx, model, resolution);
}

function resolveAistartLabBaseModel(ctx: AdapterContext) {
  const protocol = isRecord(ctx.model.protocol) ? ctx.model.protocol : {};
  const defaults = isRecord(ctx.model.defaults) ? ctx.model.defaults : {};
  const providerDefaults = isRecord(ctx.provider.defaultParams) ? ctx.provider.defaultParams : {};
  const model = String(firstDefined(
    ctx.params.upstreamModel,
    ctx.params.upstream_model,
    ctx.params.model,
    protocol.model,
    protocol.upstreamModel,
    protocol.upstream_model,
    defaults.model,
    providerDefaults.model,
    ctx.model.name,
    ctx.provider.defaultModel,
  ) || '').trim();
  if (!model) fail(400, 'AIStartLab 视频渠道缺少上游 model，请在模型 name/model 或 protocol.model 中配置', 'AISTARTLAB_MODEL_REQUIRED');
  return model;
}

function assembleAistartLabModelName(ctx: AdapterContext, baseModel: string, resolution?: string) {
  const protocol = isRecord(ctx.model.protocol) ? ctx.model.protocol : {};
  const modelAssembly = isRecord(ctx.model.modelAssembly) ? ctx.model.modelAssembly : {};
  const rawMode = String(firstDefined(
    ctx.params.modelNameMode,
    ctx.params.model_name_mode,
    ctx.params.modelAssemblyType,
    ctx.params.model_assembly_type,
    protocol.modelNameMode,
    protocol.model_name_mode,
    modelAssembly.type,
  ) || '').trim().toLowerCase();
  if (!['template', 'resolution', 'resolution-suffix', 'suffix'].includes(rawMode)) return baseModel;
  const normalizedResolution = normalizeAistartLabModelResolution(resolution || ctx.params.resolution || ctx.params.quality);
  if (!normalizedResolution) return baseModel;
  if (new RegExp(`-${escapeRegExp(normalizedResolution)}$`, 'i').test(baseModel)) return baseModel;
  const resolutionModels = firstDefined(
    modelAssembly.resolutionModels,
    modelAssembly.modelsByResolution,
    protocol.resolutionModels,
    protocol.modelsByResolution,
  );
  if (isRecord(resolutionModels)) {
    const mapped = String(firstDefined(
      resolutionModels[normalizedResolution],
      resolutionModels[normalizedResolution.toLowerCase()],
      resolutionModels[normalizedResolution.toUpperCase()],
    ) || '').trim();
    if (mapped) return mapped;
  }
  const template = String(firstDefined(
    ctx.params.modelTemplate,
    ctx.params.model_template,
    protocol.modelTemplate,
    protocol.model_template,
    modelAssembly.template,
    '{model}-{resolution}',
  ) || '{model}-{resolution}').trim() || '{model}-{resolution}';
  return template
...
function normalizeAistartLabModelResolution(value: unknown) {
  const raw = String(value || '').trim().toLowerCase();
  if (!raw) return '';
...
function resolveAistartLabResolution(ctx: AdapterContext) {
...
function resolveAistartLabSeconds(ctx: AdapterContext) {
  const caps = isRecord(ctx.model.capabilities) ? ctx.model.capabilities : {};
  const defaults = isRecord(ctx.model.defaults) ? ctx.model.defaults : {};
  const raw = firstDefined(
    ctx.params.seconds,
    ctx.params.duration,
    ctx.params.durationSeconds,
    ctx.params.duration_seconds,
    defaults.seconds,
    defaults.duration,
  ...
  if (!Number.isFinite(seconds) || seconds <= 0) fail(400, `AIStartLab seconds 必须为正整数，当前为 ${String(raw || '空')}`, 'AISTARTLAB_SECONDS_INVALID');
  ...
  if (seconds < min || seconds > max) fail(400, `AIStartLab seconds 必须在 ${min}-${max} 秒之间，当前 ${seconds} 秒`, 'AISTARTLAB_SECONDS_UNSUPPORTED');
...
function resolveAistartLabSize(ctx: AdapterContext) {
  const caps = isRecord(ctx.model.capabilities) ? ctx.model.capabilities : {};
  const defaults = isRecord(ctx.model.defaults) ? ctx.model.defaults : {};
  const aspectRatios = normalizeStringList(caps.aspectRatios ?? caps.aspect_ratios);
  const size = String(firstDefined(
    ctx.params.size,
    ctx.params.aspectRatio,
    ctx.params.aspect_ratio,
    ctx.params.requestedRatio,
...
function resolveAistartLabModeType(ctx: AdapterContext, images: string[]) {
  const raw = String(firstDefined(
    ctx.params.modeType,
  ...
  if (/frames2video|first[-_ ]?last|start[-_ ]?end|首尾/.test(raw)) return 'frames2video';
  if (/image2video|image[-_ ]?to[-_ ]?video|img2video|i2v|reference[-_ ]?to[-_ ]?video|首帧|参考/.test(raw)) return images.length ? 'image2video' : 'text2video';
  if (/text2video|text[-_ ]?to[-_ ]?video|txt2video|t2v|文生/.test(raw)) return 'text2video';
  return images.length ? 'image2video' : 'text2video';
}

function assertAistartLabVideoRequest(
  ctx: AdapterContext,
  request: { modeType: string; images: string[]; videos: string[]; audios: string[]; seconds: number; size: string; resolution?: string },
) {
  const caps = isRecord(ctx.model.capabilities) ? ctx.model.capabilities : {};
  ...
  if (request.images.length > resolveAistartLabMaxImages(ctx)) fail(400, 'AIStartLab 参考图片最多 9 张', 'AISTARTLAB_TOO_MANY_IMAGES');
  if (request.videos.length > resolveAistartLabMaxVideos(ctx)) fail(400, 'AIStartLab 参考视频最多 3 条', 'AISTARTLAB_TOO_MANY_VIDEOS');
  if (request.audios.length > resolveAistartLabMaxAudios(ctx)) fail(400, 'AIStartLab 参考音频最多 3 条', 'AISTARTLAB_TOO_MANY_AUDIOS');
}

async function collectAistartLabReferenceMediaUrls(ctx: AdapterContext, mediaType: 'video' | 'audio', limit: number) {
...
function resolveAistartLabMaxImages(ctx: AdapterContext) {
  const caps = isRecord(ctx.model.capabilities) ? ctx.model.capabilities : {};
  const maxImages = caps.maxImages;
  if (isRecord(maxImages)) return normalizePositiveInt(maxImages.full ?? maxImages.max ?? maxImages.default) || 9;
  return normalizePositiveInt(caps.maxImages ?? caps.max_images) || 9;
}

function resolveAistartLabMaxVideos(ctx: AdapterContext) {
  const caps = isRecord(ctx.model.capabilities) ? ctx.model.capabilities : {};
  return normalizePositiveInt(caps.maxVideos ?? caps.max_videos) || 3;
}

function resolveAistartLabMaxAudios(ctx: AdapterContext) {
  const caps = isRecord(ctx.model.capabilities) ? ctx.model.capabilities : {};
  return normalizePositiveInt(caps.maxAudios ?? caps.max_audios) || 3;
}

async function normalizeAistartLabVideoTaskResult(
  ctx: AdapterContext,
  upstream: unknown,
  requestJson: Prisma.InputJsonValue,
...
async function normalizeAistartLabVideoTaskResult(
  ...
  requestJson?: Prisma.InputJsonValue,
  fallbackTaskId = '',
): Promise<AdapterSubmitResult | AdapterQueryResult> {
  ...
  const base = {
    status: failed || missingDoneResult ? 'FAILED' as const : (done ? 'SUCCESS' as const : (taskId ? 'RUNNING' as const : 'FAILED' as const)),
    progress,
    upstreamTaskId: taskId,
...
function aistartLabStatusText(statusCode: number) {
  if (statusCode === 1) return 'created';
...
function normalizeAistartLabProgress(value: unknown, statusCode: number, statusText: string, urlCount: number) {
  const explicit = normalizeGenericProgressValue(value);
  if (explicit !== null) return explicit;
  if (statusCode === 3 || urlCount > 0) return 100;
  if (statusCode === 4) return 100;
  if (statusCode === 2) return 35;
  if (statusCode === 1) return 8;
