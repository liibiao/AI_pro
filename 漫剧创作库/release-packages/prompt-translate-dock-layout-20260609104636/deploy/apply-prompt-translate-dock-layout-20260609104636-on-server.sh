...
PKG="prompt-translate-dock-layout-20260609104636"
...
verify_html_markers(){
  ...
  grep -Fq '.prompt-translate-plain-row{' "$file"
  grep -Fq 'function renderPromptTranslateDock(n)' "$file"
  grep -Fq 'renderPromptTranslateToggle(n,{compact:true})' "$file"
  grep -Fq "translateDock?' has-translate-dock':''" "$file"
  grep -Fq '<b class="prompt-translate-cost">${PROMPT_TRANSLATE_COST}</b>' "$file"
  grep -Fq '<div class="prompt-translate-stack-spacer">${renderPromptTranslateDock(n)}</div>' "$file"
  if grep -Fq '${renderPromptTranslateToggle(n)}' "$file"; then
    fail "legacy bottom-bar prompt translate toggle still present in $file"
  fi
}
verify_html_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq '.prompt-translate-dock{' "$file"
  run_sudo grep -Fq '.prompt-translate-plain-row{' "$file"
  run_sudo grep -Fq 'function renderPromptTranslateDock(n)' "$file"
  run_sudo grep -Fq 'renderPromptTranslateToggle(n,{compact:true})' "$file"
  run_sudo grep -Fq "translateDock?' has-translate-dock':''" "$file"
  run_sudo grep -Fq '<b class="prompt-translate-cost">${PROMPT_TRANSLATE_COST}</b>' "$file"
  run_sudo grep -Fq '<div class="prompt-translate-stack-spacer">${renderPromptTranslateDock(n)}</div>' "$file"
  if run_sudo grep -Fq '${renderPromptTranslateToggle(n)}' "$file"; then
    fail "legacy bottom-bar prompt translate toggle still present in $file"
  fi
}
