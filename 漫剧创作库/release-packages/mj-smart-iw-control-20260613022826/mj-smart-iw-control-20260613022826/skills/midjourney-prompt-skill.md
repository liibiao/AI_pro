...
- 图生图的参考图权重 `--iw` 由画布智能模式下拉控制，是用户确定参数；不要根据画面内容自行改大或改小，也不要输出与当前选择冲突的 `--iw`。
...
```json
{
  ...
  "params": "--ar 16:9 --v 8.1 --style raw --s 100 --c 6 --q 2 --hd",
  "aspectRatio": "16:9",
  "qualityProfile": "high",
...
- 图生图参考强度：`--iw` 只使用画布传入的当前选择；低值更听文字，高值更贴近参考图。
