...
PKG="canvas-storyboard-selected-pill-bottom-right-20260604114018"
...
verify_markers(){
  local dest="$1"
  grep -Fq ".node.node-type-storyboardImage .storygrid-topline{position:absolute!important;inset:0!important" "$dest"
  grep -Fq ".node.node-type-storyboardImage .storygrid-topline span{position:absolute!important;left:8px!important;top:8px!important}" "$dest"
