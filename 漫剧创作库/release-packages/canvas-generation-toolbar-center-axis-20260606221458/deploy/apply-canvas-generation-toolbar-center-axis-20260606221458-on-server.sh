...
PKG="canvas-generation-toolbar-center-axis-20260606221458"
...
verify_html(){
  local file="$1"
  grep -Fq "tapnow-rewrite.css?v=20260606-toolbar-center-anchor" "$file"
  grep -Fq "final center-axis correction for image-generation prompt toolbars" "$file"
  grep -Fq "generation node bottom bars are centered as row boxes" "$file"
  grep -Fq "margin:10px 0 0!important" "$file"
  grep -Fq -- "--node-toolbar-center-shift" "$file"
  grep -Fq "node-type-shotStoryboard .shot-storyboard-local-bar" "$file"
}
verify_css(){
  local file="$1"
  grep -Fq "20260606: final node bottom toolbar layout" "$file"
  grep -Fq "20260606: storyboard-specific hard override" "$file"
  grep -Fq "generation node bottom bars are centered as row boxes" "$file"
  grep -Fq -- "--node-toolbar-center-shift" "$file"
...
verify_html_sudo(){
  local file="$1"
  run_sudo grep -Fq "tapnow-rewrite.css?v=20260606-toolbar-center-anchor" "$file"
  run_sudo grep -Fq "final center-axis correction for image-generation prompt toolbars" "$file"
  run_sudo grep -Fq "generation node bottom bars are centered as row boxes" "$file"
  run_sudo grep -Fq "margin:10px 0 0!important" "$file"
  run_sudo grep -Fq -- "--node-toolbar-center-shift" "$file"
  run_sudo grep -Fq "node-type-shotStoryboard .shot-storyboard-local-bar" "$file"
}
verify_css_sudo(){
  local file="$1"
  run_sudo grep -Fq "20260606: final node bottom toolbar layout" "$file"
  run_sudo grep -Fq "20260606: storyboard-specific hard override" "$file"
  run_sudo grep -Fq "generation node bottom bars are centered as row boxes" "$file"
  run_sudo grep -Fq -- "--node-toolbar-center-shift" "$file"
