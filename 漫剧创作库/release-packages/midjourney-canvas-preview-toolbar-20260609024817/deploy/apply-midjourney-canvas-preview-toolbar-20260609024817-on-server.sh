...
PKG="midjourney-canvas-preview-toolbar-20260609024817"
...
verify_markers(){
  local file="$1"
  grep -Fq ".mj-toolbar" "$file"
  grep -Fq "MIDJOURNEY_EXTENSION_TASKS" "$file"
  grep -Fq "renderMidjourneyPreview" "$file"
  grep -Fq "appendMidjourneyPromptParams" "$file"
  grep -Fq "MidjourneyCanvasBridge" "$file"
}

verify_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq ".mj-toolbar" "$file"
  run_sudo grep -Fq "MIDJOURNEY_EXTENSION_TASKS" "$file"
  run_sudo grep -Fq "renderMidjourneyPreview" "$file"
  run_sudo grep -Fq "appendMidjourneyPromptParams" "$file"
  run_sudo grep -Fq "MidjourneyCanvasBridge" "$file"
}
