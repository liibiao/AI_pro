...
PKG="canvas-multiselect-text-node-autofill-20260604211533"
...
verify_markers(){
  local dest="$1"
  grep -Fq "textPrompt:{cat:'input',name:'文本节点'" "$dest"
  grep -Fq "orderedMultiSelectSourceIds" "$dest"
  grep -Fq "connectMultiSelectedNodesToTarget" "$dest"
  grep -Fq "syncSelectedTextPromptsToTarget" "$dest"
  grep -Fq "maybeAutofillEditablePromptFromTextPrompt(promptIds[0],toId,{rerender,forceEmpty:true})" "$dest"
  grep -Fq "if(!source)return toast('请先在文本节点输入或生成内容','warn')" "$dest"
}
