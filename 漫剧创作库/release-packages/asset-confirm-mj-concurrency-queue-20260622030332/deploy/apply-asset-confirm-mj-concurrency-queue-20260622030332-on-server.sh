...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/asset-confirm-mj-concurrency-queue-20260622030332-${STAMP}"
...
check_markers(){
  local file="$1"
  test -f "$file"
  grep -Fq "function assetConfirmGenerationDisplayProgress(state)" "$file"
  grep -Fq "if(state?.status==='queued')return 0;" "$file"
  grep -Fq "const showMask=!!(state.active||state.failed);" "$file"
  grep -Fq "card.generation={status:'queued',progress:0,label:'准备中'" "$file"
  grep -Fq "const mjConcurrency=Math.min(4,Math.max(1,mjQueue.length));" "$file"
  grep -Fq "await Promise.all(Array.from({length:mjConcurrency},mjWorker));" "$file"
}
...
echo "deployed asset-confirm-mj-concurrency-queue-20260622030332"
