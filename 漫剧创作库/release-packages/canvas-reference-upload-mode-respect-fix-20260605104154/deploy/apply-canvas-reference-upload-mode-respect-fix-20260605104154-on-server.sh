...
PKG="canvas-reference-upload-mode-respect-fix-20260605104154"
...
verify_canvas_markers(){
  ...
  grep -Fq "if(String(item?.kind||'')==='text')return '';" "$file"
  grep -Fq "if(kind==='text')return false;" "$file"
  grep -Fq "const REFERENCE_IMAGE_UPLOAD_TIMEOUT_MS=300000" "$file"
  grep -Fq "function referenceImageUploadTimeoutMessage" "$file"
  grep -Fq "async function waitReferenceImageUploadPromise" "$file"
  grep -Fq "if(n.type==='shotStoryboard')return shotStoryboardReferencedImages(n,uploadMode);" "$file"
  grep -Fq '等待${label}上传' "$file"
  grep -Fq "function promptImageEntryHasPreview" "$file"
  grep -Fq "function promptImageEntriesFromNodeData" "$file"
  grep -Fq "function refreshLoadedPromptReferenceConsumers" "$file"
  grep -Fq "refreshLoadedPromptReferenceConsumers();" "$file"
  grep -Fq "'assetDesign','textPrompt','shotStoryboard','storyboardImage'" "$file"
  grep -Fq "function chat(payload,options={})" "$file"
  grep -Fq "if(isTemporaryGenerationResultUrl(ref))return false;" "$file"
  grep -Fq "function isInvalidGenerationInputFileRef" "$file"
  grep -Fq "function assertGenerationInputFilesReady" "$file"
  grep -Fq "已阻止提交，避免参考图失效" "$file"
  grep -Fq "assertGenerationInputFilesReady(taskInputFiles,'分镜图参考图')" "$file"
  grep -Fq "function getExecutionReferenceUploadMode(n)" "$file"
  grep -Fq "return getNodeImageUploadMode(n);" "$file"
  grep -Fq "const targetUploadMode=conn.kind==='image'&&target?.type!=='seedanceVideo'?getExecutionReferenceUploadMode(target):'object_storage';" "$file"
  grep -Fq "if(targetUploadMode!=='object_storage')return;" "$file"
  grep -Fq "已上传 \${count} 张参考图到 \${imageUploadModeLabel(getExecutionReferenceUploadMode(n))}" "$file"
  grep -Fq "已导入参考图 \${added} 张，后台上传 \${imageUploadModeLabel(getExecutionReferenceUploadMode(n))}" "$file"
  grep -Fq "上传超过 \${Math.round(timeoutMs/1000)} 秒未完成" "$file"
  grep -Fq "if(isServerGenerationResultUrl(ref))return true;" "$file"
  grep -Fq "return !!serverPublicAssetUrl(ref)||isServerGenerationResultUrl(ref)" "$file"
  grep -Fq "const SHOT_STORYBOARD_CHAT_TIMEOUT_MS=900000" "$file"
  grep -Fq "function canvasAccountChatWithTimeout" "$file"
  grep -Fq "fetchWithTimeout(dataUrl,{},18000)" "$file"
  grep -Fq "参考图已就绪，分镜表推演中" "$file"
  grep -Fq "提交分镜图生成任务" "$file"
  grep -Fq "等待分镜图生成任务" "$file"
  grep -Fq "label:'推演资产+分镜图'" "$file"
  grep -Fq "shortLabel:'推演资产+分镜图'" "$file"
  grep -Fq "function estimateAssetDeriveAssetCount" "$file"
  grep -Fq "function estimateAssetDeriveAssetImageCredits" "$file"
  grep -Fq "function estimateAssetDeriveShotStoryboardCredits" "$file"
  grep -Fq "function estimateAssetDeriveVideoCredits" "$file"
  grep -Fq "if(assetDeriveFlowUsesShot(flow.id))parts.push(estimateAssetDeriveShotStoryboardCredits(source));" "$file"
  grep -Fq "if(assetDeriveFlowNeedsVideo(flow.id))" "$file"
  grep -Fq "const SHOT_STORYBOARD_TABLE_CHAT_TIMEOUT_MS=150000" "$file"
  grep -Fq "const SHOT_STORYBOARD_REPAIR_CHAT_TIMEOUT_MS=90000" "$file"
  grep -Fq "function buildShotStoryboardFallbackTable" "$file"
  grep -Fq "function shouldFallbackShotStoryboardTableError" "$file"
  grep -Fq "function startShotStoryboardTableWaitProgress" "$file"
  grep -Fq "if(!shouldFallbackShotStoryboardTableError(err))throw err;" "$file"
  grep -Fq "reason:'shot-storyboard-table-timeout'" "$file"
  grep -Fq "reason:'shot-count-repair-fallback'" "$file"
  grep -Fq "分镜表上游超时，已启用本地分镜表草案继续生成分镜图" "$file"
}
...
verify_canvas_markers_sudo(){
  ...
  run_sudo grep -Fq "filter(item=>String(item?.kind||'file')!=='text')" "$file"
  run_sudo grep -Fq "if(String(item?.kind||'')==='text')return '';" "$file"
  run_sudo grep -Fq "if(kind==='text')return false;" "$file"
  run_sudo grep -Fq "const REFERENCE_IMAGE_UPLOAD_TIMEOUT_MS=300000" "$file"
  run_sudo grep -Fq "function referenceImageUploadTimeoutMessage" "$file"
  run_sudo grep -Fq "async function waitReferenceImageUploadPromise" "$file"
  run_sudo grep -Fq "if(n.type==='shotStoryboard')return shotStoryboardReferencedImages(n,uploadMode);" "$file"
  run_sudo grep -Fq '等待${label}上传' "$file"
  run_sudo grep -Fq "function promptImageEntryHasPreview" "$file"
  run_sudo grep -Fq "function promptImageEntriesFromNodeData" "$file"
  run_sudo grep -Fq "function refreshLoadedPromptReferenceConsumers" "$file"
  run_sudo grep -Fq "refreshLoadedPromptReferenceConsumers();" "$file"
  run_sudo grep -Fq "'assetDesign','textPrompt','shotStoryboard','storyboardImage'" "$file"
  run_sudo grep -Fq "function chat(payload,options={})" "$file"
  run_sudo grep -Fq "if(isTemporaryGenerationResultUrl(ref))return false;" "$file"
  run_sudo grep -Fq "function isInvalidGenerationInputFileRef" "$file"
  run_sudo grep -Fq "function assertGenerationInputFilesReady" "$file"
  run_sudo grep -Fq "已阻止提交，避免参考图失效" "$file"
  run_sudo grep -Fq "assertGenerationInputFilesReady(taskInputFiles,'分镜图参考图')" "$file"
  run_sudo grep -Fq "function getExecutionReferenceUploadMode(n)" "$file"
  run_sudo grep -Fq "return getNodeImageUploadMode(n);" "$file"
  run_sudo grep -Fq "const targetUploadMode=conn.kind==='image'&&target?.type!=='seedanceVideo'?getExecutionReferenceUploadMode(target):'object_storage';" "$file"
  run_sudo grep -Fq "if(targetUploadMode!=='object_storage')return;" "$file"
  run_sudo grep -Fq "已上传 \${count} 张参考图到 \${imageUploadModeLabel(getExecutionReferenceUploadMode(n))}" "$file"
  run_sudo grep -Fq "已导入参考图 \${added} 张，后台上传 \${imageUploadModeLabel(getExecutionReferenceUploadMode(n))}" "$file"
  run_sudo grep -Fq "上传超过 \${Math.round(timeoutMs/1000)} 秒未完成" "$file"
  run_sudo grep -Fq "if(isServerGenerationResultUrl(ref))return true;" "$file"
  run_sudo grep -Fq "return !!serverPublicAssetUrl(ref)||isServerGenerationResultUrl(ref)" "$file"
  run_sudo grep -Fq "const SHOT_STORYBOARD_CHAT_TIMEOUT_MS=900000" "$file"
  run_sudo grep -Fq "function canvasAccountChatWithTimeout" "$file"
  run_sudo grep -Fq "fetchWithTimeout(dataUrl,{},18000)" "$file"
  run_sudo grep -Fq "参考图已就绪，分镜表推演中" "$file"
  run_sudo grep -Fq "提交分镜图生成任务" "$file"
  run_sudo grep -Fq "等待分镜图生成任务" "$file"
  run_sudo grep -Fq "label:'推演资产+分镜图'" "$file"
  run_sudo grep -Fq "shortLabel:'推演资产+分镜图'" "$file"
  run_sudo grep -Fq "function estimateAssetDeriveAssetCount" "$file"
  run_sudo grep -Fq "function estimateAssetDeriveAssetImageCredits" "$file"
  run_sudo grep -Fq "function estimateAssetDeriveShotStoryboardCredits" "$file"
  run_sudo grep -Fq "function estimateAssetDeriveVideoCredits" "$file"
  run_sudo grep -Fq "if(assetDeriveFlowUsesShot(flow.id))parts.push(estimateAssetDeriveShotStoryboardCredits(source));" "$file"
  run_sudo grep -Fq "if(assetDeriveFlowNeedsVideo(flow.id))" "$file"
  run_sudo grep -Fq "const SHOT_STORYBOARD_TABLE_CHAT_TIMEOUT_MS=150000" "$file"
  run_sudo grep -Fq "const SHOT_STORYBOARD_REPAIR_CHAT_TIMEOUT_MS=90000" "$file"
  run_sudo grep -Fq "function buildShotStoryboardFallbackTable" "$file"
  run_sudo grep -Fq "function shouldFallbackShotStoryboardTableError" "$file"
  run_sudo grep -Fq "function startShotStoryboardTableWaitProgress" "$file"
  run_sudo grep -Fq "if(!shouldFallbackShotStoryboardTableError(err))throw err;" "$file"
  run_sudo grep -Fq "reason:'shot-storyboard-table-timeout'" "$file"
  run_sudo grep -Fq "reason:'shot-count-repair-fallback'" "$file"
  run_sudo grep -Fq "分镜表上游超时，已启用本地分镜表草案继续生成分镜图" "$file"
}
...
verify_bridge_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "input.timeoutMs || input.upstreamTimeoutMs || input.requestTimeoutMs || input.queryTimeoutMs || 60000" "$file"
}
...
if run_sudo test -f "$MIRROR_TARGET/smart-vision/app/scripts/bridge-server.mjs"; then
  verify_bridge_markers_sudo "$MIRROR_TARGET/smart-vision/app/scripts/bridge-server.mjs"
fi
