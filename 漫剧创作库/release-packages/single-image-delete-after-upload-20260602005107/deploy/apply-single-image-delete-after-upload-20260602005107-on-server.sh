...
PKG="single-image-delete-after-upload-20260602005107"
...
verify_markers(){
  ...
  grep -Fq "if(S.selected===n.id)selectNode(n.id);" "$dest"
  grep -Fq "if(type==='file'||type==='hidden'||a.hidden||a.style?.display==='none')return false;" "$dest"
