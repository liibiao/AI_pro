...
PKG="midjourney-canvas-next-20260608021205"
...
verify_canvas_markers(){
  local file="$1"
  grep -Fq "function isMidjourneyImageAdapter" "$file"
  grep -Fq "midjourney-imagine" "$file"
  grep -Fq "/mj/submit/imagine" "$file"
  grep -Fq "/mj/task/{taskId}/fetch" "$file"
  grep -Fq "payload.botType" "$file"
  grep -Fq "payload.speedMode" "$file"
  grep -Fq "payload.statusEndpointPath" "$file"
  grep -Fq "imageAdapterRequiresObjectStorage" "$file"
  grep -Fq "isMidjourneyImageAdapter(adapter)" "$file"
}

verify_canvas_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "function isMidjourneyImageAdapter" "$file"
  run_sudo grep -Fq "midjourney-imagine" "$file"
  run_sudo grep -Fq "/mj/submit/imagine" "$file"
  run_sudo grep -Fq "/mj/task/{taskId}/fetch" "$file"
  run_sudo grep -Fq "payload.botType" "$file"
  run_sudo grep -Fq "payload.speedMode" "$file"
  run_sudo grep -Fq "payload.statusEndpointPath" "$file"
  run_sudo grep -Fq "imageAdapterRequiresObjectStorage" "$file"
  run_sudo grep -Fq "isMidjourneyImageAdapter(adapter)" "$file"
}
...
if [ -d "$RUNTIME_ROOT/tools/workbench-web" ]; then
  log "install runtime workbench mirror: $RUNTIME_ROOT/tools/workbench-web"
...
if run_sudo test -f "$RUNTIME_ROOT/tools/workbench-web/image-studio-canvas-next.html"; then
  verify_canvas_markers_sudo "$RUNTIME_ROOT/tools/workbench-web/image-studio-canvas-next.html"
fi
