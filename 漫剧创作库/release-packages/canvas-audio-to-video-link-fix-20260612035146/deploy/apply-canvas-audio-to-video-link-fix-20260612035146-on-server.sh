...
PKG="canvas-audio-to-video-link-fix-20260612035146"
...
verify_canvas_markers(){
  local file="$1"
  grep -Fq "if((n?.type==='singleAudio'||n?.type==='singleVideo')&&(kind==='audio'||kind==='video'))preferred.push('seedanceVideo')" "$file"
  grep -Fq "const mediaSpecial=(n.type==='singleAudio'||n.type==='singleVideo')&&(kind==='audio'||kind==='video')" "$file"
  grep -Fq "if((n.type==='singleAudio'||n.type==='singleVideo')&&(first.kind==='audio'||first.kind==='video'))return showMediaDownstreamPopup(id,first,opts)" "$file"
  grep -Fq "function showMediaDownstreamPopup(id,outPort,opts={})" "$file"
  grep -Fq "if(kind==='audio')return ['seedanceVideo','videoEditor','shotStoryboard','singleAudio']" "$file"
  grep -Fq "const addTarget=stack.map(el=>el.closest?.('.node-add,#selBboxBtn')).find(Boolean)||target?.closest?.('.node-add,#selBboxBtn')" "$file"
}
