...
PKG="canvas-asset-library-mj-tab-20260613123358"
...
verify_canvas_markers(){
  local file="$1"
  grep -Fq "['mj','MJ']" "$file"
  grep -Fq "['image:mj','MJ']" "$file"
  grep -Fq "function isMidjourneyAssetTask(task)" "$file"
  grep -Fq "if(type==='IMAGE'&&isMidjourneyAssetTask(task))return 'mj';" "$file"
  grep -Fq "if(isMidjourneyAssetTask(task))return 'mj';" "$file"
}
