...
import {installCanvasTabletTouchAdapter} from './tablet-touch-adapter.js';
...
function bindEvents(){
  installCanvasTabletTouchAdapter({stage,world,svg,state,drawConnections,saveWorkflow,setSelected,screenToWorld,addNode,render});
...
function onInput(e){
  ...
  if(el.dataset.modeField){
    ...
    const field=el.dataset.modeField;
    n.values.modeValues[mode][field]=valueOf(el);
    syncImageModeSize(n.values.modeValues[mode],mode,field);
...
function syncImageModeSize(values,mode,changedField){
  if(!['txt2img','img2img','storyboard'].includes(mode))return;
  if(!['aspectRatio','resolution'].includes(changedField))return;
  values.size=imageSizeFromRatioResolution(values.aspectRatio,values.resolution,values.size||'1024x1024');
}

function imageSizeFromRatioResolution(aspectRatio,resolution,fallback){
  const ratio=String(aspectRatio||'').trim().toLowerCase();
  const res=String(resolution||'1K').trim().toUpperCase();
  const supported={
  ...
  return supported[res]?.[ratio]||supported['1K'][ratio]||fallback||'1024x1024';
}
...
function onPointerDown(e){
  const blankTouchTarget=e.target===stage||e.target===world||e.target===svg||(
    e.target?.closest&&stage.contains(e.target)&&!e.target.closest('.node,.node-picker,.conn-target-popup,.float,.sel-bbox,.sel-toolbar,button,input,textarea,select,label,a,[data-action]')
  );
  if(blankTouchTarget&&(e.pointerType==='touch'||e.pointerType===''||e.pointerType==null)&&window.__canvasIsTouchViewport?.()){
    e.preventDefault();
    e.stopPropagation();
    return;
  }
...
function setProgress(node,message){
  ...
  if(Number.isFinite(exact)){
    node.progress=Math.min(99,Math.max(8,Number(node.progress||0),exact));
  }else{
...
function imageOutputUrl(entry){
  ...
  return entry.url||entry.dataUrl||inlineUrl||entry.previewUrl||entry.localUrl||entry.remoteUrl||entry.objectStorageUrl||entry.publicUrl||entry.resultUrl||entry.outputUrl||entry.outputImageUrl||saved.localUrl||saved.url||saved.remoteUrl||saved.assetPreviewUrl||saved.objectStorageUrl||saved.publicUrl||saved.resultUrl||saved.outputUrl||saved.outputImageUrl||'';
}

function imageOutputImages(out){
  if(!out)return [];
  const list=Array.isArray(out.images)&&out.images.length?out.images:[out];
...
function propagateOutputs(fromId,seen=new Set()){
  ...
  state.conns.filter(c=>c.from===fromId).forEach(c=>{
    ...
    if(target.type==='singleImage'&&out){
      ...
      target.data={...out,images,url:imageOutputUrl(out)||imageOutputUrl(images[0])};
