...
PKG="canvas-toolbar-gap-5px-20260606125019"
...
verify_canvas_markers(){
  ...
  grep -Fq "flex:0 0 5px!important;" "$file"
  grep -Fq ":is(select,input[type=\"number\"],input[type=\"range\"],input[type=\"color\"]).tn-native-control" "$file"
  grep -Fq "position:absolute!important;" "$file"
