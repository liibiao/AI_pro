...
PKG="storyboard-toolbar-compact-grouped-20260607105940"
...
verify_markers(){
  ...
  grep -Fq "height:50px!important;" "$file"
  grep -Fq "width:106px!important;" "$file"
  grep -Fq "select[data-field=\"frameAspect\"] + .tn-custom-control" "$file"
  grep -Fq "select[data-field=\"cutCount\"] + .tn-custom-control" "$file"
  grep -Fq ".node.node-type-storyboard .aio-layout>.aio-bar:not(.prompt-zoomed-item)>.node-toolbar-right" "$file"
  grep -Fq "function nodeToolbarMinWidthScale(bar){" "$file"
  grep -Fq "node?.classList?.contains('node-type-storyboard')&&bar?.matches?.('.aio-bar')" "$file"
  grep -Fq "return 1.1;" "$file"
}
verify_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "20260607: keep storyboard bottom toolbar compact, separated, and grouped." "$file"
  run_sudo grep -Fq "height:50px!important;" "$file"
  run_sudo grep -Fq "width:106px!important;" "$file"
  run_sudo grep -Fq "select[data-field=\"frameAspect\"] + .tn-custom-control" "$file"
  run_sudo grep -Fq "select[data-field=\"cutCount\"] + .tn-custom-control" "$file"
  run_sudo grep -Fq ".node.node-type-storyboard .aio-layout>.aio-bar:not(.prompt-zoomed-item)>.node-toolbar-right" "$file"
  run_sudo grep -Fq "function nodeToolbarMinWidthScale(bar){" "$file"
  run_sudo grep -Fq "node?.classList?.contains('node-type-storyboard')&&bar?.matches?.('.aio-bar')" "$file"
  run_sudo grep -Fq "return 1.1;" "$file"
}
