...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/canvas-multigrid-refresh-aspect-fix-20260626110415-${STAMP}"
...
validate_html(){
  local file="$1"
  test -f "$file"
  require_marker "function multiGridSourceAspectFromEntry(entry)" "$file"
  require_marker "function multiGridPreviewAspect(entries,nodeOrId=null)" "$file"
  require_marker "_multiGridSourceRatio:sourceRatio||undefined" "$file"
  require_marker "entry.multiGridSourceRatio=sourceRatio||entry.multiGridSourceRatio;" "$file"
  require_marker "const nodeRatio=generationRatioFromValueBag(node?.values)||generationRatioFromValueBag(node?.data)||Number(node?.cardRatio||0)||0;" "$file"
  require_marker "multiGridPreviewAspect(previewList,n)" "$file"
}
...
echo "deployed canvas-multigrid-refresh-aspect-fix-20260626110415"
