...
PKG="canvas-preview-progress-glass-scope-20260604143122"
...
verify_markers(){
  ...
  grep -Fq -- ".preview-card>.gen-progress-mask" "$dest"
  grep -Fq -- ".aio-preview>.gen-progress-mask" "$dest"
  grep -Fq -- ".vn2-preview>.gen-progress-mask" "$dest"
  grep -Fq -- ".storygrid-panel>.storygrid-shot-progress" "$dest"
  grep -Fq -- ".aio-preview>.asset-card-derive-progress" "$dest"
  grep -Fq -- "prompt-run-progress" "$dest"
  grep -Fq -- ".node.node-type-shotStoryboard .shot-run-mask{display:none!important}" "$dest"
  grep -Fq -- "backdrop-filter:blur(16px) saturate(1.16)" "$dest"
  grep -Fq -- "backdrop-filter:blur(18px) saturate(1.18)" "$dest"
  grep -Fq -- "const genMask=el.querySelector('.preview-card>.gen-progress-mask:not([data-pano-upload-mask])" "$dest"
