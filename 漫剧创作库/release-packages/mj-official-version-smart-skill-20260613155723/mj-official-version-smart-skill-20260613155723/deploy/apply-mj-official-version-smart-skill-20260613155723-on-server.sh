...
PKG="mj-official-version-smart-skill-20260613155723"
...
verify_html_markers(){
  ...
  grep -Fq "V8.1 不允许输出 --cref/--cw/--oref/--ow" "$file"
}
...
verify_html_markers_sudo(){ local file="$1"; run_sudo grep -Fq "function midjourneyReferenceCompatibility" "$file"; run_sudo grep -Fq "smartMidjourneyEffectiveImagePromptUrls" "$file"; run_sudo grep -Fq "characterMode==='oref'" "$file"; run_sudo grep -Fq "midjourneyNodeSupportsImageReferenceControls(n)&&!isSmart" "$file"; run_sudo grep -Fq "V8.1 不允许输出 --cref/--cw/--oref/--ow" "$file"; }
verify_skill_markers_sudo(){ local file="$1"; run_sudo grep -Fq "官方版本兼容硬规则" "$file"; run_sudo grep -Fq 'V7 用 `--oref --ow`' "$file"; run_sudo grep -Fq "图生图的参考图权重" "$file"; }
verify_backend_markers_sudo(){ local file="$1"; run_sudo grep -Fq "_sanitize_midjourney_prompt_for_version" "$file"; run_sudo grep -Fq "prompt_urls_to_prefix" "$file"; run_sudo grep -Fq -- "--oref {cref_url} --ow 100" "$file"; }
