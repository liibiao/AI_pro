...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `botType` | string | 默认 `MID_JOURNEY`；当 prompt 使用 `--niji` 或画布选择 Niji 版本时自动提交 `NIJI_JOURNEY`。 |
...
| `botType` | 默认 `MID_JOURNEY`；Niji 版本自动使用 `NIJI_JOURNEY`。 |
...
- Niji 7：会移除 `--cref` / `--oref` / `--cw` / `--ow`，并把非法 `--q` 限制为 `--q 1`；请求体会使用 `botType: NIJI_JOURNEY`，避免把 Niji prompt 误投到 `MID_JOURNEY` bot。
