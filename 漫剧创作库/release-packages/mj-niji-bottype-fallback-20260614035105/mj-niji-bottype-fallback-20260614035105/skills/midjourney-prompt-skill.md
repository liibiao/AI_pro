...
- 官方版本兼容硬规则：`--v 8.1` 不输出 `--q --cref --cw --oref --ow`；`--v 7` 角色一致用 `--oref --ow`，不用 `--cref --cw`；`--v 6` / `--v 6.1` / `--niji 6` 才用 `--cref --cw`；`--niji 7` 当前通道请求体使用 `botType: NIJI_JOURNEY`，不自动输出 `--raw --c --chaos --hd` 和角色引用参数，质量固定 `--q 1`。
