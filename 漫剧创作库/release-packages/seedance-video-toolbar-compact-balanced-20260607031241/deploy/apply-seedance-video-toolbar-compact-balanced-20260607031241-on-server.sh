...
PKG="seedance-video-toolbar-compact-balanced-20260607031241"
...
verify_markers(){
  local file="$1"
  grep -Fq "20260607: keep video generation bottom toolbar compact-balanced and separated from the prompt panel." "$file"
  grep -Fq ".node.node-type-seedanceVideo .vn2-layout>.vn2-bar:not(.prompt-zoomed-item){" "$file"
  grep -Fq "height:36px!important;" "$file"
  grep -Fq "gap:3px!important;" "$file"
  grep -Fq "height:20px!important;" "$file"
  grep -Fq "width:94px!important;" "$file"
  grep -Fq "width:35px!important;" "$file"
  grep -Fq "width:28px!important;" "$file"
  grep -Fq "transform:translateX(8px)!important;" "$file"
}
verify_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "20260607: keep video generation bottom toolbar compact-balanced and separated from the prompt panel." "$file"
  run_sudo grep -Fq ".node.node-type-seedanceVideo .vn2-layout>.vn2-bar:not(.prompt-zoomed-item){" "$file"
  run_sudo grep -Fq "height:36px!important;" "$file"
  run_sudo grep -Fq "gap:3px!important;" "$file"
  run_sudo grep -Fq "height:20px!important;" "$file"
  run_sudo grep -Fq "width:94px!important;" "$file"
  run_sudo grep -Fq "width:35px!important;" "$file"
  run_sudo grep -Fq "width:28px!important;" "$file"
  run_sudo grep -Fq "transform:translateX(8px)!important;" "$file"
}
