...
PKG="minimal-connection-flow-fix-20260601115417"
...
log "install minimal connection flow fix"
install -m "$DEST_MODE" "$SRC" "$DEST"
chown "$DEST_OWNER" "$DEST"

log "verify markers"
grep -q "connMinimalFlow" "$DEST"
grep -q "conn-flow-fade" "$DEST"
grep -q "conn-flow-mid" "$DEST"
grep -q "conn-flow-peak" "$DEST"
grep -q "stroke-width:1.55" "$DEST"
grep -q "\['conn-flow-fade','conn-flow-mid','conn-flow-peak'\]" "$DEST"
