...
ARCHIVE="${1:-/tmp/prompt-zoom-ref-control-i2i-style-20260601010953.tar.gz}"
TARGET="${2:-/home/ubuntu/漫剧创作库}"
STAMP="$(date +%Y%m%d%H%M%S)"
WORK="$(mktemp -d /tmp/prompt-zoom-ref-control-i2i-style-20260601010953-XXXXXX)"
...
SRC="$WORK/prompt-zoom-ref-control-i2i-style-20260601010953"
test -f "$SRC/tools/workbench-web/image-studio-canvas-next.html" || { echo "部署包缺少 canvas-next HTML" >&2; exit 1; }

BACKUP_DIR="$TARGET/.deploy-backups/prompt-zoom-ref-control-i2i-style-20260601010953-$STAMP"
...
log "verify markers"
grep -q "prompt-zoom-large-group .prompt-ref-area .vn2-ref-collapsed" "$TARGET/tools/workbench-web/image-studio-canvas-next.html"
...
grep -q "prompt-zoom-large-group .prompt-ref-area .vn2-ref-chip" "$TARGET/tools/workbench-web/image-studio-canvas-next.html"
