...
PKG="prompt-input-panning-overflow-fix-20260606134131"
...
verify_markers(){
  local file="$1"
  grep -Fq "20260606: 平移画布时外层可见" "$file"
  grep -Fq ".stage.viewport-moving :is(.i2i-input-box,.vn2-input-row):has" "$file"
  grep -Fq ".stage.panning :is(.prompt-node-inputbox,.prompt-node-rich-input" "$file"
  grep -Fq ".node:is(.node-type-textPrompt,.node-type-assetDesign,.node-type-shotStoryboard) .aio-editor-wrap:not(.prompt-zoomed-item) .prompt-node-rich-input" "$file"
  grep -Fq "max-height:116px!important;padding:12px 48px 12px 14px!important" "$file"
}
verify_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "20260606: 平移画布时外层可见" "$file"
  run_sudo grep -Fq ".stage.viewport-moving :is(.i2i-input-box,.vn2-input-row):has" "$file"
  run_sudo grep -Fq ".stage.panning :is(.prompt-node-inputbox,.prompt-node-rich-input" "$file"
  run_sudo grep -Fq ".node:is(.node-type-textPrompt,.node-type-assetDesign,.node-type-shotStoryboard) .aio-editor-wrap:not(.prompt-zoomed-item) .prompt-node-rich-input" "$file"
  run_sudo grep -Fq "max-height:116px!important;padding:12px 48px 12px 14px!important" "$file"
}
