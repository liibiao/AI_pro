...
PKG="mj-canvas-describe-iw-translate-20260609083148"
...
verify_html_markers(){
  local file="$1"
  grep -Fq 'function mjTipAttrs(text)' "$file"
  grep -Fq 'const MIDJOURNEY_IMAGE_WEIGHT_OPTIONS=' "$file"
  grep -Fq 'function midjourneyFunctionOptionsForNode(n)' "$file"
  grep -Fq 'function normalizeMidjourneyTaskKindForNode(n,kind)' "$file"
  grep -Fq 'function finalizeMidjourneyImaginePayload(payload,params={})' "$file"
  grep -Fq 'function appendMidjourneyImageWeight(text,value,hasImageRefs=false)' "$file"
  grep -Fq 'function runMidjourneyDescribeTextPromptNode(id,n,activeModel)' "$file"
  grep -Fq 'const PROMPT_TRANSLATE_COST=2' "$file"
  grep -Fq 'function runPromptTranslateForNode(id)' "$file"
  grep -Fq 'Midjourney /describe ·' "$file"
  grep -Fq 'Blend 模式会直接混合 2-6 张参考图' "$file"
  grep -Fq 'mjImageWeight:' "$file"
  grep -Fq '提示词英译预计' "$file"
  grep -Fq 'if(n.type===&#39;textPrompt&#39;)' "$file" || grep -Fq "if(n.type==='textPrompt')" "$file"
...
verify_html_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq 'function mjTipAttrs(text)' "$file"
  run_sudo grep -Fq 'const MIDJOURNEY_IMAGE_WEIGHT_OPTIONS=' "$file"
  run_sudo grep -Fq 'function midjourneyFunctionOptionsForNode(n)' "$file"
  run_sudo grep -Fq 'function normalizeMidjourneyTaskKindForNode(n,kind)' "$file"
  run_sudo grep -Fq 'function finalizeMidjourneyImaginePayload(payload,params={})' "$file"
  run_sudo grep -Fq 'function appendMidjourneyImageWeight(text,value,hasImageRefs=false)' "$file"
  run_sudo grep -Fq 'function runMidjourneyDescribeTextPromptNode(id,n,activeModel)' "$file"
  run_sudo grep -Fq 'const PROMPT_TRANSLATE_COST=2' "$file"
  run_sudo grep -Fq 'function runPromptTranslateForNode(id)' "$file"
  run_sudo grep -Fq 'Midjourney /describe ·' "$file"
  run_sudo grep -Fq 'Blend 模式会直接混合 2-6 张参考图' "$file"
  run_sudo grep -Fq 'mjImageWeight:' "$file"
  run_sudo grep -Fq '提示词英译预计' "$file"
