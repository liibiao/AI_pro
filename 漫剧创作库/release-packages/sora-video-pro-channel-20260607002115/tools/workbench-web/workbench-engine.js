  ...
  async function uploadReferenceImage(file, options) {
    ...
    if (mode === 'object_storage' && typeof XMLHttpRequest === 'function') {
      ...
      try {
        ...
        if (options.disableServerUploadFallback) throw directErr;
        console.warn('[workbench-engine] COS direct upload failed, fallback to server upload:', directErr);
        onProgress({ loaded: 0, total: file && file.size ? file.size : 1, percent: 1, phase: 'fallback', mode: mode, label: '直传失败，改用服务器兜底' });
      }
    }
