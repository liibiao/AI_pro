...
def build_gpt_image_v2_payload(payload: dict[str, Any], images: list[str] | None = None) -> tuple[ImageStudioConfig, dict[str, Any]]:
    ...
    config.model = GPT_IMAGE_V2_MODELS_BY_RESOLUTION[resolution]
...
def build_endpoint_url(config: ImageStudioConfig, default_path: str = "") -> str:
    endpoint_path = (config.endpoint_path or default_path or "").strip().replace("{model}", config.model)
...
def generate_panorama(payload: dict[str, Any], reference_images: list[str], output_dir: Path) -> dict[str, Any]:
    ...
    config, request_payload = build_gpt_image2_edits_payload(payload, images=reference_values)
    prompt = str(request_payload.get("prompt") or "").strip()
...
def generate_seedance_chat_video(payload: dict[str, Any], output_dir: Path) -> dict[str, Any]:
    ...
    response_payload = _request_json(endpoint, _json_headers(config.api_key), request_body, timeout=600)
...
def start_notevideo_seedance_video(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    create_resp = _request_json(config.endpoint_url, headers, request_body, timeout=120)
...
def build_notevideo_request_body(payload: dict[str, Any], config: VideoStudioConfig) -> dict[str, Any]:
    ...
    if ref_mode == "full":
        video_url = str(payload.get("video") or payload.get("videoUrl") or payload.get("refVideo") or "").strip()
        if _looks_like_http_url(video_url):
            request_body["video_url"] = video_url
    
    if ref_mode == "full":
        audio_url = str(payload.get("audio") or payload.get("audioUrl") or payload.get("refAudio") or "").strip()
        if _looks_like_http_url(audio_url):
...
def generate_notevideo_seedance_video(payload: dict[str, Any], output_dir: Path) -> dict[str, Any]:
    ...
    while elapsed < max_poll_seconds:
        ...
        if status == "completed":
            video_bytes, _ = _download_binary_with_headers(content_endpoint, {"Authorization": f"Bearer {config.api_key}"}, timeout=600, retries=2)
...
def _finalize_video(
    ...
    """下载远程视频并保存到本地，返回结果字典。"""
    video_bytes, _ = _download_binary(video_url, timeout=600, retries=2)
