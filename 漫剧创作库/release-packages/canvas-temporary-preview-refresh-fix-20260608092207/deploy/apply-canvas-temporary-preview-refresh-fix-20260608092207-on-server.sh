...
PKG="canvas-temporary-preview-refresh-fix-20260608092207"
...
verify_canvas_markers(){
  ...
  grep -Fq "keep inline preview while COS sync continues" "$file"
  grep -Fq "entry.downloadError=String(err?.message||err||'本地预览保存失败')" "$file"
  grep -Fq "temporaryUrl:temporaryUrls[0]||''" "$file"
  grep -Fq "saved.temporaryUrl||saved.temporary_url" "$file"
  grep -Fq "'temporaryUrl','temporary_url'" "$file"
  grep -Fq "if(!saved.temporaryUrl&&(entry.temporaryUrl||entry.temporary_url))" "$file"
  if grep -Fq "图片生成返回 base64，但本地预览保存失败" "$file"; then
    fail "old fatal generated base64 preview error still exists in $file"
  fi
  if grep -Fq "分镜图返回 base64，但本地预览保存失败" "$file"; then
    fail "old fatal storyboard base64 preview error still exists in $file"
  fi
  if grep -Fq "分镜格返回 base64，但本地预览保存失败" "$file"; then
    fail "old fatal storyboard shot base64 preview error still exists in $file"
  fi
}
...
verify_canvas_markers_sudo(){
  ...
  run_sudo grep -Fq "keep inline preview while COS sync continues" "$file"
  run_sudo grep -Fq "entry.downloadError=String(err?.message||err||'本地预览保存失败')" "$file"
  run_sudo grep -Fq "temporaryUrl:temporaryUrls[0]||''" "$file"
  run_sudo grep -Fq "saved.temporaryUrl||saved.temporary_url" "$file"
  run_sudo grep -Fq "'temporaryUrl','temporary_url'" "$file"
  run_sudo grep -Fq "if(!saved.temporaryUrl&&(entry.temporaryUrl||entry.temporary_url))" "$file"
  if run_sudo grep -Fq "图片生成返回 base64，但本地预览保存失败" "$file"; then
    fail "old fatal generated base64 preview error still exists in $file"
  fi
  if run_sudo grep -Fq "分镜图返回 base64，但本地预览保存失败" "$file"; then
    fail "old fatal storyboard base64 preview error still exists in $file"
  fi
  if run_sudo grep -Fq "分镜格返回 base64，但本地预览保存失败" "$file"; then
    fail "old fatal storyboard shot base64 preview error still exists in $file"
  fi
}
