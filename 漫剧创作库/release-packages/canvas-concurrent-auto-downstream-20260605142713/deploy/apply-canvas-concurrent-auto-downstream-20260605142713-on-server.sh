...
PKG="canvas-concurrent-auto-downstream-20260605142713"
...
verify_canvas_markers(){
  local file="$1"
  grep -Fq "function ensureGeneratedPreviewDownstream" "$file"
  grep -Fq "function ensureGeneratedImagePreviewDownstream(id,opts={})" "$file"
  grep -Fq "function ensureGeneratedVideoPreviewDownstream(id,opts={})" "$file"
  grep -Fq "ensureGeneratedImagePreviewDownstream(id,{immediate:true})" "$file"
  grep -Fq "ensureGeneratedVideoPreviewDownstream(id,{immediate:true})" "$file"
  grep -Fq "ensureShotStoryboardEditDownstream(id)" "$file"
  grep -Fq "refreshDownstreamAfterConnectionChange(downstreamNodeIds(id))" "$file"
  grep -Fq "existingGeneratedPreviewConnection" "$file"
  grep -Fq "function renderConcurrentToggle" "$file"
  grep -Fq "function beginConcurrentTask" "$file"
  grep -Fq "function concurrentPreviewAccepts" "$file"
  grep -Fq "_concurrentPreviewTaskId" "$file"
  grep -Fq "forceNew:true" "$file"
  grep -Fq "concurrent-task-panel" "$file"
  grep -Fq "function shotStoryboardOutputSizeSpec" "$file"
  grep -Fq "选择单格比例" "$file"
  grep -Fq "storyboardCellRatio" "$file"
}
