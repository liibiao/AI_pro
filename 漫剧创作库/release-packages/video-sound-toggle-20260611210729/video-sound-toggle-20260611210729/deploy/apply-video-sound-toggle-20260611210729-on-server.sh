...
PKG="video-sound-toggle-20260611210729"
...
verify_canvas_file "$SRC/smart-vision/canvas/legacy-workbench/workbench-web/image-studio-canvas.html"
...
if [ -f "$WORKBENCH_DIR/image-studio-canvas-next.html" ]; then
  run_sudo grep -Fq "const soundToggle=caps.supportsGeneratedAudio" "$WORKBENCH_DIR/image-studio-canvas-next.html"
fi
if [ -f "$WORKBENCH_DIR/canvas-next/generation-service.js" ]; then
  run_sudo grep -Fq "const soundFields=soundField?" "$WORKBENCH_DIR/canvas-next/generation-service.js"
fi
