...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/asset-card-derive-stability-fix-20260622115757-${STAMP}"
...
check_markers(){
  local file="$1"
  test -f "$file"
  grep -Fq "const ASSET_CARD_DERIVE_CHAT_TIMEOUT_MS=900000;" "$file"
  grep -Fq "function cleanAssetCardJsonCandidateText" "$file"
  grep -Fq "function tryParseAssetCardJsonCandidate" "$file"
  grep -Fq "function balancedAssetCardJsonSnippets" "$file"
  grep -Fq "async function requestAssetCardPlanTextAgent" "$file"
  grep -Fq "async function repairAssetCardPlanJsonWithModel" "$file"
  grep -Fq "onProgress?.(26,'文本 Agent 推演')" "$file"
  grep -Fq "onProgress?.(84,'修复 JSON')" "$file"
  grep -Fq "route:'text-agent'" "$file"
}
...
echo "deployed asset-card-derive-stability-fix-20260622115757"
