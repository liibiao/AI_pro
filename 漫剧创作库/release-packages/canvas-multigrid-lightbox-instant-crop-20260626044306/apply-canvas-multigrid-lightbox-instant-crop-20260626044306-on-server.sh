...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/canvas-multigrid-lightbox-instant-crop-20260626044306-${STAMP}"
...
validate_html(){
  local file="$1"
  test -f "$file"
  require_marker "function multiGridCellPlaceholderDataUrl(entry)" "$file"
  require_marker "function completeLightboxMultiGridCropAsync(entry,token)" "$file"
  require_marker "LB.multiGridBusy='crop'" "$file"
  require_marker "multi-grid-crop-status" "$file"
  require_marker "裁切中" "$file"
  require_marker "setImgSrcSafe(img,src,{fallbackUrls:[multiGridCellSourceUrl(entry)].filter(Boolean)})" "$file"
  require_marker "function openMultiGridCellLightbox(entry,context={})" "$file"
  require_marker "const ready=readyMaterializedMultiGridCellEntry(entry)" "$file"
}
...
echo "deployed canvas-multigrid-lightbox-instant-crop-20260626044306"
