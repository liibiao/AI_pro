...
;(function (root) {
  ...
  function requestJson(url, options) {
    ...
    function xhrRequest() {
      return new Promise(function (resolve, reject) {
        ...
        xhr.onload = function () {
          var data = {};
          try { data = JSON.parse(xhr.responseText || '{}'); } catch (err) { reject(err); return; }
    ...
    if (typeof fetch === 'function') {
      return fetch(url, options).then(function (response) {
        ...
        if (typeof XMLHttpRequest === 'function') return xhrRequest();
        throw err;
      });
    }
    ...
    return Promise.reject(new Error('当前浏览器缺少 fetch/XMLHttpRequest，无法请求 JSON'));
  }
  ...
  async function buildStandaloneResult(responsePayload) {
    ...
    if (b64) {
      throw new Error('上游只返回了 base64 图片数据；新画布要求生图结果走 URL 响应，请检查当前渠道 response_format=url 是否生效');
    }
  ...
  function isAbortError(err) {
    return err && (err.name === 'AbortError' || /AbortError|已取消/i.test(String(err.message || err)));
  }

  function isNonRetryableGenerateError(message) {
    var text = String(message || '').toLowerCase();
    return /认证失败|api key|invalid token|unauthorized|forbidden|请先填写|未检测到|不支持|not supported|参考图必须|mask 必须|invalid image|invalid mask|content policy|safety/.test(text);
  }

  function wait(ms, signal) {
    return new Promise(function (resolve, reject) {
      if (signal && signal.aborted) return reject(new DOMException('已取消', 'AbortError'));
  ...
  async function generateImageStandalone(payload) {
    ...
    var maxAttempts = Number(payload.maxRetries || payload.retryAttempts || 3) || 3;
    var lastError = null;
    for (var attempt = 1; attempt <= maxAttempts; attempt += 1) {
      try {
        if (typeof payload.onRetry === 'function' && attempt > 1) payload.onRetry(attempt, maxAttempts, lastError);
        ...
        lastError = err;
        if (attempt >= maxAttempts || isNonRetryableGenerateError(err && err.message || err)) break;
        await wait(1000 * attempt, payload.signal);
      }
    }
    var finalMessage = lastError && lastError.message || String(lastError || '生成失败');
    if (!isNonRetryableGenerateError(finalMessage) && maxAttempts > 1) finalMessage = '已自动重试 ' + maxAttempts + ' 次仍失败：' + finalMessage;
  ...
  async function loadModelsFromServer() {
    var models = [];
    try {
      var response = await requestJson(API_BASE + '/models', { cache: 'no-store' });
      if (response.ok && response.data && response.data.ok) models = response.data.models || [];
      else console.warn('[workbench] 模型列表接口返回异常', response.status, response.data);
    } catch (err) {
      console.warn('[workbench] 模型列表加载失败', err && err.message ? err.message : err);
    }
    return models;
  }
  ...
  function getModelBaseUrl(raw) {
    return normalizeModelUrl(raw && (raw.baseUrl || raw.base_url || raw.apiBase || raw.api_base || raw.url || raw.endpoint));
  }
  function getModelApiKey(raw) {
    return String(raw && (raw.key || raw.apiKey || raw.api_key || raw.token) || '').trim();
  }
  function getModelName(raw) {
    return String(raw && (raw.realModelName || raw.upstreamModel || raw.modelName || raw.model || raw.name) || '').trim();
  }
  ...
  var OBJECT_STORAGE_UPLOAD_TARGET_API = API_BASE + '/object-storage-upload-target';
  function normalizeReferenceUploadMode(mode) {
    var value = String(mode || '').trim().toLowerCase();
    if (value === 'files' || value === 'file' || value === 'provider_files' || value === 'provider-files') return 'files';
    if ([
    ...
    ].indexOf(value) >= 0) return 'object_storage';
    return 'files';
  }
  function buildFilesUploadApi(baseUrl) {
    var root = normalizeModelUrl(baseUrl);
    if (!root) return FILES_UPLOAD_API;
    try {
      var u = new URL(root, location.href);
      var path = String(u.pathname || '').replace(/\/+$/, '');
      if (/\/files$/i.test(path)) return u.toString().replace(/\/+$/, '');
      if (/\/v1$/i.test(path)) {
        u.pathname = path + '/files';
        return u.toString();
      }
      u.pathname = path + '/v1/files';
      return u.toString();
    } catch (_) {
      return root.replace(/\/+$/, '') + (/(?:^|\/)v1$/i.test(root) ? '/files' : '/v1/files');
    }
  }
  function getReferenceUploadApi(mode, options) {
    options = options || {};
    return normalizeReferenceUploadMode(mode) === 'object_storage' ? OBJECT_STORAGE_UPLOAD_API : buildFilesUploadApi(options.baseUrl || options.endpointUrl || options.url || '');
  }
  async function requestObjectStorageUploadTarget(file, options) {
    options = options || {};
    var timeoutMs = Number(options.targetTimeoutMs || options.signingTimeoutMs || 12000);
    ...
    var resp;
    try {
      resp = await fetch(OBJECT_STORAGE_UPLOAD_TARGET_API, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        filename: file && file.name ? file.name : 'reference.png',
      ...
      if (err && err.name === 'AbortError') throw new Error('获取 COS 直传地址超时，正在切换后端 COS 上传');
      throw err;
    } finally {
      if (timer) clearTimeout(timer);
    }
    var data = await resp.json().catch(function () { return {}; });
    if (!resp.ok || !data.ok && !data.uploadUrl) {
      throw new Error(data && (data.error || data.message || data.msg) || '获取 COS 直传地址失败 HTTP ' + resp.status);
    }
    return data;
  }
  async function putObjectStorageDirect(file, target, onProgress, options) {
    options = options || {};
    return await new Promise(function (resolve, reject) {
      var xhr = new XMLHttpRequest();
      var settled = false;
      ...
      var stallTimer = null;
      var confirmTimeoutMs = Number(options.confirmTimeoutMs || options.postUploadTimeoutMs || 45000);
      var stallTimeoutMs = Number(options.directStallTimeoutMs || 20000);
      ...
      function rejectOnce(err, abortXhr) {
        if (settled) return;
        settled = true;
        clearConfirmTimer();
        clearStallTimer();
      ...
      function armStallTimer() {
        if (stallTimer || !stallTimeoutMs || stallTimeoutMs <= 0) return;
        stallTimer = setTimeout(function () {
          rejectOnce(new Error('COS 直传启动超时，正在切换后端 COS 上传'), true);
        }, stallTimeoutMs);
      }
      function armConfirmTimer() {
        if (confirmTimer || !confirmTimeoutMs || confirmTimeoutMs <= 0) return;
        confirmTimer = setTimeout(function () {
          rejectOnce(new Error('COS 直传已上传 100%，但服务端确认超时，正在切换后端 COS 上传'), true);
        }, confirmTimeoutMs);
      }
      xhr.open('PUT', target.uploadUrl, true);
      xhr.timeout = Number(options.timeoutMs || 240000);
      var headers = target.headers || {};
      Object.keys(headers).forEach(function (key) {
        try { xhr.setRequestHeader(key, headers[key]); } catch (_) {}
      });
      var contentType = target.contentType || file && file.type || 'application/octet-stream';
      try { xhr.setRequestHeader('Content-Type', contentType); } catch (_) {}
      xhr.upload.onprogress = function (evt) {
        ...
        if (loaded > 0 || percent > 0) clearStallTimer();
        if (percent >= 100) {
          onProgress({ loaded: loaded, total: total, percent: 99, phase: 'confirming', mode: 'object_storage', label: '确认公网 URL', id: target.publicUrl, fileId: target.publicUrl, uploadRef: target.publicUrl, providerRef: target.publicUrl, remoteUrl: target.publicUrl, url: target.publicUrl, uploaded: true, provider: target.provider || 'object_storage' });
          armConfirmTimer();
        } else {
          onProgress({ loaded: loaded, total: total, percent: percent, phase: 'uploading', mode: 'object_storage', label: '直传 COS' });
        }
      };
      xhr.onerror = function () { rejectOnce(new Error('COS 直传网络失败：请检查 COS Bucket CORS 是否允许 PUT/OPTIONS 和 Content-Type')); };
      xhr.onabort = function () { rejectOnce(new Error('COS 直传已取消')); };
      xhr.ontimeout = function () { rejectOnce(new Error('COS 直传超时')); };
      xhr.onload = function () {
        if (settled) return;
        if (xhr.status < 200 || xhr.status >= 300) {
          rejectOnce(new Error('COS 直传失败 HTTP ' + xhr.status + (xhr.responseText ? '：' + String(xhr.responseText).slice(0, 180) : '')));
          return;
        }
        onProgress({ loaded: file && file.size ? file.size : 1, total: file && file.size ? file.size : 1, percent: 100, phase: 'done', mode: 'object_storage', label: 'COS 上传完成' });
        resolveOnce({
          ok: true,
          id: target.publicUrl,
          ...
          provider: target.provider || 'object_storage',
          key: target.key || '',
          contentType: contentType,
          bytes: file && file.size ? file.size : target.size || 0,
          size: file && file.size ? file.size : target.size || 0,
          filename: target.filename || file && file.name || 'reference.png',
          directUpload: true
        });
      };
      xhr.send(file);
  ...
  function uploadErrorMessage(err) {
    return String(err && (err.message || err.error || err) || '').trim() || '未知错误';
  }
  async function uploadObjectStorageViaBackend(file, onProgress, options, directErr) {
    options = options || {};
    var form = new FormData();
    form.append('file', file, file && file.name ? file.name : 'reference.png');
    form.append('purpose', 'vision');
    form.append('mode', 'object_storage');
    var fallbackLabel = directErr ? '直传失败，改用后端 COS' : '后端上传 COS';
    onProgress({ loaded: 0, total: file && file.size ? file.size : 1, percent: 74, phase: 'uploading', mode: 'object_storage', label: fallbackLabel });
    if (typeof XMLHttpRequest !== 'function') {
      var fallbackResp = await fetch(OBJECT_STORAGE_UPLOAD_API, { method: 'POST', body: form });
      var fallbackData = await fallbackResp.json().catch(function () { return {}; });
      if (!fallbackResp.ok || (!fallbackData.ok && !fallbackData.remoteUrl && !fallbackData.url)) {
        throw new Error(fallbackData && (fallbackData.error || fallbackData.message || fallbackData.msg) || '后端 COS 上传失败 HTTP ' + fallbackResp.status);
      }
      onProgress({ loaded: file && file.size ? file.size : 1, total: file && file.size ? file.size : 1, percent: 100, phase: 'done', mode: 'object_storage', label: 'COS 上传完成' });
      return fallbackData;
    }
    return await new Promise(function (resolve, reject) {
      ...
      var settled = false;
      var confirmTimer = null;
      var confirmTimeoutMs = Number(options.backendConfirmTimeoutMs || options.backendPostUploadTimeoutMs || 120000);
      function clearConfirmTimer() {
        if (confirmTimer) {
          clearTimeout(confirmTimer);
          confirmTimer = null;
      ...
      function armConfirmTimer() {
        if (confirmTimer || !confirmTimeoutMs || confirmTimeoutMs <= 0) return;
        confirmTimer = setTimeout(function () {
          rejectOnce(new Error('后端 COS 上传已接收文件，但转存确认超时'), true);
        }, confirmTimeoutMs);
      }
      xhr.open('POST', OBJECT_STORAGE_UPLOAD_API, true);
      xhr.timeout = Number(options.backendTimeoutMs || options.timeoutMs || 300000);
      xhr.upload.onprogress = function (evt) {
        if (settled) return;
        var total = Number(evt && evt.total || file && file.size || 0) || 0;
        var loaded = Number(evt && evt.loaded || 0) || 0;
        var rawPercent = total > 0 ? Math.max(0, Math.min(100, Math.round(loaded / total * 100))) : 0;
        var percent = Math.max(74, Math.min(96, Math.round(74 + rawPercent / 100 * 22)));
        onProgress({ loaded: loaded, total: total, percent: percent, phase: 'uploading', mode: 'object_storage', label: fallbackLabel });
        if (rawPercent >= 100) {
          armConfirmTimer();
          onProgress({ loaded: loaded, total: total, percent: 99, phase: 'confirming', mode: 'object_storage', label: '后端转存 COS' });
        }
      };
      xhr.onerror = function () { rejectOnce(new Error('后端 COS 上传网络失败')); };
      xhr.onabort = function () { rejectOnce(new Error('后端 COS 上传已取消')); };
      xhr.ontimeout = function () { rejectOnce(new Error('后端 COS 上传超时')); };
      xhr.onload = function () {
        if (settled) return;
        var data = {};
        try {
          ...
          rejectOnce(new Error('后端 COS 上传返回了无效 JSON'));
          return;
        }
        if (xhr.status < 200 || xhr.status >= 300 || (!data.ok && !data.remoteUrl && !data.url)) {
          rejectOnce(new Error(data && (data.error || data.message || data.msg) || '后端 COS 上传失败 HTTP ' + xhr.status));
          return;
        }
        onProgress({ loaded: file && file.size ? file.size : 1, total: file && file.size ? file.size : 1, percent: 100, phase: 'done', mode: 'object_storage', label: 'COS 上传完成' });
        resolveOnce(data);
  ...
  async function uploadReferenceImage(file, options) {
    ...
    var baseUrl = String(options.baseUrl || options.endpointUrl || options.url || '').trim();
    var apiKey = String(options.apiKey || options.key || '').trim();
    if (mode === 'files' && (!baseUrl || !apiKey)) throw new Error('FILES 上传缺少模型 API 地址或 API Key');
    var onProgress = typeof options.onProgress === 'function' ? options.onProgress : function () {};
    if (mode === 'object_storage' && typeof XMLHttpRequest === 'function') {
      onProgress({ loaded: 0, total: file && file.size ? file.size : 1, percent: 2, phase: 'signing', mode: mode, label: '获取 COS 直传地址' });
      try {
        var target = await requestObjectStorageUploadTarget(file, options);
        onProgress({ loaded: 0, total: file && file.size ? file.size : 1, percent: 8, phase: 'signing', mode: mode, label: 'COS 地址已获取' });
        return await putObjectStorageDirect(file, target, onProgress, options);
      } catch (directErr) {
        try {
          return await uploadObjectStorageViaBackend(file, onProgress, options, directErr);
        } catch (backendErr) {
          throw new Error('COS 上传失败：' + uploadErrorMessage(directErr) + '；后端转存失败：' + uploadErrorMessage(backendErr));
        }
      }
    }
    if (mode === 'object_storage') {
      throw new Error('当前浏览器环境不支持 COS 直传，请使用支持 XMLHttpRequest 的浏览器');
    }
    var uploadApi = getReferenceUploadApi(mode, { baseUrl: baseUrl });
    var form = new FormData();
    form.append('file', file, file && file.name ? file.name : 'reference.png');
    form.append('purpose', 'vision');
    if (mode !== 'files') form.append('mode', mode);
    var uploadHeaders = mode === 'files' ? buildDirectMultipartHeaders(apiKey) : undefined;
    if (typeof XMLHttpRequest !== 'function') {
      var fallbackResp = await fetch(uploadApi, { method: 'POST', headers: uploadHeaders, body: form });
    ...
    return await new Promise(function (resolve, reject) {
      ...
      var settled = false;
      var confirmTimer = null;
      var confirmTimeoutMs = Number(options.confirmTimeoutMs || options.postUploadTimeoutMs || 90000);
      function clearConfirmTimer() {
        if (confirmTimer) {
          clearTimeout(confirmTimer);
          confirmTimer = null;
      ...
      function armConfirmTimer() {
        if (confirmTimer || !confirmTimeoutMs || confirmTimeoutMs <= 0) return;
        confirmTimer = setTimeout(function () {
          rejectOnce(new Error('参考图已上传 100%，但 /v1/files 云端确认超时'), true);
        }, confirmTimeoutMs);
      }
      ...
      xhr.upload.onprogress = function (evt) {
        ...
        var percent = total > 0 ? Math.max(0, Math.min(100, Math.round(loaded / total * 100))) : 0;
        if (percent >= 100) {
          onProgress({ loaded: loaded, total: total, percent: 99, phase: 'confirming', mode: mode, label: '云端确认中' });
          armConfirmTimer();
        } else {
          onProgress({ loaded: loaded, total: total, percent: percent, phase: 'uploading', mode: mode });
        }
      };
      xhr.onerror = function () { rejectOnce(new Error('上传参考图失败，网络连接中断')); };
      xhr.onabort = function () { rejectOnce(new Error('上传参考图已取消')); };
      xhr.ontimeout = function () { rejectOnce(new Error('上传参考图超时，请检查 /v1/files 或对象存储服务')); };
      if (uploadHeaders) {
        Object.keys(uploadHeaders).forEach(function (key) {
          try { xhr.setRequestHeader(key, uploadHeaders[key]); } catch (_) {}
        });
      }
      xhr.onload = function () {
        if (settled) return;
        ...
        try {
          ...
          rejectOnce(new Error('上传参考图返回了无效 JSON'));
          return;
        }
        if (xhr.status < 200 || xhr.status >= 300 || (!data.ok && !data.id)) {
          rejectOnce(new Error(data && data.error || data && data.message || data && data.msg || 'HTTP ' + xhr.status));
          return;
        }
        onProgress({ loaded: file && file.size ? file.size : 1, total: file && file.size ? file.size : 1, percent: 100, phase: 'done', mode: mode });
        resolveOnce(data);
