...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/asset-confirm-mj-task-routing-failure-fix-20260622174046-${STAMP}"
...
check_markers(){
  local file="$1"
  test -f "$file"
  grep -q "function assetConfirmGenerationLooksMidjourney" "$file"
  grep -q "function normalizeAssetConfirmMidjourneyTaskOptions" "$file"
  grep -q "assetConfirmNodeType:'assetConfirm'" "$file"
  grep -q "submitKind:'imagine'" "$file"
  grep -q "generationFailureMode=assetConfirmGenerationLooksMidjourney" "$file"
  grep -q "Midjourney 生图失败" "$file"
}
...
echo "deployed asset-confirm-mj-task-routing-failure-fix-20260622174046"
