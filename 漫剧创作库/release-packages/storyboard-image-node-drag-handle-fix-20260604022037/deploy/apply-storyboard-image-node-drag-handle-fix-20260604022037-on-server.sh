...
PKG="storyboard-image-node-drag-handle-fix-20260604022037"
...
verify_markers(){
  ...
  grep -Fq "shotGridSurface" "$dest"
  grep -Fq "allowNodeDrag" "$dest"
  grep -Fq "if(S._justDragged||e.target.closest('[data-shot-drag]'))return;" "$dest"
