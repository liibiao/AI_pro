...
PKG="mj-smart-reference-final-prompt-fix-20260613142215"
...
verify_html_markers_sudo(){ local file="$1"; run_sudo grep -Fq "afterHasActorAction" "$file"; run_sudo grep -Fq "payload.mjPromptImageUrls=imageRefs" "$file"; run_sudo grep -Fq "lockedAsCharacterOrStyle" "$file"; }
...
verify_skill_markers_sudo(){ local file="$1"; run_sudo grep -Fq "同一张图默认只承担一个主控制职责" "$file"; run_sudo grep -Fq '角色身份锁定：`--cref`' "$file"; }
