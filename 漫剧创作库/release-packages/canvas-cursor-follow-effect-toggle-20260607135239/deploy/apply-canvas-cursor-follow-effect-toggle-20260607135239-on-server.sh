...
PKG="canvas-cursor-follow-effect-toggle-20260607135239"
...
verify_markers(){
  ...
  grep -Fq "stageCursorField" "$file"
  grep -Fq "settingsCursorEffectToggle" "$file"
  grep -Fq "mjb_canvas_cursor_effect_enabled" "$file"
  grep -Fq "function initStageCursorField()" "$file"
  grep -Fq "canvas-cursor-effect-change" "$file"
  grep -Fq "setCursorEffectEnabled" "$file"
}
verify_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "auth-cursor-field" "$file"
  run_sudo grep -Fq "stage-cursor-field" "$file"
  run_sudo grep -Fq "stageCursorField" "$file"
  run_sudo grep -Fq "settingsCursorEffectToggle" "$file"
  run_sudo grep -Fq "mjb_canvas_cursor_effect_enabled" "$file"
  run_sudo grep -Fq "function initStageCursorField()" "$file"
  run_sudo grep -Fq "canvas-cursor-effect-change" "$file"
  run_sudo grep -Fq "setCursorEffectEnabled" "$file"
}
