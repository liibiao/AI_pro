...
PKG="canvas-storyboard-shot-large-bottom-right-20260612134430"
...
verify_html_markers(){
  local file="$1"
  grep -Fq 'storygrid-panel-large{position:absolute;right:7px;bottom:7px' "$file"
  grep -Fq 'storygrid-panel.active::after{content:"✓";position:absolute;left:8px;bottom:8px' "$file"
  grep -Fq 'data-shot-large="${esc(no)}"' "$file"
  grep -Fq 'function openStoryboardShotLargePreview(id,shotNo,sourceEl=null)' "$file"
  grep -Fq '_shotToggleNo:shotToggleNo' "$file"
  grep -Fq 'toggleStoryboardShotSelection(previewId,dragInfo._shotToggleNo)' "$file"
}
verify_html_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq 'storygrid-panel-large{position:absolute;right:7px;bottom:7px' "$file"
  run_sudo grep -Fq 'storygrid-panel.active::after{content:"✓";position:absolute;left:8px;bottom:8px' "$file"
  run_sudo grep -Fq 'data-shot-large="${esc(no)}"' "$file"
  run_sudo grep -Fq 'function openStoryboardShotLargePreview(id,shotNo,sourceEl=null)' "$file"
  run_sudo grep -Fq '_shotToggleNo:shotToggleNo' "$file"
  run_sudo grep -Fq 'toggleStoryboardShotSelection(previewId,dragInfo._shotToggleNo)' "$file"
}
