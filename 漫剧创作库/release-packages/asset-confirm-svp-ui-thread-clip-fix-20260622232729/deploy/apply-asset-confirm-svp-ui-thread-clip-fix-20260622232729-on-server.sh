...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/asset-confirm-svp-ui-thread-clip-fix-20260622232729-${STAMP}"
...
check_markers(){
  local file="$1"
  test -f "$file"
  grep -Fq ".then(()=>yieldForUiFeedback())" "$file"
  grep -Fq "function syncAssetConfirmCreatePromptButtons(n)" "$file"
  grep -Fq "function ensureAssetConfirmCreateStatusElement(n)" "$file"
  grep -Fq "raw.startsWith('@Image',i)" "$file"
  grep -Fq "renderShotVideoPromptMissingResourceClip(token.label,{mini:true})" "$file"
  grep -Fq "if(!deferInitialBuild)rerenderNode(nid,{animate:false})" "$file"
}
...
echo "deployed asset-confirm-svp-ui-thread-clip-fix-20260622232729"
