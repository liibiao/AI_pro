...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/canvas-multigrid-node-lightbox-upscale-fix-20260626022334-${STAMP}"
...
validate_html(){
  local file="$1"
  test -f "$file"
  require_marker "function stripMaterializedMultiGridCells(entry)" "$file"
  require_marker "function scheduleMultiGridGeneratedEntryMaterialize(entry,nodeId='')" "$file"
  require_marker "function renderMultiGridLightboxWorkbar(bar)" "$file"
  require_marker "data-mg-import-canvas" "$file"
  require_marker "data-mg-upscale-run" "$file"
  require_marker "node.values.refinePresetId='upscale-pure'" "$file"
  require_marker "const shouldPreviewClick=isMiddlePanoramaDrag?false" "$file"
}
...
echo "deployed canvas-multigrid-node-lightbox-upscale-fix-20260626022334"
