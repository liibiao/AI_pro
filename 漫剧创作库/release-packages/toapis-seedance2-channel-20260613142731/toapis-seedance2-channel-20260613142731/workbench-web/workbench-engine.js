...
;(function (root) {
  ...
  async function putObjectStorageDirect(file, target, onProgress, options) {
    ...
    return await new Promise(function (resolve, reject) {
      ...
      xhr.onerror = function () { reject(new Error('COS 直传网络失败：请检查 COS Bucket CORS 是否允许 PUT/OPTIONS 和 Content-Type')); };
      xhr.onabort = function () { reject(new Error('COS 直传已取消')); };
      xhr.ontimeout = function () { reject(new Error('COS 直传超时')); };
      xhr.onload = function () {
        if (xhr.status < 200 || xhr.status >= 300) {
          reject(new Error('COS 直传失败 HTTP ' + xhr.status + (xhr.responseText ? '：' + String(xhr.responseText).slice(0, 180) : '')));
          return;
        }
  ...
  async function uploadObjectStorageViaBackend(file, onProgress, options, directErr) {
    ...
    return await new Promise(function (resolve, reject) {
      ...
      xhr.upload.onprogress = function (evt) {
        ...
        if (rawPercent >= 100) onProgress({ loaded: loaded, total: total, percent: 100, phase: 'uploading', mode: 'object_storage', label: '后端转存 COS' });
      };
      xhr.onerror = function () { reject(new Error('后端 COS 上传网络失败')); };
      xhr.onabort = function () { reject(new Error('后端 COS 上传已取消')); };
      xhr.ontimeout = function () { reject(new Error('后端 COS 上传超时')); };
      xhr.onload = function () {
        var data = {};
        try {
          data = JSON.parse(xhr.responseText || '{}');
        } catch (_) {
          reject(new Error('后端 COS 上传返回了无效 JSON'));
          return;
        }
        if (xhr.status < 200 || xhr.status >= 300 || (!data.ok && !data.remoteUrl && !data.url)) {
          reject(new Error(data && (data.error || data.message || data.msg) || '后端 COS 上传失败 HTTP ' + xhr.status));
          return;
        }
        onProgress({ loaded: file && file.size ? file.size : 1, total: file && file.size ? file.size : 1, percent: 100, phase: 'done', mode: 'object_storage', label: 'COS 上传完成' });
        resolve(data);
  ...
  async function uploadReferenceImage(file, options) {
    ...
    return await new Promise(function (resolve, reject) {
      ...
      xhr.ontimeout = function () { reject(new Error('上传参考图超时，请检查 /v1/files 或对象存储服务')); };
      if (uploadHeaders) {
