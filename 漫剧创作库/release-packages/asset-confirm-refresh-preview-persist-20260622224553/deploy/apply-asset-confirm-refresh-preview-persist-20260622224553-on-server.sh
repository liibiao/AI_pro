...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/asset-confirm-refresh-preview-persist-20260622224553-${STAMP}"
...
check_markers(){
  local file="$1"
  test -f "$file"
  grep -Fq "function assetConfirmIsPersistableImageUrl" "$file"
  grep -Fq "function assetConfirmNormalizePersistedImageEntry" "$file"
  grep -Fq "20260622-persistable-preview-v1" "$file"
  grep -Fq "return assetConfirmNormalizePersistedImageEntry(entry,{card,task:finalTask})" "$file"
  grep -Fq "next.image=assetConfirmNormalizePersistedImageEntry" "$file"
}
...
echo "deployed asset-confirm-refresh-preview-persist-20260622224553"
