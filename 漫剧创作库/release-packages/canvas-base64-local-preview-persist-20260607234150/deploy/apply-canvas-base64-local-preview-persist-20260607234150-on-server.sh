...
PKG="canvas-base64-local-preview-persist-20260607234150"
...
verify_canvas_markers(){
  ...
  grep -Fq "function persistDerivedEntryOnNode" "$file"
  grep -Fq "写入本地预览" "$file"
  grep -Fq "cachedDataUrl:''" "$file"
  grep -Fq "saveCurrentProjectNow({silent:true,deferRender:true})" "$file"
...
verify_canvas_markers_sudo(){
  ...
  run_sudo grep -Fq "function persistDerivedEntryOnNode" "$file"
  run_sudo grep -Fq "写入本地预览" "$file"
  run_sudo grep -Fq "cachedDataUrl:''" "$file"
  run_sudo grep -Fq "saveCurrentProjectNow({silent:true,deferRender:true})" "$file"
