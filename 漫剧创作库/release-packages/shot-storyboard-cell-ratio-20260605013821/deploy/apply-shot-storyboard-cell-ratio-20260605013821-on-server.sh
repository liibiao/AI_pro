...
PKG="shot-storyboard-cell-ratio-20260605013821"
...
verify_canvas_markers(){
  ...
  grep -Fq "function renderShotStoryboardSizeControls" "$file"
  grep -Fq "选择单格比例" "$file"
  grep -Fq "工具栏比例语义：该比例只代表每个分镜格的画幅" "$file"
  grep -Fq "工具栏比例是每个分镜格的比例" "$file"
  grep -Fq "allowLegacySize:true" "$file"
  grep -Fq "storyboardCellRatio" "$file"
  grep -Fq "storyboardCanvasRatio" "$file"
  grep -Fq "shot-cell-ratio-trigger" "$file"
}
