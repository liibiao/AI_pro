...
ARCHIVE="${1:?usage: apply-canvas-next-asset-confirm-state-perf-fix-20260616232251-on-server.sh <package.tar.gz>}"
PKG_NAME="canvas-next-asset-confirm-state-perf-fix-20260616232251"
WORK_DIR="$(mktemp -d)"
...
verify_file(){
  local file="$1"
  test -f "$file"
  grep -Fq "assetConfirmFinalGenerationState" "$file"
  grep -Fq "settleAssetConfirmCardOwnImage" "$file"
  grep -Fq "scheduleAssetConfirmBatchRerender" "$file"
  grep -Fq "persistAssetConfirmNodeState" "$file"
  grep -Fq "normalizeLoadedAssetConfirmCard" "$file"
  grep -Fq "const concurrency=Math.min(2" "$file"
  grep -Fq "S.assetConfirmProgressTimers={}" "$file"
  grep -Fq "replacing,replaceExisting:replacing" "$file"
  grep -Fq "assetConfirmCardNeedsRecoveredImage" "$file"
}
