...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/canvas-multigrid-cell-reference-cos-fix-20260626103935-${STAMP}"
...
validate_html(){
  local file="$1"
  test -f "$file"
  require_marker "function getMultiGridCellReferenceEntryFromNode(node,opts={})" "$file"
  require_marker "function scheduleReferenceConsumerConnectionSync(fromId,toId)" "$file"
  require_marker "if(multiGridCellCandidatesFromSourceNode(n).some(isMultiGridCellEntry))return null;" "$file"
  require_marker "startDerivedImageAssetSync(nodeId,entry.dataUrl||entry.url,entry,{" "$file"
  require_marker "const pendingCell=from.type==='singleImage'&&multiGridCellCandidatesFromSourceNode(from).some(isMultiGridCellEntry)&&!getMultiGridCellReferenceEntryFromNode(from,{allowSchedule:true});" "$file"
}
...
echo "deployed canvas-multigrid-cell-reference-cos-fix-20260626103935"
