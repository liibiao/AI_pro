...
PKG="canvas-video-429-polling-fix-20260612120430"
...
verify_canvas_markers(){
  local file="$1"
  grep -Fq "function isGenerationRateLimitMessage(msg)" "$file"
  grep -Fq "return isGenerationRateLimitMessage(msg)||/stream disconnected before completion" "$file"
  grep -Fq "return isGenerationRateLimitMessage(msg)||/超时|timeout|timed out" "$file"
  grep -Fq "function generationTransientRetryDelayMs(err,baseDelay=1800)" "$file"
  grep -Fq "function sleepGenerationPoll(ms,signal)" "$file"
  grep -Fq "else if(!isGenerationFailureStatus(localStatus)||isTransientGenerationTaskFailure(localTask))current=localTask" "$file"
  grep -Fq "await sleepGenerationPoll(Math.max(0,generationTransientRetryDelayMs(err,delay)-delay),signal)" "$file"
}
