...
function fullbloodModelWhere() {
  return {
    ...
    OR: [
      ...
      { modelKey: { contains: 'fullblood', mode: 'insensitive' } },
      { name: { contains: '满血', mode: 'insensitive' } },
      { displayName: { contains: '满血', mode: 'insensitive' } },
    ],
  };
}
...
async function main() {
  ...
  for (const model of models) {
    ...
    await prisma.aiModel.update({ where: { id: model.id }, data });
    if (model.modelKey) {
    ...
    console.log(`[repair-fullblood-hardguard-binding] model=${model.id} modelKey=${model.modelKey} real=${model.name} adapter=${adapter}`);
  }

  console.log(`[repair-fullblood-hardguard-binding] provider=${provider.providerKey} adapter=${provider.adapter} baseUrl=${provider.baseUrl} key=${key.source}`);
}

main()
  .catch(error => {
    console.error('[repair-fullblood-hardguard-binding] failed:', error);
    process.exitCode = 1;
  })
  .finally(async () => {
