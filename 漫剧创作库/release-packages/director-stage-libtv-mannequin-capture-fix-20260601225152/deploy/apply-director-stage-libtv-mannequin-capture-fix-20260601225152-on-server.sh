...
PKG="director-stage-libtv-mannequin-capture-fix-20260601225152"
...
log "install libtv-style mannequin and capture fix"
...
grep -Fq "makeJointGroove('knee'+side,.072" "$DEST"
grep -Fq "directorStageCanvasSnapshotDataUrl" "$DEST"
grep -Fq "renderer.setViewport?.(0,0,rt.renderer.domElement.width,rt.renderer.domElement.height)" "$DEST"
grep -Fq "props:[]" "$DEST"
