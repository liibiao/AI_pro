...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/asset-confirm-editor-mj-pano-actions-20260622163842-${STAMP}"
...
check_markers(){
  local file="$1"
  test -f "$file"
  grep -q "function renderAssetConfirmEditorPreviewHtml" "$file"
  grep -q "function bindAssetConfirmEditorMediaControls" "$file"
  grep -q "function bindAssetConfirmEditorPanoramaControls" "$file"
  grep -q "data-asset-confirm-editor-preview-stage" "$file"
  grep -q "data-asset-confirm-editor-pano-host" "$file"
  grep -q "asset-confirm-editor-row{display:flex" "$file"
  grep -q "asset-confirm-editor-stage .asset-confirm-mj-preview" "$file"
  grep -q "runAssetConfirmCardMidjourneyAction(n.id,card.id" "$file"
  grep -q "generateAssetConfirmCardPanorama(n.id,card.id" "$file"
  grep -q "captureAssetConfirmCardPanoramaGrid(n.id,card.id" "$file"
}
...
echo "deployed asset-confirm-editor-mj-pano-actions-20260622163842"
