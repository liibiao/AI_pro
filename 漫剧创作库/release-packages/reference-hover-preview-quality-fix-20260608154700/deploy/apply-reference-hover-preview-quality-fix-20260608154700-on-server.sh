...
PKG="reference-hover-preview-quality-fix-20260608154700"
...
verify_html_markers(){
  local file="$1"
  grep -Fq '.vn2-chip-preview-popup{position:fixed;' "$file"
  grep -Fq 'function renderHoverPreviewImg(src,maxW=900,maxH=900)' "$file"
  grep -Fq 'original!==toDisplayImageSrc(preview)' "$file"
  grep -Fq 'pop.innerHTML=renderHoverPreviewImg(src,900,900);' "$file"
  grep -Fq 'function shouldSuppressPromptZoomChipHoverPreview(chip){' "$file"
...
verify_html_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq '.vn2-chip-preview-popup{position:fixed;' "$file"
  ...
  run_sudo grep -Fq 'pop.innerHTML=renderHoverPreviewImg(src,900,900);' "$file"
  run_sudo grep -Fq 'function shouldSuppressPromptZoomChipHoverPreview(chip){' "$file"
