...
PKG="seedance-video-toolbar-compact-overlap-fix-20260607015403"
...
verify_markers(){
  local file="$1"
  grep -Fq "20260607: keep video generation bottom toolbar compact and separated from the prompt panel." "$file"
  grep -Fq ".node.node-type-seedanceVideo .vn2-layout>.vn2-bar:not(.prompt-zoomed-item){" "$file"
  grep -Fq "height:40px!important;" "$file"
  grep -Fq "max-height:40px!important;" "$file"
  grep -Fq "gap:5px!important;" "$file"
  grep -Fq "width:82px!important;" "$file"
  grep -Fq "width:128px!important;" "$file"
  grep -Fq "width:32px!important;" "$file"
  grep -Fq "transform:translateX(10px)!important;" "$file"
}
verify_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "20260607: keep video generation bottom toolbar compact and separated from the prompt panel." "$file"
  run_sudo grep -Fq ".node.node-type-seedanceVideo .vn2-layout>.vn2-bar:not(.prompt-zoomed-item){" "$file"
  run_sudo grep -Fq "height:40px!important;" "$file"
  run_sudo grep -Fq "max-height:40px!important;" "$file"
  run_sudo grep -Fq "gap:5px!important;" "$file"
  run_sudo grep -Fq "width:82px!important;" "$file"
  run_sudo grep -Fq "width:128px!important;" "$file"
