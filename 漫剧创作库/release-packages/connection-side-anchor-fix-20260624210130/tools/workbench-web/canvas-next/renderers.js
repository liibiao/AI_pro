...
function firstImage(node){
  const entry=node.data?.images?.[0]||node.values.images?.[0]||node.data||{};
  const saved=entry.saved||node.data?.saved||{};
  return entry.url||entry.dataUrl||entry.previewUrl||entry.preview_url||entry.localUrl||entry.remoteUrl||entry.objectStorageUrl||entry.object_storage_url||entry.assetPreviewUrl||entry.asset_preview_url||entry.resultUrl||entry.result_url||entry.outputUrl||entry.output_url||entry.outputImageUrl||entry.output_image_url||node.data?.url||node.data?.dataUrl||node.data?.previewUrl||saved.localUrl||saved.url||saved.remoteUrl||saved.assetPreviewUrl||saved.objectStorageUrl||'';
}
...
function renderModeControls(node,mode,values,model){
  ...
  if(mode==='video'){
    ...
    const resolution=resolutionOptions.includes(requestedResolution)?requestedResolution:resolutionOptions[0];
...
export function drawConnections(svg,state){
  ...
  const paths=state.conns.map(c=>{
    ...
    if(!a||!b)return '';
    const start=connectionAnchor(a,sr,'out');
    const end=connectionAnchor(b,sr,'in');
    if(!start||!end)return '';
  ...
  if(state.connect&&state.tempPoint){
    ...
    if(a){
      ...
      const anchor=connectionAnchor(a,sr,isReverse?'in':'out');
      if(!anchor)return;
      const ax=anchor.x;
      const ay=anchor.y;
...
function rectSideAnchor(r,dir){
  const y=r.top+r.height/2;
  return dir==='out'?{x:r.left+r.width,y}:{x:r.left,y};
}

function connectionAnchor(port,stageRect,dir='in'){
  ...
  return rectSideAnchor({left:r.left-stageRect.left,top:r.top-stageRect.top,width:r.width,height:r.height},dir);
}
