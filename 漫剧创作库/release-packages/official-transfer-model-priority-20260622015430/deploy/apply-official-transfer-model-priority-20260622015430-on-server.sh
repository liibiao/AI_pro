...
PKG="official-transfer-model-priority-20260622015430"
...
verify_canvas_next_html(){
  local file="$1"
  grep -Fq "{key:'official-transfer',rank:0,pattern:/^官转/}" "$file"
  grep -Fq "data-label-tone=\"gold\"" "$file"
  grep -Fq ".tn-menu-label.tone-gold" "$file"
  grep -Fq "label-tone-gold" "$file"
  grep -Fq "modelOptionLabelToneAttrs" "$file"
}
...
if run_sudo test -f "$WORKBENCH_DIR/image-studio-canvas-next.html"; then
  run_sudo grep -Fq "{key:'official-transfer',rank:0,pattern:/^官转/}" "$WORKBENCH_DIR/image-studio-canvas-next.html"
  run_sudo grep -Fq "data-label-tone=\"gold\"" "$WORKBENCH_DIR/image-studio-canvas-next.html"
  run_sudo grep -Fq ".tn-menu-label.tone-gold" "$WORKBENCH_DIR/image-studio-canvas-next.html"
fi
if run_sudo test -f "$WORKBENCH_DIR/image-studio-canvas.html"; then
  run_sudo grep -Fq "{key:'official-transfer',rank:0,pattern:/^官转/}" "$WORKBENCH_DIR/image-studio-canvas.html"
fi
...
echo "URL: http://124.156.137.236/image-studio-canvas-next.html"
