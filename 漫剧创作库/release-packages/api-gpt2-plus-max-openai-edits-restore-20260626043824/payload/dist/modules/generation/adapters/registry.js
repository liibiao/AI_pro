...
const MIDJOURNEY_SPECIAL_ENDPOINTS = {
...
const registry = {
    ...
    'aistartlab-video': { submit: submitAistartLabVideo, query: queryAistartLabVideo },
...
let midjourneySubmitQueue = Promise.resolve();
...
function openAiImageGeometryOptions(params, ctx) {
    const aspectRatio = resolveOpenAiImageAspectRatio(params);
    return compactJson({
        size: ctx && shouldUseGptImage2VariantSizeToken(ctx) ? resolveOpenAiImageResolution(params).toLowerCase() : resolveOpenAiImageSize(params, aspectRatio),
        aspect_ratio: aspectRatio,
    });
}
function shouldUseGptImage2VariantSizeToken(ctx) {
    ...
    if (hint.includes('aiyunzhi-gpt-image-2'))
        return false;
    return hint.includes('gpt-image-2-max')
        || hint.includes('gpt-image-2-plus')
        || hint.includes('model_gtp-2-max')
        || hint.includes('model_gpt-2-plus')
        || hint.includes('gpt 2 稳定max')
        || hint.includes('gpt 2 优化plus');
}
function resolveOpenAiImageSize(params, aspectRatio) {
...
function resolveOpenAiImageAspectRatio(params) {
...
function resolveOpenAiImageResolution(params) {
...
function openAiImageSizeFromRatioResolution(aspectRatio, resolution) {
    const map = {
...
async function submitOpenAiImage(ctx) {
    ...
    const requestJson = withOpenAiImageRequestOptions(ctx, compactJson({
        ...
        ...openAiImageGeometryOptions(ctx.params, ctx),
...
function openAiResponsesImageGeometryOptions(ctx) {
    ...
    return compactJson({ size });
}
async function queryOpenAiResponsesImage(ctx) {
    return {
        status: 'FAILED',
        progress: 100,
        upstreamTaskId: ctx.upstreamTaskId,
        upstreamRequestId: ctx.upstreamTaskId,
        responseJson: { upstreamTaskId: ctx.upstreamTaskId },
...
function shouldUseDirectOpenAiResponsesImageModel(ctx) {
    const hint = [
        ctx.model.name,
        ctx.model.modelKey,
        ctx.model.displayName,
...
async function normalizeOpenAiResponsesImageSubmit(ctx, upstream, requestJson) {
...
function openAiResponsesImageMissingResultMessage(upstream) {
    const message = extractErrorMessage(upstream);
    if (message && message !== '上游任务失败')
        return message;
    const outputTypes = Array.isArray(upstream?.output)
        ? upstream.output.map((item) => String(item?.type || item?.role || '').trim()).filter(Boolean).join(', ')
        : '';
...
async function callOpenAiResponsesImageJson(ctx, endpoint, requestJson) {
    let lastErr;
    for (let attempt = 0; attempt <= OPENAI_RESPONSES_IMAGE_RETRY_DELAYS_MS.length; attempt += 1) {
        try {
            ...
            lastErr = err;
            if (attempt >= OPENAI_RESPONSES_IMAGE_RETRY_DELAYS_MS.length || !shouldRetryOpenAiResponsesImageError(err))
                throw err;
...
function shouldRetryOpenAiResponsesImageError(err) {
    const message = err instanceof Error ? err.message : String(err || '');
    if (/chatgpt service unavailable|service unavailable|temporarily unavailable|bad gateway|gateway timeout|upstream request timeout|timeout|socket hang up|connection reset|stream disconnected|premature close/i.test(message))
        return true;
    if (err instanceof HttpError) {
        const upstreamStatus = Number(err.upstreamStatus || 0);
        return err.status >= 500 || [502, 503, 504].includes(upstreamStatus);
    }
    return false;
}
function formatOpenAiResponsesImageRetryError(err) {
...
function parseDelayMsList(value, fallback) {
    const raw = String(value || '').trim();
    if (!raw)
        return fallback;
...
async function withMidjourneySubmitSlot(label, fn) {
    let release = () => { };
    const previous = midjourneySubmitQueue.catch(() => undefined);
    midjourneySubmitQueue = new Promise(resolve => {
        release = resolve;
    });
    await previous;
    try {
...
async function callMidjourneySubmitJson(ctx, endpoint, requestJson, label) {
    let lastError;
    for (let attempt = 0; attempt <= MIDJOURNEY_SUBMIT_RETRY_DELAYS_MS.length; attempt += 1) {
        try {
            ...
            if (!midjourneySubmitRejected(upstream))
                return upstream;
            if (attempt >= MIDJOURNEY_SUBMIT_RETRY_DELAYS_MS.length || !isMidjourneySubmitRetryablePayload(upstream))
                return upstream;
            ...
            lastError = err;
            if (attempt >= MIDJOURNEY_SUBMIT_RETRY_DELAYS_MS.length || !isMidjourneySubmitRetryableError(err))
                throw err;
            const delayMs = MIDJOURNEY_SUBMIT_RETRY_DELAYS_MS[attempt] || 0;
...
function isMidjourneySubmitRetryablePayload(payload) {
    const parts = [
...
function isMidjourneySubmitRetryableError(err) {
    if (err instanceof HttpError) {
        const upstreamStatus = Number(err.upstreamStatus || 0);
        if ([408, 409, 425, 429, 500, 502, 503, 504].includes(upstreamStatus) || err.status >= 500)
            return true;
        if (isMidjourneySubmitRetryablePayload(err.responseJson))
            return true;
    }
    return isMidjourneySubmitRetryableMessage(formatMidjourneySubmitRetryError(err));
}
function isMidjourneySubmitRetryableMessage(message) {
...
function formatMidjourneySubmitRetryError(err) {
    const responseMessage = extractErrorMessage(err?.responseJson);
...
async function submitAiyunzhiFireflyGptImage(ctx) {
    assertImageModelName(ctx);
...
function resolveAiyunzhiFireflyEndpointPath(ctx) {
    const protocol = isRecord(ctx.model.protocol) ? ctx.model.protocol : {};
    const providerDefaults = isRecord(ctx.provider.defaultParams) ? ctx.provider.defaultParams : {};
    const configured = String(firstDefined(ctx.params.endpointPath, ctx.params.endpoint_path, protocol.endpointPath, protocol.endpoint_path, ctx.model.endpointPath, ctx.provider.endpointPath, providerDefaults.endpointPath, providerDefaults.endpoint_path) || '').trim();
    if (!configured)
        return AIYUNZHI_FIREFLY_CHAT_ENDPOINT_PATH;
    if (/\/(?:v1\/)?images\/(?:generations|edits)(?:[?#].*)?$/i.test(configured))
        return AIYUNZHI_FIREFLY_CHAT_ENDPOINT_PATH;
    if (/\/v1\/chat\/completions(?:[?#].*)?$/i.test(configured))
        return configured;
...
async function buildAiyunzhiFireflyImageContent(ctx) {
    const groups = await collectPublicImageReferenceUrlGroups(ctx, 15000, { allowLocalFiles: true });
    if (groups.length > 6)
        fail(400, `当前模型最多支持 6 张参考图，当前 ${groups.length} 张`, 'AIYUNZHI_FIREFLY_TOO_MANY_REFERENCES');
    const content = [];
...
function resolveAiyunzhiFireflyDirectReferenceUrl(refs) {
    for (const ref of refs || []) {
        ...
        if (isPublicHttpUrl(raw))
            return normalizeAiyunzhiFireflyImageUrl(raw);
    }
    return '';
}
...
function aiyunzhiFireflyModelNameMode(ctx) {
    const protocol = isRecord(ctx.model.protocol) ? ctx.model.protocol : {};
    const modelAssembly = isRecord(ctx.model.modelAssembly) ? ctx.model.modelAssembly : {};
    const raw = String(firstDefined(ctx.params.modelNameMode, ctx.params.model_name_mode, protocol.modelNameMode, protocol.model_name_mode, modelAssembly.type) || '').trim().toLowerCase();
    if (['literal', 'base', 'passthrough', 'pass-through', 'none'].includes(raw))
        return 'literal';
    return 'template';
}
function fireflyGptImageBaseModel(value) {
...
async function submitAiyunzhiGptImage2(ctx) {
    assertImageModelName(ctx);
    const refGroups = await collectPublicImageReferenceUrlGroups(ctx, 15000, { allowLocalFiles: true });
    if (refGroups.length > 3)
        fail(400, `Aiyunzhi GPT Image 2 最多支持 3 张参考图，当前 ${refGroups.length} 张`, 'AIYUNZHI_GPT_IMAGE2_TOO_MANY_REFERENCES');
    ...
    const prompt = withAiyunzhiGptImage2AspectRatioPrompt(ctx.prompt, ctx);
...
function resolveAiyunzhiGptImage2ModelName(ctx) {
    const raw = String(firstDefined(ctx.params.model, ctx.params.imageModel, ctx.params.image_model, ctx.model.name, ctx.model.displayName, ctx.provider.defaultModel, 'gpt-image-2') || '').trim();
    return normalizeGptImageV2BaseModel(raw) || 'gpt-image-2';
}
function withAiyunzhiGptImage2AspectRatioPrompt(prompt, ctx) {
    const base = String(prompt || '').trim();
    const ratio = resolveAiyunzhiGptImage2AspectRatio(ctx);
    if (!base || !ratio)
        return base;
    const ratioPattern = escapeRegExp(ratio);
    if (new RegExp(`(?:图片比例|画面比例|比例)\\s*${ratioPattern}(?!\\d)`, 'i').test(base))
        return base;
    return `${base}\n图片比例${ratio}`;
}
function resolveAiyunzhiGptImage2AspectRatio(ctx) {
    const raw = String(firstDefined(ctx.params.aspect_ratio, ctx.params.aspectRatio, ctx.params.requestedRatio, ctx.params.requested_ratio, ctx.params.ratio, ctx.params.requestedPixelSize, ctx.params.pixelSize, ctx.params.size) || '').trim().toLowerCase().replace('×', 'x');
    if (!raw || raw === 'auto')
        return '';
    if (/^\d+(?:\.\d+)?\s*:\s*\d+(?:\.\d+)?$/.test(raw))
        return ratioFromSize(raw);
    return ratioFromSize(raw);
}
function normalizeAiyunzhiGptImage2Response(payload) {
...
function extractAiyunzhiGptImage2ImageResult(payload, depth = 0) {
    ...
    if (typeof payload === 'string') {
        ...
        if (!raw)
            return null;
        if (/^data:image\//i.test(raw))
            return { url: raw, mime_type: inferImageMimeType(raw) };
        if (/^[A-Za-z0-9+/=\s]{800,}$/.test(raw) && /^(?:iVBOR|\/9j\/|UklGR|R0lGOD)/.test(raw.replace(/\s+/g, '')))
            return { b64_json: raw, mime_type: 'image/png' };
    ...
    if (url)
        return { url, mime_type: inferImageMimeType(url) };
...
function normalizeExplicitAiyunzhiGptImage2Resolution(value) {
    const raw = String(value || '').trim().toLowerCase().replace(/\s+/g, '').replace('×', 'x');
    if (!raw || raw === 'auto')
        return '';
    if (/^[1-4]k$/.test(raw))
        return normalizeGptImageV2Resolution(raw);
    if (/^(?:uhd|4k|4096|3840|2160p|3k|3072|2880|1800p|qhd|2k|2048|1920|1440p)$/.test(raw)) {
        return normalizeGptImageV2Resolution(raw);
    }
...
function firstAiyunzhiGptImage2Resolution(...values) {
    for (const value of values) {
        const resolution = normalizeExplicitAiyunzhiGptImage2Resolution(value);
        if (resolution)
            return resolution;
    }
    return '';
}
function normalizeAiyunzhiGptImage2PixelSize(value) {
    const raw = String(value || '').trim().toLowerCase().replace(/\s+/g, '').replace('×', 'x');
    return /^\d{3,5}x\d{3,5}$/.test(raw) ? raw : '';
}
function firstAiyunzhiGptImage2PixelSize(...values) {
    for (const value of values) {
        const size = normalizeAiyunzhiGptImage2PixelSize(value);
        if (size)
            return size;
    }
    return '';
}
function resolveAiyunzhiGptImage2Resolution(ctx) {
    const defaults = isRecord(ctx.model.defaults) ? ctx.model.defaults : {};
    const providerDefaults = isRecord(ctx.provider.defaultParams) ? ctx.provider.defaultParams : {};
    const resolution = firstAiyunzhiGptImage2Resolution(ctx.params.resolution, ctx.params.requestedResolution, ctx.params.imageSize, ctx.params.image_size, ctx.params.requestedPixelSize, ctx.params.requested_pixel_size, ctx.params.pixelSize, ctx.params.pixel_size, ctx.params.size, defaults.resolution, defaults.imageSize, defaults.image_size, defaults.size, providerDefaults.resolution, providerDefaults.imageSize, providerDefaults.image_size, providerDefaults.size) || '1K';
    if (!['1K', '2K', '3K', '4K'].includes(resolution)) {
        fail(400, `Aiyunzhi GPT Image 2 只支持 resolution=1k、2k、3k、4k，当前为 ${String(resolution || '').trim() || '空'}`, 'AIYUNZHI_GPT_IMAGE2_SIZE_UNSUPPORTED');
    }
    return resolution;
}
function resolveAiyunzhiGptImage2Size(ctx) {
    const defaults = isRecord(ctx.model.defaults) ? ctx.model.defaults : {};
    const providerDefaults = isRecord(ctx.provider.defaultParams) ? ctx.provider.defaultParams : {};
    const resolution = resolveAiyunzhiGptImage2Resolution(ctx);
    const ratio = resolveAiyunzhiGptImage2AspectRatio(ctx) || '1:1';
    const explicitPixelSize = firstAiyunzhiGptImage2PixelSize(ctx.params.requestedPixelSize, ctx.params.requested_pixel_size, ctx.params.pixelSize, ctx.params.pixel_size, ctx.params.size, defaults.requestedPixelSize, defaults.requested_pixel_size, defaults.pixelSize, defaults.pixel_size, defaults.size, providerDefaults.requestedPixelSize, providerDefaults.requested_pixel_size, providerDefaults.pixelSize, providerDefaults.pixel_size, providerDefaults.size);
    if (explicitPixelSize && normalizeExplicitAiyunzhiGptImage2Resolution(explicitPixelSize) === resolution)
        return explicitPixelSize;
    return openAiImageSizeFromRatioResolution(ratio, resolution) || explicitPixelSize || '1024x1024';
}
function resolveAiyunzhiGptImage2Count(ctx) {
...
function resolveAiyunzhiGptImage2ResponseType(ctx) {
    const protocol = isRecord(ctx.params.protocol) ? ctx.params.protocol : {};
    const modelProtocol = isRecord(ctx.model.protocol) ? ctx.model.protocol : {};
    const defaults = isRecord(ctx.model.defaults) ? ctx.model.defaults : {};
    const providerDefaults = isRecord(ctx.provider.defaultParams) ? ctx.provider.defaultParams : {};
    return normalizeImageResponseTypeValue(firstDefined(modelProtocol.response_type, defaults.response_type, providerDefaults.response_type, ctx.params.response_type, protocol.response_type, modelProtocol.responseType, defaults.responseType, providerDefaults.responseType, ctx.params.responseType, protocol.responseType)) || resolveImageResponseType(ctx);
}
function resolveAiyunzhiGptImage2Endpoint(ctx, kind) {
    const protocol = isRecord(ctx.params.protocol) ? ctx.params.protocol : {};
    const modelProtocol = isRecord(ctx.model.protocol) ? ctx.model.protocol : {};
    const defaults = isRecord(ctx.model.defaults) ? ctx.model.defaults : {};
    const providerDefaults = isRecord(ctx.provider.defaultParams) ? ctx.provider.defaultParams : {};
    const explicit = String(firstDefined(kind === 'generation' ? ctx.params.generationEndpointPath : ctx.params.editEndpointPath, kind === 'generation' ? ctx.params.generation_endpoint_path : ctx.params.edit_endpoint_path, kind === 'generation' ? protocol.generationEndpointPath : protocol.editEndpointPath, kind === 'generation' ? protocol.generation_endpoint_path : protocol.edit_endpoint_path, kind === 'generation' ? modelProtocol.generationEndpointPath : modelProtocol.editEndpointPath, kind === 'generation' ? modelProtocol.generation_endpoint_path : modelProtocol.edit_endpoint_path, kind === 'generation' ? defaults.generationEndpointPath : defaults.editEndpointPath, kind === 'generation' ? defaults.generation_endpoint_path : defaults.edit_endpoint_path, kind === 'generation' ? providerDefaults.generationEndpointPath : providerDefaults.editEndpointPath, kind === 'generation' ? providerDefaults.generation_endpoint_path : providerDefaults.edit_endpoint_path, ctx.params.endpointPath, ctx.params.endpoint_path, protocol.endpointPath, protocol.endpoint_path, modelProtocol.endpointPath, modelProtocol.endpoint_path, defaults.endpointPath, defaults.endpoint_path, providerDefaults.endpointPath, providerDefaults.endpoint_path) || '').trim();
    if (explicit)
        return explicit;
    const configured = String(ctx.model.endpointPath || ctx.provider.endpointPath || '').trim();
    if (kind === 'generation' && /\/images\/generations(?:$|[?#])/i.test(configured))
        return configured;
    if (kind === 'edit' && /\/images\/edits(?:$|[?#])/i.test(configured))
        return configured;
    return kind === 'generation' ? '/v1/images/generations' : '/v1/images/edits';
}
async function callAiyunzhiFireflyJson(ctx, endpointPath, requestJson) {
    const apiKey = decryptSecret(ctx.provider.apiKeyEncrypted || '');
    if (!apiKey)
        fail(400, '上游 API Key 未配置', 'PROVIDER_KEY_MISSING');
    ...
    try {
        ...
        if (err instanceof Error && err.name === 'AbortError')
            fail(504, `Firefly GPT Image 上游请求超时（${ctx.timeoutMs}ms）`, 'UPSTREAM_TIMEOUT');
...
function normalizeAiyunzhiFireflyResponse(rawText) {
    const text = String(rawText || '').trim();
    if (!text)
        return { errorMessage: 'Firefly GPT Image 返回为空' };
    try {
        const payload = JSON.parse(text);
        const imageUrl = extractAiyunzhiFireflyImageUrlFromText(JSON.stringify(payload));
        if (imageUrl)
            return { data: [{ url: imageUrl }], rawResponse: payload };
        return payload;
    }
    catch {
    ...
    if (imageUrl)
        return { data: [{ url: imageUrl }], rawResponse: { streamText: text } };
    return { errorMessage: `Firefly GPT Image 返回中未找到图片 URL：${text.slice(0, 500)}`, rawResponse: { streamText: text } };
}
function collectAiyunzhiFireflyContentParts(value, parts) {
    if (!value)
        return;
    ...
    if (Array.isArray(item.choices))
        item.choices.forEach(choice => collectAiyunzhiFireflyContentParts(choice, parts));
}
function extractAiyunzhiFireflyImageUrlFromText(value) {
    const source = String(value || '');
    const markdown = source.match(/!\[[^\]]*]\((https?:\/\/[^)\s]+)\)/i);
    if (markdown?.[1])
        return normalizeAiyunzhiFireflyImageUrl(markdown[1]);
...
function normalizeAiyunzhiFireflyImageUrl(value) {
...
function isAiyunzhiFireflyPresignedImageUrl(value) {
...
function decodeUrlEntityEscapes(value) {
...
async function submitMidjourneyImagine(ctx) {
    ...
    if (submitKind !== 'imagine')
        return submitMidjourneySpecialTask(ctx, submitKind);
...
async function submitMidjourneySpecialTask(ctx, kind) {
        ...
        if (customId) {
            ...
            if (midjourneySubmitRejected(actionUpstream) || !actionTaskId) {
                return {
                    ...
                    upstreamRequestId: String(actionUpstream?.request_id || actionUpstream?.requestId || actionUpstream?.id || actionTaskId || ''),
                    requestJson: { action: actionRequest, modal: requestJson },
                    responseJson: sanitizeUpstreamPayload(actionUpstream),
                    resultJson: { url: '', outputs: [], taskId: actionTaskId, taskKind: kind },
                    resultUrls: [],
                    errorCode: 'MIDJOURNEY_MODAL_ACTION_PREPARE_FAILED',
                    errorMessage: extractErrorMessage(actionUpstream) || 'Midjourney 局部重绘交互初始化失败',
                };
            }
            requestJson = compactJson({ ...requestJson, taskId: actionTaskId });
    ...
    if (rejected) {
        return {
            ...
            upstreamRequestId: String(upstream?.request_id || upstream?.requestId || upstream?.id || taskId || ''),
            requestJson,
            responseJson: responseJson || sanitizeUpstreamPayload(upstream),
            resultJson: { url: '', outputs: [], taskId, taskKind: kind },
            resultUrls: [],
            errorCode: `MIDJOURNEY_${kind.toUpperCase()}_SUBMIT_FAILED`,
            errorMessage: extractErrorMessage(upstream) || String(upstream?.description || `Midjourney ${kind} 提交失败`),
        };
    }
    const directResult = await normalizeImageSubmit(ctx.provider, upstream, requestJson, resolveImageResponseType(ctx));
    if (directResult.status === 'SUCCESS' || directResult.resultUrls?.length)
        return withMidjourneySpecialResultJson(directResult, upstream, kind);
    if (taskId) {
        return {
            status: 'RUNNING',
            progress: extractGenericImageProgress(upstream) || 1,
            ...
            requestJson,
            responseJson: responseJson || sanitizeUpstreamPayload(upstream),
            resultJson: { url: '', outputs: [], taskId, taskKind: kind, mjActions: extractMidjourneyActionButtons(upstream) },
            resultUrls: [],
        };
    }
    return {
...
function normalizeMidjourneySubmitKind(ctx) {
    const explicit = String(firstDefined(ctx.params.mjTaskKind, ctx.params.mj_task_kind, ctx.params.midjourneyTaskKind, ctx.params.midjourney_task_kind, ctx.params.submitKind, ctx.params.submit_kind, ctx.params.actionKind, ctx.params.action_kind) || '').trim().toLowerCase();
    if (isMidjourneySubmitKind(explicit))
        return explicit;
    ...
    if (modeMatch && isMidjourneySubmitKind(modeMatch[1]))
        return modeMatch[1];
    ...
    if (endpointMatch && isMidjourneySubmitKind(endpointMatch[1]))
        return endpointMatch[1];
    return 'imagine';
}
function isMidjourneySubmitKind(value) {
    return ['imagine', 'describe', 'blend', 'action', 'modal'].includes(String(value || ''));
}
function resolveMidjourneySpecialEndpoint(ctx, kind) {
    const explicit = String(firstDefined(ctx.params.endpointPath, ctx.params.endpoint_path, '') || '').trim();
    if (explicit && /\/mj\/submit\/(?:describe|blend|action|modal)\b/i.test(explicit))
        return explicit;
    return MIDJOURNEY_SPECIAL_ENDPOINTS[kind];
}
async function buildMidjourneySpecialTaskRequest(ctx, kind) {
    if (kind === 'describe')
        return buildMidjourneyDescribeRequest(ctx);
    if (kind === 'blend')
        return buildMidjourneyBlendRequest(ctx);
    if (kind === 'modal')
        return buildMidjourneyModalRequest(ctx);
    return buildMidjourneyActionRequest(ctx);
}
function midjourneyCommonSubmitFields(ctx) {
    const defaults = isRecord(ctx.model.defaults) ? ctx.model.defaults : {};
    const providerDefaults = isRecord(ctx.provider.defaultParams) ? ctx.provider.defaultParams : {};
    return {
...
function midjourneyBotType(ctx) {
    const defaults = isRecord(ctx.model.defaults) ? ctx.model.defaults : {};
    const providerDefaults = isRecord(ctx.provider.defaultParams) ? ctx.provider.defaultParams : {};
...
async function buildMidjourneyDescribeRequest(ctx) {
    const image = await collectMidjourneyDescribeImage(ctx);
    if (!image.link && !image.base64)
        fail(400, 'Midjourney Describe 需要当前图片或参考图 URL/base64', 'MIDJOURNEY_DESCRIBE_IMAGE_REQUIRED');
...
async function buildMidjourneyBlendRequest(ctx) {
    const base64Array = await collectMidjourneyBlendBase64Array(ctx);
    if (base64Array.length < 2)
        fail(400, 'Midjourney Blend 至少需要 2 张参考图', 'MIDJOURNEY_BLEND_IMAGES_REQUIRED');
    const accountFilter = buildMidjourneyAccountFilter(ctx);
    return compactJson({
        ...midjourneyCommonSubmitFields(ctx),
        botType: midjourneyBotType(ctx),
...
function buildMidjourneyActionRequest(ctx) {
    ...
    const customId = String(firstDefined(ctx.params.customId, ctx.params.custom_id, ctx.params.mjCustomId, ctx.params.mj_custom_id, '') || '').trim();
    if (!taskId)
        fail(400, 'Midjourney Action 缺少 taskId', 'MIDJOURNEY_ACTION_TASK_ID_REQUIRED');
    if (!customId)
        fail(400, 'Midjourney Action 缺少 customId，请先刷新任务或选择带有上游按钮数据的 MJ 结果', 'MIDJOURNEY_ACTION_CUSTOM_ID_REQUIRED');
    return compactJson({
        ...midjourneyCommonSubmitFields(ctx),
        taskId,
        customId,
...
function buildMidjourneyModalRequest(ctx) {
    const taskId = String(firstDefined(ctx.params.taskId, ctx.params.task_id, ctx.params.sourceTaskId, ctx.params.source_task_id, ctx.params.upstreamTaskId, '') || '').trim();
    if (!taskId)
        fail(400, 'Midjourney Modal 缺少 taskId', 'MIDJOURNEY_MODAL_TASK_ID_REQUIRED');
...
async function collectMidjourneyDescribeImage(ctx) {
    ...
    if (explicitLink || explicitBase64)
        return { link: explicitLink, base64: explicitBase64 };
    const groups = await collectPublicImageReferenceUrlGroups(ctx, 15000, { allowLocalFiles: true });
    for (const group of groups) {
...
async function collectMidjourneyBlendBase64Array(ctx) {
    const explicit = ctx.params.base64Array || ctx.params.base64_array;
    if (Array.isArray(explicit))
        return uniqueStrings(explicit).slice(0, 6);
    const groups = await collectPublicImageReferenceUrlGroups(ctx, 15000, { allowLocalFiles: true });
    const out = [];
    for (const group of groups) {
        for (const ref of group) {
...
function normalizeMidjourneyBlendDimensions(value) {
    const raw = String(value || '').trim().toLowerCase();
    if (['portrait', '竖图', '竖版', '2:3', '3:4', '9:16'].includes(raw))
        return 'PORTRAIT';
    if (['landscape', '横图', '横版', '3:2', '4:3', '16:9'].includes(raw))
        return 'LANDSCAPE';
    return 'SQUARE';
}
async function midjourneyBase64DataUriFromReference(ref, timeoutMs) {
    const raw = String(ref || '').trim();
    if (!raw)
        return '';
    if (/^data:image\//i.test(raw))
    ...
    try {
        ...
        if (!resp.ok)
            fail(400, `Midjourney Blend 参考图下载失败 HTTP ${resp.status}`, 'MIDJOURNEY_BLEND_REFERENCE_FETCH_FAILED');
...
function mergeResultJson(base, patch) {
    const cleaned = Object.fromEntries(Object.entries(patch).filter(([, item]) => item !== undefined && item !== null && item !== ''));
    return { ...(isRecord(base) ? base : {}), ...cleaned };
}
function withMidjourneySpecialResultJson(result, upstream, kind) {
    return {
        ...result,
...
function extractMidjourneyDescribeTexts(payload) {
    const out = [];
    const visit = (value, key = '', depth = 0) => {
        ...
        if (typeof value === 'string') {
            ...
            if (text && /description|descriptions|prompt|prompts|imagine/i.test(key) && !/提交成功|success|completed/i.test(text))
                out.push(text);
            return;
        }
        if (Array.isArray(value)) {
...
function extractMidjourneyActionButtons(payload) {
    const out = [];
    const seen = new Set();
...
function cleanMidjourneyPromptText(value) {
    return String(value || '')
        ...
        if (defaultMatch)
            return String(defaultMatch[2] ?? defaultMatch[1] ?? '').trim();
        const valueMatch = raw.match(/\bvalue\s*=\s*(["'])(.*?)\1/i) || raw.match(/\bvalue\s*=\s*([^\s}]+)/i);
        if (valueMatch)
            return String(valueMatch[2] ?? valueMatch[1] ?? '').trim();
...
function buildMidjourneyParamSuffix(value) {
    if (value == null)
        return '';
    ...
    const command = cleanMidjourneyPromptText(isRecord(value)
        ...
        : value);
    const versionValue = firstDefined(source.version, source.modelVersion, source.model_version, source.v, midjourneyVersionFromPrompt(command), 'v7');
...
function stripManagedMidjourneyPromptParams(value) {
    return (` ${String(value || '')}`)
...
function midjourneyVersionFromPrompt(value) {
    const match = String(value || '').match(/--(niji|v)\s+(\d+(?:\.\d+)?)/i);
    return match ? `${match[1].toLowerCase()}${match[2]}` : '';
}
function midjourneyNumericParamFromPrompt(value, names) {
    const match = String(value || '').match(new RegExp(`--(?:${names})\\s+(-?\\d+(?:\\.\\d+)?)`, 'i'));
    return match ? match[1] : undefined;
}
function normalizeMidjourneyQualityForVersion(value, version) {
...
function midjourneyVersionFlagFromValue(value) {
    const raw = String(value || '').trim().toLowerCase().replace(/\s+/g, '');
    if (!raw)
        return '';
    if (raw.startsWith('niji'))
        return `--niji ${raw.replace(/^niji/, '') || '7'}`;
    return `--v ${raw.replace(/^v/, '') || '7'}`;
}
function escapeRegExp(value) {
    return String(value).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
}
function appendMissingMidjourneyPromptSuffixes(suffixes, prompt, suffix) {
    ...
    for (const piece of pieces) {
        ...
        if (flag && new RegExp(`${escapeRegExp(flag)}\\b`, 'i').test(base))
            continue;
        if (!flag && base.toLowerCase().includes(piece.toLowerCase()))
            continue;
        suffixes.push(piece);
    }
}
...
function midjourneySubmitRejected(payload) {
    ...
    if (isMidjourneyAcceptedPendingSubmit(payload, raw))
        return false;
    return !['0', '1', '200', 'success', 'ok'].includes(raw);
}
function isMidjourneyAcceptedPendingSubmit(payload, code) {
    if (!['21'].includes(code))
        return false;
    const taskId = extractMidjourneyTaskId(payload);
    if (!taskId)
        return false;
    const description = String(payload?.description || payload?.data?.description || '').toLowerCase();
...
function openAiImageResponseFormat(ctx) {
    ...
    if (responseType === 'base64' || responseType === 'server_base64_object_storage' || responseType === 'server_base64_async_object_storage')
        return 'b64_json';
    if (responseType === 'provider_url' || responseType === 'object_storage' || responseType === 'server_object_storage' || responseType === 'server_async_object_storage')
        return 'url';
...
function withGptImage2ResolutionModel(ctx, requestJson) {
    if (!shouldUseGptImage2ResolutionTierModel(ctx))
        return requestJson;
    const request = isRecord(requestJson) ? { ...requestJson } : {};
...
function resolveGptImage2ResolutionModelName(ctx, requestModel, requestSize) {
    ...
    if (shouldUseGptImage2FullResolutionSuffixModel(ctx))
        return `${baseModel}-${resolution}`;
    if (resolution === '1K')
        return baseModel;
    return resolveGptImageV2ModelName(baseModel, resolution);
}
function resolveGptImage2Resolution(ctx, requestSize) {
    const defaults = isRecord(ctx.model.defaults) ? ctx.model.defaults : {};
    const providerDefaults = isRecord(ctx.provider.defaultParams) ? ctx.provider.defaultParams : {};
    const resolution = normalizeGptImageV2Resolution(firstDefined(ctx.params.resolution, ctx.params.requestedResolution, ctx.params.imageSize, ctx.params.requestedPixelSize, ctx.params.size, requestSize, defaults.resolution, defaults.imageSize, providerDefaults.resolution, providerDefaults.imageSize));
    if (isGptImage2ProOfficialProxyRequest(ctx))
        return resolution === '1K' ? '1K' : '2K';
    return resolution;
}
function resolveGptImage2AspectRatio(ctx, requestSize) {
    ...
    if (isGptImage2ProOfficialProxyRequest(ctx))
        return resolveGptImage2ProOfficialProxyAspectRatio(raw);
    ...
    if (raw === 'landscape' || raw === 'wide')
        return '16:9';
    if (raw === 'portrait' || raw === 'vertical')
        return '9:16';
    if (raw === 'square')
        return '1:1';
    return '16:9';
}
function resolveGptImage2ProOfficialProxyAspectRatio(rawValue) {
...
function aspectRatioNumberFromValue(value) {
...
function isHongniaoGptImage2GenerationRequest(ctx) {
    if (!isGptImage2Request(ctx))
        return false;
    const protocol = isRecord(ctx.params.protocol) ? ctx.params.protocol : {};
    const hint = [
...
function shouldUseGptImage2GenerationsEndpoint(ctx) {
    if (!isGptImage2Request(ctx))
        return false;
    const protocol = isRecord(ctx.params.protocol) ? ctx.params.protocol : {};
    const hint = [
...
function shouldUseGptImage2FullResolutionSuffixModel(ctx) {
    return isHongniaoGptImage2GenerationRequest(ctx);
}
function shouldUseGptImage2ResolutionTierModel(ctx) {
    if (!isGptImage2Request(ctx))
        return false;
    const hint = [
        ctx.provider.providerKey,
...
function isGptImage2ProOfficialProxyRequest(ctx) {
    if (!isGptImage2Request(ctx))
        return false;
    if (isHongniaoGptImage2GenerationRequest(ctx))
        return false;
    const hint = [
        ctx.provider.providerKey,
        ctx.provider.name,
...
function resolveOpenAiTextImageEndpoint(ctx) {
    if (shouldUseGptImage2ResolutionTierModel(ctx) || shouldUseGptImage2GenerationsEndpoint(ctx))
...
function resolveOpenAiEditsEndpoint(ctx) {
    if (shouldUseGptImage2GenerationsEndpoint(ctx))
        return '/v1/images/generations';
    if (isGptImage2Request(ctx))
        return '/images/edits';
...
async function submitGenericVideo(ctx) {
    ...
    const responseKeys = upstream && typeof upstream === 'object' && !Array.isArray(upstream)
        ? Object.keys(upstream).slice(0, 20).join(', ')
        : typeof upstream;
    return {
...
async function submitAistartLabVideo(ctx) {
    const requestJson = await buildAistartLabVideoRequestJson(ctx);
    const upstream = await callUpstreamJson(ctx.provider, resolveEndpointPath(resolveAistartLabSubmitEndpointPath(ctx), ctx), requestJson, ctx.timeoutMs);
    return normalizeAistartLabVideoTaskResult(ctx, upstream, requestJson);
}
async function queryAistartLabVideo(ctx) {
    const endpointPath = resolveEndpointPath(resolveAistartLabStatusEndpointPath(ctx), { ...ctx, taskId: ctx.upstreamTaskId });
    const query = /(?:^|[?&])taskId=|(?:^|[?&])task_id=/i.test(endpointPath) ? {} : { taskId: ctx.upstreamTaskId };
    const upstream = await callUpstreamGetJson(ctx.provider, endpointPath, query, ctx.timeoutMs);
    return normalizeAistartLabVideoTaskResult(ctx, upstream, undefined, ctx.upstreamTaskId);
}
async function buildAistartLabVideoRequestJson(ctx) {
    const prompt = String(ctx.prompt || '').trim();
    if (!prompt)
        fail(400, 'AIStartLab 视频生成 prompt 不能为空', 'AISTARTLAB_PROMPT_REQUIRED');
    if (prompt.length > 5000)
        fail(400, `AIStartLab 视频生成 prompt 最多 5000 字符，当前 ${prompt.length} 字符`, 'AISTARTLAB_PROMPT_TOO_LONG');
    const images = buildVideoReferenceUrls(ctx);
    const videos = await collectAistartLabReferenceMediaUrls(ctx, 'video', resolveAistartLabMaxVideos(ctx));
    const audios = await collectAistartLabReferenceMediaUrls(ctx, 'audio', resolveAistartLabMaxAudios(ctx));
    const resolution = resolveAistartLabResolution(ctx);
    const model = resolveAistartLabModel(ctx, resolution);
    const seconds = resolveAistartLabSeconds(ctx);
    const size = resolveAistartLabSize(ctx);
    const modeType = resolveAistartLabModeType(ctx, images);
    assertAistartLabVideoRequest(ctx, { modeType, images, videos, audios, seconds, size, resolution });
    return compactJson({
        channel: resolveAistartLabChannel(ctx),
        model,
        resolution,
        prompt,
        seconds,
...
function resolveAistartLabSubmitEndpointPath(ctx) {
    const protocol = isRecord(ctx.model.protocol) ? ctx.model.protocol : {};
    const providerDefaults = isRecord(ctx.provider.defaultParams) ? ctx.provider.defaultParams : {};
    return String(firstDefined(ctx.params.endpointPath, ctx.params.endpoint_path, protocol.endpointPath, protocol.endpoint_path, ctx.model.endpointPath, ctx.provider.endpointPath, providerDefaults.endpointPath, providerDefaults.endpoint_path, '/video/task/v2') || '/video/task/v2').trim();
}
function resolveAistartLabStatusEndpointPath(ctx) {
    const protocol = isRecord(ctx.model.protocol) ? ctx.model.protocol : {};
    const providerDefaults = isRecord(ctx.provider.defaultParams) ? ctx.provider.defaultParams : {};
    return String(firstDefined(ctx.params.statusEndpointPath, ctx.params.status_endpoint_path, protocol.statusEndpointPath, protocol.status_endpoint_path, ctx.model.statusEndpointPath, ctx.provider.statusEndpointPath, providerDefaults.statusEndpointPath, providerDefaults.status_endpoint_path, '/video/task/status') || '/video/task/status').trim();
}
function resolveAistartLabChannel(ctx) {
    const protocol = isRecord(ctx.model.protocol) ? ctx.model.protocol : {};
    const defaults = isRecord(ctx.model.defaults) ? ctx.model.defaults : {};
    const providerDefaults = isRecord(ctx.provider.defaultParams) ? ctx.provider.defaultParams : {};
    const value = firstDefined(ctx.params.aistartlabChannel, ctx.params.aistartlab_channel, ctx.params.upstreamChannel, ctx.params.upstream_channel, ctx.params.channel, protocol.channel, protocol.upstreamChannel, protocol.upstream_channel, defaults.channel, defaults.upstreamChannel, defaults.upstream_channel, providerDefaults.channel, providerDefaults.upstreamChannel, providerDefaults.upstream_channel);
    const channel = String(value || '').trim();
    if (!channel)
        fail(400, 'AIStartLab 视频渠道缺少上游 channel，请在模型 protocol.channel 或 defaults.channel 中配置', 'AISTARTLAB_CHANNEL_REQUIRED');
    return channel;
}
function resolveAistartLabModel(ctx, resolution) {
    const model = resolveAistartLabBaseModel(ctx);
    return assembleAistartLabModelName(ctx, model, resolution);
}
function resolveAistartLabBaseModel(ctx) {
    const protocol = isRecord(ctx.model.protocol) ? ctx.model.protocol : {};
    const defaults = isRecord(ctx.model.defaults) ? ctx.model.defaults : {};
    const providerDefaults = isRecord(ctx.provider.defaultParams) ? ctx.provider.defaultParams : {};
    const model = String(firstDefined(ctx.params.upstreamModel, ctx.params.upstream_model, ctx.params.model, protocol.model, protocol.upstreamModel, protocol.upstream_model, defaults.model, providerDefaults.model, ctx.model.name, ctx.provider.defaultModel) || '').trim();
    if (!model)
        fail(400, 'AIStartLab 视频渠道缺少上游 model，请在模型 name/model 或 protocol.model 中配置', 'AISTARTLAB_MODEL_REQUIRED');
    return model;
}
function assembleAistartLabModelName(ctx, baseModel, resolution) {
    const protocol = isRecord(ctx.model.protocol) ? ctx.model.protocol : {};
    const modelAssembly = isRecord(ctx.model.modelAssembly) ? ctx.model.modelAssembly : {};
    const rawMode = String(firstDefined(ctx.params.modelNameMode, ctx.params.model_name_mode, ctx.params.modelAssemblyType, ctx.params.model_assembly_type, protocol.modelNameMode, protocol.model_name_mode, modelAssembly.type) || '').trim().toLowerCase();
    if (!['template', 'resolution', 'resolution-suffix', 'suffix'].includes(rawMode))
        return baseModel;
    const normalizedResolution = normalizeAistartLabModelResolution(resolution || ctx.params.resolution || ctx.params.quality);
    if (!normalizedResolution)
        return baseModel;
    if (new RegExp(`-${escapeRegExp(normalizedResolution)}$`, 'i').test(baseModel))
        return baseModel;
    const resolutionModels = firstDefined(modelAssembly.resolutionModels, modelAssembly.modelsByResolution, protocol.resolutionModels, protocol.modelsByResolution);
    if (isRecord(resolutionModels)) {
        const mapped = String(firstDefined(resolutionModels[normalizedResolution], resolutionModels[normalizedResolution.toLowerCase()], resolutionModels[normalizedResolution.toUpperCase()]) || '').trim();
        if (mapped)
            return mapped;
    }
    const template = String(firstDefined(ctx.params.modelTemplate, ctx.params.model_template, protocol.modelTemplate, protocol.model_template, modelAssembly.template, '{model}-{resolution}') || '{model}-{resolution}').trim() || '{model}-{resolution}';
    return template
        .replace(/\{(?:baseModel|base_model|model)\}/g, baseModel)
        .replace(/\{resolution\}/g, normalizedResolution)
        .trim() || baseModel;
}
function normalizeAistartLabModelResolution(value) {
    const raw = String(value || '').trim().toLowerCase();
    if (!raw)
        return '';
    if (raw.includes('1080'))
        return '1080p';
    if (raw.includes('720'))
        return '720p';
    if (raw.includes('480'))
...
function resolveAistartLabResolution(ctx) {
    const caps = isRecord(ctx.model.capabilities) ? ctx.model.capabilities : {};
    const defaults = isRecord(ctx.model.defaults) ? ctx.model.defaults : {};
    const resolutions = normalizeStringList(caps.resolutions);
    const value = String(firstDefined(ctx.params.resolution, ctx.params.quality, ctx.params.requestedResolution, defaults.resolution, caps.defaultResolution, resolutions[0]) || '').trim();
    if (value && resolutions.length && !resolutions.includes(value)) {
        fail(400, `AIStartLab 当前模型不支持 resolution=${value}，可选：${resolutions.join(', ')}`, 'AISTARTLAB_RESOLUTION_UNSUPPORTED');
    }
    return value || undefined;
}
function resolveAistartLabSeconds(ctx) {
    const caps = isRecord(ctx.model.capabilities) ? ctx.model.capabilities : {};
    const defaults = isRecord(ctx.model.defaults) ? ctx.model.defaults : {};
    const raw = firstDefined(ctx.params.seconds, ctx.params.duration, ctx.params.durationSeconds, ctx.params.duration_seconds, defaults.seconds, defaults.duration, caps.defaultDuration, 5);
    const seconds = Number.parseInt(String(raw || '').replace(/s$/i, ''), 10);
    if (!Number.isFinite(seconds) || seconds <= 0)
        fail(400, `AIStartLab seconds 必须为正整数，当前为 ${String(raw || '空')}`, 'AISTARTLAB_SECONDS_INVALID');
    const min = normalizePositiveInt(caps.secondsMin ?? caps.minSeconds ?? caps.minVideoDurationSeconds ?? defaults.secondsMin ?? defaults.minSeconds) || 1;
    const max = normalizePositiveInt(caps.secondsMax ?? caps.maxSeconds ?? caps.maxVideoDurationSeconds ?? defaults.secondsMax ?? defaults.maxSeconds) || 120;
    if (seconds < min || seconds > max)
        fail(400, `AIStartLab seconds 必须在 ${min}-${max} 秒之间，当前 ${seconds} 秒`, 'AISTARTLAB_SECONDS_UNSUPPORTED');
    const allowed = normalizeVideoDurationList(caps.durations);
    if (allowed.length && !allowed.includes(seconds)) {
        fail(400, `AIStartLab 当前模型不支持 ${seconds} 秒，可选：${allowed.join(', ')}`, 'AISTARTLAB_SECONDS_UNSUPPORTED');
    }
    return seconds;
}
function resolveAistartLabSize(ctx) {
    const caps = isRecord(ctx.model.capabilities) ? ctx.model.capabilities : {};
    const defaults = isRecord(ctx.model.defaults) ? ctx.model.defaults : {};
    const aspectRatios = normalizeStringList(caps.aspectRatios ?? caps.aspect_ratios);
    const size = String(firstDefined(ctx.params.size, ctx.params.aspectRatio, ctx.params.aspect_ratio, ctx.params.requestedRatio, defaults.size, defaults.aspectRatio, defaults.aspect_ratio, caps.defaultAspectRatio, aspectRatios[0], '16:9') || '16:9').trim().replace(/\s+/g, '');
    if (aspectRatios.length && /^\d+(?:\.\d+)?:\d+(?:\.\d+)?$/.test(size) && !aspectRatios.includes(size)) {
        fail(400, `AIStartLab 当前线路不支持 size=${size}，可选：${aspectRatios.join(', ')}`, 'AISTARTLAB_SIZE_UNSUPPORTED');
    }
    return size;
}
function resolveAistartLabModeType(ctx, images) {
    const raw = String(firstDefined(ctx.params.modeType, ctx.params.mode_type, ctx.params.videoMode, ctx.params.video_mode, ctx.params.refMode, ctx.params.ref_mode, ctx.mode) || '').trim().toLowerCase();
    if (/frames2video|first[-_ ]?last|start[-_ ]?end|首尾/.test(raw))
        return 'frames2video';
    if (/image2video|image[-_ ]?to[-_ ]?video|img2video|i2v|reference[-_ ]?to[-_ ]?video|首帧|参考/.test(raw))
        return images.length ? 'image2video' : 'text2video';
    if (/text2video|text[-_ ]?to[-_ ]?video|txt2video|t2v|文生/.test(raw))
        return 'text2video';
    return images.length ? 'image2video' : 'text2video';
}
function assertAistartLabVideoRequest(ctx, request) {
    const caps = isRecord(ctx.model.capabilities) ? ctx.model.capabilities : {};
    const supportedModes = normalizeStringList(caps.supportedModeTypes ?? caps.supported_mode_types);
    if (supportedModes.length && !supportedModes.includes(request.modeType)) {
        fail(400, `AIStartLab 当前线路不支持 modeType=${request.modeType}，可选：${supportedModes.join(', ')}`, 'AISTARTLAB_MODE_UNSUPPORTED');
    }
    if (request.modeType === 'text2video' && request.images.length) {
        fail(400, 'AIStartLab text2video 不允许传参考图片', 'AISTARTLAB_TEXT2VIDEO_IMAGES_UNSUPPORTED');
    }
    if (request.modeType === 'image2video' && !request.images.length) {
        fail(400, 'AIStartLab image2video 至少需要 1 张参考图片', 'AISTARTLAB_IMAGE2VIDEO_IMAGE_REQUIRED');
    }
    if (request.modeType === 'frames2video' && request.images.length !== 2) {
        fail(400, `AIStartLab frames2video 必须正好传 2 张首尾帧图片，当前 ${request.images.length} 张`, 'AISTARTLAB_FRAMES2VIDEO_IMAGE_COUNT');
    }
    if (request.modeType === 'frames2video' && request.audios.length && caps.frames2VideoSupportsAudio === false) {
        fail(400, 'AIStartLab 当前线路首尾帧模式不支持参考音频', 'AISTARTLAB_FRAMES2VIDEO_AUDIO_UNSUPPORTED');
    }
    if (request.images.length > resolveAistartLabMaxImages(ctx))
        fail(400, 'AIStartLab 参考图片最多 9 张', 'AISTARTLAB_TOO_MANY_IMAGES');
    if (request.videos.length > resolveAistartLabMaxVideos(ctx))
        fail(400, 'AIStartLab 参考视频最多 3 条', 'AISTARTLAB_TOO_MANY_VIDEOS');
    if (request.audios.length > resolveAistartLabMaxAudios(ctx))
        fail(400, 'AIStartLab 参考音频最多 3 条', 'AISTARTLAB_TOO_MANY_AUDIOS');
}
async function collectAistartLabReferenceMediaUrls(ctx, mediaType, limit) {
    const keys = mediaType === 'video'
        ? ['videos', 'video', 'videoUrl', 'video_url', 'refVideo', 'reference_videos', 'referenceVideos', 'reference_video_urls', 'referenceVideoUrls', 'reference_video', 'referenceVideo']
        : ['audios', 'audio', 'audioUrl', 'audio_url', 'refAudio', 'reference_audios', 'referenceAudios', 'reference_audio_urls', 'referenceAudioUrls', 'reference_audio', 'referenceAudio'];
    const urls = buildSoraVideoProUrlList(ctx, keys, mediaType, limit);
    if (urls.length > limit) {
        fail(400, mediaType === 'video' ? 'AIStartLab 参考视频最多 3 条' : 'AIStartLab 参考音频最多 3 条', mediaType === 'video' ? 'AISTARTLAB_TOO_MANY_VIDEOS' : 'AISTARTLAB_TOO_MANY_AUDIOS');
    }
    return materializeSoraVideoProMediaUrls(ctx, urls, mediaType, limit);
}
function resolveAistartLabMaxImages(ctx) {
    const caps = isRecord(ctx.model.capabilities) ? ctx.model.capabilities : {};
    const maxImages = caps.maxImages;
    if (isRecord(maxImages))
        return normalizePositiveInt(maxImages.full ?? maxImages.max ?? maxImages.default) || 9;
    return normalizePositiveInt(caps.maxImages ?? caps.max_images) || 9;
}
function resolveAistartLabMaxVideos(ctx) {
    const caps = isRecord(ctx.model.capabilities) ? ctx.model.capabilities : {};
    return normalizePositiveInt(caps.maxVideos ?? caps.max_videos) || 3;
}
function resolveAistartLabMaxAudios(ctx) {
    const caps = isRecord(ctx.model.capabilities) ? ctx.model.capabilities : {};
    return normalizePositiveInt(caps.maxAudios ?? caps.max_audios) || 3;
}
async function normalizeAistartLabVideoTaskResult(ctx, upstream, requestJson, fallbackTaskId = '') {
    const envelope = isRecord(upstream) ? upstream : {};
    const apiCode = firstDefined(envelope.code, envelope.statusCode);
    const data = isRecord(envelope.data) ? envelope.data : envelope;
    const taskId = String(firstDefined(data.taskId, data.task_id, envelope.taskId, envelope.task_id, extractVideoTask(upstream), extractUpstreamTaskId(upstream), fallbackTaskId) || '').trim();
    const statusCode = Number(firstDefined(data.status, envelope.status));
    const statusText = aistartLabStatusText(statusCode) || extractTaskStatusText(upstream);
    const apiFailed = apiCode !== undefined && apiCode !== null && Number(apiCode) !== 0;
    const failed = apiFailed || statusCode === 4 || isFinalFailedTaskStatus(upstream, statusText, taskId);
    const outputUrl = String(firstDefined(data.outputUrl, data.output_url, data.videoUrl, data.video_url, envelope.outputUrl, envelope.output_url, extractVideoResultUrl(upstream)) || '').trim();
    const urls = await resolveVideoResultUrls(ctx, outputUrl ? [outputUrl] : extractUrls(upstream));
    const done = statusCode === 3 || isDoneTaskStatus(statusText) || Boolean(urls.length && !failed);
    const missingDoneResult = done && !urls.length;
    const progress = normalizeAistartLabProgress(firstDefined(data.progress, envelope.progress), statusCode, statusText, urls.length);
    const base = {
        status: failed || missingDoneResult ? 'FAILED' : (done ? 'SUCCESS' : (taskId ? 'RUNNING' : 'FAILED')),
        progress,
        upstreamTaskId: taskId,
        upstreamRequestId: taskId,
        responseJson: upstream,
        resultJson: compactJson({
            url: urls[0] || '',
            outputs: urls,
            outputType: firstDefined(data.outputType, data.output_type),
            costCredits: firstDefined(data.costCredits, data.cost_credits),
        }),
        resultUrls: urls,
        errorCode: failed
            ? String(firstDefined(data.errorCode, data.error_code, envelope.code, 'AISTARTLAB_VIDEO_TASK_FAILED') || 'AISTARTLAB_VIDEO_TASK_FAILED')
            : (missingDoneResult || (!taskId && !urls.length) ? 'AISTARTLAB_VIDEO_TASK_MISSING' : undefined),
        errorMessage: failed
            ? (String(firstDefined(data.errorMessage, data.error_message, envelope.msg, envelope.message, extractErrorMessage(upstream)) || 'AIStartLab 视频任务失败'))
            : (missingDoneResult || (!taskId && !urls.length) ? 'AIStartLab 上游响应未包含任务 ID 或视频 URL' : undefined),
    };
    return requestJson ? { ...base, requestJson } : base;
}
function aistartLabStatusText(statusCode) {
    if (statusCode === 1)
        return 'created';
    if (statusCode === 2)
        return 'processing';
...
function normalizeAistartLabProgress(value, statusCode, statusText, urlCount) {
    const explicit = normalizeGenericProgressValue(value);
    if (explicit !== null)
        return explicit;
    if (statusCode === 3 || urlCount > 0)
        return 100;
    if (statusCode === 4)
        return 100;
    if (statusCode === 2)
    ...
    if (statusCode === 1)
        return 8;
    return extractGenericVideoProgress({}, statusText);
}
...
async function normalizeFullbloodVideoSubmitResult(ctx, upstream, requestJson) {
    const taskId = extractVideoTask(upstream) || extractUpstreamTaskId(upstream) || String(upstream?.id || upstream?.data?.id || upstream?.data?.taskId || upstream?.data?.task_id || '');
    const resultUrl = extractVideoResultUrl(upstream);
    const urls = await resolveVideoResultUrls(ctx, resultUrl ? [resultUrl] : extractUrls(upstream));
    const statusText = extractTaskStatusText(upstream);
    const failed = isFinalFailedTaskStatus(upstream, statusText, taskId);
...
function resolveFullbloodVideoSubmitEndpointPath(ctx) {
    const protocol = isRecord(ctx.model.protocol) ? ctx.model.protocol : {};
    const configured = String(firstDefined(protocol.endpoint_path, protocol.endpointPath, ctx.model.endpointPath, ctx.provider.endpointPath, '/videos') || '').trim();
    return /\/videos(?:$|[?#])/i.test(configured) ? configured : '/videos';
}
function resolveFullbloodVideoStatusEndpointPath(ctx) {
    const protocol = isRecord(ctx.model.protocol) ? ctx.model.protocol : {};
    const configured = String(firstDefined(protocol.status_endpoint_path, protocol.statusEndpointPath, ctx.model.statusEndpointPath, ctx.provider.statusEndpointPath, '/videos/{taskId}') || '').trim();
    return /\/videos\/\{?taskId\}?(?:$|[?#])/i.test(configured) ? configured : '/videos/{taskId}';
}
function resolveFullbloodVideoSize(ctx, aspectRatio) {
    const protocol = isRecord(ctx.model.protocol) ? ctx.model.protocol : {};
    const defaults = isRecord(ctx.model.defaults) ? ctx.model.defaults : {};
    const sizeByAspectRatio = isRecord(protocol.sizeByAspectRatio) ? protocol.sizeByAspectRatio : {};
    return String(firstDefined(ctx.params.size, sizeByAspectRatio[aspectRatio], defaults.size, '1280x720') || '1280x720');
}
...
async function submitSeedance2Video(ctx) {
    const modelName = resolveSeedance2ModelName(ctx);
    const usesAiyunzhiProtocol = isAiyunzhiSeedance2Protocol(ctx, modelName);
    if (!usesAiyunzhiProtocol)
        return submitLegacySeedance2Video(ctx, modelName);
    if (!isSeedance2FullModelName(modelName))
        return submitOfficialSd2Video(ctx, modelName);
    const prompt = resolveSeedance2Prompt(ctx);
    if (!prompt)
        fail(400, 'Seedance 2.0 prompt is required', 'SEEDANCE2_PROMPT_REQUIRED');
    ...
    if (multipartRefs.total > 0) {
        ...
        try {
            ...
            if (!isSeedance2PromptRequiredError(err) || fallbackRefs.total === 0)
                throw err;
...
function resolveSeedance2Prompt(ctx) {
    const direct = [
        ctx.prompt,
        ctx.params.prompt,
        ctx.params.text,
...
function isSeedance2PromptRequiredError(err) {
    const message = err instanceof Error ? err.message : String(err || '');
    return /prompt\s+is\s+required|提示词.{0,12}(?:必填|不能为空|缺失)/i.test(message);
}
async function submitOfficialSd2Video(ctx, modelName) {
    const resolution = resolveSeedance2SdResolution(ctx, modelName);
    const duration = resolveSeedance2SdDuration(ctx, modelName);
    const aspectRatio = resolveSeedance2SdAspectRatio(ctx, modelName);
    const references = await collectSeedance2SdReferences(ctx);
...
async function submitZaomengSeedance2Video(ctx) {
    const prompt = resolveSeedance2Prompt(ctx);
    if (!prompt)
        fail(400, '造梦 Seedance 2.0 prompt 为必填。', 'ZAOMENG_SEEDANCE2_PROMPT_REQUIRED');
...
async function submitToapisSeedance2Video(ctx) {
    const prompt = resolveSeedance2Prompt(ctx);
    if (!prompt)
        fail(400, 'ToAPIs Seedance 2 prompt 为必填。', 'TOAPIS_SEEDANCE2_PROMPT_REQUIRED');
    ...
    const aspectRatio = normalizeAllowedString(ctx.params.aspectRatio || ctx.params.aspect_ratio || ctx.params.requestedRatio || ctx.params.ratio, ['21:9', '16:9', '4:3', '1:1', '3:4', '9:16'], '16:9');
...
function resolveToapisSeedance2Resolution(ctx, modelName) {
...
function resolveToapisSeedance2Duration(ctx) {
...
async function collectToapisSeedance2References(ctx) {
...
function collectToapisSeedance2AssetUrlReferences(ctx) {
    const refs = { images: [], videos: [], audios: [] };
    const visit = (value, expectedKind) => {
        if (!value)
            return;
        if (Array.isArray(value)) {
        ...
        if (typeof value === 'string') {
            ...
            if (expectedKind === 'image')
                refs.images.push({ kind: 'image', url: raw });
            if (expectedKind === 'video')
                refs.videos.push({ kind: 'video', url: raw });
            if (expectedKind === 'audio')
                refs.audios.push({ kind: 'audio', url: raw });
            return;
        }
        if (!isRecord(value))
            return;
        ...
        if (!url)
            return;
        if (kind === 'image')
            refs.images.push({ kind: 'image', url });
        if (kind === 'video')
            refs.videos.push({ kind: 'video', url });
        if (kind === 'audio')
            refs.audios.push({ kind: 'audio', url });
    };
    const assetSources = [
        ...
        ['image', ctx.params.image_with_roles], ['video', ctx.params.video_with_roles], ['audio', ctx.params.audio_with_roles],
    ];
    assetSources.forEach(([kind, value]) => visit(value, kind));
    ctx.inputFiles.forEach(value => visit(value));
    return refs;
}
function buildToapisSeedance2ImageRoles(ctx, images) {
    ...
    return imageRefs.map((item, index) => {
        if (item.role)
            return { url: item.url, role: item.role };
        if (imageRefs.length === 1 && !/reference|参考/.test(refMode))
            return { url: item.url, role: 'first_frame' };
        if (index === 0 && (ctx.params.first_frame_image_url || ctx.params.firstFrameImageUrl))
            return { url: item.url, role: 'first_frame' };
        if (index === 1 && (ctx.params.last_frame_image_url || ctx.params.lastFrameImageUrl))
            return { url: item.url, role: 'last_frame' };
        return { url: item.url, role: 'reference_image' };
    });
}
function explicitToapisSeedance2ImageRoleMap(value) {
    const map = new Map();
    const visit = (item) => {
        if (!isRecord(item))
            return;
        ...
        if (url && ['first_frame', 'last_frame', 'reference_image'].includes(role))
            map.set(url, role);
    };
    if (Array.isArray(value))
        value.forEach(visit);
...
function toapisSeedance2ReferenceUrl(ref) {
...
function assertToapisSeedance2References(imageWithRoles, videoWithRoles, audioWithRoles) {
    ...
    if (firstFrameCount > 1)
        fail(400, 'ToAPIs Seedance 2 first_frame 最多 1 张。', 'TOAPIS_SEEDANCE2_TOO_MANY_FIRST_FRAMES');
    if (lastFrameCount > 1)
        fail(400, 'ToAPIs Seedance 2 last_frame 最多 1 张。', 'TOAPIS_SEEDANCE2_TOO_MANY_LAST_FRAMES');
    if (referenceImageCount > 9)
        fail(400, 'ToAPIs Seedance 2 reference_image 最多 9 张。', 'TOAPIS_SEEDANCE2_TOO_MANY_REFERENCE_IMAGES');
    if (hasFrameMode && referenceImageCount)
        fail(400, 'ToAPIs Seedance 2 首帧/首尾帧模式不能与 reference_image 混用。', 'TOAPIS_SEEDANCE2_FRAME_REFERENCE_MIXED');
    if (videoWithRoles.length > 3)
        fail(400, 'ToAPIs Seedance 2 reference_video 最多 3 条。', 'TOAPIS_SEEDANCE2_TOO_MANY_VIDEOS');
    if (audioWithRoles.length > 3)
        fail(400, 'ToAPIs Seedance 2 reference_audio 最多 3 段。', 'TOAPIS_SEEDANCE2_TOO_MANY_AUDIOS');
...
function resolveToapisSeedance2GenerateAudio(ctx) {
    const explicit = firstDefined(ctx.params.generate_audio, ctx.params.generateAudio, ctx.params.enableSound, ctx.params.enable_sound);
    if (explicit === undefined)
        return true;
    return normalizeBooleanValue(explicit);
}
function toapisSeedance2Seed(ctx) {
...
async function normalizeToapisSeedance2Result(ctx, upstream, requestJson) {
...
function resolveZaomengSeedance2ModelName(ctx) {
    const raw = String(ctx.params.model || ctx.model.name || ctx.provider.defaultModel || '').trim().toLowerCase();
    if (raw === 'seedance-2.0-fast')
        return 'seedance-2.0-fast';
    if (raw === 'seedance-2.0-svip')
        return 'seedance-2.0-svip';
    const hint = [
        ctx.model.name,
        ctx.model.displayName,
        ctx.model.modelKey,
...
function resolveZaomengSeedance2Resolution(ctx) {
    return normalizeAllowedString(ctx.params.resolution || ctx.params.quality || ctx.params.requestedResolution, ['720p', '1080p'], '720p');
}
function resolveZaomengSeedance2Duration(ctx, resolution) {
    const maxSeconds = resolution === '1080p' ? 12 : 15;
    return resolveVideoDurationSeconds(ctx, resolution, ctx.params.duration || ctx.params.durationSeconds || ctx.params.duration_seconds || ctx.params.seconds, 5, Array.from({ length: maxSeconds }, (_, index) => index + 1));
}
async function collectZaomengSeedance2References(ctx) {
...
function zaomengGenericFileReferences(ctx) {
    const refs = { images: [], videos: [], audios: [] };
    const push = (value) => {
        if (!value)
            return;
        ...
        if (kind === 'image')
            refs.images.push({ kind, url });
        else if (kind === 'video')
            refs.videos.push({ kind, url });
        else if (kind === 'audio')
            refs.audios.push({ kind, url });
    };
    push(ctx.params.files);
    return refs;
}
function assertZaomengSeedance2References(ctx, refs) {
    const total = refs.images.length + refs.videos.length + refs.audios.length;
    if (refs.images.length > 9)
        fail(400, `造梦 Seedance 2.0 参考图最多 9 张，当前 ${refs.images.length} 张。`, 'ZAOMENG_SEEDANCE2_TOO_MANY_IMAGES');
    if (refs.videos.length > 3)
        fail(400, `造梦 Seedance 2.0 参考视频最多 3 条，当前 ${refs.videos.length} 条。`, 'ZAOMENG_SEEDANCE2_TOO_MANY_VIDEOS');
    if (refs.audios.length > 3)
        fail(400, `造梦 Seedance 2.0 参考音频最多 3 条，当前 ${refs.audios.length} 条。`, 'ZAOMENG_SEEDANCE2_TOO_MANY_AUDIOS');
    if (total > 9)
        fail(400, `造梦 Seedance 2.0 files 最多 9 个，当前 ${total} 个。`, 'ZAOMENG_SEEDANCE2_TOO_MANY_FILES');
    const badVideo = refs.videos.find(ref => ref.url && !isZaomengMp4Url(ref.url));
    if (badVideo?.url)
        fail(400, '造梦 Seedance 2.0 参考视频 URL 必须以 .mp4 结尾。', 'ZAOMENG_SEEDANCE2_VIDEO_URL_NOT_MP4');
    assertZaomengAudioDurations(ctx);
}
function assertZaomengAudioDurations(ctx) {
    const values = [...ctx.inputFiles, ctx.params.files].flat();
    const tooShort = values.find(item => {
        if (!isRecord(item) || inferReferenceMediaType(item) !== 'audio')
            return false;
        const metadata = isRecord(item.metadata) ? item.metadata : {};
        const seconds = Number(firstDefined(item.duration, item.durationSeconds, item.duration_seconds, item.rawDuration, metadata.duration, metadata.durationSeconds, metadata.duration_seconds) || 0);
        return Number.isFinite(seconds) && seconds > 0 && seconds < 3;
    });
    if (tooShort)
        fail(400, '造梦 Seedance 2.0 参考音频每条不少于 3 秒。', 'ZAOMENG_SEEDANCE2_AUDIO_TOO_SHORT');
}
function zaomengSeedance2Files(refs) {
    return [...refs.images, ...refs.videos, ...refs.audios]
        .map(ref => String(ref.url || '').trim())
        .filter(Boolean);
}
function zaomengSeedance2PromptWithMarkers(prompt, refs) {
    const missingMarkers = [];
    const addMissing = (prefix, count) => {
        for (let index = 1; index <= count; index += 1) {
            const pattern = new RegExp(`@${prefix}_?${index}\\b`, 'i');
            if (!pattern.test(prompt))
                missingMarkers.push(`@${prefix}${index}`);
        }
    };
...
function isZaomengMp4Url(value) {
    const raw = String(value || '').trim();
    if (!raw)
        return false;
    try {
...
async function normalizeZaomengSeedance2SubmitResult(ctx, upstream, requestJson) {
    ...
    return {
        ...
        upstreamTaskId: taskId,
        upstreamRequestId: String(upstream?.task_id || upstream?.taskId || upstream?.request_id || upstream?.requestId || taskId || ''),
        requestJson,
        responseJson: upstream,
        resultJson: { url: urls[0] || '', outputs: urls },
        resultUrls: urls,
...
function extractZaomengSeedance2PollTaskId(upstream) {
...
async function submitLegacySeedance2Video(ctx, modelName = resolveSeedance2ModelName(ctx)) {
    const prompt = String(ctx.prompt || '').trim();
    if (!prompt)
        fail(400, 'Seedance 2 prompt is required', 'SEEDANCE2_PROMPT_REQUIRED');
    ...
    if (usesArtifexSeedance2Channel) {
        ...
        if (oversized)
            fail(400, 'Seedance 2 单个参考素材不能超过 25MB', 'SEEDANCE2_MATERIAL_TOO_LARGE');
    ...
    if (usesArtifexSeedance2Channel) {
        if (rawImageUrls.length > 9)
            fail(400, 'Seedance 2 extra_images 最多 9 个', 'SEEDANCE2_TOO_MANY_IMAGES');
        if (rawVideoUrls.length > 3)
            fail(400, 'Seedance 2 extra_videos 最多 3 个', 'SEEDANCE2_TOO_MANY_VIDEOS');
        if (rawAudioUrls.length > 3)
            fail(400, 'Seedance 2 extra_audios 最多 3 个', 'SEEDANCE2_TOO_MANY_AUDIOS');
    }
    ...
    const duration = resolveVideoDurationSeconds(ctx, resolution, ctx.params.duration || ctx.params.durationSeconds || ctx.params.duration_seconds || ctx.params.seconds, 5, Array.from({ length: 12 }, (_, index) => index + 4));
    const aspectRatio = normalizeAllowedString(ctx.params.aspectRatio || ctx.params.aspect_ratio, usesArtifexSeedance2Channel ? ['16:9', '9:16', '1:1'] : ['16:9', '9:16', '1:1', '21:9', '3:4', '4:3'], '16:9');
    const requestJson = compactJson({
        model: modelName,
        prompt,
        duration,
...
async function querySeedance2Video(ctx) {
    const modelName = resolveSeedance2ModelName(ctx);
    if (!isAiyunzhiSeedance2Protocol(ctx, modelName))
        return queryGenericVideo(ctx);
    ...
    const upstream = await callUpstreamGetJson(ctx.provider, endpoint, {}, ctx.timeoutMs);
    const normalized = await normalizeSeedance2VideoTaskResult(ctx, upstream, { taskId: ctx.upstreamTaskId });
...
async function submitSeedanceFullVideo(ctx) {
    const prompt = String(ctx.prompt || '').trim();
    if (!prompt)
        fail(400, 'Seedance2.0 满血 prompt is required', 'SEEDANCE_FULL_PROMPT_REQUIRED');
    const imageUrls = buildVideoReferenceUrls(ctx).slice(0, 9);
    const mediaRefs = buildVideoMediaReferences(ctx);
...
async function querySeedanceFullVideo(ctx) {
    ...
    const upstream = await callUpstreamGetJson(ctx.provider, endpoint, {}, ctx.timeoutMs);
    return normalizeSeedanceFullTaskResult(ctx, upstream, { taskId: ctx.upstreamTaskId });
}
async function normalizeSeedanceFullTaskResult(ctx, upstream, requestJson) {
    const taskId = extractVideoTask(upstream) || extractUpstreamTaskId(upstream) || String(upstream?.taskId || upstream?.task_id || upstream?.id || ctx.upstreamTaskId || '');
    const resultUrl = extractVideoResultUrl(upstream);
    const urls = await resolveVideoResultUrls(ctx, resultUrl ? [resultUrl] : extractUrls(upstream));
    const statusText = extractTaskStatusText(upstream);
    const failed = isFinalFailedTaskStatus(upstream, statusText, taskId);
    ...
    const missingDoneResult = done && !urls.length;
    const base = {
        status: failed || missingDoneResult ? 'FAILED' : (done && urls.length ? 'SUCCESS' : (taskId ? 'RUNNING' : 'FAILED')),
        progress: done && urls.length ? 100 : extractGenericVideoProgress(upstream, statusText),
        upstreamTaskId: taskId,
        upstreamRequestId: String(upstream?.request_id || upstream?.requestId || upstream?.taskId || upstream?.task_id || upstream?.id || taskId || ''),
        responseJson: upstream,
        resultJson: { url: urls[0] || '', outputs: urls },
        resultUrls: urls,
...
async function submitLingdongSd2VipVideo(ctx) {
    ...
    const duration = resolveVideoDurationSeconds(ctx, '720p', ctx.params.duration || ctx.params.durationSeconds || ctx.params.duration_seconds || ctx.params.seconds, 15, [15]);
    const videoParams = withoutKeys(ctx.params, [
        'model', 'prompt', 'negative_prompt', 'stream',
        'duration', 'durationSeconds', 'duration_seconds', 'seconds',
    ...
    const endpoint = resolveEndpointPath(ctx.model.endpointPath || ctx.provider.endpointPath || '/videos', ctx);
    const upstream = await callUpstreamJson(ctx.provider, endpoint, requestJson, ctx.timeoutMs);
    const taskId = extractVideoTask(upstream) || extractUpstreamTaskId(upstream) || String(upstream?.id || '');
    const resultUrl = extractVideoResultUrl(upstream);
    const urls = await resolveVideoResultUrls(ctx, resultUrl ? [resultUrl] : extractUrls(upstream));
    const statusText = extractTaskStatusText(upstream);
    const failed = isFinalFailedTaskStatus(upstream, statusText, taskId);
...
async function queryLingdongSd2VipVideo(ctx) {
    ...
    for (const endpointPath of endpoints) {
        ...
        try {
            ...
            if (normalized.status !== 'RUNNING')
                return normalized;
            if (!bestRunning || Number(normalized.progress || 0) > Number(bestRunning.progress || 0))
                bestRunning = normalized;
        }
        catch (err) {
            if (!isRetryableVideoStatusEndpointError(err))
                throw err;
...
async function submitSeedanceTaskVideo(ctx) {
...
async function querySeedanceTaskVideo(ctx) {
...
function resolveSeedanceTaskSubmitEndpointPath(ctx) {
    const configured = String(ctx.model.endpointPath || ctx.provider.endpointPath || '').trim();
    if (!configured || configured === '/video/generations')
        return '/api/v3/contents/generations/tasks';
    return configured;
}
function resolveSeedanceTaskStatusEndpointPath(ctx) {
...
export async function buildSeedanceTaskRequestJson(ctx) {
...
function normalizeSeedanceTaskMode(ctx, imageUrls, videoUrls, audioUrls) {
    ...
    if (ctx.model.name === 'gemini-omni')
        return videoUrls.length ? 'edit' : hasReference ? 'r2v' : 't2v';
    if (['firstlast', 'first-last', 'i2v_first_last'].includes(refMode) && imageUrls.length >= 2)
        return 'i2v_first_last';
    return hasReference ? 'reference_material' : 't2v';
}
function seedanceTaskSizeFromRatio(ratio, resolution) {
...
async function normalizeSeedanceTaskSubmitResult(ctx, upstream, requestJson) {
    const normalized = await normalizeSeedanceTaskResult(ctx, upstream);
    return { ...normalized, requestJson };
}
async function normalizeSeedanceTaskQueryResult(ctx, upstream, fallbackTaskId = '') {
    return normalizeSeedanceTaskResult(ctx, upstream, fallbackTaskId);
}
async function normalizeSeedanceTaskResult(ctx, upstream, fallbackTaskId = '') {
...
function seedanceTaskMissingResultMessage(ctx, upstream) {
    ...
    if (emptyResponse)
        return 'Seedance Task 上游返回空响应，未包含任务 ID 或视频 URL';
    return 'Seedance Task 上游响应未包含任务 ID 或视频 URL';
}
function parseSeedanceTaskItems(payload) {
    if (!isRecord(payload))
        return [];
    ...
    if (isRecord(data))
        return parseSeedanceTaskItems(data).length ? parseSeedanceTaskItems(data) : [data];
    return [];
}
function extractSeedanceTaskId(payload) {
    const data = isRecord(payload?.data) ? payload.data : {};
    const direct = String(payload?.id || payload?.task_id || payload?.taskId || data.id || data.task_id || data.taskId || '').trim();
    if (direct)
        return direct;
...
function extractSeedanceTaskStatus(payload) {
    const data = isRecord(payload?.data) ? payload.data : {};
    const direct = String(payload?.status || data.status || '').trim().toLowerCase();
    if (direct)
        return direct;
    for (const item of parseSeedanceTaskItems(payload)) {
...
function extractSeedanceTaskProgress(payload) {
    const data = isRecord(payload?.data) ? payload.data : {};
    const candidates = [payload?.progress, data.progress, ...parseSeedanceTaskItems(payload).map(item => item.progress)];
    for (const candidate of candidates) {
...
function extractSeedanceTaskVideoUrl(payload) {
    for (const item of parseSeedanceTaskItems(payload)) {
...
async function normalizeVideoQueryResult(ctx, upstream) {
    ...
    return {
        ...
        upstreamTaskId: ctx.upstreamTaskId,
        responseJson: upstream,
        resultJson: { url: urls[0] || '', outputs: urls, ...(coverUrl ? { coverUrl } : {}) },
        resultUrls: urls,
...
function normalizeSeedanceFullResolution(value) {
    const raw = String(value || '').trim().toLowerCase();
    if (raw === '1080' || raw === '1080p')
        return '1080p';
    return '720p';
}
function resolveSeedanceFullModelName(ctx, resolution) {
    const protocol = isRecord(ctx.model.protocol) ? ctx.model.protocol : {};
    const defaults = isRecord(ctx.model.defaults) ? ctx.model.defaults : {};
    const providerDefaults = isRecord(ctx.provider.defaultParams) ? ctx.provider.defaultParams : {};
    const modelByResolution = firstDefined(ctx.params.modelByResolution, ctx.params.model_by_resolution, protocol.modelByResolution, protocol.model_by_resolution, defaults.modelByResolution, defaults.model_by_resolution, providerDefaults.modelByResolution, providerDefaults.model_by_resolution);
...
function resolveSeedanceFullSubmitEndpointPath(ctx) {
    const protocol = isRecord(ctx.model.protocol) ? ctx.model.protocol : {};
    const providerDefaults = isRecord(ctx.provider.defaultParams) ? ctx.provider.defaultParams : {};
    const configured = String(firstDefined(ctx.params.endpointPath, ctx.params.endpoint_path, protocol.endpoint_path, protocol.endpointPath, ctx.model.endpointPath, ctx.provider.endpointPath, providerDefaults.endpointPath, providerDefaults.endpoint_path, '/seedance-full/generate')).trim();
    return /\/seedance-full\/generate(?:\/test)?(?:$|[?#])/i.test(configured) ? configured : '/seedance-full/generate';
}
function resolveSeedanceFullStatusEndpointPath(ctx) {
    const protocol = isRecord(ctx.model.protocol) ? ctx.model.protocol : {};
    const providerDefaults = isRecord(ctx.provider.defaultParams) ? ctx.provider.defaultParams : {};
    const configured = String(firstDefined(ctx.params.statusEndpointPath, ctx.params.status_endpoint_path, protocol.status_endpoint_path, protocol.statusEndpointPath, ctx.model.statusEndpointPath, ctx.provider.statusEndpointPath, providerDefaults.statusEndpointPath, providerDefaults.status_endpoint_path, '/seedance-full/task/{taskId}')).trim();
...
function resolveSeedance2ModelName(ctx) {
    ...
    if (providerKey === 'artifex-seedance2' && /^seedance-2$/i.test(sdModel))
        return 'video-pro-720p';
    if (providerKey === 'artifex-seedance2-fast' && /^seedance-2-fast$/i.test(sdModel))
        return 'video-fast-720p';
    if (isArtifexSeedance2ModelName(sdModel))
        return sdModel.toLowerCase();
    if (/^(?:sd2-(?:720p|1080p)(?:-fast)?|seedance-2)$/i.test(sdModel))
        return sdModel.toLowerCase();
...
function isAiyunzhiSeedance2Protocol(ctx, modelName = resolveSeedance2ModelName(ctx)) {
    const hay = [
        modelName,
        ctx.provider.baseUrl,
        ctx.provider.providerKey,
...
function resolveSeedance2SdResolution(ctx, modelName) {
...
function resolveSeedance2SdDuration(ctx, modelName) {
    if (String(modelName || '').toLowerCase() === 'seedance-2')
        return 15;
    return resolveVideoDurationSeconds(ctx, resolveSeedance2SdResolution(ctx, modelName), ctx.params.duration || ctx.params.durationSeconds || ctx.params.duration_seconds || ctx.params.seconds, 5, Array.from({ length: 12 }, (_, index) => index + 4));
}
function resolveSeedance2SdAspectRatio(ctx, modelName) {
...
function resolveOfficialSd2ModeType(ctx, imageRefs) {
    ...
    if (/frames2video|first[-_ ]?last|start[-_ ]?end|首尾/.test(raw) && imageRefs.length >= 2)
        return 'frames2video';
    if (/image2video|image[-_ ]?to[-_ ]?video|img2video|i2v|首帧/.test(raw) && imageRefs.length >= 1)
        return 'image2video';
    if (/text2video|text[-_ ]?to[-_ ]?video|t2v|文生/.test(raw))
        return 'text2video';
    return imageRefs.length ? 'image2video' : 'text2video';
}
function resolveOfficialSd2EnableSound(ctx) {
    return resolveOfficialSd2GenerateAudio(ctx) ? 'on' : 'off';
}
function resolveOfficialSd2GenerateAudio(ctx) {
    const metadata = isRecord(ctx.params.metadata) ? ctx.params.metadata : {};
    const explicit = firstDefined(ctx.params.generate_audio, ctx.params.generateAudio, metadata.generate_audio, metadata.generateAudio, metadata.enableSound, metadata.enable_sound, ctx.params.enableSound, ctx.params.enable_sound);
    if (explicit == null)
        return true;
    return normalizeBooleanValue(explicit);
}
async function collectSeedance2SdReferences(ctx) {
    const imageUrls = await materializeSoraVideoProImageUrls(ctx, buildSoraVideoProUrlList(ctx, [
...
function collectSeedance2AssetReferences(ctx) {
    const refs = [];
    const visit = (value, expectedKind) => {
        if (!value)
            return;
        ...
        if (typeof value === 'string') {
            const raw = value.trim();
            if (expectedKind && /^asset[_:-][\w.-]+$/i.test(raw))
                refs.push({ kind: expectedKind, assetId: raw });
            return;
        }
        if (!isRecord(value))
            return;
        const inferred = inferReferenceMediaType(value) || expectedKind || '';
        const assetId = String(firstDefined(value.assetId, value.asset_id, isRecord(value.image_asset) ? value.image_asset.asset_id : undefined, isRecord(value.video_asset) ? value.video_asset.asset_id : undefined, isRecord(value.audio_asset) ? value.audio_asset.asset_id : undefined) || '').trim();
        const kind = value.image_asset ? 'image' : value.video_asset ? 'video' : value.audio_asset ? 'audio' : inferred;
        if (assetId && (kind === 'image' || kind === 'video' || kind === 'audio'))
            refs.push({ kind, assetId });
    };
...
function dedupeSeedance2References(refs) {
    const seen = new Set();
    return refs.filter(ref => {
        const key = `${ref.kind}:${ref.url || ''}:${ref.assetId || ''}`;
        if (!ref.url && !ref.assetId || seen.has(key))
            return false;
        seen.add(key);
        return true;
    });
}
function buildSeedance2SdContent(prompt, refs) {
    const content = [];
...
function seedance2ContentReference(ref) {
    if (ref.assetId) {
...
function seedance2MultipartReferences(refs) {
    const needsMultipart = [...refs.images, ...refs.videos, ...refs.audios]
        .some(ref => ref.url && !/^https?:\/\//i.test(ref.url));
    if (!needsMultipart)
        return { images: [], videos: [], audios: [], total: 0 };
    const withUrl = (items) => items.filter(ref => ref.url);
...
function seedance2AllUrlReferences(refs) {
    const withUrl = (items) => items.filter(ref => ref.url);
    const images = withUrl(refs.images);
    const videos = withUrl(refs.videos);
    const audios = withUrl(refs.audios);
...
function resolveSeedance2GenerateAudio(ctx) {
    if ('generate_audio' in ctx.params)
        return normalizeBooleanValue(ctx.params.generate_audio);
    if (ctx.params.generateAudio === true)
        return true;
    return undefined;
}
...
function resolveSeedance2SubmitEndpoint(ctx) {
    const configured = String(ctx.model.endpointPath || ctx.provider.endpointPath || '').trim();
    if (!configured || configured === '/videos')
        return '/video/generations';
    if (/\/video\/generations\/multipart$/i.test(configured))
        return configured.replace(/\/multipart$/i, '');
    return configured;
}
function resolveOfficialSd2SubmitEndpoint(ctx) {
    const configured = String(ctx.model.endpointPath || ctx.provider.endpointPath || '').trim();
    if (!configured || /\/video\/generations(?:\/multipart)?$/i.test(configured))
        return '/videos';
    return resolveEndpointPath(configured, ctx);
}
function resolveSeedance2StatusEndpoint(ctx, modelName) {
    const configured = String(ctx.model.statusEndpointPath || ctx.provider.statusEndpointPath || '').trim();
    if (isSeedance2FullModelName(modelName))
        return configured || '/video/generations/{taskId}';
    if (!configured || /\/video\/generations\/\{taskId\}$/i.test(configured))
        return '/videos/{taskId}';
    return configured;
}
function resolveSeedance2MultipartEndpoint(ctx) {
    const protocol = isRecord(ctx.model.protocol) ? ctx.model.protocol : {};
    const providerDefaults = isRecord(ctx.provider.defaultParams) ? ctx.provider.defaultParams : {};
    const configured = String(firstDefined(ctx.params.multipartEndpointPath, ctx.params.multipart_endpoint_path, protocol.multipartEndpointPath, protocol.multipart_endpoint_path, providerDefaults.multipartEndpointPath, providerDefaults.multipart_endpoint_path) || '').trim();
    if (configured)
        return resolveEndpointPath(configured, ctx);
    const submit = resolveSeedance2SubmitEndpoint(ctx);
    if (/\/video\/generations$/i.test(submit))
        return resolveEndpointPath(`${submit}/multipart`, ctx);
    return resolveEndpointPath('/video/generations/multipart', ctx);
}
async function buildSeedance2VideoForm(requestJson, refs, timeoutMs) {
    const request = isRecord(requestJson) ? requestJson : {};
    const form = new FormData();
    ...
    if ('generate_audio' in request)
        form.set('generate_audio', String(Boolean(request.generate_audio)));
...
function seedance2RequestPrompt(request) {
...
async function queryOfficialSd2ContentResult(ctx, statusEndpoint, statusPayload) {
    const protocol = isRecord(ctx.model.protocol) ? ctx.model.protocol : {};
    ...
    try {
        ...
        return {
            ...
            progress: 100,
            upstreamTaskId: ctx.upstreamTaskId,
            responseJson: { status: statusPayload, content: upstream },
            resultJson: { url: urls[0] || '', outputs: urls },
            resultUrls: urls,
        };
    }
...
async function normalizeSeedance2VideoTaskResult(ctx, upstream, requestJson) {
    const taskId = extractVideoTask(upstream) || extractUpstreamTaskId(upstream);
    const resultUrl = extractVideoResultUrl(upstream);
    const urls = await resolveVideoResultUrls(ctx, resultUrl ? [resultUrl] : extractUrls(upstream));
    const statusText = extractTaskStatusText(upstream);
...
function extractSeedance2CoverUrl(upstream) {
    ...
    return String(source.cover_url
        || source.coverUrl
        || data.cover_url
        || data.coverUrl
        || result.cover_url
...
function assertLingdongSd2VipRequest(ctx) {
    const prompt = String(ctx.prompt || '').trim();
    const caps = isRecord(ctx.model.capabilities) ? ctx.model.capabilities : {};
    const maxPromptLength = normalizePositiveInt(caps.maxPromptLength ?? caps.max_prompt_length) || 4000;
    if (!prompt)
        fail(400, 'sd-2-vip prompt 为必填。', 'LINGDONG_SD2_VIP_PROMPT_REQUIRED');
    if (prompt.length > maxPromptLength) {
...
function lingdongSd2VipReferenceUrls(ctx, mediaType, keys, limit) {
    const urls = buildSoraVideoProUrlList(ctx, keys, mediaType, limit)
        .map(item => String(item || '').trim())
...
function resolveLingdongSd2VipSize(ctx) {
    const raw = String(ctx.params.size || ctx.params.quality || ctx.params.resolution || '').trim().toLowerCase();
    if (raw === 'small' || raw.includes('480'))
        return 'small';
    if (raw === 'large' || raw.includes('720'))
        return 'large';
    return 'large';
}
function assertLingdongSd2VipMaterialSizes(ctx) {
    const limitsMb = { image: 30, video: 50, audio: 15 };
...
function extractGenericVideoProgress(payload, statusText = '') {
    const data = payload?.data;
    const first = Array.isArray(data) ? data[0] : data;
    const explicit = [
        payload?.progress,
    ...
    ].map(normalizeGenericProgressValue).find((value) => value !== null && value > 0);
    if (explicit !== undefined)
        return explicit;
    const normalized = normalizeTaskStatusValue(statusText) || extractTaskStatusText(payload);
    if (['pending', 'queued', 'queue', 'created', 'submitted'].includes(normalized))
        return 8;
    if (['dispatched', 'dispatching'].includes(normalized))
        return 18;
    if (['processing', 'running', 'in_progress', 'generating'].includes(normalized))
        return 35;
    return extractVideoTask(payload) ? 5 : 0;
}
function normalizeGenericProgressValue(value) {
    if (value === undefined || value === null || value === '')
        return null;
...
function findGenericProgressValue(value, depth) {
    ...
    for (const [key, item] of Object.entries(value)) {
        if (/^(progress|progress_percent|progressPercent|percentage|percent)$/i.test(key))
            return item;
...
async function compatibleMediaRefToBlob(ref, timeoutMs, mediaType, label) {
    const raw = String(ref || '').trim();
    if (mediaType === 'image')
        return compatibleImageRefToBlob(raw, timeoutMs, label);
    const dataMatch = raw.match(/^data:((?:video|audio)\/[^;]+);base64,(.+)$/i);
    if (dataMatch) {
        const mime = dataMatch[1].toLowerCase();
        if (!mime.startsWith(`${mediaType}/`))
            fail(400, `${label}类型不匹配`, 'MEDIA_REFERENCE_TYPE_MISMATCH');
    ...
    if (!/^https?:\/\//i.test(raw))
        fail(400, `${label}必须是公网 URL 或 data URI`, 'MEDIA_REFERENCE_INVALID');
    const controller = new AbortController();
    const timer = timeoutMs > 0 ? setTimeout(() => controller.abort(), timeoutMs) : null;
    try {
        const resp = await fetch(raw, { signal: controller.signal });
        if (!resp.ok)
            fail(400, `${label}下载失败 HTTP ${resp.status}`, 'MEDIA_REFERENCE_FETCH_FAILED');
...
function mediaExtensionFromMime(mime, mediaType) {
...
function isUrlOnlyImageResponseType(responseType) {
...
function imageOutputMissingErrorCode(payload) {
    ...
    if (/policy|safety|safe|moderation|blocked|refusal|refused|content|can(?:not|'?t|’t)\s+generate|unable\s+to\s+generate/i.test(message))
...
async function imageResultUrl(provider, result, responseType = 'object_storage') {
    ...
    if (responseType === 'server_async_object_storage') {
        if (directUrl && !/^data:image\//i.test(directUrl))
            return directUrl;
        return '';
    }
...
async function resolveImageResultUrls(provider, urls, responseType) {
    ...
    if (responseType === 'provider_url' || responseType === 'server_object_storage' || responseType === 'server_async_object_storage' || responseType === 'base64')
...
async function resolveVideoResultUrls(ctx, urls) {
    ...
    if ((responseType === 'provider_url' || responseType === 'base64') && !hasProtectedProviderUrl)
        return absolute;
...
export async function materializeMediaResultUrl(provider, value, mediaType = 'video') {
    const raw = String(value || '').trim();
    if (!raw)
        return '';
    if (!/^https?:\/\//i.test(raw))
...
function providerResultFetchHeaders(provider, rawUrl) {
    try {
...
export function isProtectedProviderResultUrl(provider, rawUrl) {
    try {
...
function resolveGptImageV2ModelName(modelName, resolution) {
    ...
    if (normalizedResolution === '1K' || normalizedResolution === '3K')
        return baseModel;
...
function buildVideoReferenceUrls(ctx) {
    ...
    const push = (value) => {
        if (!value)
            return;
        if (!referenceMatchesMediaType(value, 'image'))
            return;
        if (typeof value === 'string') {
            const raw = value.trim();
            const publicUrl = resolvePublicReferenceImageUrl(raw);
...
function buildSoraVideoProUrlList(ctx, keys, mediaType, _limit) {
    const refs = new Set();
    const push = (value) => {
        if (!value)
            return;
        ...
        if (!referenceMatchesMediaType(value, mediaType))
            return;
        if (typeof value === 'string') {
            const raw = value.trim();
            if (!raw)
...
function referenceMatchesMediaType(value, expected) {
    const actual = inferReferenceMediaType(value);
    return !actual || actual === expected;
}
function inferReferenceMediaType(value) {
    if (!value)
        return '';
    if (Array.isArray(value)) {
    ...
    if (typeof value === 'string')
        return inferReferenceMediaTypeFromString(value);
    if (!isRecord(value))
        return '';
    const declared = [
        value.mediaType,
    ...
    if (declared) {
        if (declared === 'image' || declared.startsWith('image/') || declared === 'image_url' || declared === 'image_asset')
            return 'image';
        if (declared === 'video' || declared.startsWith('video/') || declared === 'video_url' || declared === 'video_asset')
            return 'video';
        if (declared === 'audio' || declared.startsWith('audio/') || declared === 'audio_url' || declared === 'audio_asset')
            return 'audio';
    }
    if (value.audioUrl || value.audio_url)
        return 'audio';
    if (value.videoUrl || value.video_url)
        return 'video';
...
function inferReferenceMediaTypeFromString(value) {
    const raw = String(value || '').trim().toLowerCase();
    if (!raw)
        return '';
    if (raw.startsWith('data:image/'))
    ...
    if (/\.(?:png|jpe?g|webp|gif|bmp|avif|heic|heif|tiff?)$/i.test(path))
        return 'image';
    if (/\.(?:mp4|mov|webm|m4v|avi|mkv|mpeg|mpg)$/i.test(path))
        return 'video';
    if (/\.(?:mp3|wav|m4a|aac|ogg|oga|flac|opus|wma)$/i.test(path))
        return 'audio';
    return '';
}
...
function extractGptImageV2Urls(payload) {
    ...
    const push = (value) => {
        const raw = String(value || '').trim();
        if (/^(https?:\/\/|\/?v1\/(?:images\/results|files|videos)\/|\/?(?:images\/results|files|videos)\/)/i.test(raw))
            urls.add(raw);
    };
...
function resolveGptImageV2Endpoint(ctx) {
    ...
    if (/\/images\/generations$/i.test(configured))
        return '/v1/images/generations';
    if (/\/v1\/images\/edits$/i.test(configured))
        return '/v1/images/generations';
    if (/\/images\/edits$/i.test(configured))
        return '/v1/images/generations';
    return configured;
}
...
function normalizeProviderRelativeResultPath(value) {
    const raw = String(value || '').trim();
    if (!raw)
        return '';
    if (/^\/?v1\/(?:images\/results|files|videos)\//i.test(raw))
        return raw.startsWith('/') ? raw : `/${raw}`;
    if (/^\/?(?:images\/results|files|videos)\//i.test(raw))
        return `/v1/${raw.replace(/^\/+/, '')}`;
    if (raw.startsWith('/'))
        return raw;
    return '';
}
...
function extractUrls(payload) {
    ...
    const visit = (value) => {
        if (!value)
            return;
        if (typeof value === 'string') {
            if (/^(https?:\/\/|\/?v1\/(?:images\/results|files|videos)\/|\/?(?:images\/results|files|videos)\/)/i.test(value))
                urls.add(value);
            return;
        }
        if (Array.isArray(value)) {
...
function normalizeTaskStatusValue(value) {
    ...
    if (/^(failed|failure|fail|error|errored|cancelled|canceled|rejected|blocked|denied)$/i.test(raw) || /失败|错误|拒绝|拦截|审核失败/.test(raw))
        return 'failed';
    if (/^(pending|queued|queue|dispatched|dispatching|processing|running|in_progress|generating|submitted|created)$/i.test(raw) || /排队|等待|处理中|进行中|生成中/.test(raw))
        return raw;
    return '';
}
...
function isQueuedOrBusyTaskMessage(message) {
...
function findErrorMessageValue(value, depth) {
    ...
    for (const [key, item] of entries) {
        if (isNonUserFacingErrorMetadataKey(key))
            continue;
...
function extractResponsesOutputTextMessage(payload) {
...
function isNonUserFacingErrorMetadataKey(key) {
