...
PKG="image-preview-original-display-20260612024201"
...
verify_html_markers(){
  local file="$1"
  grep -Fq 'function previewImageSrc(src,maxW=COS_PREVIEW_THUMB_MAX,maxH=COS_PREVIEW_THUMB_MAX)' "$file"
  grep -Fq 'return imageOriginalDimensionProbeSrc(raw);' "$file"
  grep -Fq 'function renderSafeImg(src,attrs=' "$file"
  grep -Fq 'data-original-src="${esc(original)}"' "$file"
  grep -Fq "return apiUrl('/api/workbench/image-studio/proxy-image?remoteUrl='+encodeURIComponent(raw));" "$file"
  ! grep -Fq 'data-adaptive-image="1"' "$file"
  ! grep -Fq 'data-cos-thumb="1"' "$file"
  ! grep -Fq 'imageMogr2/thumbnail' "$file"
}
verify_html_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq 'function previewImageSrc(src,maxW=COS_PREVIEW_THUMB_MAX,maxH=COS_PREVIEW_THUMB_MAX)' "$file"
  run_sudo grep -Fq 'return imageOriginalDimensionProbeSrc(raw);' "$file"
  run_sudo grep -Fq 'function renderSafeImg(src,attrs=' "$file"
  run_sudo grep -Fq 'data-original-src="${esc(original)}"' "$file"
  run_sudo grep -Fq "return apiUrl('/api/workbench/image-studio/proxy-image?remoteUrl='+encodeURIComponent(raw));" "$file"
  if run_sudo grep -Fq 'data-adaptive-image="1"' "$file"; then fail "adaptive thumbnail marker still present in $file"; fi
  if run_sudo grep -Fq 'data-cos-thumb="1"' "$file"; then fail "COS thumbnail marker still present in $file"; fi
  if run_sudo grep -Fq 'imageMogr2/thumbnail' "$file"; then fail "COS thumbnail transform still present in $file"; fi
}
