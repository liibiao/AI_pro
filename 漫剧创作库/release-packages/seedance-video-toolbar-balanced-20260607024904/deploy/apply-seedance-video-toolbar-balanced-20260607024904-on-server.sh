...
PKG="seedance-video-toolbar-balanced-20260607024904"
...
verify_markers(){
  local file="$1"
  grep -Fq "20260607: keep video generation bottom toolbar balanced and separated from the prompt panel." "$file"
  grep -Fq ".node.node-type-seedanceVideo .vn2-layout>.vn2-bar:not(.prompt-zoomed-item){" "$file"
  grep -Fq "height:42px!important;" "$file"
  grep -Fq "gap:5px!important;" "$file"
  grep -Fq "height:24px!important;" "$file"
  grep -Fq "width:117px!important;" "$file"
  grep -Fq "width:44px!important;" "$file"
  grep -Fq "width:32px!important;" "$file"
  grep -Fq "transform:translateX(10px)!important;" "$file"
}
verify_markers_sudo(){
  local file="$1"
  run_sudo grep -Fq "20260607: keep video generation bottom toolbar balanced and separated from the prompt panel." "$file"
  run_sudo grep -Fq ".node.node-type-seedanceVideo .vn2-layout>.vn2-bar:not(.prompt-zoomed-item){" "$file"
  run_sudo grep -Fq "height:42px!important;" "$file"
  run_sudo grep -Fq "gap:5px!important;" "$file"
  run_sudo grep -Fq "height:24px!important;" "$file"
  run_sudo grep -Fq "width:117px!important;" "$file"
  run_sudo grep -Fq "width:44px!important;" "$file"
  run_sudo grep -Fq "width:32px!important;" "$file"
  run_sudo grep -Fq "transform:translateX(10px)!important;" "$file"
}
