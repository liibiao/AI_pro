...
async function submitMidjourneySpecialTask(ctx, kind) {
    ...
    if (kind === 'modal') {
        ...
        if (customId) {
            ...
            const actionUpstream = await callUpstreamJson(ctx.provider, actionEndpoint, actionRequest, ctx.timeoutMs);
            ...
            upstream = await callUpstreamJson(ctx.provider, modalEndpoint, requestJson, ctx.timeoutMs);
            responseJson = {
...
function buildMidjourneyImaginePrompt(ctx, promptUrls) {
    ...
    const basePrompt = stripManagedMidjourneyPromptParams(cleanMidjourneyPromptText(ctx.prompt));
...
async function submitSeedance2Video(ctx) {
    ...
    const content = buildSeedance2SdContent(ctx.prompt, references);
    const requestJson = compactJson({
        model: modelName,
        ...(content.length > 1 ? { content } : { prompt: ctx.prompt }),
        aspect_ratio: aspectRatio,
        resolution,
        duration,
    ...
    const upstream = multipartRefs.total > 0
        ? await callUpstreamMultipart(ctx.provider, resolveSeedance2MultipartEndpoint(ctx), await buildSeedance2VideoForm(requestJson, multipartRefs, ctx.timeoutMs), ctx.timeoutMs)
        : await callUpstreamJson(ctx.provider, resolveSeedance2SubmitEndpoint(ctx), requestJson, ctx.timeoutMs);
...
async function submitOfficialSd2Video(ctx, modelName) {
    ...
    const enableSound = resolveOfficialSd2EnableSound(ctx);
    const requestJson = compactJson({
        model: modelName,
        prompt: ctx.prompt,
        duration,
...
async function submitLegacySeedance2Video(ctx, modelName = resolveSeedance2ModelName(ctx)) {
    ...
    const duration = resolveVideoDurationSeconds(ctx, resolution, ctx.params.duration || ctx.params.durationSeconds || ctx.params.duration_seconds || ctx.params.seconds, 6, Array.from({ length: 12 }, (_, index) => index + 4));
    const aspectRatio = normalizeAllowedString(ctx.params.aspectRatio || ctx.params.aspect_ratio, ['16:9', '9:16', '1:1', '21:9', '3:4', '4:3'], '16:9');
    const requestJson = compactJson({
        model: modelName,
        prompt: ctx.prompt,
...
function resolveOfficialSd2EnableSound(ctx) {
    const metadata = isRecord(ctx.params.metadata) ? ctx.params.metadata : {};
    const explicit = firstDefined(metadata.enableSound, metadata.enable_sound, ctx.params.enableSound, ctx.params.enable_sound);
    if (explicit == null)
        return 'on';
    const normalized = normalizeBooleanValue(explicit);
