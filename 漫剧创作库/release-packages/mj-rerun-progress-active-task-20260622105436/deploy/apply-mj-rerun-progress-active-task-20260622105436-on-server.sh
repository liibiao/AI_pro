...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/mj-rerun-progress-active-task-20260622105436-${STAMP}"
...
check_markers(){
  local file="$1"
  test -f "$file"
  grep -Fq "function generatedImageNodeHasActiveRun(n)" "$file"
  grep -Fq "function generatedImageNodeAcceptsTaskKeys(n,keys=[])" "$file"
  grep -Fq "function beginGeneratedImageNodeRunState(n,runId,label='图片生成中')" "$file"
  grep -Fq "function addGeneratedImageNodeActiveTaskKeys(n,keys={})" "$file"
  grep -Fq "const generationRunId=\`image-run-" "$file"
  grep -Fq "if(!generatedImageNodeAcceptsTaskKeys(n,[taskId]))return;" "$file"
  grep -Fq "if(!generatedImageNodeAcceptsTaskKeys(n,keys))return false;" "$file"
}
...
echo "deployed mj-rerun-progress-active-task-20260622105436"
