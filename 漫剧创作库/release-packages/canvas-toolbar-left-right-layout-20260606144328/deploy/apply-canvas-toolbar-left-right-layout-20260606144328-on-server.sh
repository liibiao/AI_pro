...
PKG="canvas-toolbar-left-right-layout-20260606144328"
...
verify_markers(){
  ...
  grep -Fq "[data-action=\"runNode\"]" "$file"
  grep -Fq ".image-action-toolbar,.refine-toolbar,.paint-toolbar,.edit-mode-toolbar,.pano-viewer-bar" "$file"
}
verify_markers_sudo(){
  local file="$1"
  ...
  run_sudo grep -Fq "[data-action=\"runNode\"]" "$file"
  run_sudo grep -Fq ".image-action-toolbar,.refine-toolbar,.paint-toolbar,.edit-mode-toolbar,.pano-viewer-bar" "$file"
}
