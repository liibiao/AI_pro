...
PKG="canvas-toolbar-transform-center-20260606123048"
...
verify_canvas_markers(){
  ...
  grep -Fq ".node :is(.aio-bar,.prompt-node-bar,.vn2-bar,.shot-storyboard-local-bar,.storygrid-action-bar):not(.prompt-zoomed-item)" "$file"
  grep -Fq "left:50%!important;" "$file"
  grep -Fq "transform:translateX(-50%)!important;" "$file"
  grep -Fq "transform:translate(-50%,-8px)!important;" "$file"
  grep -Fq "transform:translate(-50%,0)!important;" "$file"
  grep -Fq "height:29px!important;" "$file"
  grep -Fq "width:37px!important;" "$file"
  grep -Fq ".prompt-node-toggle.concurrent-toggle .concurrent-switch" "$file"
  grep -Fq "class=\"concurrent-switch\"" "$file"
}
