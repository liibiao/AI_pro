...
WORKDIR="$(mktemp -d)"
BACKUP_DIR="/home/ubuntu/漫剧创作库/.deploy-backups/asset-confirm-progress-full-overlay-20260622022441-${STAMP}"
...
check_markers(){
  ...
  grep -Fq "height:100%!important;display:block!important" "$file"
  grep -Fq "asset-confirm-gen-mask>.gen-progress-text em{display:none!important}" "$file"
  grep -Fq "asset-confirm-detail-preview .asset-confirm-gen-mask>.gen-progress-text b{font-size:46px!important}" "$file"
}
...
echo "deployed asset-confirm-progress-full-overlay-20260622022441"
