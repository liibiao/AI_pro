...
PKG="video-model-billing-badges-20260620040715"
...
verify_canvas_next_html(){
  local file="$1"
  grep -Fq ".tn-billing-badge" "$file"
  grep -Fq "has-billing-badge" "$file"
...
if run_sudo test -f "$WORKBENCH_DIR/image-studio-canvas-next.html"; then
  run_sudo grep -Fq ".tn-billing-badge" "$WORKBENCH_DIR/image-studio-canvas-next.html"
