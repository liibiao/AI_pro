...
- **Seedance 长版默认执行格式（新增红线）**：用户或上游要求“长版提示词 / Seedance 长版 / 长版母稿”时，Studio 必须按 `画面主体【完整】 + @Image 起手 + 逐条时间轴 + 六固定段名` 输出；旧式 `主体 / 空间 / 光影 / 镜头 / 台词 / 音效` 字段块只可作为内部分析，不能作为正式长版交付。
- **Seedance 2.5 连续叙事执行（新增）**：当执行 `seedance2.5-15s / seedance2.5-30s` 时，必须先检查生成任务、主要主体、场景与环境状态、情绪目标、视觉风格和分段镜头脚本是否齐全；30s 版本若出现主体丢失、空间断裂、后半段空转或情绪失控，优先拆回两个 15s 版本，而不是继续增加描述长度。
- **Seedance 2.5 正式稿结构保护（新增）**：正式流水线投喂稿默认保留六大模块与六固定段名；“去模板化”只处理动作节奏、镜头句式和招式推进同构，不得把结构删成纯自然段。动作段必须避免固定套用“起势 → 推进 → 压制 → 反击 → 爆点 → 收势”。
- **Seedance 2.5 可复制稿执行（新增）**：交付给用户或平台的正式投喂稿不使用 Markdown 表格，`分段镜头脚本` 必须写成 `0-3s：...` 的自然时间轴；不得裸写 `HZW 材质画质风格`，必须展开为具体材质、画质、光影、反射、空气层、体积雾、颗粒和清晰度边界。
...
- `05-prompts/seedance/第XX集/seedance2.5/ep*-seedance2.5-15s-paste.md` (Seedance 2.5 / 2.0 15s 连续叙事投喂稿)
- `05-prompts/seedance/第XX集/seedance2.5/ep*-seedance2.5-30s-paste.md` (Seedance 2.5 30s 连续叙事投喂稿)
