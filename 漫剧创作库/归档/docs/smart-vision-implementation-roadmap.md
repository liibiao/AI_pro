...
1. **先封装现状，不重造流程**：所有任务从 `docs/workflow.md`、`docs/agent-protocol.md`、`docs/phase-gate-checklist.md`、`docs/project-memory-system.md` 映射。
2. **先现有节点，后新增节点**：MVP 先用当前 `txt2img`、`img2imgAll`、`seedanceVideo`、`videoEditor` 等节点生成 JSON。
3. **先故事板级，后单集级，再全剧级**。
4. **先人工审核闭环，后全自动队列**。
5. **先本地文件系统和工程记忆，后可选数据库与云存储增强**。
6. **所有关键结果写回项目文件和 `.smart-vision/` 工程状态，不依赖单一客户端或数据库**。
7. **先能力包 Runtime，后重型服务端**：MVP 不把 FastAPI、SQLite、Redis、RQ/Celery、PostgreSQL 作为核心前置依赖。
