...
function isAiyunzhiGptImage2SiblingModel(model={},adapter=''){
...
function aiyunzhiGptImage2ConfiguredModelName(model={}){
  const provider=model?.provider||{};
  const candidates=[
    model?.model,model?.modelName,model?.realModelName,model?.upstreamModel,model?.defaultModel,
    provider.defaultModel,model?.name
  ].map(value=>String(value||'').trim()).filter(Boolean);
  const selected=candidates.find(value=>/gpt[-_ ]?image[-_ ]?2/i.test(value))||'gpt-image-2';
  return selected.replace(/-(?:1|2|3|4)k$/i,'')||'gpt-image-2';
}
...
function isAiyunzhiGptImage2Model(model={},adapter=''){
  ...
  return isAiyunzhiGptImage2Adapter(adapter)||
    ...
    isAiyunzhiGptImage2SiblingModel(model,adapter)||
...
function forceAiyunzhiGptImage2ModelConfig(model={}){
  ...
  const imageModel=aiyunzhiGptImage2ConfiguredModelName(model);
  ...
  return {
    ...
    upload_mode:'object_storage',
    protocol,
    provider:{...(model.provider||{}),adapter:'aiyunzhi-gpt-image-2',endpointPath:'/v1/images/generations',uploadMode:'object_storage',defaultModel:imageModel}
  };
}
...
function taskResultUrls(task){
  ...
  const visit=(value,key='')=>{
    ...
    if(typeof value==='string'){
      if(/^data:image\//i.test(value)||/^https?:\/\//i.test(value)||/^\/api\//i.test(value)||/^\/v1\//i.test(value)||/^file:/i.test(value))push(value);
      return;
    }
    if(Array.isArray(value)){value.forEach(item=>visit(item,key));return;}
    if(typeof value==='object'){
      const mime=String(value.mimeType||value.mime_type||value.mime||value.contentType||value.content_type||'image/png');
      const inline=value.inlineData||value.inline_data;
      const inlineData=String(inline?.data||value.b64_json||value.b64||'').trim();
      if(inlineData&&/^image\//i.test(String(inline?.mimeType||inline?.mime_type||mime)))push(/^data:image\//i.test(inlineData)?inlineData:`data:${String(inline?.mimeType||inline?.mime_type||mime||'image/png')};base64,${inlineData}`);
      Object.entries(value).forEach(([key,item])=>{
        if(/url|uri|output|image|video|result|preview|temporary|public|cos|storage/i.test(key)||typeof item==='object')visit(item,key);
      });
    }
  };
...
function imageEntryFromTask(task,prompt,payload,{isPanorama=false}={}){
  ...
  const saved={url,remoteUrl:url,assetPreviewUrl:url,assetPreviewUrls:[url],imageTaskId:task.id,taskId:task.id,status:task.status};
  const entry={url,dataUrl:url,previewUrl:url,localUrl:'',remoteUrl:url,assetPreviewUrl:url,assetPreviewUrls:[url],saved,prompt,payload,width:0,height:0,isPanorama,images:[]};
  entry.images=[entry];
  return entry;
}
